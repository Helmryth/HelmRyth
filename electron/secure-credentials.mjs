...
const credentialDocument = (value) =>
  Object(value) === value && !Array.isArray(value) ? value : null;
...
export async function readSecureCredentials({
  ...
  for (let attempt = 0; attempt <= delays.length; attempt++) {
    ...
    try {
      ...
      const text = primitiveString(decrypted) ?? decrypted?.result;
      try {
        const parsed = credentialDocument(JSON.parse(text));
        if (parsed === null) {
