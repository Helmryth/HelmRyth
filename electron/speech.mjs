...
const BUNDLE = app.isPackaged
  ? path.join(process.resourcesPath, "Helmryth Speech.app")
  : speechHelperBundle;
...
function parseSpeechFrame(line) {
  const payload = JSON.parse(line);
  if (payload === null) return null;
  return {
    payload,
    error: isStringValue(payload.error) ? payload.error : null,
    completed: payload.partial === false && isStringValue(payload.text),
  };
}
...
export function startSpeech(win, options = {}) {
  ...
  const sessionDir = mkdtempSync(path.join(app.getPath("temp"), "helmryth-speech-"));
  ...
  const drain = () => {
    ...
    while ((nl = buf.indexOf("\n")) !== -1) {
      ...
      if (!line) continue;
      try {
        const frame = parseSpeechFrame(line);
        if (!frame) continue;
        if (frame.error !== null) reportedError = frame.error;
        ...
        if (child === speechSession && !win.isDestroyed()) {
          win.webContents.send("speech:transcript", frame.payload);
        }
      } catch {
