...
export function registerUpdaterIpc() {
  ...
  ipcMain.handle("update:check", () => {
    if (updaterCoordinator) return updaterCoordinator.check(true);
    if (updaterUnavailableReason) {
      setState({ status: "error", message: updaterUnavailableReason });
    }
    return state;
  });
...
export function startUpdater(mainWindow) {
  ...
  if (!channel.available) {
    ...
    updaterUnavailableReason = channel.reason === "not-provisioned"
  ...
  try {
    ...
    updaterCoordinator = null;
    setState({ status: "error", message: "The updater is unavailable." });
    return;
  }
