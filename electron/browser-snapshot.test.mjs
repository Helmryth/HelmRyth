...
describe("browser snapshot", () => {
  ...
  it("presents as the Chrome it is", () => {
    expect(
      browserUserAgent("Mozilla/5.0 (Macintosh) AppleWebKit/537.36 (KHTML, like Gecko) Helmryth/0.1.0 Chrome/140.0.0.0 Electron/43.4.0 Safari/537.36"),
    ).toBe("Mozilla/5.0 (Macintosh) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/140.0.0.0 Safari/537.36");
  });

  it("derives one durable partition per operator from safe characters only", () => {
    expect(browserPartition("operator_1-A")).toBe("persist:helmryth-browser-operator_1-A");
    expect(browserPartition("../../evil")).toBe("persist:helmryth-browser-evil");
