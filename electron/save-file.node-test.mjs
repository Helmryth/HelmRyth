...
const canSymlink = (() => {
  const probe = fs.mkdtempSync(path.join(os.tmpdir(), "helmryth-symlink-probe-"));
  try {
...
before(() => {
  home = fs.mkdtempSync(path.join(os.tmpdir(), "helmryth-save-file-"));
  dataHome = path.join(home, ".helmryth");
  fs.mkdirSync(path.join(dataHome, "workspaces", "operator"), { recursive: true });
  fs.writeFileSync(path.join(dataHome, "workspaces", "operator", "report.docx"), "docx");
...
describe("save-file path validation", () => {
  it("accepts a file inside the operator home, as a path or a file:// URL", async () => {
    const file = path.join(dataHome, "workspaces", "operator", "report.docx");
  ...
  it("accepts a file under a symlinked operator home", { skip: !canSymlink }, async () => {
    const realHome = fs.mkdtempSync(path.join(os.tmpdir(), "helmryth-real-home-"));
    const linkedHome = fs.mkdtempSync(path.join(os.tmpdir(), "helmryth-linked-home-"));
    const realDataHome = path.join(realHome, "operator-data");
    fs.mkdirSync(realDataHome, { recursive: true });
    fs.writeFileSync(path.join(realDataHome, "report.docx"), "docx");
    fs.symlinkSync(realDataHome, path.join(linkedHome, ".helmryth"));

    const viaLink = path.join(linkedHome, ".helmryth", "report.docx");
  ...
  it("rejects paths outside the operator home, including via traversal", async () => {
    const rejected = "Only files created by your operators can be saved";
    await assert.rejects(resolveSavablePath(path.join(home, "secret.txt"), { home }), { message: rejected });
    await assert.rejects(resolveSavablePath(path.join(dataHome, "..", "secret.txt"), { home }), { message: rejected });
  });

  it("rejects a symlink inside the operator home pointing outside it", { skip: !canSymlink }, async () => {
    const escape = path.join(dataHome, "escape.txt");
    fs.symlinkSync(path.join(home, "secret.txt"), escape);
    await assert.rejects(resolveSavablePath(escape, { home }), {
      message: "Only files created by your operators can be saved",
    });
    fs.rmSync(escape);
  });
  ...
  it("rejects empty, relative, and non-file targets", async () => {
    ...
    await assert.rejects(resolveSavablePath("workspaces/operator/report.docx", { home }), { message: "That file path is invalid" });
    await assert.rejects(resolveSavablePath(path.join(dataHome, "nope.docx"), { home }), { message: "That file no longer exists" });
    await assert.rejects(resolveSavablePath(path.join(dataHome, "workspaces"), { home }), { message: "That path is not a file" });
  });
});

describe("save-file dialog default name", () => {
  it("suggests a name that does not overwrite an existing file", async () => {
    const downloads = fs.mkdtempSync(path.join(os.tmpdir(), "helmryth-downloads-"));
    const source = path.join(dataHome, "workspaces", "operator", "report.docx");
  ...
  it("keeps the extension on the suggestion", async () => {
    const downloads = fs.mkdtempSync(path.join(os.tmpdir(), "helmryth-downloads-ext-"));
    const source = path.join(dataHome, "workspaces", "operator", "report.docx");
    fs.writeFileSync(path.join(downloads, "report.docx"), "");
