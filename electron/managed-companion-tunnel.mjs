 ...
 * the reusable LAN port, so it must be reconciled by the Registry before
...
/** Validate a Registry provision response and return a copy to persist. */
export function withManagedCompanionTunnelAccess(credentials, provision) {
  ...
  if (!endpoint || !validConnectorToken(token)) {
    throw new Error("The Reach service returned an invalid managed endpoint");
  }
  return {
    ...credentials,
...
export function resolveCloudflaredBinary({
  ...
  const override = environment.HELMRYTH_CLOUDFLARED_PATH?.trim();
...
function ensurePrivateDirectory(directory, fileSystem, currentUid) {
  ...
  if (!stat.isDirectory() || stat.isSymbolicLink()) {
    throw new Error("The managed Reach runtime path is unsafe");
  }
  if (currentUid !== undefined && stat.uid !== currentUid) {
    throw new Error("The managed Reach runtime path has an unexpected owner");
  }
...
/** Remove only token files created by a dead Helmryth process. Suspicious
...
async function verifyHostedEndpoint(
  ...
  try {
    return JSON.parse(text)?.app === "helmryth";
  } catch {
    return false;
  }
}
...
export function createManagedCompanionTunnel({
  ...
  const removeTokenFile = (file = tokenFile) => {
    ...
    try {
      ...
      if (error?.code !== "ENOENT") log("managed Reach connector token cleanup failed");
    }
  };
  ...
  const attempt = async (ownedGeneration) => {
    ...
    if (
      ...
      publish({
        status: "unavailable",
        ready: false,
        error: "The managed Reach connector is missing from this build.",
      });
      return state;
    }
