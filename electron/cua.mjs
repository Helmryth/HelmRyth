...
//    EmbeddedCuaDriverHost so TCC grants attribute to Helmryth and the
//    driver inherits them. One prompt, named Helmryth, out of the box.
...
function parseStateListener(listener) {
  try {
...
export function setCuaStateListener(listener) {
  stateListener = parseStateListener(listener);
}
...
async function loadEmbeddedSdk() {
  ...
  process.env.HELMRYTH_CUA_SDK_LIBRARY = path.join(
...
async function attachStandalone() {
  ...
  if (!(await socketAlive(STANDALONE_SOCKET))) {
    ...
    // machine already granted — instead of the freshly signed Helmryth.
...
async function startEmbedded(binary) {
  ...
  // Helmryth rather than to a terminal or helper process.
  ...
  if (!sdk.hasRequiredMacOSPermissions(permissionStatus)) {
    ...
    throw new Error(`${missing || "macOS permissions"} required; grant access in System Settings and restart Helmryth`);
  }
...
export async function startCua() {
  ...
  const wantEmbedded =
    app.isPackaged || process.env.HELMRYTH_CUA_EMBEDDED === "1";
  let nextConnection;
