...
function desktopCommandEnvironment(source = process.env, additions = {}) {
  ...
  // Keep every Cua child owned by Helmryth deterministic and local-only.
...
function sameDriverFileIdentity(expected, actual) {
  ...
  return DRIVER_FILE_IDENTITY_KEYS.every(
    (key) => isStringValue(expected[key]) && expected[key] === actual[key],
  );
}
...
function runGetent(args, {
  ...
  if (result?.error || result?.status !== 0 || !isStringValue(result?.stdout)) {
    return {
      ok: false,
...
function privatePrimaryGroup(identity, { getent = runGetent } = {}) {
  if (
    ...
    !isStringValue(identity?.username) ||
    !identity.username
  ) {
  ...
  if (!isStringValue(groupResult.stdout)) {
  ...
  if (!isStringValue(passwdResult.stdout)) {
    return { exclusive: false, reason: "lookup-malformed" };
  }
...
function createPermissionCheck(identity, lookupPrivateGroup) {
  ...
  const failure = (component, stat) => {
    ...
    const details = { affectedPaths: [component] };
    if (groupProof?.reason) details.permissionReason = groupProof.reason;
    return unavailable(
...
function validateManifest(manifest, binaryPath) {
  ...
  if (
    !isObjectRecord(invocation) ||
    !isStringValue(invocation.command) ||
...
function validateDoctor(report, { session = "x11" } = {}) {
  if (!isObjectRecord(report) || !isBooleanValue(report.ok) || !Array.isArray(report.probes)) {
  ...
  const probes = report.probes.map((probe) => {
    if (
      ...
      !isStringValue(probe.label) ||
      !["ok", "warn", "err"].includes(probe.status) ||
      !isStringValue(probe.message)
    ) {
    ...
    if (isStringValue(probe.detail)) normalizedProbe.detail = probe.detail;
...
async function inspectLinuxCuaDriver({
  ...
  try {
    ...
    if (session === "wayland") ready.compositor = "gnome-mutter";
    return ready;
  } catch (error) {
    const details = {
      path: discovered.path,
    ...
    if (error?.probes) details.probes = error.probes;
    return unavailable(error?.code ?? "diagnostics-failed", error?.message ?? String(error), details);
  }
}
