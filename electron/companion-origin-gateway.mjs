...
const RUNTIME_PREFIX = "helmryth-reach-origin-";
...
export function validCompanionOriginTarget(target, platform = process.platform) {
  ...
  if (platform === "win32") {
    return /^\\\\\.\\pipe\\helmryth-reach-origin-[1-9][0-9]*-[0-9a-f-]{36}$/i.test(
      target.socketPath,
    );
  }
  return (
...
export function createCompanionOriginEndpoint({
  ...
  if (platform === "win32") {
    return Object.freeze({
      pid: processId,
      socketPath: `\\\\.\\pipe\\helmryth-reach-origin-${processId}-${identifier()}`,
      directory: null,
    });
  }
  ...
  try {
    ...
    if (
      ...
      throw new Error("The Reach origin directory is unsafe");
    }
    const socketPath = path.join(directory, SOCKET_NAME);
    if (Buffer.byteLength(socketPath) > 96) {
      throw new Error("The Reach origin socket path is too long");
    }
    ...
    const outgoing = request(
      ...
      (response) => {
        ...
        response.on("end", () => {
          ...
          try {
            finish(JSON.parse(Buffer.concat(chunks).toString("utf8"))?.app === "helmryth");
          } catch {
            finish(false);
          }
        });
...
function unavailable(response) {
  ...
  const body = JSON.stringify({ error: "Reach origin unavailable" });
...
export function createCompanionOriginGateway({
  ...
  if (!validCompanionOriginTarget(target)) {
    throw new Error("The Reach origin target is invalid");
  }
  let accepting = true;
  let listening = false;
