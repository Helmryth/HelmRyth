...
const INSTALLATION_CREDENTIAL = `hry_node_${"b".repeat(22)}.${"c".repeat(43)}`;
const CONNECTOR_TOKEN = `eyJ${"d".repeat(100)}`;
const ENDPOINT = "https://c-opaque.example.com";
...
describe("Companion account service", () => {
  it("maps the Registry Node and Reach error contract to precise recovery copy", () => {
    const cases = [
    ...
    for (const [code, message] of cases) {
      expect(friendlyCompanionAccountError(new RegistryError(code))).toBe(message);
    }
  });

  it("fails closed without provisioning and reads only the desktop-owned service config", () => {
    expect(resolveCompanionRegistryOrigin()).toBe("");
    expect(resolveCompanionRegistryOrigin({
      managedServiceConfig: { registryOrigin: "http://127.0.0.1:8787/" },
    })).toBe("http://127.0.0.1:8787");
    expect(resolveCompanionRegistryOrigin({
      managedServiceConfig: { registryOrigin: "http://accounts.example.com" },
    })).toBe("");
    expect(resolveCompanionRegistryOrigin({
      managedServiceConfig: { registryOrigin: new String("https://accounts.example.com") },
    })).toBe("");
    expect(resolveCompanionRegistryOrigin({
      managedServiceConfig: { registryOrigin: "https://accounts.example.com" },
    })).toBe("https://accounts.example.com");
  });

  it("signs out locally without a configured Registry client and retains no reusable secret", async () => {
    ...
    await expect(service.signOut()).resolves.toEqual({
      available: false,
      status: "signed-out",
      message: "Signed out on this device. Remote cleanup will finish after you reconnect this account.",
    });
  ...
  it("hides account onboarding until the configured registry is healthy", async () => {
    const client = readyClient({
      health: vi.fn(async () => {
        throw new RegistryError("request_failed", 404);
      }),
    });
  ...
  it("keeps an existing account recoverable while the registry is unhealthy", async () => {
    const client = readyClient({
      health: vi.fn(async () => {
  ...
  it("discovers a registry that becomes healthy without restarting the app", async () => {
    const health = vi
      .fn()
      .mockRejectedValueOnce(new RegistryError("request_failed", 404))
  ...
  it("shows a specific rate-limit message instead of the generic secure-access error", async () => {
    ...
    const client = readyClient({
      requestOTP: vi.fn(async () => {
        throw new RegistryError("rate_limited", 429, requestId);
      }),
    });
    const { service } = serviceFixture({ client });
  ...
  it("keeps an invalid code on the signed-out path with a friendly message", async () => {
    const client = readyClient({
      verifyOTP: vi.fn(async () => {
        throw new RegistryError("invalid_otp", 400);
      }),
    });
    const { service } = serviceFixture({ client });
  ...
  it("recovers from a network provisioning failure on retry", async () => {
    const ensureEndpoint = vi
      .fn()
      .mockRejectedValueOnce(new RegistryError("network_unavailable"))
  ...
  it("drops a response-lost session bearer but retains cleanup intent when reconciliation is offline", async () => {
    const client = readyClient({
      ensureInstallation: vi.fn(async () => {
        throw new RegistryError("network_unavailable");
      }),
    ...
    await expect(service.signOut()).resolves.toMatchObject({
      status: "signed-out",
      message: expect.stringContaining("Remote cleanup"),
    });
  ...
  it("stops locally, drops reusable credentials, and finishes pending cleanup after reauthentication", async () => {
    ...
    const recovered = await service.verifyCode("ada@example.com", "12345678");
    expect(recovered).toEqual({ available: true, status: "signed-out" });
    expect(store.read()).toEqual({ [COMPANION_CLIENT_INSTANCE_FIELD]: UUID });
    expect(listInstallations).toHaveBeenCalledTimes(2);
  });

  it("stops hosted access before remote cleanup when the registry is offline", async () => {
    const offline = async () => {
      throw new RegistryError("network_unavailable");
    };
    const client = readyClient({
  ...
  it("treats an already removed installation as an idempotent sign-out", async () => {
    const client = readyClient({
      revokeInstallation: vi.fn(async () => {
        throw new RegistryError("not_found", 404);
      }),
    });
    const { service, store } = serviceFixture({ initial: signedCredentials(), client });
