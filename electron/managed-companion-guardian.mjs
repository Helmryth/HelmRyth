...
export async function runManagedCompanionGuardian({
  ...
  if (!validCompanionOriginTarget(target, platform)) {
    throw new Error("The managed Reach origin target is invalid");
  }
  if (!Number.isInteger(originPort) || originPort < 1 || originPort > 65_535) {
    throw new Error("The managed Reach origin port is invalid");
  }
...
function guardianArguments(argv) {
  if (argv.length !== 5) throw new Error("The managed Reach guardian arguments are invalid");
  ...
  if (!validCompanionOriginTarget(target)) {
    throw new Error("The managed Reach guardian target is invalid");
  }
