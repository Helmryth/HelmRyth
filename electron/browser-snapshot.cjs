...
// the cloud box's CDP helper (server/remote-computer.ts) so an operator that learned
...
/**
 * The only addresses the surface will load. Operators (and the address bar) may
...
/** A operator id becomes a durable session partition: logins survive restarts
 * and no two operators share a cookie jar. Only the safe id characters are kept
...
function browserPartition(botId) {
  ...
  if (!safe) throw new Error("An operator id is required");
  return `persist:helmryth-browser-${safe}`;
}

/** A named profile is a partition several operators may share — "Work", "Client
 * A" — so one sign-in serves every operator pointed at it. */
function browserProfilePartition(profileId) {
  ...
  return `persist:helmryth-browser-profile-${safe}`;
}
