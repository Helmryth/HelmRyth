...
//
// Profiles: an operator has one view per profile it has used — its own private
...
// native modals), and only http(s) navigations honoured. A operator's browser can
 ...
 * compact preview scales it down, the expanded view shows it 1:1. Operators see
 * one consistent desktop viewport regardless of how wide the panel is. */
const VIEWPORT = Object.freeze({ width: 1280, height: 800 });

/** Keys an operator may press by name → CDP key event fields. `text` is what makes
...
function botIdOf(value) {
  ...
  if (!BOT_ID.test(id)) throw new Error("An operator id is required");
  return id;
}
 ...
 * @param {(botId: string) => string} [options.partitionFor] test seam for the per-operator partition
...
function createBrowserSurfaceManager({
  ...
  /** the view an operator currently shows / acts on */
  const active = new Map();
  let guestCounter = 0;

  const partitionForProfile = (botId, profile) => {
    if (profile === GUEST_PROFILE) return `helmryth-browser-guest-${botId}-${++guestCounter}`;
  ...
  const evictIfNeeded = () => {
    ...
    if (!victim) throw new Error(`Only ${maxViews} operator browser surfaces can be open at once`);
  ...
  const secure = (entry) => {
    ...
    // A download would land on the user's disk under an operator's control; refuse
    // until there is a reviewed place for it to go.
    ses.on("will-download", (event) => event.preventDefault());
    contents.setWindowOpenHandler(({ url }) => {
      // target=_blank links stay in this operator's one tab: a second window would
  ...
  /** Things the page does on its own that an operator must hear about. */
  const onProtocolEvent = (entry, method, params) => {
    if (method === "Page.javascriptDialogOpening") {
      // alert/confirm/prompt would otherwise be a native modal over the app
      // window that nobody can answer for the operator. Accept confirm/beforeunload,
      ...
      if (type === "prompt") response.promptText = String(params.defaultPrompt ?? "");
      void cdp(entry, "Page.handleJavaScriptDialog", response).catch(() => {});
  ...
  const create = (botId, profile) => {
    evictIfNeeded();
    if (owner.isDestroyed?.()) throw new Error("The Helmryth window is unavailable");
  ...
  /** The view an operator should be looking at: `undefined` keeps whatever is
   * active (callers that don't know the profile never evict a tab); "" is
   * the operator's own session; "guest" a throwaway; anything else a named
  ...
  const cdp = async (entry, method, params = {}) => {
    ...
    if (!entry.attached) {
      ...
      try {
        ...
        // never show a native file picker for an operator; the event is reported instead
   ...
   * loading must not hang the operator). */
  ...
  const api = {
    /** Create or switch the operator's view; hidden until laid out. */
    ...
    /** Position the operator's active view over the renderer's rectangle (or hide
     ...
     * simply settles — bounded, so an operator never hangs on a page that stalls. */
    ...
    /** Drop every operator's view on a named profile — before its data is cleared. */
    ...
    /** Drop every view an operator has (all profiles). */
    close(botId) {
