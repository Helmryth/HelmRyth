...
function parseSourcePath(rawPath) {
  const isString =
    Object(rawPath) !== rawPath &&
    Object.prototype.toString.call(rawPath) === "[object String]";
  if (!isString || !rawPath.trim()) {
...
function normalizeSourcePath(rawPath) {
  ...
  if (!path.isAbsolute(sourcePath)) throw new Error("That file path is invalid");
  return sourcePath;
}
...
function assertInside(root, target) {
  if (target !== root && !target.startsWith(root + path.sep)) {
    throw new Error("Only files created by your operators can be saved");
  }
}
...
async function resolveSource(rawPath, { home, fsp, platform }) {
  ...
  const root = await canonicalPath(
    path.join(home, ".helmryth"),
    fsp,
    "Only files created by your operators can be saved",
  );
