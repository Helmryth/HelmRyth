...
function temporaryDirectory() {
  ...
  const directory = fs.mkdtempSync(path.join(base, "helmryth-cua-runtime-"));
...
function harness({
  ...
  const inspect = vi.fn(async () => {
    ...
    if (session === "wayland") result.compositor = "gnome-mutter";
    return result;
  });
...
describe.skipIf(process.platform === "win32")("Linux CUA opt-in and lifecycle", () => {
  ...
  it("passes the exact packaged candidate and architecture into inspection", async () => {
    const bundledDriverPath = "/opt/Helmryth/resources/cua-linux-x64/cua-driver";
    const context = harness({
