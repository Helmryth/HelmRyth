...
describe("workspace credential env", () => {
  it("maps each stored secret to exactly its server env var", () => {
    expect(
      ...
      HELMRYTH_TTS_KEY: "tts-secret",
      OPENCODE_API_KEY: "ocg-secret",
      HELMRYTH_OPENAI_IMAGE_KEY: "image-secret",
    });
  });
  ...
  it("replaces inherited provider configuration with only encrypted stored credentials", () => {
    const inherited = {
      PATH: "/usr/bin",
      XAI_API_KEY: "shell-xai",
      OPENAI_COMPAT_API_KEY: "shell-compat",
      OPENAI_COMPAT_URL: "https://attacker.invalid/v1",
      OPENAI_COMPAT_MODEL: "attacker/model",
      OPENAI_COMPAT_PROVIDER: "attacker",
      BOX_TOKEN: "shell-box",
    };
    expect(workspaceCredentialChildEnvironment({
      openaiCompatApiKey: "stored-compat",
    }, inherited)).toEqual({
      PATH: "/usr/bin",
      OPENAI_COMPAT_API_KEY: "stored-compat",
    });
  });
});
