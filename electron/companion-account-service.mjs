...
const INSTALLATION_CREDENTIAL = /^hry_node_[A-Za-z0-9_-]{22}\.[A-Za-z0-9_-]{43}$/;
...
const ownString = (document, field) => stringValue(document?.[field]);

/** Hosted access is disabled until a build is given an exact provisioned
 * HTTPS origin (or an HTTP loopback Worker for development). An invalid or
 * absent setting fails closed so an OTP or bearer never reaches an inherited
 * or assumed host. */
export function resolveCompanionRegistryOrigin({ managedServiceConfig } = {}) {
  return normalizeRegistryOrigin(managedServiceConfig?.registryOrigin);
}
...
const FRIENDLY_MESSAGES = Object.freeze({
  ...
  network_unavailable: "Helmryth could not reach its secure connection service. Check your internet and try again.",
  ...
  node_limit_reached: "This account has reached its computer limit. Remove an old computer and try again.",
  node_exists: "This computer is already connected. Try again to recover it.",
  reach_busy: "The secure connection is still being prepared. Try again in a moment.",
  reach_unavailable: "The secure connection service could not finish setup. Local pairing still works; try again shortly.",
  reach_cleanup_pending: "The secure connection is still being removed. Try signing out again shortly.",
  remote_cleanup_pending: "Signed out on this device. Remote cleanup will finish after you reconnect this account.",
  registry_unavailable: "Secure access is not available right now. Local pairing still works.",
...
export function friendlyCompanionAccountError(error) {
  ...
  const reference = error instanceof RegistryError && error.requestId
...
function publicState({ available, status, email, endpoint, message }) {
  ...
  const accessEndpoint = (() => {
    const decodedEndpoint = stringValue(endpoint);
    if (!decodedEndpoint) return "";
    try {
      const parsed = new URL(decodedEndpoint);
  ...
  const decodedMessage = stringValue(message);
  const safeMessage = decodedMessage.length >= 1 && decodedMessage.length <= 280
    ? decodedMessage
    : null;
...
export function createCompanionAccountService({
  ...
  const probeRegistry = async ({ force = false } = {}) => {
  ...
  const requireHealthyRegistry = async () => {
    if (!(await probeRegistry({ force: true }))) {
  ...
  const settledState = () => {
    ...
    const cleanupPending = companionAccountCleanupPending(document);
    if (!configured) {
      return publicState({
        available: false,
        status: "signed-out",
        message: cleanupPending
          ? FRIENDLY_MESSAGES.remote_cleanup_pending
          : FRIENDLY_MESSAGES.registry_unavailable,
      });
    }
    if (cleanupPending && !account) {
      return publicState({
        available: true,
        status: "signed-out",
        message: FRIENDLY_MESSAGES.remote_cleanup_pending,
      });
    }
    ...
    if (!healthy) {
      return publicState({
        ...
        message: FRIENDLY_MESSAGES.registry_unavailable,
      });
    }
    if (
      phase &&
  ...
  const clearLocalAccountForPendingCleanup = async () => {
    await updateCredentials((document) => ({
      ...withoutCompanionAccount(document),
      [COMPANION_ACCOUNT_CLEANUP_PENDING_FIELD]: true,
    }));
    phase = null;
  };

  const clearAfterCleanup = async ({ stopLocal = true } = {}) => {
    await updateCredentials(withoutCompanionAccount);
    if (stopLocal) await stopManagedEndpoint();
    phase = null;
  };
  ...
  const cleanupCurrentAccount = async ({ markPending = true, stopLocal = true } = {}) => {
    const document = credentials();
    const account = storedAccount(document);
    if (!account) {
      await clearAfterCleanup({ stopLocal });
      return true;
    }
    ...
    await clearAfterCleanup({ stopLocal: false });
    return true;
  };
  ...
  const failAction = (
    ...
    phase = {
      status:
        signedOut ||
        (expiredSessionIsSignedOut && error instanceof RegistryError && error.status === 401)
          ? "signed-out"
          : "error",
      email,
      message,
  ...
  const requestCode = (rawEmail) => serialize(async () => {
    if (!configured) throw new Error(FRIENDLY_MESSAGES.registry_unavailable);
    const email = normalizeAccountEmail(rawEmail);
    let requested;
    try {
      await requireHealthyRegistry();
  ...
  const verifyCode = (rawEmail, rawCode) => serialize(async () => {
    ...
    const resumingSecretlessCleanup = Boolean(
      !previous && companionAccountCleanupPending(credentials()),
    );
    const finishingPendingCleanup = refreshingPendingCleanup || resumingSecretlessCleanup;
    ...
    try {
      ...
      await updateCredentials((document) =>
        withAuthenticatedAccount(document, {
          ...verified,
          clientInstanceId,
          preserveCleanupPending: finishingPendingCleanup,
        }),
      );
      authenticatedPersisted = true;
      if (finishingPendingCleanup) {
  ...
  const signOut = () => serialize(async () => {
    ...
    phase = { status: "connecting", email };
    try {
      // Local access must stop even when the hosted registry is down.
      ...
      await cleanupCurrentAccount({ stopLocal: false });
      ...
      // Sign-out is locally terminal even if the Registry cannot revoke its
      // remote Node yet. Keep only a non-secret intent marker and the stable
      // client UUID; fresh authentication can later finish owner-scoped cleanup.
      await clearLocalAccountForPendingCleanup();
      return settledState();
    }
  });
  ...
  const restore = () => serialize(async () => {
    if (!configured) return settledState();
    if (!(await probeRegistry({ force: true }))) return settledState();
