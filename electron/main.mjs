...
import {
  ...
  workspaceCredentialChildEnvironment,
...
import { createExternalWebWindowOpenHandler, openExternalWebUrl } from "./external-url.mjs";
import {
...
import {
  ...
  resolveCompanionRegistryOrigin,
...
const PRODUCT_NAME = "Helmryth";
const APPLICATION_ID = "com.helmryth.app";
const STRING_VALUE_TAG = "[object String]";
const RECORD_VALUE_TAG = "[object Object]";

function parseStringValue(value) {
  if (Object(value) === value || Object.prototype.toString.call(value) !== STRING_VALUE_TAG) return null;
  return value;
}

function parseRecordValue(value) {
  if (Object.prototype.toString.call(value) !== RECORD_VALUE_TAG) return null;
  const prototype = Object.getPrototypeOf(value);
  return prototype === Object.prototype || prototype === null ? value : null;
}

// Establish every OS-visible identity before Electron resolves userData/logs.
// This also gives development launches the same process title as packages.
app.setName(PRODUCT_NAME);
process.title = PRODUCT_NAME;
// 127.0.0.1 explicitly — vite binds IPv4; a bare "localhost" here can
// resolve to ::1 and leave the app surface unavailable.
const DEV_URL = developmentRendererOrigin();
...
// live in this process; operators reach them through the loopback host whose
...
function applyUnreadBadge(win = mainWindow) {
  ...
  if (process.platform === "win32") {
    ...
    win.setOverlayIcon(
      ...
      count > 0 ? `${count} unread workstream${count === 1 ? "" : "s"}` : "No unread workstreams",
    );
    return;
  }
...
if (process.platform === "linux") {
  ...
  app.setDesktopName("com.helmryth.app.desktop");
}
...
if (!app.requestSingleInstanceLock()) {
  console.log("[desktop] Helmryth is already running — focusing that window");
  process.exit(0);
}
...
async function secureComposioConfig() {
  const dataDir = process.env.HELMRYTH_DATA_DIR || path.join(app.getPath("home"), ".helmryth");
  const configPath = path.join(dataDir, "config.json");
  try {
    const config = parseRecordValue(JSON.parse(fs.readFileSync(configPath, "utf8")));
    const composio = parseRecordValue(config?.composio);
    if (!config || !composio) return;
    let changed = false;
    const apiKey = parseStringValue(composio.apiKey);
    if (apiKey !== null && apiKey.trim().startsWith("ak_")) {
    ...
    for (const field of ["key", "url"]) {
      if (Object.hasOwn(composio, field)) {
        delete composio[field];
        changed = true;
      }
    }
    if (!changed) return;
...
function conduitUrl() {
  return normalizeConduitUrl(managedServices.conduitOrigin);
}

// The packaged app has no terminal: everything about the server child's life
// goes to server.log in the OS log dir (~/Library/Logs/Helmryth on macOS,
// Console.app-visible; %APPDATA%\Helmryth\logs on Windows), which is also
...
function ensureManagedCompanionConnector() {
  ...
  managedCompanionConnector = createManagedCompanionTunnel({
    ...
    onChange: (status) => {
      slog(`managed Reach connection ${status.status}`);
...
setCompanionLifecycleListener(({ expected, pid }) => {
  if (expected) return;
  slog(`owned Reach process exited unexpectedly pid=${pid ?? "unknown"}`);
  companionDesiredThisLaunch = false;
  companionLaunchGeneration += 1;
  syncCompanionKeepAwake(false, false);
...
/** Called only after the registry has revoked/deleted the endpoint. */
 ...
 * the registry to revoke anything, but it must not erase the retry
...
function ensureCompanionAccountService() {
  ...
  const registryOrigin = resolveCompanionRegistryOrigin({
    managedServiceConfig: managedServices,
  });
  let client = null;
  if (registryOrigin) {
    try {
      client = createRegistryClient({ registryOrigin });
    } catch {
...
async function startServerOn(port) {
  ...
  let baseEnvironment = managedServiceChildEnvironment(managedServices, {
    ...process.env,
    HELMRYTH_STATIC_DIR: path.join(process.resourcesPath, "ui"),
    HELMRYTH_RESOURCES_PATH: process.resourcesPath,
    HELMRYTH_SKILLS_DIR: path.join(process.resourcesPath, "skills"),
    HELMRYTH_PORT: String(port),
    HELMRYTH_UI_ORIGIN: packagedRendererOrigin(port),
    HELMRYTH_USER_DATA: app.getPath("userData"),
  });
  baseEnvironment = workspaceCredentialChildEnvironment(secureCredentials, baseEnvironment);
  delete baseEnvironment.COMPOSIO_API_KEY;
  if (secureCredentials.composioApiKey) {
    baseEnvironment.COMPOSIO_API_KEY = secureCredentials.composioApiKey;
  }
  // "we could not read your keys" must not reach the UI as "you have none"
  baseEnvironment.HELMRYTH_CREDENTIAL_STORE = credentialStoreUnavailable ? "unavailable" : "ok";
  const childEnv = managedConduitChildEnvironment(conduitUrl(), secureCredentials, baseEnvironment);
  slog(`fork ${entry} port=${port}`);
  const proc = utilityProcess.fork(entry, [], {
    serviceName: "Helmryth Core",
    env: childEnv,
...
function syncManagedComposioCredentials() {
  ...
  try {
    serverProc.postMessage({
      type: "helmryth:conduit-access",
      access: managedConduitAccess(conduitUrl(), secureCredentials),
    });
  } catch (error) {
...
function buildErrorPage({ allPortsOccupied }) {
  ...
  const reason = allPortsOccupied
    ? "Every Helmryth port answered health checks from another process — likely a second copy of the app, or another program on ports 8799–28799. Quit that program, then quit and reopen Helmryth."
    : "The background server didn't come up in time — this is usually slow startup, not a port conflict. Quit and reopen Helmryth.";
  return (
    "data:text/html;charset=utf-8," +
    encodeURIComponent(
      `<body style="margin:0;display:flex;align-items:center;justify-content:center;height:100vh;background:#f6f1e7;color:#273447;font:15px Atkinson Hyperlegible Next,-apple-system,system-ui"><main style="max-width:420px;padding:40px"><div aria-hidden="true" style="width:34px;height:34px;background:#c44f36;clip-path:polygon(0 0,70% 0,100% 30%,100% 100%,30% 100%,0 70%)"></div><h2 style="font:600 24px Geologica,-apple-system,system-ui;margin:20px 0 8px">Helmryth couldn't start</h2><p style="color:#526071;line-height:1.55;margin:0">${escapeHtml(reason)} If it keeps happening, open <a target="_blank" rel="noopener" href="${serverLogHref}" style="color:#a13e2d">${escapeHtml(serverLogPath)}</a>.</p></main></body>`,
    )
  );
}
...
function rendererOrigin() {
  return app.isPackaged ? packagedRendererOrigin(SERVER_PORT) : DEV_URL;
}
...
function desktopViewerErrorPage(message, retryUrl) {
  ...
  return (
    "data:text/html;charset=utf-8," +
    encodeURIComponent(`<!doctype html><meta name="color-scheme" content="light"><title>Desktop unavailable</title>
      <body style="margin:0;display:grid;place-items:center;height:100vh;background:#f6f1e7;color:#273447;font:14px Atkinson Hyperlegible Next,-apple-system,system-ui,sans-serif">
        ...
        <p style="margin:0 0 20px;color:#526071;line-height:1.5">${escape(message)}</p>
        <a href="${escape(retryUrl)}" target="_blank" rel="noreferrer" style="display:inline-block;border:1px solid #a13e2d;border-radius:6px;background:#c44f36;color:#fffdf8;padding:9px 14px;text-decoration:none;font-weight:600">Open in browser</a></main>
      </body>`)
  );
}

function openDesktopViewer(owner, rawUrl, rawTitle, contextId) {
  if (!owner || owner.isDestroyed()) throw new Error("The Helmryth window is unavailable");
  ...
  // Clear the ref first so the stale window's close handler no-ops; on an operator
  // change, tell the previous operator to release (same-operator reopen stays quiet).
  ...
  const viewer = new BrowserWindow({
    ...
    webPreferences: {
      ...
      partition: "helmryth-desktop-viewer",
    },
  });
...
function ensureDesktopWorkspace(owner) {
  ...
  const manager = createDesktopWorkspaceManager({
    ...
    partitionPrefix: `helmryth-desktop-workspace-${randomUUID()}`,
    notify: (state) => {
...
/** The built-in browser: WebContentsViews per operator inside the app window,
 * plus the loopback host the operator's tools call. The host (and the token in
...
async function startBrowserSurface(owner) {
  try {
    ...
    // themselves stay alive — an operator mid-task must not lose its tab.
    owner.webContents.on("did-start-navigation", (_event, _url, isInPlace, isMainFrame) => {
...
// Deleting a profile: every operator's view on it goes, then its cookies, storage
...
function createWindow() {
  ...
  const win = new BrowserWindow({
    ...
    minWidth: 900,
    minHeight: 600,
    // The sole marketed surface is light and matches this native background,
    // so Windows can paint immediately without a theme-sync flash.
    show: true,
    icon: APP_ICON,
    backgroundColor: "#f6f1e7",
  ...
  win.webContents.setWindowOpenHandler(
    createExternalWebWindowOpenHandler(shell.openExternal, {
      reportError: (error, target) => {
        slog(`external link open failed ${target?.toString?.() ?? "unknown"}: ${error?.message ?? error}`);
      },
    }),
  );
  ...
  if (process.env.HELMRYTH_SMOKE_TEST === "1") {
    win.webContents.once("did-finish-load", async () => {
      try {
        const result = await win.webContents.executeJavaScript(`
          (async () => {
            if (!window.helmryth?.getCapabilities) throw new Error("desktop preload bridge is unavailable");
            let crashPromise = null;
            if (${JSON.stringify(process.env.HELMRYTH_SMOKE_CUA === "1")}) {
              crashPromise = new Promise((resolve, reject) => {
                const timeout = setTimeout(() => {
                  unsubscribe?.();
                  reject(new Error("timed out waiting for CUA crash invalidation"));
                }, 10000);
                const unsubscribe = window.helmryth.onCapabilitiesChanged((next) => {
            ...
            const [initialCapabilities, healthResponse, configResponse] = await Promise.all([
              window.helmryth.getCapabilities(),
              fetch("/api/health"),
              fetch("/api/config"),
            ]);
            ...
            if (crashPromise) {
              ...
              cuaRetryStatus = await window.helmryth.localControl.retry();
              capabilities = await window.helmryth.getCapabilities();
            }
            return {
              initialCapabilities,
              capabilities,
        ...
        if (process.env.HELMRYTH_SMOKE_BUNDLED_CUA === "1") {
        ...
        if (process.env.HELMRYTH_SMOKE_KEEP_OPEN !== "1") win.close();
      }
    });
  }

  if (app.isPackaged) {
    win.loadURL(serverReady ? packagedRendererOrigin(SERVER_PORT) : buildErrorPage({ allPortsOccupied: serverStartConflictOnly }));
  } else {
    win.loadURL(DEV_URL);
  }
  return win;
...
ipcMain.handle("engine:open-terminal", async (_event, command) => {
  const parsedCommand = parseStringValue(command);
  if (parsedCommand === null || !parsedCommand.trim()) return false;
  clipboard.writeText(parsedCommand);
...
// A operator's working folder: the native picker, so the path is real and the
...
ipcMain.handle("desktop:pick-folder", async (event, current) => {
  ...
  const currentPath = parseStringValue(current);
  if (currentPath) options.defaultPath = currentPath;
...
// Operators hand users files as markdown links to paths inside the Helmryth
...
// renderer-controlled, so it must resolve inside ~/.helmryth and be a
...
ipcMain.handle("desktop:open-external", async (_event, rawUrl) => {
  return openExternalWebUrl(shell.openExternal, rawUrl);
});
...
// live desktop inside Helmryth instead of sending the person to a browser.
...
// Close only when the caller owns the current viewer — otherwise one operator's
// "Hand control back" would close (and release) another operator's viewer.
...
// ── Helmryth Reach and Registry ────────────────────────────────────────
// The renderer can control the owned Reach lifecycle, but it cannot reach the
// service's control port or read Registry/connector credentials.
function registerReachIpc(prefix) {
  ipcMain.handle(`${prefix}:state`, () => desktopCompanionState());
  ipcMain.handle(`${prefix}:start`, () => startDesktopCompanion());
  ipcMain.handle(`${prefix}:stop`, () => stopDesktopCompanion());
  ipcMain.handle(`${prefix}:keep-awake`, async (_event, enabled) => {
    rememberCompanionKeepAwake(Boolean(enabled));
    return desktopCompanionState();
  });
  ipcMain.handle(`${prefix}:pairing`, (_event, open, expectedToken) =>
    companionPairing(Boolean(open), expectedToken).then(decorateDesktopCompanionState),
  );
  ipcMain.handle(`${prefix}:cloud-desktop`, (_event, deviceId, allowed) =>
    companionCloudDesktopAccess(deviceId, Boolean(allowed)).then(() => desktopCompanionState()),
  );
  ipcMain.handle(`${prefix}:revoke`, (_event, deviceId) =>
    companionRevoke(deviceId).then(() => desktopCompanionState()),
  );
}

function registerRegistryIpc(prefix) {
  ipcMain.handle(`${prefix}:state`, () => ensureCompanionAccountService().state());
  ipcMain.handle(`${prefix}:request-code`, (_event, email) =>
    ensureCompanionAccountService().requestCode(email),
  );
  ipcMain.handle(`${prefix}:verify-code`, (_event, email, code) =>
    ensureCompanionAccountService().verifyCode(email, code),
  );
  ipcMain.handle(`${prefix}:retry`, () => ensureCompanionAccountService().retry());
  ipcMain.handle(`${prefix}:sign-out`, () => ensureCompanionAccountService().signOut());
}

registerReachIpc("reach");
registerRegistryIpc("registry");
// Decode-only bridge aliases; remove in the next versioned renderer contract.
registerReachIpc("companion");
registerRegistryIpc("companion-account");
...
ipcMain.handle("assemblyai:set-key", async (_event, value) => {
  const credential = parseStringValue(value);
  if (credential === null) throw new Error("Unsupported credential");
  if (!(await safeStorage.isAsyncEncryptionAvailable())) {
    throw new Error("The operating-system credential store is unavailable");
  }
...
app.whenReady().then(async () => {
  ...
  if (app.isPackaged) app.setAsDefaultProtocolClient("helmryth");
  if (process.platform === "darwin") app.dock.setIcon(APP_ICON);
  managedServices = loadManagedServiceConfig({
    isPackaged: app.isPackaged,
    resourcesPath: process.resourcesPath,
    environment: process.env,
  });
  if (managedServices.state !== "ready") {
    slog(`managed service configuration ${managedServices.state}; hosted services remain disabled`);
  }
  ...
  // The Reach service the user left on comes back without anyone finding the
  ...
  // Package smoke proves signed-resource discovery but must never enroll a
  // real production Node from an ephemeral CI identity.
  if (
    app.isPackaged &&
    process.env.HELMRYTH_SMOKE_TEST !== "1" &&
    conduitUrl() &&
  ...
  // In-app updates remain dormant until a provisioned build supplies a
  // signed release channel. Downloads and installs are always user-driven.
  startUpdater(win);
  app.on("activate", () => {
