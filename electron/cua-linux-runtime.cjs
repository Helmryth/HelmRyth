...
const HOST_BUNDLE_ID = "com.helmryth.app";
...
function decodeString(value) {
  if (Object.prototype.toString.call(value) !== "[object String]" || Object(value) === value) {
    return null;
  }
  return value;
}
...
function validateDaemonMetadata(response, { childPid } = {}) {
  const metadata = decodeRecord(response?.ok === true ? response.result : null);
  if (!metadata) {
...
function validateToolSurface(response) {
  const manifest = decodeRecord(response?.ok === true ? response.result : null);
  if (
    !manifest ||
  ...
  const names = new Set(
    tools
      .map((tool) => decodeString(decodeRecord(tool)?.name))
...
function validateWaylandHealthReport(response) {
  const result = decodeRecord(response?.ok === true ? response.result : null);
  const report = decodeRecord(result?.structuredContent ?? result?.structured_content);
  if (
    !report ||
  ...
  for (const value of report.checks) {
    const check = decodeRecord(value);
    const name = decodeString(check?.name);
    const message = decodeString(check?.message);
    const hint = decodeString(check?.hint);
    if (
      !check ||
      name === null ||
      !["pass", "fail", "skip"].includes(check.status) ||
      message === null ||
      (check.status === "fail" && hint === null) ||
      checks.has(name)
    ) {
...
function createLinuxCuaRuntime({
  ...
  const runtimeRoot = () => {
    ...
    if (configured && path.isAbsolute(configured)) {
      try {
        ...
        if (
          ...
          const root = ensurePrivateDirectory(path.join(configured, "helmryth-cua"));
    ...
    const currentUid = process.getuid?.() ?? os.userInfo().uid;
    const root = ensurePrivateDirectory(path.join(os.tmpdir(), `helmryth-cua-${currentUid}`));
    cleanupStaleRuntimeDirectories(root);
    return root;
  };
  ...
  const start = async () => {
    ...
    startPromise = (async () => {
      ...
      if (runtimeSession === "wayland") {
        childEnvOverrides.CUA_DRIVER_RS_ENABLE_WAYLAND = "1";
      }
      const childEnv = desktopCommandEnvironment(env, childEnvOverrides);
      const args = [
        "serve",
        "--embedded",
      ...
      try {
        ...
        if (runtimeSession === "wayland") {
          mcpEnvironment.CUA_DRIVER_RS_ENABLE_WAYLAND = "1";
        }
        const readyConnectionPayload = {
          schemaVersion: CONNECTION_SCHEMA_VERSION,
          mode:
        ...
        if (inspected.compositor) {
          readyConnectionPayload.compositor = inspected.compositor;
        }
        const readyConnection = publish(readyConnectionPayload);
