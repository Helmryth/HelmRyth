...
function parseConnectionDetails(connection) {
  return {
    message: parseStringField(connection?.message),
    driverPath: parseStringField(connection?.driver?.path),
    driverVersion: parseStringField(connection?.driver?.version),
    driverSource: parseStringField(connection?.driver?.source),
    session: parseStringField(connection?.session),
    compositor: parseStringField(connection?.compositor),
  };
}
...
function desktopCapabilities({
  ...
  const connectionDetails = parseConnectionDetails(localConnection);
  const screenPreview = {
  ...
  if (connectionDetails.message !== null) {
    localComputer.message = connectionDetails.message;
  }
  if (connectionDetails.driverPath !== null) {
    localComputer.driverPath = connectionDetails.driverPath;
  }
  if (connectionDetails.driverVersion !== null) {
    localComputer.driverVersion = connectionDetails.driverVersion;
  }
  if (connectionDetails.driverSource !== null) {
    localComputer.driverSource = connectionDetails.driverSource;
  }
  if (connectionDetails.session !== null) {
    localComputer.session = connectionDetails.session;
  }
  if (connectionDetails.compositor !== null) {
    localComputer.compositor = connectionDetails.compositor;
  }
  if (!localAvailable) {
