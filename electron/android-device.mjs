...
function isInputPayload(value) {
  return value !== null && Object(value) === value && !isCallableValue(value);
}
...
export function createAndroidDeviceController(options = {}) {
  ...
  const readyDevice = async (serial) => {
    if (!isStringValue(serial) || !serial) throw new Error("An Android device is required");
  ...
  const input = async (serial, payload) => {
    const { binary } = await readyDevice(serial);
    if (!isInputPayload(payload)) throw new Error("Invalid Android input");
