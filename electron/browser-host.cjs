...
// renderer, never a debugging port on Helmryth itself. The manager is
...
function safeCorrectableError(error, token) {
  ...
  message = message
    ...
    .replace(/[\r\n\t]+/g, " ")
    .trim();
  return (message || "invalid browser request").slice(0, MAX_ERROR_CHARS);
}
...
function createBrowserHost({ manager, token = randomBytes(32).toString("hex") }) {
  ...
  const handle = async (req, res) => {
    ...
    if (!surface) return json(res, 503, { error: "the Helmryth window is closed — open it to use the browser" });
    let body;
    try {
      body = await readJson(req);
    } catch (error) {
      const status = error?.statusCode === 413 ? 413 : 400;
      return json(res, status, { error: safeCorrectableError(error, token) });
    }
    try {
      ...
      const status = /stale|unknown|not visible|gone|required|invalid|limited|unsupported|Only |no previous|no next|must be|timed out|no option|not a select|changed since/i.test(message)
        ? 400
        : 500;
      return json(res, status, { error: status === 400 ? safeCorrectableError(error, token) : "browser operation failed" });
    }
  };
    ...
    start() {
      ...
      server = http.createServer((req, res) => {
        handle(req, res).catch(() => {
          try {
            json(res, 500, { error: "browser host failure" });
          } catch {}
        });
      });
