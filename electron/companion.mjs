...
// sidecar's ~/.helmryth-companion, which is the child process's directory,
...
async function control(method, urlPath, body) {
  ...
  if (!res.ok && res.status !== 404) throw new Error(`Reach control ${res.status}`);
  return res.json();
}
...
async function start({ resourcesPath, harnessPort, hostedUrl = null, log }) {
  ...
  if (!resolved) {
    ...
    lastError = app.isPackaged
      ? "Helmryth Reach is missing from this build"
      : "Helmryth Reach sources are missing from this checkout";
    return companionState();
  }
  log?.(`Reach fork ${resolved.entry}`);

  let allocatedOrigin;
  try {
    allocatedOrigin = createCompanionOriginEndpoint();
  } catch {
    lastError = "the private Reach origin could not be created";
    return companionState();
  }
  ...
  // HELMRYTH_COMPANION_* remains a private, versioned child-process contract
  // for the current Reach sidecar. It is not exposed in product copy or the
  // canonical renderer bridge and will migrate atomically with companion/src.
  ...
  delete childEnvironment.HELMRYTH_COMPANION_HOSTED_URL;
  delete childEnvironment.HELMRYTH_COMPANION_INTERNAL_ORIGIN;
  if (hostedUrl) childEnvironment.HELMRYTH_COMPANION_HOSTED_URL = hostedUrl;
  childEnvironment.HELMRYTH_COMPANION_INTERNAL_ORIGIN = allocatedOrigin.socketPath;

  let child;
  try {
    child = utilityProcess.fork(resolved.entry, [], {
      serviceName: "Helmryth Reach",
      env: {
        ...childEnvironment,
        HELMRYTH_PORT: String(harnessPort),
        HELMRYTH_COMPANION_PORT: String(COMPANION_PORT),
        HELMRYTH_CONTROL_PORT: String(CONTROL_PORT),
      },
    ...
    cleanupOrigin();
    lastError = "Helmryth Reach could not be started";
    return companionState();
  }
  child.stdout?.on("data", (d) => log?.(`[reach] ${String(d).trimEnd()}`));
  child.stderr?.on("data", (d) => log?.(`[reach err] ${String(d).trimEnd()}`));
  ...
  for (let i = 0; i < 40; i++) {
    if (exited) {
      lastError = "Helmryth Reach could not start — check the log";
      return companionState();
    }
    try {
      ...
      if (state?.pid !== undefined && child.pid !== undefined && state.pid !== child.pid) {
        ...
        lastError = `port ${CONTROL_PORT} is already serving another Reach service — stop it and try again`;
        return companionState();
      }
  ...
  lastError = "Helmryth Reach did not come up in time";
  return companionState();
}
...
export async function companionState() {
  ...
  try {
    ...
    return {
      ...
      connectedDeviceIds: [],
      pairing: null,
      error: "Helmryth Reach is not responding",
    };
  }
}
...
export async function companionPairing(open, expectedToken) {
  ...
  const token = /^hry_pair_[A-Za-z0-9_-]{43}$/.test(candidate)
    ? candidate
