...
// extension, so the two agree on what an operator's island says.
//
// One activity per operator that is doing something: needs you, or running.
// Quiet operators have no activity, the same rule as the Updates pill.
import ActivityKit
import AppIntents
import Foundation
...
struct BotActivityAttributes: ActivityAttributes {
    public struct ContentState: Codable, Hashable {
        /// `SigilState.rawValue` — the face to wear.
        var face: String
        /// "needsYou" | "working" | "toReview"
        var kind: String
        /// "Scout reached a gate", "Forge is running"
        var headline: String
