// Keeping the Dynamic Island in step with the operators.
//
// One Live Activity per operator that is doing something — needs you, or running
...
// launch if the operator has moved on.
import ActivityKit
import Combine
import Foundation
...
final class LiveActivityCoordinator {
    ...
    /// When each operator's current kind began, so an update does not reset the clock.
    ...
    private func sync(_ state: CompanionState) {
        ...
        for update in wanted {
            ...
            let face = OperatorSignalState.forBot(bot, last: state.visibleTranscript(forThread: bot.threadId).last)
            ...
            let content = BotActivityAttributes.ContentState(
                ...
                headline: update.kind == .needsYou ? "\(bot.name) reached a gate" : "\(bot.name) is running",
            ...
            // An operator stopping for you is worth an alert: the island pops
            // open on its own and the lock screen lights up. Ordinary running is not.
        ...
        // operators that went quiet: let the island go
