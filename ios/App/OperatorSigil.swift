// The operator mark — the same silhouette the desktop draws.
//
// The desktop renders Blob Studio's "cursor" sigil from an SVG path
// (`src/components/CursorAvatar.tsx`, `SHAPE.body`). The phone used to draw a
// rounded blob instead, which meant an operator you know by its shape looked
// like a different one on the two screens. This is that path, verbatim.
//
...
enum HelmrythPalette {
    /// Shared desktop operator palette copied into the iOS mark renderer.
...
/// The sigil silhouette, as an SVG path. Absolute `M`, `C` and `Z` only —
/// which is what makes the parser below twenty lines rather than a library.
enum SigilSilhouette {
    static let path =
        """
    ...
    static func fit(_ rect: CGRect) -> CGAffineTransform {
        let side = SigilFaceData.faceBox + margin * 2
...
/// An operator mark, at whatever size the row needs — and alive, exactly the
/// way it is
/// on the desktop. `SigilFaceEngine` is a port of the frame loop in
/// `SigilSigilios/OperatorSigil.tsx`: a state picks a pool of expressions and
...
struct OperatorSigil: View {
    ...
    /// Pass true only where motion carries meaning — a busy operator, an open
    ...
    @State private var engine = SigilFaceEngine()
...
struct OperatorSigilStill: View {
    let color: String
    var state: OperatorSignalState = .idle
    var size: CGFloat = 52
    var comets: Bool = false
...
/// The face, frame by frame. One per drawn sigil; holds the morph in
/// progress, the blink, and when the next expression or blink is due.
final class SigilFaceEngine {
    private(set) var state: OperatorSignalState = .idle
    private var expression = 0
    private var currentRings: [[CGPoint]] = [SigilFaceData.ring(0, eye: 0), SigilFaceData.ring(0, eye: 1)]
    private var targetRings: [[CGPoint]] = [SigilFaceData.ring(0, eye: 0), SigilFaceData.ring(0, eye: 1)]
    private var currentMouth = SigilFaceData.mouth(0)
    private var targetMouth = SigilFaceData.mouth(0)
    private var currentGaze = SigilFaceData.gaze(0)
    private var targetGaze = SigilFaceData.gaze(0)
    ...
    func setState(_ new: OperatorSignalState, now: Date) {
        guard !started || new != state else { return }
        started = true
        state = new
        stateStart = now
        select(SigilFaceData.pools[new]?.first ?? 0)
        if last == nil { morph = 1; velocity = 0 } // first frame: rest on it, no morph in
        nextExpressionAt = schedule(SigilFaceData.expressionCadence[new], from: now)
        nextBlinkAt = schedule(SigilFaceData.blink[new], from: now)
    }
    ...
    private func select(_ index: Int) {
        let i = ((index % SigilFaceData.expressionCount) + SigilFaceData.expressionCount) % SigilFaceData.expressionCount
        if i == expression && morph >= 1 { return }
        currentRings = displayedRings()
        currentMouth = displayedMouth()
        currentGaze = displayedGaze()
        targetRings = [SigilFaceData.ring(i, eye: 0), SigilFaceData.ring(i, eye: 1)]
        targetMouth = SigilFaceData.mouth(i)
        targetGaze = SigilFaceData.gaze(i)
        expression = i
        morph = 0
        velocity = 0
    }
    ...
    func step(now: Date) {
        ...
        if let due = nextExpressionAt, now >= due {
            let pool = SigilFaceData.pools[state] ?? [0]
            let alternatives = pool.filter { $0 != expression }
            select(alternatives.randomElement() ?? pool[0])
            nextExpressionAt = schedule(SigilFaceData.expressionCadence[state], from: now)
        }
        if let due = nextBlinkAt, now >= due {
            blinkStart = now
            nextBlinkAt = schedule(SigilFaceData.blink[state], from: now)
        }
    }
    ...
    static let cometTrails: [OperatorSignalState: CometSpec] = [
    ...
    func draw(in context: inout GraphicsContext, size: CGSize, color: String, bodyMotion: Bool, comets: Bool = false, at now: Date = Date()) {
        ...
        context.concatenate(SigilSilhouette.fit(rect))
        ...
        let centre = CGPoint(x: SigilFaceData.faceBox / 2, y: SigilFaceData.faceBox / 2)
        ...
        if bodyMotion {
            bodyContext.concatenate(bodyTransform(SigilFaceData.motion[state] ?? SigilBodyMotion(), elapsed: elapsed))
        }
    ...
    private func drawBody(in context: inout GraphicsContext, color: String, now: Date) {
        let body = SigilSilhouette.inFaceBox
        let bounds = SigilSilhouette.faceBoxBounds
        context.fill(body, with: .linearGradient(
            Gradient(stops: HelmrythPalette.gradientStops(color)),
        ...
        let a = SigilFaceData.anchor
        context.translateBy(x: a.x, y: a.y)
        context.scaleBy(x: a.scale, y: a.scale)
        context.translateBy(x: -SigilFaceData.faceCentre.x, y: -SigilFaceData.faceCentre.y)
        ...
        context.stroke(
            ...
            with: .color(.white),
            style: StrokeStyle(lineWidth: SigilFaceData.mouthStroke, lineCap: .round)
        )
    }

    /// The desktop's `bodyTransform`, in face-box units, elapsed in ms.
    private func bodyTransform(_ m: SigilBodyMotion, elapsed: CGFloat) -> CGAffineTransform {
        let centre = SigilFaceData.faceBox / 2
        let ground = SigilFaceData.faceBox
...
/// The person, not an operator — the roster header and the settings row. A
/// letter rather than a sigil, deliberately: the sigils mean "this is an operator", and
...
struct ProfileAvatar: View {
    ...
    var body: some View {
        Circle()
            .fill(HelmrythPalette.color("green"))
            .frame(width: size, height: size)
            .overlay {
                Text(initial)
