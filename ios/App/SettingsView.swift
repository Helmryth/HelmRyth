    ...
    var body: some View {
        Form {
            ...
            Section {
                ...
                Text("Alerts arrive while Helmryth is open or was recently in the background. Closed-app delivery is not available yet.")
            }
        ...
        HStack(spacing: 12) {
            ZStack {
                RoundedRectangle(cornerRadius: 10, style: .continuous)
                    .fill(HelmrythPalette.color("blue").opacity(0.14))
...
struct ConnectionSecurityView: View {
    ...
    var body: some View {
        Form {
            if let connection = session.connection {
                ...
                ContentUnavailableView("No workbench connected", systemImage: "laptopcomputer.slash")
            }
        }
        .navigationTitle("Connection & Security")
        .navigationBarTitleDisplayMode(.inline)
        .alert("Edit address", isPresented: $editingAddress) {
            TextField("Workbench address", text: $addressText)
                .textInputAutocapitalization(.never)
                .autocorrectionDisabled()
            Button("Save") {
                if !session.updateAddress(addressText) {
                    session.actionError = "That address doesn't look right. Copy it from Reach settings and try again."
                }
            }
            Button("Cancel", role: .cancel) {}
        } message: {
            Text("Use the address shown in Reach settings on your workbench. Your pairing is kept.")
        }
        .confirmationDialog(
            ...
            Text("This removes the connection from this iPhone only. It does not revoke this phone on your Mac. To remove Mac-side access, open Helmryth → Settings → Reach and remove this device.")
        }
    }
    ...
    private var troubleshootingText: String {
        ...
        case .live:
            return "This workbench is connected and responding normally."
        case .connecting:
            return "Helmryth is trying the saved connection automatically."
        case let .offline(reason):
            return reason
        case .unauthorized:
            return "This phone was removed from the workbench. Pair it again to reconnect."
        case .unpaired:
            return "This phone is not paired with a workbench."
        }
    }
