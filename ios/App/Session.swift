...
private let log = Logger(subsystem: "com.helmryth.companion", category: "stream")
...
final class Session: ObservableObject {
    enum Status: Equatable {
        ...
        case connecting
        case live
        /// The token stopped working — revoked on the workbench, most likely.
        case unauthorized
        case offline(String)
    }
    ...
    /// How many workbench panels are open. A count rather than a flag: the
    ...
    private func restore() {
        ...
        do {
            ...
            // costs a walk to the workbench.
            connection = saved
            restorePending = true
            status = .offline(
                (error as? KeychainError)?.isLocked == true
                    ? "Unlock this phone to reach your workbench."
    ...
    func pair(
        ...
        // prefer the name the workbench calls itself over the Bonjour label
        var stored = outcome.connection
        if !paired.serverName.isEmpty { stored.name = paired.serverName }
        // The workbench knows every address it answers on, but redemption may
    ...
    func receivePairingURL(_ url: URL) {
        guard CompanionPairingInvitePolicy.allowsIncomingInvite(
            ...
            actionError = "This phone is already paired. Unpair it in Settings before connecting it to another workbench."
            return
        }
        guard let invite = PairingInvite.parse(url) else {
            actionError = "That pairing invitation is not valid. Start pairing again on your workbench."
            return
        }
        pairingInvite = CompanionPairingInvitePolicy.nextInvite(
            current: pairingInvite,
    ...
    /// Ask the harness to include this operator's workbench in the stream, for as
    ...
    private func hydrate() async throws {
        ...
        log.info("hydrated \(fleet.bots.count, privacy: .public) operators, \(fleet.groups.count, privacy: .public) crews")
        state.hydrate(fleet)
        NotificationCoordinator.shared.setBadge(state.unreadCount)
    }
    ...
    /// Make a new operator. The harness chooses its name, colour and greeting,
    /// so one made here is indistinguishable from one made on the desktop.
    ///
    /// Creating an operator does not broadcast — the desktop adds it
    /// optimistically too — so the new operator is folded in here rather than
    /// waited for. Return
    ...
    /// Make a crew from the phone. Same shape as `createBot`: fold it in
    ...
    func cloudDesktop(for bot: Bot) async throws -> URL {
        guard let client else { throw APIError.transport("This workbench is offline.") }
        do {
    ...
    func previewVoice(_ voiceId: String, for bot: Bot) async -> Data? {
        guard let client else { return nil }
        do { return try await client.previewVoice(text: "Hello, \(bot.name) here.", voiceId: voiceId) }
    ...
    func openNotification(_ target: NotificationTarget) async {
        guard let client else {
            ...
            if restorePending {
                ...
                actionError = "Pair this phone with your workbench to open that run."
            }
            return
        }
        pendingNotification = nil
        do {
            ...
            guard var selected = bot else { throw APIError.status(code: 404, message: "That operator no longer exists.") }
...
/// A chat is an operator or a crew. They share a thread, which is what every
/// message, gate, and page is keyed by.
enum Chat: Identifiable, Hashable {
    ...
    var subtitle: String {
        ...
        case let .room(room): return "\(room.memberIds.count) operators"
        }
    }
...
extension CompanionState {
    ...
    /// then most recently active. Hidden operators stay hidden.
    ///
