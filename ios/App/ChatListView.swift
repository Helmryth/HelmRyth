// The roster.
//
// Messages-shaped: a glass header, your crews across the top, every operator
// below with the unread dot in that operator's own colour at the left edge,
// and a glass bar floating at the bottom. The bar's pill is Updates — only
// the operators that need you, are running, or have something you have not
// read — beside round search and new-operator buttons. Everything scrolls
// under the glass, which is the whole point of the glass.
import SwiftUI
...
struct ChatListView: View {
    ...
    /// Driven so that making an operator can open it. Value-based navigation alone
    /// cannot push without a tap, and a new operator appearing silently at the
    ...
    var body: some View {
        NavigationStack(path: $path) {
                        ...
                        ForEach(Array(rows.enumerated()), id: \.element.id) { index, summary in
                            NavigationLink(value: summary.chat) {
                                ChatRow(
                                    ...
                                    state: OperatorSignalState.forChat(summary.chat, in: session.state),
                ...
                .overlay {
                    if chats.isEmpty && searchHits.isEmpty {
                        ContentUnavailableView(
                            query.isEmpty ? "No operators yet" : "Nothing matches",
                            systemImage: query.isEmpty ? "bubble.left.and.bubble.right" : "magnifyingglass",
                            description: Text(
                                query.isEmpty
                                    ? "Operators you create on your workbench show up here."
            ...
            // an operator that stopped for you grows out of the island
    ...
    /// The paired workbench's profile on the left, one settings action on the
    ...
    private var header: some View {
        HStack(alignment: .center) {
            ProfileAvatar(name: session.connection?.name ?? "You", size: 30)
                ...
                .accessibilityLabel("Connected to \(session.connection?.name ?? "your workbench")")
        ...
        GlassGroup(spacing: 8) {
            HStack(spacing: 8) {
                if searchOpen {
                    ...
                    .accessibilityLabel("New operator")
                }
            }
        }
        .padding(.horizontal, 16)
    ...
    private var chats: [ChatSummary] {
        ...
        guard !query.isEmpty else {
            // crews live in the strip; the list is operators
...
/// A crew as a round tile: the first two members' marks stacked, its name
        ...
        VStack(spacing: 7) {
            ZStack {
                if let room {
                    ...
                    if room.unread {
                        Circle()
                            .fill(HelmrythPalette.color("blue"))
...
struct ChatRow: View {
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 0) {
            // the unread dot, in the operator's own colour, at the very edge
            ZStack {
                if chat.unread && !chat.busy {
                    Circle()
                        .fill(HelmrythPalette.color(chat.color))
                        .frame(width: 10, height: 10)
                }
            }
            .frame(width: 22)
            ...
            HStack(alignment: .top, spacing: 14) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    HStack(spacing: 8) {
                        ...
                        // the operator's role, the way the desktop shows it
                    ...
                    HStack(alignment: .top, spacing: 8) {
                        // one line for every operator, so the rows keep one rhythm
                    ...
                    if waiting {
                        Label("Waiting on you", systemImage: "hand.raised.fill")
                            ...
                            .padding(.vertical, 4)
                            .background(Capsule().fill(HelmrythPalette.color(chat.color)))
                            .padding(.top, 4)
                    }
                }
                .padding(.vertical, 12)
...
struct UpdatesPill: View {
    ...
    var body: some View {
        Button(action: action) {
            HStack(spacing: 8) {
                if !updates.isEmpty {
                    SigilStack(colors: Array(updates.prefix(3).map(\.chat.color)))
                }
                        ...
                        if let first = updates.first {
                            ...
                            case .needsYou:
                                Image(systemName: "hand.raised.fill")
                                    ...
                                    .foregroundStyle(HelmrythPalette.color(first.chat.color))
...
/// Up to three sigils overlapping, the way a group of faces reads at a glance.
struct SigilStack: View {
    let colors: [String]
    var size: CGFloat = 28
    ...
    var body: some View {
        HStack(spacing: -overlap) {
            ForEach(Array(colors.enumerated()), id: \.offset) { _, color in
                OperatorSigil(color: color, size: size, state: .idle, animated: false)
                    .padding(2)
                    .background(Circle().fill(Color(uiColor: .systemBackground)))
            }
        }
...
struct StatusBanner: View {
    ...
    var body: some View {
        Group {
            ...
            case .unauthorized:
                banner("This phone was unpaired on the workbench.", systemImage: "lock.slash", tint: .red)
            }
        }
