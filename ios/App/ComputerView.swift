// An operator's workbench, live.
//
// The harness already screenshots a running operator every few seconds and pushes
...
struct ComputerView: View {
    ...
    /// The operator as the stream last described it — `busy` is what tells us
    ...
    var body: some View {
        ZStack {
            ...
            if let image = frame.flatMap(\.data).flatMap(UIImage.init(data:)) {
                Image(uiImage: image)
                    ...
                    .accessibilityLabel("\(current.name)'s workbench")
            } else {
                waiting
            }
        }
        ...
        .safeAreaInset(edge: .bottom) {
            // A VPS-backed operator is "cloud" too, but the server refuses to mint
                ...
                VStack(spacing: 8) {
                    ...
                    Button {
                        ...
                        if openingDesktop {
                            ...
                            Label("Open live cloud workbench", systemImage: "display")
                                .frame(maxWidth: .infinity)
                        }
                    }
                    .buttonStyle(.borderedProminent)
                    .disabled(openingDesktop)
                    Text("Interactive VNC session. Access must be enabled for this phone in the Mac's Reach settings.")
                        .font(.caption)
        ...
        .alert("Open live cloud workbench?", isPresented: $confirmingDesktop) {
            Button("Cancel", role: .cancel) {}
            Button("Open workbench") { Task { await openDesktop() } }
        } message: {
            Text("This gives this phone full control of the cloud workbench, including anything signed in inside it.")
        }
        .sheet(
            isPresented: Binding(
                get: { desktopURL != nil },
    ...
    private var waiting: some View {
        VStack(spacing: 12) {
            ...
            // An idle operator is not being screenshotted at all, so this would
            // otherwise be an indefinite spinner with no explanation.
            if current.busy != true {
                Text("This operator's workbench is only captured while it is running.")
                    .font(.system(size: 13))
