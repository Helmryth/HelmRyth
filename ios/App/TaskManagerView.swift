...
/// Separate contexts for either an operator or a crew. Keeping one sheet for
/// both makes "run" mean the same operation everywhere in the app.
struct TaskManagerView: View {
    ...
    var body: some View {
        NavigationStack {
                ...
                Section {
                    HStack(spacing: 12) {
                        ...
                        VStack(alignment: .leading, spacing: 2) {
                            ...
                            Text(current.isBot ? "Operator runs" : "Crew runs")
                                .font(.subheadline).foregroundStyle(.secondary)
                        }
                    }
                } footer: {
                    Text("A run is one conversation and result, with its own context and working folder.")
                }
                        ...
                        Button {
                            ...
                            HStack {
                                VStack(alignment: .leading, spacing: 3) {
                                    Text(task.title.isEmpty ? "Untitled run" : task.title)
            ...
            .navigationTitle("\(current.name)’s runs")
        ...
        .alert("New run", isPresented: $showingNewTask) {
