...
// sigil engine does the rest — it already morphs every state and
                ...
                if let shown {
                    VStack(spacing: 10) {
                        ...
                        Button { open(shown.chat) } label: {
                            ChatAvatarView(chat: shown.chat, size: 120, state: OperatorSignalState.forChat(shown.chat, in: session.state), animated: attentionLive, comets: attentionLive)
                        }
                        .buttonStyle(.plain)
                        ...
                        VStack(spacing: 4) {
                            Label("\(shown.chat.name) needs you", systemImage: "hand.raised.fill")
                                ...
                                .foregroundStyle(HelmrythPalette.color(shown.chat.color))
                                ...
                                ForEach(card.options, id: \.self) { option in
                                    Button {
                                        ...
                                        Text(option)
                                            ...
                                            .frame(height: 40)
                                            .background(
                                                Capsule().fill(CardStyle.isRefusal(option) ? Color.white.opacity(0.16) : HelmrythPalette.color(shown.chat.color))
                                            )
                                    }
                                    .buttonStyle(.plain)
                                    .disabled(answering)
