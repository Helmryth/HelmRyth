...
struct PairingView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle("Connect workbench")
            ...
            .fullScreenCover(isPresented: $showingScanner) {
                PairingScannerSheet { payload in
                    guard let url = URL(string: payload), let invite = PairingInvite.parse(url) else {
                        return "That isn't an Helmryth pairing QR code."
                    }
                    accept(invite)
                    return nil
                }
        ...
        VStack(spacing: 16) {
            ZStack {
                RoundedRectangle(cornerRadius: 28, style: .continuous)
                    .fill(HelmrythPalette.color("blue").opacity(0.12))
                    .frame(width: 124, height: 124)
                Image(systemName: "laptopcomputer.and.iphone")
                    .font(.system(size: 46, weight: .medium))
                    .foregroundStyle(HelmrythPalette.color("blue"))
            }
            .accessibilityHidden(true)

            VStack(spacing: 8) {
                Text("Connect to your workbench")
                    .font(.title.bold())
                    .multilineTextAlignment(.center)
                Text("Scan the QR code in Helmryth. We'll securely choose the best way to connect.")
                    .font(.body)
    ...
    private var qrAction: some View {
        VStack(spacing: 12) {
            ...
            Text("On your workbench, open Settings → Reach → Set up a phone.")
                .font(.footnote)
                .foregroundStyle(.secondary)
                .multilineTextAlignment(.center)
        }
    ...
    @ViewBuilder
    private var discoveredWorkbenches: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack {
                Label("Nearby workbenches", systemImage: "desktopcomputer")
                    .font(.subheadline.weight(.semibold))
                Spacer()
                if discovery.browsing && discovery.found.isEmpty {
                    ProgressView()
                        .controlSize(.small)
                        .accessibilityLabel("Looking for workbenches")
                }
            }
            ...
            if let discoveryFailure = discovery.failure {
                ...
                Text("Workbenches ready to pair will appear here.")
                    .font(.footnote)
                    .foregroundStyle(.secondary)
            } else {
                ForEach(discovery.found) { service in
                    ...
                    .buttonStyle(.plain)
                    .accessibilityHint("Enter the code shown on this workbench")
                }
            }
        }
    }
    ...
    private var manualEntry: some View {
        VStack(alignment: .leading, spacing: 12) {
            TextField("Workbench address", text: $manualAddress)
            ...
            Text("Use the address shown in Reach settings on your workbench.")
                .font(.footnote)
                .foregroundStyle(.secondary)
            ...
            Button("Continue") {
                ...
                guard let connection = Self.parse(manualAddress) else {
                    failure = "That address doesn't look right. Copy it from Reach settings and try again."
                    return
                }
                choiceGeneration += 1
    ...
    private func confirmationView(for connection: Connection) -> some View {
        ...
        VStack(spacing: 22) {
            ZStack {
                Circle()
                    .fill(HelmrythPalette.color("green").opacity(0.12))
                    .frame(width: 92, height: 92)
                Image(systemName: "desktopcomputer")
                    .font(.system(size: 36, weight: .medium))
                    .foregroundStyle(HelmrythPalette.color("green"))
            }
            .accessibilityHidden(true)
            ...
            VStack(alignment: .leading, spacing: 7) {
                ...
                Text(connection.pairingConsentOrigin)
                    ...
                    .fixedSize(horizontal: false, vertical: true)
                Text("Make sure this is the workbench you expect before connecting.")
                    .font(.footnote)
                    .foregroundStyle(.secondary)
            }
            ...
            if let credential = scannedCredential {
                if !connectionIsProtected(connection) {
                    Text("Only continue on a network you trust. Local connections are authenticated but are not encrypted by Helmryth.")
                        .font(.footnote)
                        .foregroundStyle(.secondary)
                        .multilineTextAlignment(.center)
                        .fixedSize(horizontal: false, vertical: true)
                ...
                VStack(spacing: 12) {
                    Text("Enter the 6-digit code shown on your workbench")
                        .font(.subheadline)
                        .foregroundStyle(.secondary)
            ...
            Button("Choose a different workbench") {
                Haptics.selection()
                chosen = nil
                code = ""
                scannedCredential = nil
        ...
        do {
            ...
            if cameFromScanner {
                if error is PairingRouteError {
                    ...
                    failure = "\(error.localizedDescription) Start pairing again on your workbench and rescan the new QR code."
                    chosen = nil
                    scannedCredential = nil
                    pairRequestId = nil
                }
