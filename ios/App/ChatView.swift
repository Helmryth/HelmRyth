...
struct ChatView: View {
    ...
    var body: some View {
        ...
        VStack(spacing: 0) {
            ScrollViewReader { proxy in
                ScrollView {
                    ...
                    VStack(alignment: .leading, spacing: 6) {
                        ...
                        if let live = session.state.streaming[threadId], !live.isEmpty {
                            ...
                            TypingIndicatorView(tintColor: HelmrythPalette.color(current.color))
                                .id(Self.liveBubbleId)
                                .accessibilityLabel("\(current.name) is running")
                        }
                    }
                ...
                // The blur is only the top strip — back, workbench — the way
                ...
                .overlay(alignment: .top) {
                    ...
                    ZStack(alignment: .top) {
                        ...
                        ChatAvatarView(chat: current, size: faceSize, state: OperatorSignalState.forChat(current, in: session.state), animated: OperatorSignalState.forChat(current, in: session.state).showsActivity || islandExpanded, comets: islandExpanded)
    ...
    /// Back on the left with the rest-of-app unread count, the operator's
    /// workbench on the right — a blurred strip to the top edge.
    private var headerBar: some View {
        HStack(alignment: .top) {
            ...
            if case .bot = current {
                ...
                .accessibilityLabel("Watch \(current.name)'s workbench")
            } else {
    ...
    private var plusActions: [PlusAction] {
        ...
        if case let .bot(bot) = current {
            out.append(PlusAction(
                id: "task", systemImage: "plus.square.on.square", title: "New run",
                subtitle: "Start a fresh run with \(bot.name)", disabled: bot.busy == true
            ) { Task { await session.createTask(for: bot, title: nil) } })
            out.append(PlusAction(
                id: "tasks", systemImage: "square.stack", title: "Runs",
                subtitle: "Switch, rename or remove one"
            ) { showingTasks = true })
            out.append(PlusAction(
                id: "computer", systemImage: "display", title: "Watch workbench",
        ...
        if case let .room(room) = current, room.dm != true {
            out.append(PlusAction(
                id: "task", systemImage: "plus.square.on.square", title: "New run",
                subtitle: "Start a fresh run in \(room.name)",
        ...
        VStack(spacing: 6) {
            ...
            if showCommandHUD {
                CommandSkillHUDView(
                    ...
                    accentColor: HelmrythPalette.color(current.color)
                ) { command in
                    switch command.id {
                    case "computer":
                        draft = ""
                ...
                PredictiveActionChipsView(accentColor: HelmrythPalette.color(current.color)) { chip in
                    submit(chip.prompt)
                }
                .transition(.opacity)
            }
        ...
        HStack(alignment: .bottom, spacing: 0) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                if let speaker, !mine {
                    Text(speaker.name)
                        .font(.system(size: 13, weight: .semibold))
                        .foregroundStyle(HelmrythPalette.color(speaker.color))
                }
                // Operators get markdown, you do not — the same split the desktop
...
struct CardView: View {
    ...
    private var tint: Color { HelmrythPalette.color(chat.color) }
...
/// A frame of the operator's workbench. In the paged shape the pixels are not
/// in the transcript — they are fetched here, once, when the row appears.
            ...
            VStack(alignment: .leading, spacing: 4) {
                if let reasoning, !reasoning.isEmpty, text?.isEmpty != false {
                    AgentThoughtChamberView(
                        ...
                        botName: "Bot",
                        sigilColor: HelmrythPalette.color(color),
                        isStreaming: true
                    )
                }
