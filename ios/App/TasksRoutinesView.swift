...
struct TasksRoutinesView: View {
    ...
    var body: some View {
        List {
            Section {
                VStack(alignment: .leading, spacing: 8) {
                    Label("Run = one conversation and result", systemImage: "bubble.left.and.text.bubble.right")
                    Label("Cadence = a schedule that creates a fresh run", systemImage: "calendar.badge.clock")
                }
                .font(.subheadline)
            } footer: {
                Text("No cron syntax. Every run uses the operator's existing model, methods, permissions, workbench, and capabilities. Times follow the paired workbench's local timezone.")
            }

            Section("Cadences") {
                if routines.isEmpty && !loading {
                    ContentUnavailableView("No cadences", systemImage: "calendar.badge.plus", description: Text("Schedule recurring or one-time operator work."))
                }
            ...
            Section {
                Label("Workbench only", systemImage: "lock.desktopcomputer")
                    .foregroundStyle(.secondary)
            } header: {
                Text("Webhooks")
            } footer: {
                Text("Creating or rotating a webhook changes an internet-reachable trigger and signing secret, so webhook management remains on the paired workbench. Webhook run receipts still appear above.")
            }
        }
        .navigationTitle("Runs & Cadences")
        .toolbar {
            ToolbarItem(placement: .primaryAction) {
                Button("New cadence", systemImage: "plus") { editor = .new }
            }
        }
        .task { await reload() }
        ...
        .confirmationDialog(
            "Delete \(deleting?.name ?? "this cadence")?",
    ...
    var body: some View {
        ...
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 3) {
                ...
                Text("\(bot?.name ?? "Deleted operator") · \(routine.schedule.summary) · \(routine.runLocation.label)")
    ...
    var body: some View {
        DisclosureGroup {
            VStack(alignment: .leading, spacing: 8) {
                ...
                if run.status == "waiting" { Text("This run is waiting for your answer.").foregroundStyle(.orange) }
                if let threadId = run.threadId,
                   let target = NotificationTarget(botId: run.botId, threadId: threadId) {
                    Button("Open run", systemImage: "arrow.up.right.square") {
            ...
            HStack {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    Text("\(bot?.name ?? "Deleted operator") · \(Date(timeIntervalSince1970: run.scheduledFor / 1_000).formatted(date: .abbreviated, time: .shortened))")
...
private struct RoutineEditorView: View {
        ...
        NavigationStack {
            Form {
                Section("Work") {
                    ...
                    Picker("Operator", selection: $botId) {
                        Text("Choose an operator").tag("")
                        ForEach(session.state.bots.filter { $0.hidden != true }) { bot in Text(bot.name).tag(bot.id) }
                    }
                    TextField("What should the operator do?", text: $prompt, axis: .vertical).lineLimit(4...10)
                    Stepper("Allow up to \(duration) minutes", value: $duration, in: 15...240, step: 15)
                }

                Section {
                    Picker("Run location", selection: $runOn) {
                        Label("This workbench", systemImage: "laptopcomputer")
                    ...
                    if runOn == .local {
                        Text("Uses this operator's selected model and workbench setting on the paired workbench.")
                    } else if runAvailability?.cloudReady == true {
                        Text("Runs the operator and its methods inside its Box virtual machine. The VM wakes automatically for each run; keep Helmryth running so its scheduler can launch the job.")
                    } else {
                        Text("This existing Cloud VM choice is preserved, but it cannot run until the paired workbench has a configured Box API key and an available Box operator.")
                    }
                }
                ...
                Section {
                    ...
                    if kind == .once {
                        ...
                        Label(
                            "This cadence uses a schedule added by a newer Helmryth. Choose One time or Selected days before saving.",
            ...
            .navigationTitle(routine == nil ? "New cadence" : "Edit cadence")
    ...
    private func save() async {
        guard kind != .unknown else {
            session.actionError = "Choose a supported schedule before saving this cadence."
            return
        }
        saving = true
