...
struct ConnectedAppsView: View {
    ...
    /// The last inventory the workbench vouched for, or `nil` if it never has.
    ...
    var body: some View {
        List {
            if credentialStoreUnreadable {
                Section {
                    ...
                    if statuses == nil {
                        Text("Your workbench could not open its credential store, so it cannot say which accounts are connected. Nothing has been disconnected — restarting Helmryth on your workbench usually clears this.")
                            .font(.caption)
                            .foregroundStyle(.secondary)
                    } else {
                        Text("Showing what was connected last time. Your workbench could not open its credential store just now, so these could not be re-checked. Nothing has been disconnected — restarting Helmryth on your workbench usually clears this.")
                            .font(.caption)
                            .foregroundStyle(.secondary)
                    }
                }
            ...
            // the workbench applies — `!configured && !stale`.
            if catalog?.configured == false, !credentialStoreUnreadable {
                Section {
                    ContentUnavailableView(
                        "Capabilities need setup",
                        systemImage: "link.badge.plus",
                        description: Text("Configure Composio on your workbench first. Provider credentials are never returned to this phone.")
                    )
                }
            }
        ...
        .alert("Account alias", isPresented: Binding(
            ...
            Text("An alias makes the account explicit when an operator uses more than one \(aliasCard?.label ?? "app") account.")
        }
    }
