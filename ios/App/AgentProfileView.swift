...
/// The paired-safe subset of an operator profile. Shared provider keys remain
/// on the workbench; the phone sees only configured/not-configured status and
/// the renderer-neutral voice/avatar operations.
struct AgentProfileView: View {
    ...
    init(bot: Bot) {
        ...
        _crop = State(initialValue: bot.avatarCrop ?? .operatorMark)
            ...
            Form {
                Section {
                    ...
                    if current.avatarUrl != nil {
                        Button("Use operator mark", systemImage: "trash", role: .destructive) {
                    ...
                    Text("PNG, JPEG, GIF, or WebP, up to 10 MB. Images are stored on your paired workbench and loaded with this phone's pairing token.")
                }

                Section {
                    TextField("Art direction", text: $prompt, axis: .vertical)
                        .lineLimit(2...5)
                    Button("Generate on workbench", systemImage: "sparkles") {
                        Task { await generateImage() }
                    }
                    .disabled(busy || !imageGenerationReady || prompt.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty)
                } header: {
                    Text("Generate an operator mark")
                } footer: {
                    Text(imageGenerationReady
                         ? "Generation uses the shared image provider configured on your workbench. No provider key is sent to or stored on this phone."
                         : "To generate images, configure the shared image provider in Helmryth on your workbench. Provider keys cannot be added from a phone.")
                }
                ...
                Section("Identity") {
                    ...
                    TextField("What this operator does", text: $description, axis: .vertical)
                        .lineLimit(3...8)
                    Toggle("Operator notifications", isOn: $notifications)
                }
                ...
                Section {
                    if voiceConfigured {
                        Picker("Voice", selection: $voice) {
                            if hasWorkspaceDefaultVoice {
                                ...
                                Text("Choose an operator voice").tag("").disabled(true)
                            }
                            if !voice.isEmpty, !voices.contains(where: { $0.id == voice }) {
                                Text("Current operator voice").tag(voice)
                            }
                        ...
                        if !hasWorkspaceDefaultVoice, voice.isEmpty {
                            Label("Pick a voice for this operator before enabling speech.", systemImage: "info.circle")
                                .font(.footnote)
                    ...
                    if !voiceConfigured {
                        ...
                        // workbench has no built-in voices to speak with.
                        if usesSystemVoices {
                            Text("Built-in Mac voices need no key, and this workbench has none available. Switch the voice engine to ElevenLabs in this operator's profile on the workbench to keep using voice.")
                        } else {
                            Text("Add the shared ElevenLabs key in this operator's profile on the workbench. The key is never returned to iOS.")
                        }
                    } else if !hasWorkspaceDefaultVoice {
                        if usesSystemVoices {
                            Text("No workspace default voice is selected. Choose an operator-specific voice above; synthesis still uses the built-in Mac voices on your workbench.")
                        } else {
                            Text("No workspace default voice is selected. Choose an operator-specific voice above; synthesis still uses the shared ElevenLabs key on your workbench.")
                        }
                    } else {
                        Text("The voice choice belongs to this operator. Workspace default uses the shared voice selected on your workbench.")
                    }
                }
    ...
    private func clearImage() async {
        ...
        if let updated = await session.updateProfile(
            BotProfilePatch(avatarUrl: .clear, avatarCrop: .operatorMark),
            for: current
        ) {
            crop = updated.avatarCrop ?? .operatorMark
            baseline.crop = crop
        }
    }
    ...
    private func upload(_ item: PhotosPickerItem) async {
        ...
        let intendedCrop = crop == .operatorMark ? AvatarCrop.circle : crop
    ...
    private func generateImage() async {
        ...
        if let updated = await session.updateProfile(shapePatch, for: generated) {
            ...
            crop = generated.avatarCrop ?? .operatorMark
            baseline.crop = crop
        }
    }
    ...
    private func previewVoice() async {
        guard selectedVoiceCanSpeak else {
            session.actionError = "Pick an operator voice or configure a workspace default on your workbench first."
            return
        }
        busy = true
        defer { busy = false }
    ...
    private func synchronizeForm(with bot: Bot) {
        ...
        crop = bot.avatarCrop ?? .operatorMark
        voice = bot.voice ?? ""
...
private extension AvatarCrop {
    var label: String {
        switch self {
        case .operatorMark: "Operator mark"
        case .circle: "Circle"
