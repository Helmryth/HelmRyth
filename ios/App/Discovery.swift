// Finding workbenches on the network, so nobody types an IP address.
//
// The other half of `server/mdns.ts`: the harness advertises
// `_helmryth._tcp` while its companion listener is up, and this browses
...
final class Discovery: ObservableObject {
    struct Found: Identifiable, Hashable {
        ...
        /// from the profile name, e.g. "Ada Lovelace's workbench".
        let name: String
    ...
    private func startBrowser() {
        ...
        let browser = NWBrowser(
            for: .bonjour(type: "_helmryth._tcp", domain: nil),
            using: parameters
        )
    ...
    private static func failureMessage(for error: NWError) -> String {
        ...
        return "Local discovery isn't available right now. Enter the address shown in Reach settings."
    }
    ...
    func resolve(_ service: Found) async throws -> Connection {
        ...
        let resolved: NWEndpoint = try await withCheckedThrowingContinuation { continuation in
            ...
            connection.stateUpdateHandler = { state in
                ...
                case .ready:
                    if let endpoint = connection.currentPath?.remoteEndpoint {
                        ...
                        finish(.failure(APIError.transport("Couldn't work out that workbench's address.")))
                    }
                case let .failed(error):
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 10) {
                finish(.failure(APIError.transport("That workbench didn't answer.")))
            }
        }

        guard case let .hostPort(host, port) = resolved else {
            throw APIError.transport("Couldn't work out that workbench's address.")
        }
