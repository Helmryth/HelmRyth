# Testing Helmryth Reach locally
...
| 2 — desktop half | + Node 24+, pnpm, and one supported CLI (`claude`, `codex`, or `grok`) signed in |
...
Use the current local checkout after Reach lands. If you need a fresh clone,
use your own canonical repository URL and branch naming:

```sh
git clone <your Helmryth repository URL>
cd Helmryth
...
In the app: **Settings → Reach**. Turn it on. You should see either

- *"Your phone will find this workbench as …"* — Bonjour is advertising, or
...
dns-sd -B _helmryth._tcp                         # macOS: should list the service
```
...
- **The firewall is prompting.** System Settings → Network → Firewall. Incoming
  connections to `node`/Helmryth must be allowed, or the phone reaches
...
```sh
xcodebuild -project HelmrythCompanion.xcodeproj -scheme HelmrythCompanion \
...
**Re-run `../scripts/prepare-xcodegen.sh --run generate` whenever a pull adds a file to `App/`.** The
...
1. **Pair.** In Helmryth → Settings → Reach, choose **Set up a
   phone**. Scan the QR code with the phone's Camera, open Helmryth Mobile,
   confirm that the workbench and six-digit code are filled in, then tap
   **Connect**. The workbench should also appear by name for the manual path:
   ...
   - If the list stays empty, check in this order:
     1. **Local Network permission.** iOS asks once, and a denial is
        permanent and silent. Settings → Helmryth → Local Network. If the
...
3. **Send a message** from the phone. It should appear on the desktop too — same
   harness, two clients.
4. **The gate.** This is the whole product. Ask an operator to do something that
   ...
   unblock the operator on the workbench.
5. **Reconnect.** Background the app for a minute while the operator keeps working,
...
7. **Revoke.** Remove the device in Settings → Reach on the workbench. The
...
3. **In Helmryth → Settings → Reach:** with Phone access on, the panel now
...
token by token as the provider emits them, and each operator has a workbench panel —
...
- **Xcode console**, subsystem `com.helmryth.companion`: `opening stream`,
  then `stream live, resumed=…`, then `hydrated N operators`. Whichever of those is
