# Helmryth Reach Mobile (iOS)

Your operators keep running on the workbench. This is the phone you watch them
from, answer their gates on, and send them the next thing.
...
streaming replies, the workbench view, and — the one that matters — a gate
raised by an operator on the Mac, answered on the phone, with the operator carrying on.
...
ios/
  ...
  App/                           SwiftUI, and everything that needs a device
    ...
    Discovery.swift              NWBrowser for _helmryth._tcp
    Keychain.swift               the device token
    OperatorSigil.swift          the operator mark, in the desktop's palette
    ...
    ChatListView.swift           roster: glass header, crews strip, operators, the Updates bar
    Updates.swift                what the Updates pill shows — only operators doing something
    UpdatesSheet.swift           the pill opened: needs you / working / to review
    NewGroupSheet.swift          make a crew from the phone
    ...
    ComputerView.swift           opt-in live view of an operator's workbench
...
open HelmrythCompanion.xcodeproj
```

**Re-run `../scripts/prepare-xcodegen.sh --run generate` after pulling any change that adds a file to
...
| Allowed | Refused |
|---|---|
| Read operators, crews, and transcripts | Write API keys (`PUT /api/config`) |
| Send messages, make an operator or a crew | Manage pairing or revoke devices |
| **Answer gates and questions** | Drive the Local VM or this workbench |
| Interrupt an operator, mark chats read | Reach `/api/internal/*` |
...
verbs. The sidecar does not expose the general operator or crew `PATCH` routes,
because those can also change execution policy, computers, connected apps, and
working directories.

Reach settings stay on the workbench on purpose: losing the phone must not
...
the host workbench remain unreachable through Reach.
...
- **Messaging-app shape, not settings-list shape.** Operator marks at roster size,
  the operator's role as a chip beside its name, timestamps that say "Yesterday"
  rather than a date, and a gap-based separator in the transcript instead of a
  stamp on every message. The palette in `SigilAvatar.swift` is copied verbatim
  from `src/lib/sigil.ts`: an operator the user knows as "the orange one" should be
...
- **Composer dictation is the mic.** Tap to talk, tap to stop, then edit or
  ...
  branch, and message; the roster's "+" creates the same basic operator the desktop
...
routes and the workbench remains the source of truth. Tailscale is supported
through manual MagicDNS entry; it is not a dependency and Helmryth does not
