...
final class PairingTests: XCTestCase {
    ...
    func testRetryCanReuseTheLogicalRequestIDAfterTheOnlyRouteDropsItsResponse() async throws {
        ...
        XCTAssertEqual(outcome.response.token, "hry_device")
        let pairRequests = PairingRequestStub.captured().filter { $0.url?.path == "/api/pair" }
    ...
    func testRejectsAServiceThatDoesNotIdentifyAsHelmryth() async throws {
    ...
    private static let credential = "hry_pair_" + String(repeating: "a", count: 43)
    private static let health = Data(#"{"app":"helmryth","pid":42,"static":true}"#.utf8)
    private static let paired = Data(
        #"{"token":"hry_device","device":{"id":"d","name":"iPhone","createdAt":1,"lastSeenAt":1},"serverName":"Mac","hosts":["192.168.1.42"]}"#.utf8
    )
}
