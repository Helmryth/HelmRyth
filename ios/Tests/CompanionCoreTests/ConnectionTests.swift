...
final class ConnectionTests: XCTestCase {
    ...
    func testParsesADesktopPairingInvite() throws {
        let token = "hry_pair_" + String(repeating: "a", count: 43)
        let url = try XCTUnwrap(URL(string: "helmryth://pair?address=macbook.tail1234.ts.net%3A8810&token=\(token)&code=004209&name=Divyam%27s%20Mac"))
        ...
        XCTAssertEqual(invite.connection.name, "Divyam's Mac")
        XCTAssertEqual(invite.credential, token)
    }

    func testPairingConsentShowsNormalizedOriginInsteadOfTrustingQRName() throws {
        let url = try XCTUnwrap(URL(string:
            "helmryth://pair?address=https%3A%2F%2FOTHER.Example%3A9443%2F" +
            "&code=004209&name=Divyam%27s%20Mac"))
        let invite = try XCTUnwrap(PairingInvite.parse(url))
    ...
    func testParsesAnOlderCodeOnlyPairingInvite() throws {
        let url = try XCTUnwrap(URL(string: "helmryth://pair?address=mac.local&code=004209"))
        let invite = try XCTUnwrap(PairingInvite.parse(url))
    ...
    func testLegacyTailnetInviteDropsUnselectedLocalFallbackKinds() throws {
        let url = try XCTUnwrap(URL(string:
            "helmryth://pair?address=macbook.tail1234.ts.net%3A8810&code=004209" +
            "&hosts=macbook.tail1234.ts.net,192.168.1.42,helmryth-aa.local"))
        let invite = try XCTUnwrap(PairingInvite.parse(url))
    ...
    func testHostedTypedInviteCannotRetainDirectFallbacks() throws {
        ...
        let token = "hry_pair_" + String(repeating: "a", count: 43)
        let url = try XCTUnwrap(URL(string:
            "helmryth://pair?address=192.168.1.42%3A8810&token=\(token)&endpoints=\(encoded)"))
    ...
    func testRejectsMalformedOrDowngradedTypedEndpoints() throws {
        ...
        let invalidBase64 = try XCTUnwrap(URL(string:
            "helmryth://pair?address=192.168.1.42%3A8810&token=\(token)&endpoints=not-json"))
    ...
    func testSanitizesFallbackHostsAndKeepsOnlyTheConfirmedLocalOrigin() throws {
        ...
        let url = try XCTUnwrap(URL(string:
            "helmryth://pair?address=mac.local&code=004209&hosts=%20mac.local%20,other.local,,bad%2Fslash,has%20space"))
        let invite = try XCTUnwrap(PairingInvite.parse(url))
        XCTAssertEqual(invite.connection.hosts, ["mac.local"])

        // and an invite with no usable candidate keeps the single address
        let empty = try XCTUnwrap(URL(string: "helmryth://pair?address=mac.local&code=004209&hosts=bad%2Fslash"))
    ...
    func testAPairResponseWithAndWithoutHostsDecodes() throws {
        let older = Data(#"{"token":"hry_x","device":{"id":"d","name":"p","createdAt":1,"lastSeenAt":1},"serverName":"Mac"}"#.utf8)
        XCTAssertNil(try JSONDecoder().decode(PairResponse.self, from: older).hosts)

        let newer = Data(#"{"token":"hry_x","device":{"id":"d","name":"p","createdAt":1,"lastSeenAt":1},"serverName":"Mac","hosts":["a.ts.net","192.168.1.42"]}"#.utf8)
        XCTAssertEqual(try JSONDecoder().decode(PairResponse.self, from: newer).hosts, ["a.ts.net", "192.168.1.42"])

        let typed = Data(#"{"token":"hry_x","device":{"id":"d","name":"p","createdAt":1,"lastSeenAt":1},"serverName":"Mac","endpoints":[{"url":"https://mac.example","kind":"hosted","priority":0}]}"#.utf8)
    ...
    func testRejectsAnUntrustedOrMalformedPairingInvite() throws {
        ...
        XCTAssertNil(PairingInvite.parse(try XCTUnwrap(URL(string: "helmryth://pair?address=mac.local&code=12345"))))
        XCTAssertNil(PairingInvite.parse(try XCTUnwrap(URL(string: "helmryth://pair?address=mac.local&token=weak"))))
        XCTAssertNil(PairingInvite.parse(try XCTUnwrap(URL(string: "helmryth://pair?address=mac.local&token=weak&code=123456"))))
        XCTAssertNil(PairingInvite.parse(try XCTUnwrap(URL(string: "helmryth://pair?address=host%2Fpath&code=123456"))))
        XCTAssertNil(PairingInvite.parse(try XCTUnwrap(URL(string: "helmryth://pair?address=one.local&address=two.local&code=123456"))))
    }
