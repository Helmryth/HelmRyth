...
public struct Bot: Codable, Hashable, Identifiable, Sendable {
    ...
    /// `operatorMark` ignores `avatarUrl`; the other values describe the image mask.
    ...
    public var sigilExpression: String?
...
public enum AvatarCrop: String, Codable, CaseIterable, Hashable, Sendable {
    ...
    public init(from decoder: Decoder) throws {
        let raw = try decoder.singleValueContainer().decode(String.self)
        switch raw {
        case "sigil", "sigillike", "operator", "operatorMark":
            self = .operatorMark
        default:
            self = Self(rawValue: raw) ?? .operatorMark
        }
    }
    ...
    public init(from decoder: Decoder) throws {
        ...
        guard !endpoints.isEmpty else {
            throw DecodingError.dataCorruptedError(
                forKey: .endpoints,
                in: container,
                debugDescription: "Reach endpoint metadata must contain at least one valid route."
            )
        }
    }
}
    ...
    public init(from decoder: Decoder) throws {
        ...
        guard let parsed = URL(string: raw),
            ...
            throw DecodingError.dataCorruptedError(
                forKey: .joinUrl,
                in: container,
                debugDescription: "Cloud workbench URL must be HTTPS"
            )
        }
        url = parsed
    }
...
public struct ConfigStatus: Codable, Sendable {
    ...
    /// Whether synthesis is available on the paired workbench. Deliberately
...
/// One frame of an operator's workbench, as it arrives on the stream.
