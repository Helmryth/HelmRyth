...
public struct PairingInvite: Equatable, Sendable {
    ...
    public static func parse(_ url: URL) -> PairingInvite? {
        guard url.scheme?.lowercased() == "helmryth",
    ...
    private static func credential(from values: [String: String]) -> String? {
        if let token = values["token"] {
            let canonicalPairingTokenPrefix = "hry_pair_"
            let legacyMigrationPairingTokenPrefix = "omb_pair_"
            let acceptedPrefixes = [canonicalPairingTokenPrefix, legacyMigrationPairingTokenPrefix]
            guard let prefix = acceptedPrefixes.first(where: { token.hasPrefix($0) }),
...
public struct PairingRouteError: Error, LocalizedError, Equatable, Sendable {
    ...
    public var errorDescription: String? {
        ...
        return "Couldn’t reach this workbench through any available route (\(routes)). Keep Reach turned on in Helmryth, then try again."
    }
}
...
public enum APIError: Error, LocalizedError, Sendable {
    ...
    public var errorDescription: String? {
        ...
        case let .status(code, message):
            ...
            switch code {
            case 401: return "This phone is not paired with that workbench."
            case 403: return "That can only be done on the workbench itself."
            case 404: return "That is no longer there."
            case 409: return "That operator is busy — stop it first."
            default: return "The workbench answered with an error (\(code))."
            }
...
public struct CompanionClient: Sendable {
    ...
    private func send<T: Decodable>(_ request: URLRequest, as type: T.Type) async throws -> T {
        ...
        do {
            ...
            throw APIError.transport("The workbench sent something this app couldn't read.")
        }
    }
    ...
    /// Only the first response that identifies itself as Helmryth receives
    ...
    private static func healthy(_ connection: Connection, session: URLSession) async -> Bool {
        do {
            ...
            guard !Task.isCancelled,
                  ...
                  try JSONDecoder().decode(HealthIdentity.self, from: data).app == "helmryth"
            else { return false }
            return true
        } catch {
            return false
