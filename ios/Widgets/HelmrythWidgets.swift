// The widget extension: the operator's Live Activity, in the Dynamic Island
// and on the lock screen. The face is the sigil engine's resting face for the
...
@main
struct HelmrythWidgets: WidgetBundle {
    ...
    var body: some WidgetConfiguration {
        ActivityConfiguration(for: BotActivityAttributes.self) { context in
            ...
            DynamicIsland {
                ...
                OperatorSigilStill(color: context.attributes.color, state: OperatorSignalState(rawValue: context.state.face) ?? .idle, size: 24)
            } compactTrailing: {
                compactTrailing(context)
            } minimal: {
                OperatorSigilStill(color: context.attributes.color, state: OperatorSignalState(rawValue: context.state.face) ?? .idle, size: 22)
            }
            .keylineTint(HelmrythPalette.color(context.attributes.color))
        }
    }
    ...
    private func compactTrailing(_ context: ActivityViewContext<BotActivityAttributes>) -> some View {
        ...
        case "needsYou":
            Image(systemName: "hand.raised.fill")
                ...
                .foregroundStyle(HelmrythPalette.color(context.attributes.color))
        case "working":
            Image(systemName: "circle.dotted")
                .font(.system(size: 13, weight: .semibold))
                .foregroundStyle(.white.opacity(0.8))
        default:
            Circle().fill(HelmrythPalette.color(context.attributes.color)).frame(width: 8, height: 8)
        }
    }
}
                        ...
                        .background(
                            Capsule().fill(
                                option.caseInsensitiveCompare("Deny") == .orderedSame
                                    ...
                                    : HelmrythPalette.color(context.attributes.color)
                            )
                        )
                }
                .buttonStyle(.plain)
...
private struct OrbitingFace: View {
    ...
    var body: some View {
        ZStack {
            ...
            OperatorSigilStill(color: context.attributes.color, state: OperatorSignalState(rawValue: context.state.face) ?? .idle, size: size, comets: true, at: Date())
        }
    }
}
