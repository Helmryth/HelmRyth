-- Local operators, workstreams, and desktop state deliberately do not belong here. This
-- database records only cloud account ownership and revocable node
-- credentials.
CREATE TABLE nodes (
  id TEXT PRIMARY KEY,
...
CREATE INDEX nodes_owner_active_idx
  ON nodes(owner_user_id, revoked_at, created_at);

CREATE UNIQUE INDEX nodes_owner_client_active_uidx
  ON nodes(owner_user_id, client_instance_id)
...
CREATE TRIGGER nodes_active_limit_before_insert
...
BEGIN
  SELECT RAISE(ABORT, 'active_node_limit');
END;
...
CREATE TRIGGER nodes_rotation_cooldown_before_update
BEFORE UPDATE OF last_rotation_at ON nodes
...
CREATE INDEX node_credentials_node_idx
  ON node_credentials(node_id, revoked_at);

CREATE UNIQUE INDEX node_credentials_one_active_uidx
  ON node_credentials(node_id)
  WHERE revoked_at IS NULL;

CREATE TRIGGER node_credentials_rotation_guard_before_insert
