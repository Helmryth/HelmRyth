...
// No upper length bound here on purpose. The limiter keys off an HMAC, which
// accepts any length, and a `.max(254)` made this schema REJECT long addresses
// — normalizedRecipient then returned null, limitedOTPResponse bailed out, and
// the per-recipient cap was never recorded. An address too long for the limiter
// was still long enough for Better Auth to send to, so overlong addresses got
// unlimited sends. The limiter must see every address the sender will accept.
...
export async function limitedOTPResponse(request: Request, env: Env): Promise<Response | null> {
  ...
  const key = await recipientKey(email, env.HELMRYTH_REGISTRY_AUTH_SECRET);
