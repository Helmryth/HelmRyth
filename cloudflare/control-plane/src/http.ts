...
function requestOriginAllowed(request: Request, config: RegistryConfig): string | null {
  const origin = request.headers.get("origin");
  if (!origin) return null;
  return config.nodeOrigins.has(origin) ? origin : null;
}

export function preflight(request: Request, config: RegistryConfig): Response {
