...
const resultEnvelopeSchema = z.object({ result: z.unknown() }).loose();
...
function providerErrorCode(envelope: ErrorEnvelope | null, status: number): string {
  const providerCode = envelope?.errors?.find((error) => error.code !== undefined)?.code;
...
export class CloudflareAPI {
  constructor(
    private readonly config: RegistryConfig["cloudflare"],
  ...
  private async request<T>(
    ...
    if (
      ...
      throw new CloudflareAPIError(providerErrorCode(envelope.success ? envelope.data : null, response.status), response.status);
    }
    const resultEnvelope = resultEnvelopeSchema.safeParse(value);
    if (!resultEnvelope.success) throw new CloudflareAPIError("cf_invalid_response");
    const result = schema.safeParse(resultEnvelope.data.result);
  ...
  async configureTunnel(tunnelId: string, hostname: string): Promise<void> {
    const result = await this.request(
      ...
      configurationSchema,
      {
        body: {
          config: {
            ingress: [
              { hostname, service: this.config.reachOrigin },
    ...
    if (
      ...
      || ingress[0]?.service !== this.config.reachOrigin
