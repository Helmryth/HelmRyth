...
type JsonValue = null | undefined | boolean | number | string | ConnectedAccountSummary | CapabilityState | JsonValue[] | JsonObject;
...
function randomToken() {
  ...
  return `hry_${Array.from(bytes, (byte) => byte.toString(16).padStart(2, "0")).join("")}`;
}
...
/** Credential shapes that must never reach a caller: this Worker authenticates
 * to Composio with its OWN api key, and Composio's auth failures quote the key
 * they were sent — middle-masked, but still leaking the leading and trailing
 * characters of a server-side secret to whoever provoked the error. Bearer and
 * sk-/ak-/pk- style tokens are redacted for the same reason. */
const CREDENTIAL_PATTERNS: RegExp[] = [
  /\b(?:ak|sk|pk|rk)[-_][A-Za-z0-9._-]{6,}/gi,
  /\bBearer\s+[A-Za-z0-9._-]{8,}/gi,
  // middle-masked echoes such as `ak_s...3bP` or `abc***xyz`
  /\b[A-Za-z0-9._-]{2,}(?:\.{3,}|\*{3,}|\u2026)[A-Za-z0-9._-]{2,}\b/g,
  /\b[A-Fa-f0-9]{32,}\b/g,
];

function redactSecrets(text: string): string {
  return CREDENTIAL_PATTERNS.reduce((out, pattern) => out.replace(pattern, "«redacted»"), text);
}

async function upstreamError(response: Response, fallback: string) {
  // An authentication failure is always about THIS Worker's credentials, never
  // the caller's, so its body can say nothing a caller should act on.
  ...
  try {
    ...
    return redactSecrets(String(body.message ?? nested ?? fallback)).slice(0, 240);
  } catch {
    // Unparsed upstream text is the highest-risk path — it used to be forwarded
    // verbatim. Prefer the canonical fallback and never echo a raw body.
    return fallback;
  }
}
...
async function ensureSession(node: NodeRow, env: Env, ctx: ExecutionContext) {
  if (!(await env.CONDUIT_SESSION_LIMITER.limit({ key: node.id })).success) {
    throw new Response(JSON.stringify({ error: "too many Conduit requests" }), { status: 429, headers: JSON_HEADERS });
  }
  let session = node.session_id ? await getSession(env, node.session_id) : null;
  ...
  if (!session?.multiAccountConfigured) {
    ...
    // Session upgrades legacy nodes without relinking OAuth grants.
    session = await createSession(env, node.composio_user_id);
    multiAccountUpgradeAttempted.add(session.sessionId);
    await env.CONDUIT_DB.prepare("UPDATE nodes SET session_id = ?, last_seen_at = ? WHERE id = ?")
      .bind(session.sessionId, Date.now(), node.id)
      .run();
  } else {
    ctx.waitUntil(
      env.CONDUIT_DB.prepare("UPDATE nodes SET last_seen_at = ? WHERE id = ?")
        .bind(Date.now(), node.id)
        .run()
        .catch((error: Error) => console.error(JSON.stringify({ message: "last-seen update failed", id: node.id, error: error.message }))),
    );
  }
  return session;
}
...
async function authenticate(request: Request, env: Env) {
  const token = request.headers.get("authorization")?.match(/^Bearer (hry_[0-9a-f]{64})$/)?.[1];
  if (!token) return null;
  const row = await env.CONDUIT_DB.prepare(
    "SELECT id, composio_user_id, session_id, disabled_at FROM nodes WHERE token_hash = ?",
  ).bind(await sha256(token)).first<NodeRow>();
  return row && row.disabled_at === null ? row : null;
}

async function enrollNode(request: Request, env: Env) {
  if (env.HELMRYTH_CONDUIT_ENROLLMENT_MODE !== "open") return json({ error: "enrollment is temporarily closed" }, 503);
  const fingerprint = `${request.headers.get("cf-connecting-ip") ?? "unknown"}|${request.headers.get("user-agent") ?? "unknown"}`;
  if (!(await env.NODE_ENROLLMENT_LIMITER.limit({ key: await sha256(fingerprint.slice(0, 512)) })).success) {
    return json({ error: "too many enrollment attempts" }, 429);
  }
  const nodeId = crypto.randomUUID();
  const token = randomToken();
  const now = Date.now();
  await env.CONDUIT_DB.prepare(
    "INSERT INTO nodes (id, token_hash, composio_user_id, created_at, last_seen_at) VALUES (?, ?, ?, ?, ?)",
  ).bind(nodeId, await sha256(token), `hry_node_${nodeId.replaceAll("-", "")}`, now, now).run();
  console.log(JSON.stringify({ message: "node enrolled", nodeId }));
  return json({ nodeId, token }, 201);
}

async function proxyMcp(request: Request, node: NodeRow, env: Env, ctx: ExecutionContext) {
  ...
  const session = await ensureSession(node, env, ctx);
...
function capabilityStateFromAccounts(accounts: ConnectedAccountSummary[]): CapabilityState {
...
function capabilityStateFromToolkit(
  ...
  const accountState = capabilityStateFromAccounts(accounts);
...
function allCapabilityStates(
  ...
  for (const toolkit of toolkits) {
    ...
    capabilities.set(slug, capabilityStateFromToolkit(toolkit, accountsBySlug.get(slug) ?? []));
  }
  return Object.fromEntries(capabilities);
}

async function connectedCapabilities(
  node: NodeRow,
  env: Env,
  ...
  const [toolkits, accounts] = await Promise.all([
    ...
    listConnectedAccounts(env, node.composio_user_id, []).catch(() => []),
  ]);
  return json({
    configured: true,
    capabilities: allCapabilityStates(summarizeAccounts(accounts, []), toolkits),
  });
}

async function capabilityStatus(url: URL, node: NodeRow, env: Env, ctx: ExecutionContext) {
  const slugs = [...new Set((url.searchParams.get("capabilities") ?? "").split(",").map((slug) => slug.toLowerCase()).filter(Boolean))].slice(0, 50);
  const session = await ensureSession(node, env, ctx);
  const [response, accounts] = await Promise.all([
    ...
    listConnectedAccounts(env, node.composio_user_id, slugs).catch(() => []),
  ]);
  ...
  return json({ capabilities: Object.fromEntries(slugs.map((slug) => {
    const item = items.get(slug);
    const capabilityAccounts = accountsBySlug.get(slug) ?? [];
    return [slug, capabilityStateFromToolkit(item, capabilityAccounts)];
  })) });
}
...
async function authorize(
  ...
  const session = await ensureSession(node, env, ctx);
  // Listing can be denied to the conduit's key scope; authorize must still
  // work, with the alias guardrails degrading to first-account behavior.
  const accounts = await listConnectedAccounts(env, node.composio_user_id, [slug]).catch(() => []);
  const capabilityAccounts = accounts.filter((account) => account.toolkit?.slug?.toLowerCase() === slug);
  const usableAccounts = capabilityAccounts.filter((account) => /^(active|initiated|initializing|pending)$/i.test(account.status ?? ""));
  ...
  if (alias && capabilityAccounts.some((account) => account.alias?.trim().toLowerCase() === alias.toLowerCase())) {
...
async function disconnect(slug: string, node: NodeRow, env: Env, ctx: ExecutionContext) {
  const session = await ensureSession(node, env, ctx);
...
async function disconnectAccount(
  ...
  await ensureSession(node, env, ctx);
  const accounts = await listConnectedAccounts(env, node.composio_user_id, [slug]);
...
async function route(request: Request, env: Env, ctx: ExecutionContext) {
  ...
  if (request.method === "GET" && url.pathname === "/healthz") return json({ service: "helmryth-conduit", ready: Boolean(env.COMPOSIO_API_KEY) });
  if (request.method === "POST" && url.pathname === "/v1/nodes") return enrollNode(request, env);
  if (!url.pathname.startsWith("/v1/")) return json({ error: "not found" }, 404);
  const node = await authenticate(request, env);
  if (!node) return json({ error: "unauthorized" }, 401);
  if (request.method === "GET" && url.pathname === "/v1/self") return json({ nodeId: node.id });
  if (request.method === "POST" && url.pathname === "/v1/mcp") return proxyMcp(request, node, env, ctx);
  if (request.method === "GET" && url.pathname === "/v1/capabilities/catalog") return catalog(env);
  if (request.method === "GET" && url.pathname === "/v1/capabilities/active") return connectedCapabilities(node, env, ctx);
  if (request.method === "GET" && url.pathname === "/v1/capabilities") return capabilityStatus(url, node, env, ctx);
  const accountMatch = url.pathname.match(/^\/v1\/capabilities\/([a-z0-9][a-z0-9_-]{0,80})\/accounts\/([A-Za-z0-9][A-Za-z0-9_-]{0,127})$/);
  if (accountMatch && request.method === "DELETE") {
    return disconnectAccount(accountMatch[1], accountMatch[2], node, env, ctx);
  }
  const match = url.pathname.match(/^\/v1\/capabilities\/([a-z0-9][a-z0-9_-]{0,80})(?:\/(authorize))?$/);
  if (match?.[2] && request.method === "POST") return authorize(match[1], await requestAlias(request), node, env, ctx);
  if (match && !match[2] && request.method === "DELETE") return disconnect(match[1], node, env, ctx);
