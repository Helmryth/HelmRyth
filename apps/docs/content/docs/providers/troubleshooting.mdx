...
3. Restart Helmryth after installation.
4. Set the exact executable in **System settings → Engines** if automatic detection still fails.
...
Run the provider's own status or login command in a terminal. Helmryth uses the CLI's authenticated session; installing the binary alone is not enough.

A `401 Unauthorized` response from `api.openai.com` means the launched Codex process did not receive valid authentication. Confirm `codex login status`, sign in again if needed, then restart Helmryth. Do not paste a ChatGPT password into Helmryth.
...
- Try the provider CLI directly with the same model to separate an account problem from a Helmryth integration problem.
