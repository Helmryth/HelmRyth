...
const directoryBotSchema = z.record(z.string(), z.json());

function text(value: JsonValue | undefined, max: number): string | null {
...
export function parseBotDirectory(value: JsonValue): DirectoryBot[] {
  const envelope = directoryEnvelopeSchema.safeParse(value);
  if (!envelope.success) {
    throw new Error("The operator directory response is not supported");
  }
  const bots: DirectoryBot[] = [];
  const seen = new Set<string>();
  for (const candidate of envelope.data.bots.slice(0, MAX_DIRECTORY_BOTS)) {
    const rawResult = directoryBotSchema.safeParse(candidate);
...
async function readBounded(response: Response, maxBytes: number): Promise<string> {
  const oversized = () => new Error("The operator directory response is too large");
...
export async function fetchBotDirectory(fetcher: Fetcher = fetch): Promise<DirectoryBot[]> {
  ...
  if (!response.ok) throw new Error(`The operator directory returned HTTP ${response.status}`);
