...
import { eraseThreadEventLogs, readThreadEvents, type InspectorEntry } from "./thread-events.ts";

const dirs: string[] = [];
function tmp() {
  const d = mkdtempSync(join(tmpdir(), "hry-thread-events-"));
  dirs.push(d);
  return d;
}
afterEach(() => {
...
const line = <Fixture extends object | null>(value: Fixture) => JSON.stringify(value) + "\n";
const runtime = <Event extends object>(event: Event) => ({ provider: "test", threadId: "t1", ...event });
const runtimeEventIds = (entries: InspectorEntry[]) =>
  entries.flatMap((entry) => entry.kind === "runtime" ? [entry.data.eventId] : []);
...
describe("readThreadEvents", () => {
  ...
  it("caps each log to its most recent `limit` lines and reports what it skipped", () => {
    ...
    expect(runtimeEventIds(page.entries)).toEqual(["e7", "e8", "e9"]);
  ...
  it("discards JSON-valid records that do not satisfy the inspector wire contract", () => {
    ...
    const page = readThreadEvents({ eventsDir, nativeDir, threadId: "t1" });
    expect(page.entries.map((entry) => [entry.kind, entry.kind === "runtime" ? entry.data.eventId : undefined])).toEqual([
      ["runtime", "valid"],
      ["native", undefined],
    ]);
  ...
  it("rejects malformed retry telemetry while retaining a valid retry event", () => {
    const eventsDir = tmp();
    const nativeDir = tmp();
    const retry = (eventId: string, attempt: number, delayMs: number, reason: string | undefined) =>
    ...
    const page = readThreadEvents({ eventsDir, nativeDir, threadId: "t1" });
    expect(runtimeEventIds(page.entries)).toEqual(["valid-retry"]);
  });
  ...
  it("keeps walking backward when a corrupt tail record would otherwise consume the limit", () => {
    ...
    const page = readThreadEvents({ eventsDir, nativeDir, threadId: "t1", limit: 3 });
    expect(runtimeEventIds(page.entries)).toEqual(["e17", "e18", "e19"]);
  ...
  it("updates cached totals from appended bytes and preserves multibyte records across read chunks", () => {
    ...
    writeFileSync(file, line(runtime({ eventId: "large", createdAt: "1", type: "runtime.error", message: "🐭".repeat(40_000) })));
    ...
    expect(runtimeEventIds(page.entries)).toEqual(["large", "latest"]);
    const large = page.entries.find((entry) => entry.kind === "runtime" && entry.data.eventId === "large");
    expect(large?.kind === "runtime" && large.data.type === "runtime.error" && large.data.message.startsWith("🐭🐭")).toBe(true);
  });
  ...
  it("physically erases both per-thread logs, invalidates cached counts, and is idempotent", () => {
    const eventsDir = tmp();
    const nativeDir = tmp();
    writeFileSync(join(eventsDir, "t1.ndjson"), line(runtime({ eventId: "one", createdAt: "1", type: "turn.started" })));
    writeFileSync(join(nativeDir, "t1.ndjson"), line({ at: "2", dir: "in", source: "test", msg: {} }));
    expect(readThreadEvents({ eventsDir, nativeDir, threadId: "t1" }).total).toEqual({ runtime: 1, native: 1 });
    expect(eraseThreadEventLogs({ eventsDir, nativeDir, threadId: "t1" })).toEqual({
      runtimeLogRemoved: true,
      nativeLogRemoved: true,
    });
    expect(readThreadEvents({ eventsDir, nativeDir, threadId: "t1" }).total).toEqual({ runtime: 0, native: 0 });
    expect(eraseThreadEventLogs({ eventsDir, nativeDir, threadId: "t1" })).toEqual({
      runtimeLogRemoved: false,
      nativeLogRemoved: false,
    });
    expect(() => eraseThreadEventLogs({ eventsDir, nativeDir, threadId: "../bots" })).toThrow(/thread id/);
  });
});
