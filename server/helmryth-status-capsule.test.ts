...
import { readHelmrythStatus, trustedHelmrythStatusSystemPrompt } from "./helmryth-status-capsule.ts";
...
function cachePath(capsule: TestCapsule): string {
  const root = mkdtempSync(join(tmpdir(), "helmryth-status-"));
  roots.push(root);
  const parent = join(root, "helmryth");
...
posixOnly("readHelmrythStatus", () => {
  it("projects only fresh normalized two-VM capability data", () => {
    const capsule = successCapsule();
    // Cross-language receipt produced by scripts/aos_helmryth_status.py
    // for this exact normalized fixture.
    expect(capsule.receipt_sha256).toBe(
      "sha256:1832093f994167acf54b8ac0c11931e89a8dc2c71941ab9c210574197d349e4a",
    );
    const path = cachePath(capsule);

    const status = readHelmrythStatus({ cachePath: path, now: new Date(NOW.getTime() + 1_000) });
    ...
    const prompt = trustedHelmrythStatusSystemPrompt({ cachePath: path, now: new Date(NOW.getTime() + 1_000) });
  ...
  it("rejects a signed success capsule whose installed and accepted hashes differ", () => {
    ...
    expect(readHelmrythStatus({ cachePath: path, now: NOW })).toMatchObject({
      freshness: "unknown",
      reason: "invalid",
  ...
  it("turns stale, future-skewed, failed, and receipt-tampered state into unknown", () => {
    const path = cachePath(successCapsule());
    expect(readHelmrythStatus({ cachePath: path, now: new Date("2026-08-22T06:35:00Z") })).toMatchObject({
      freshness: "stale", reason: "stale", runtimeState: "unknown", readyCount: 0,
      ui: { twoUp: false },
    });
    expect(readHelmrythStatus({ cachePath: path, now: new Date("2026-08-22T06:29:59Z") })).toMatchObject({
      freshness: "unknown", reason: "clock_skew", runtimeState: "unknown", readyCount: 0,
    });

    const failurePath = cachePath(failedCapsule());
    expect(readHelmrythStatus({ cachePath: failurePath, now: new Date(NOW.getTime() + 1_000) })).toMatchObject({
    ...
    expect(readHelmrythStatus({ cachePath: tamperedPath, now: new Date(NOW.getTime() + 1_000) })).toMatchObject({
  ...
  it("rejects signed extra fields, insecure modes, and same-path symlinks", () => {
    ...
    expect(readHelmrythStatus({ cachePath: extraPath, now: NOW }).reason).toBe("invalid");

    const insecurePath = cachePath(successCapsule());
    chmodSync(insecurePath, 0o644);
    expect(readHelmrythStatus({ cachePath: insecurePath, now: NOW }).reason).toBe("missing_or_insecure");

    const targetPath = cachePath(successCapsule());
    const linkPath = join(dirname(targetPath), "latest-link.json");
    symlinkSync(targetPath, linkPath);
    expect(readHelmrythStatus({ cachePath: linkPath, now: NOW }).reason).toBe("missing_or_insecure");
  });
  ...
  it("rejects a non-0700 parent and a symlinked parent", () => {
    ...
    expect(readHelmrythStatus({ cachePath: looseParentPath, now: NOW }).reason).toBe("missing_or_insecure");

    const targetPath = cachePath(successCapsule());
    const linkRoot = mkdtempSync(join(tmpdir(), "helmryth-status-parent-link-"));
    roots.push(linkRoot);
    const linkedParent = join(linkRoot, "helmryth");
    symlinkSync(dirname(targetPath), linkedParent, "dir");
    expect(
      readHelmrythStatus({ cachePath: join(linkedParent, "latest.json"), now: NOW }).reason,
...
it.skipIf(process.getuid !== undefined)(
  ...
  () => {
    ...
    expect(trustedHelmrythStatusSystemPrompt({ cachePath: path, now: NOW })).toContain(
      "runtime_state=unknown",
    );
  },
);
