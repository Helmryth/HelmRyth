...
export function engineIsFresh(input: {
  ...
  resumeCursors: Partial<Record<string, string>>;
  transcript: Array<{ role: "user" | "assistant"; text: string }>;
}): boolean {
...
const FRESH_PREAMBLE =
  "[You are joining this workstream in progress because the user switched this operator over to you. The workstream so far:]";
...
export function buildTurnContext(input: TurnContextInput): BuiltTurnContext {
  ...
  return {
    turnText: [
      ...
      "",
      ...transcript.map((m) => `${m.role === "user" ? "User" : "Operator"}: ${m.text}`),
      "",
