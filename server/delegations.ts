...
const pendingDelegationSchema = z.object({
  id: z.string().trim().min(1).optional(),
  toBotId: z.string(),
  message: z.string(),
  reason: z.string().optional(),
  depth: z.number().finite(),
  attempts: z.number().finite().optional(),
});
const delegationReceiptSchema = z.object({
  id: z.string().trim().min(1),
  sourceThreadId: z.string(),
  toBotId: z.string(),
  toBotName: z.string(),
  status: z.enum(["done", "failed", "denied", "busy_gave_up", "dropped", "error"]),
...
export function _loadPending(): void {
  ...
  try {
    const raw = z.record(z.string(), z.array(z.json())).parse(JSON.parse(readFileSync(DELEGATIONS_FILE, "utf8")));
    for (const [threadId, list] of Object.entries(raw)) {
      const items = list.flatMap((value): PendingDelegationItem[] => {
        const parsed = pendingDelegationSchema.safeParse(value);
        if (!parsed.success) return [];
        const item: PendingDelegationItem = {
          id: parsed.data.id ?? newId(),
          toBotId: parsed.data.toBotId,
          message: parsed.data.message,
          depth: Math.max(0, Math.trunc(parsed.data.depth)),
          attempts: Math.max(0, Math.trunc(parsed.data.attempts ?? 0)),
        };
        if (parsed.data.reason !== undefined) item.reason = parsed.data.reason;
        return [item];
      });
  ...
  receipts = [];
  try {
    const rawReceipts = z.array(z.json()).parse(JSON.parse(readFileSync(RECEIPTS_FILE, "utf8")));
    if (rawReceipts.length > 0) {
      const now = Date.now();
      const loaded: DelegationReceipt[] = [];
      for (const value of rawReceipts) {
        const parsed = delegationReceiptSchema.safeParse(value);
        if (!parsed.success || now - parsed.data.finishedAt > RECEIPT_MAX_AGE_MS) continue;
        const receipt: DelegationReceipt = { ...parsed.data };
        loaded.push(receipt);
      }
...
export interface PendingDelegationSnapshot {
  sourceThreadId: string;
  toBotId: string;
  reason?: string;
}
...
export function pendingDelegationSnapshot(): PendingDelegationSnapshot[] {
  return [...pendingDelegations.entries()].flatMap(([sourceThreadId, items]) =>
    items.map((item) => {
      const snapshot: PendingDelegationSnapshot = {
        sourceThreadId,
...
async function processOne(
  ...
  if (!target) {
    ...
    bus.store.appendMessage(sourceThreadId, {
      role: "bot",
      kind: "activity",
      tool: { name: `error: delegation to ${item.toBotId} failed — no such operator`, ok: false },
    });
    return "settled";
  }
  if (target.busy) {
  ...
  const prefixed = `[Delegated by @${sender.name}, another operator in this Helmryth workspace. Do the work and reply directly.]\n\n${item.message}${reasonLine}`;
