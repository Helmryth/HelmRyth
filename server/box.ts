...
import { wholeScreenshot } from "./container-computer.ts";
...
const BOX_API = process.env.HELMRYTH_BOX_API || "https://ascii.dev/api/box/v1";
const READY = new Set(["idle", "ready", "running"]);
const DEFAULT_BOX_TTL_SECONDS = 8 * 60 * 60;
const TRIAL_BOX_TTL_SECONDS = 2 * 60 * 60;

const boxDetailsSchema = z.object({
  billingUrl: z.string().optional(),
  maxTtlSeconds: z.number().int().optional(),
  maximumTtlSeconds: z.number().int().optional(),
  maxAutoStopSeconds: z.number().int().optional(),
}).passthrough();
const boxRecordSchema = z.object({
  id: z.string().min(1),
  name: z.string().optional(),
  state: z.string(),
  desktopAvailable: z.boolean().nullable().optional(),
}).passthrough();
const boxApiBodySchema = z.object({
  ok: z.boolean().optional(),
  box: boxRecordSchema.optional(),
  boxes: z.array(boxRecordSchema).optional(),
  ...
  details: boxDetailsSchema.optional(),
  error: z.object({ code: z.string().optional(), details: boxDetailsSchema.optional() }).passthrough().optional(),
}).passthrough();
type BoxApiBody = z.infer<typeof boxApiBodySchema>;

async function safeBoxJson(res: Response): Promise<BoxApiBody | null> {
  const raw = await res.json().catch(() => null);
  const parsed = boxApiBodySchema.safeParse(raw);
...
async function boxJson(cfg: AppConfig, path: string, opts: RequestInit = {}) {
  ...
  const body = await safeBoxJson(res);
...
async function boxNameFor(botId: string) {
  ...
  const hash = [...new Uint8Array(digest)]
    ...
    .join("")
    .slice(0, 6);
  return `helmryth-${botId.slice(0, 8).toLowerCase().replace(/[^a-z0-9]/g, "")}-${hash}`;
}
...
async function mintDesktopUrl(cfg: AppConfig, boxId: string, { vncBudgetMs = 60_000 } = {}) {
  const safeDesktopUrl = (raw: string | undefined): string | null => {
    if (!raw || raw.length > 16_384) return null;
    try {
      const url = new URL(raw);
      // Box is a remote provider. Its viewer is never entitled to the
      // loopback-only HTTP exception used by Helmryth's own VM/VPS tunnels.
  ...
  while (Date.now() - t0 < vncBudgetMs) {
    ...
    const url = safeDesktopUrl(body?.desktopUrl ?? body?.url);
    if (url) return url;
    if (!body?.provisioning) break;
    await new Promise((r) => setTimeout(r, 3000));
  }
  const { body } = await boxJson(cfg, `/boxes/${boxId}/desktop`, { method: "POST" });
  return safeDesktopUrl(body?.desktopUrl ?? body?.url);
}
...
async function waitReady(cfg: AppConfig, boxId: string, budgetMs = 90_000) {
  ...
  while (Date.now() - t0 < budgetMs) {
    ...
    if (state && READY.has(state)) return body.box;
...
export async function findBox(cfg: AppConfig, botId: string) {
  ...
  const found = (body?.boxes ?? []).find((box) => box.name === name && box.state !== "error") ?? null;
...
export function boxErrorMessage(status: number, what: string, body?: BoxApiBody | null): string {
  const theirs = body?.message?.trim() ?? "";
  const link = body?.error?.details?.billingUrl ?? "";
  if (status === 402) {
...
function trialBoxTtlSeconds(body: BoxApiBody | null): number | null {
  const code = body?.error?.code ?? body?.code;
  if (code !== "trial_auto_stop_required") return null;
  const details = body?.error?.details ?? body?.details;
  if (!details) return TRIAL_BOX_TTL_SECONDS;
  for (const value of [details.maxTtlSeconds, details.maximumTtlSeconds, details.maxAutoStopSeconds]) {
    if (value !== undefined && Number.isInteger(value) && value > 0 && value <= DEFAULT_BOX_TTL_SECONDS) return value;
  }
...
export async function provisionBox(cfg: AppConfig, botId: string, botName: string) {
  if (!boxConfigured(cfg)) {
    throw new Error('box provider not enabled — add {"box":{"token":"…"}} to ~/.helmryth/config.json');
  }
...
export async function sleepBox(cfg: AppConfig, botId: string) {
  const box = await findBox(cfg, botId);
  if (!box) throw new Error("No Workbench is assigned to this operator");
...
export async function execOnBox(cfg: AppConfig, botId: string, command: string) {
  const box = await findBox(cfg, botId);
  if (!box) throw new Error("This operator does not have a Workbench yet");
...
const PANEL_PATH = "/tmp/helmryth-panel.jpg";
...
async function readFileBase64(cfg: AppConfig, boxId: string, path: string): Promise<string | null> {
  ...
  return ok && content ? content : null;
}
...
export async function screenshotBox(cfg: AppConfig, botId: string, knownBoxId?: string) {
  ...
  if (!checked.ok) {
    throw Object.assign(new Error("the Workbench returned an incomplete screenshot"), { status: 502 });
  }
