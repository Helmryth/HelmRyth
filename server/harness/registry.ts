...
const cliConfigSchema = z.looseObject({ cli: z.string().optional() });

function cliDefaultOf(driver: AnyProviderDriver | undefined): string | undefined {
  if (!driver) return undefined;
  try {
    const parsed = cliConfigSchema.safeParse(driver.defaultConfig());
    return parsed.success ? parsed.data.cli : undefined;
  } catch {
    return undefined;
  }
}
...
function cliOfRaw(raw: ProviderConfigInput): string | undefined {
  const parsed = cliConfigSchema.safeParse(raw);
  return parsed.success && parsed.data.cli ? parsed.data.cli : undefined;
}
