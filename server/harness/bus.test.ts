...
import { runtimeEventSchema } from "../thread-events.ts";
...
describe("EventBus", () => {
  ...
  it("creates one canonical redacted event for disk and every live listener", () => {
    ...
    const original = testEvent({
      ...
      raw: { source: "provider", payload: { authorization: `Bearer ${key}` } },
    });
    bus.publish(original);

    if (original.type !== "runtime.error") throw new Error("runtime-error fixture parsed as the wrong event type");
    expect(original.message).toContain(key);
    expect(first).toHaveLength(1);
    expect(second).toHaveLength(1);
    expect(first[0]).toBe(second[0]);
    expect(JSON.stringify(first[0])).not.toContain(key);
    expect(JSON.stringify(first[0])).toContain("«redacted");
    const written = readFileSync(join(EVENTS_DIR, "redacted-log.ndjson"), "utf8");
    const logged = runtimeEventSchema.parse(JSON.parse(written.trim()));
    expect(logged).toEqual(first[0]);
  });
  ...
  it("never falls back to credential-bearing live delivery when persistence fails", () => {
    const key = `sk-ant-api03-${"abcdefghijklmnopqrstuvwxyz0123456789"}`;
    const append: typeof appendFileSync = vi.fn(() => {
    ...
    bus.publish(testEvent({ type: "content.delta", streamKind: "assistant_text", delta: `leaked ${key}` }));

    expect(seen).toHaveLength(2);
    expect(seen[0].type).toBe("runtime.error");
    expect(seen[1].type).toBe("content.delta");
    expect(JSON.stringify(seen[1])).not.toContain(key);
    expect(JSON.stringify(seen[1])).toContain("«redacted");
  });
