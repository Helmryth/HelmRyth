...
// HELMRYTH_EXTRA_PATH: the server runs in its own process, so injection happens
...
posixOnly("VPS turn routing e2e (fake ACP fleet + fake docker over SSH)", () => {
  ...
  beforeAll(async () => {
    ...
    home = mkdtempSync(join(tmpdir(), "hry-vps-routing-"));
  ...
  it(
    ...
    async () => {
      ...
      expect(echo).toContain("self-hosted remote Linux Workbench");
