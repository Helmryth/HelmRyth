...
const defaultLoginShellProbe: LoginShellProbe = (file, args, options, callback) => {
  execFile(file, args, options, callback);
};
let runLoginShellProbe: LoginShellProbe = defaultLoginShellProbe;
...
export function augmentedPath(): string {
  if (cached === null) {
    cached = mergePaths([
      ...(process.env.HELMRYTH_EXTRA_PATH ? process.env.HELMRYTH_EXTRA_PATH.split(delimiter) : []),
  ...
  if (!probed && !process.env.VITEST && process.platform !== "win32") {
    probed = true;
    probeLoginShellPath(probeGeneration);
  }
  return cached;
}
...
function probeLoginShellPath(generation: number): void {
  ...
  runLoginShellProbe(
    shell,
    ["-l", "-i", "-c", 'printf "__HELMRYTH_PATH__%s" "$PATH"'],
    { timeout: 5000 },
    (err, stdout) => {
      if (generation !== probeGeneration) return;
      if (err || !stdout) return;
      const text = String(stdout);
      const m = /__HELMRYTH_PATH__([^\n]*)/.exec(text);
      if (!m || !m[1]) return;
      loginShellPath = m[1];
...
export function resetPathCacheForTests(): void {
  ...
  loginShellPath = null;
  probeGeneration += 1;
  runLoginShellProbe = defaultLoginShellProbe;
}

export function setLoginShellProbeForTests(probe: LoginShellProbe): void {
  runLoginShellProbe = probe;
}
...
function nodeExe(near: string): string | null {
  ...
  return "electron" in process.versions ? null : process.execPath;
}
