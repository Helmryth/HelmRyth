...
import type { CommsBroadcastFrame, CommsBus } from "./comms-visibility.ts";
...
interface BusPair {
  commsBus: CommsBus;
  approvalBus: { store: Store; broadcast: (payload: CommsBroadcastFrame) => void };
  broadcasts: TestBroadcastFrame[];
}

interface TestBroadcastFrame extends CommsBroadcastFrame {
  threadId?: string;
}

function setupBuses(store: Store): BusPair {
  const broadcasts: TestBroadcastFrame[] = [];
  const broadcast = (payload: CommsBroadcastFrame) => {
...
describe("queueDelegation", () => {
  ...
  it("queues, broadcasts, and drops a 'Delegated to @Target' chip on the source thread", () => {
    ...
    // polling /api/bots
    const broadcastSchema = z.object({ kind: z.literal("message"), threadId: z.literal(from.threadId) });
    const broadcast = broadcasts.find((candidate) => broadcastSchema.safeParse(candidate).success);
...
describe("drainDelegations", () => {
  ...
  let target: BotRecord;
  let commsBus: CommsBus;
  let approvalBus: { store: Store; broadcast: (payload: CommsBroadcastFrame) => void };
  ...
  it("skips runTarget and emits a 'no such operator' chip when the target was deleted", async () => {
    queueDelegation(commsBus, from, { toBotId: target.id, message: "do this", depth: 0 }, 1);
    ...
    const chip = await waitFor(() =>
      store
        .messagesFor(from.threadId)
        .find((m) => m.kind === "activity" && (m.tool?.name ?? "").includes("no such operator")),
    );
...
describe("delegations survive a restart", () => {
  ...
  it("writes the queue to disk on queue, and clears it on drain and discard", async () => {
    ...
    const onDisk = z.record(z.string(), z.array(z.json())).parse(JSON.parse(readFileSync(file(), "utf8")));
