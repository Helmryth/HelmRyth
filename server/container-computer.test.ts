...
const readinessProbe =
  ...
  "--screenshot-out-file /tmp/helmryth-readiness.png";
const readinessRead = `docker exec ${CONTAINER} base64 -w0 /tmp/helmryth-readiness.png`;
...
function readyInspect(overrides: ReadyInspectOverride = {}) {
  return JSON.stringify([
    {
      Config: {
        Image: IMAGE,
...
describe("containerComputerStatus", () => {
  ...
  it("accepts exact Podman-on-Windows hardening and its WSL-translated durable mount", async () => {
    ...
    const target: LocalVmTarget = {
      ...derived,
      workspaceDir: "C:\\Users\\light\\.helmryth\\vm-homes\\win-target",
    };
    const detail = JSON.parse(perBotReadyInspect("bot-win", 41629))[0];
    detail.Mounts[0].Source = "/mnt/c/Users/light/.helmryth/vm-homes/win-target";
    detail.HostConfig = {
      ...detail.HostConfig,
    ...
    const fake = runner({
      ...
      [`${targetDriverExec} call get_desktop_state {} --socket ${CUA_SOCKET} --screenshot-out-file /tmp/helmryth-readiness.png`]: "{}\n",
      [`podman exec ${target.containerName} base64 -w0 /tmp/helmryth-readiness.png`]: validPng.toString("base64"),
    });
  ...
  it("keeps per-bot identities, workspaces, and ephemeral viewer ports separate", async () => {
    ...
    const fake = runner({
      ...
      [`${targetDriverExec} call get_desktop_state {} --socket ${CUA_SOCKET} --screenshot-out-file /tmp/helmryth-readiness.png`]: "{}\n",
      [`docker exec ${target.containerName} base64 -w0 /tmp/helmryth-readiness.png`]: validPng.toString("base64"),
    });
  ...
  it("refuses a per-bot container carrying another target's label", async () => {
    ...
    expect(status.problem).toContain("not created by Helmryth");
  });
...
describe("Cua integration", () => {
  ...
  it("builds an exact, checksum-verified Cua Driver 0.20.0 image", () => {
    ...
    expect(dockerfile).toContain("prepare-helmryth-workspace.sh");
  ...
  it("captures the preview through Cua Driver rather than xdotool or VNC", async () => {
    const screenshotCall =
      `${driverExec} call get_desktop_state {} --socket ${CUA_SOCKET} ` +
      "--screenshot-out-file /tmp/helmryth-preview.png";
    const png = validPng;
    const fake = runner({
      "/usr/bin/which docker": "docker\n",
      "/usr/bin/which podman": new Error("missing"),
      ...
      [screenshotCall]: "{}\n",
      [`docker exec ${CONTAINER} base64 -w0 /tmp/helmryth-preview.png`]: png.toString("base64"),
    });
...
describe("setupCommands", () => {
  ...
  it("uses an explicit local image name so Podman never resolves the managed build on Docker Hub", () => {
    expect(IMAGE).toMatch(/^localhost\/helmryth\/cua-local-vm:/);
    expect(setupCommands("podman", "darwin").run).toContain(IMAGE);
    expect(setupCommands("podman", "darwin").run).not.toContain("docker.io/helmryth");
  });
