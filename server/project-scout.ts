...
import {
  CREW_MANIFEST_FORMAT,
  CREW_MANIFEST_VERSION,
  parseCrewManifest,
  type CrewManifestOperator,
  type CrewManifestV1,
...
interface ScoutedPackageJson {
  name?: string;
  description?: string;
  deps: Set<string>;
}

const packageObjectSchema = z.record(z.string(), z.json());

function packageJson(cwd: string): ScoutedPackageJson {
  const raw = readText(join(cwd, "package.json"));
  if (!raw) return { deps: new Set() };
  try {
    const parsed = packageObjectSchema.safeParse(JSON.parse(raw));
    if (!parsed.success) return { deps: new Set() };
    const pkg = parsed.data;
    const deps = new Set<string>();
    for (const field of ["dependencies", "devDependencies"]) {
      const block = pkg[field];
      const dependencyBlock = packageObjectSchema.safeParse(block);
      if (dependencyBlock.success) for (const dep of Object.keys(dependencyBlock.data)) deps.add(dep);
    }
    const result: ScoutedPackageJson = { deps };
    const name = z.string().safeParse(pkg.name);
    if (name.success) result.name = name.data;
    const description = z.string().safeParse(pkg.description);
...
interface ScoutedReadme {
  title?: string;
  summary?: string;
}

function readme(cwd: string): ScoutedReadme {
  const raw =
...
/** Turn a scouted profile into an importable crew: a lead plus one operator
 * per detected role. Suggesting is all this does — creating operators and
 * the crew stays behind the existing import endpoint and
 * its human click. */
export function suggestTeam(profile: ProjectProfile): CrewSuggestion {
  const reasons = Object.fromEntries([
    ["lead", "Every project crew needs one operator who briefs, splits, and reviews."],
  ]);
  const operators: CrewManifestOperator[] = [
    {
      key: "lead",
      name: LEAD.name,
      title: LEAD.title,
  ...
  // No signals at all: the crew still gets a working pair of hands.
  ...
  const manifest: CrewManifestV1 = {
    format: CREW_MANIFEST_FORMAT,
    version: CREW_MANIFEST_VERSION,
    crew: {
      name: `${profile.name} crew`.slice(0, 100),
      operators,
    },
  };
  if (profile.summary) manifest.crew.description = profile.summary.slice(0, 2_000);
  ...
  return { roomName: profile.name, manifest: parseCrewManifest(manifest), reasons };
}
