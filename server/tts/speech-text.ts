...
// The pattern nests a quantifier inside a repeated group, so on a long run of
// path characters that contains no slash the engine tries every split before
// failing — quadratic. Run over a whole transcript that made one in-cap
// POST /api/tts/prepare hold the core's single event loop for minutes, which
// stops every other route and the SSE stream with it. Two bounds keep it linear
// in practice: only whitespace-delimited tokens are considered, and only tokens
// short enough to actually be a path are handed to the pattern at all.
const PATH_SEGMENTS = /(?:[\w.@-]+\/){1,}([\w.-]+\.\w{1,6})\b/g;
const LONGEST_REAL_PATH = 512;

function shortenPaths(text: string): string {
  if (!text.includes("/")) return text;
  return text.replace(/\S+/g, (token) => (
    token.length <= LONGEST_REAL_PATH && token.includes("/")
      ? token.replace(PATH_SEGMENTS, "$1")
      : token
  ));
}
...
export function speakable(input: string): string {
  ...
  //
  // The old shape — `^\s*\|?[\s:-]*\|[\s|:-]*$` — was quadratic. On a line with
  // no pipe, `\s*` had to hand back one character at a time so the required
  // `\|` could be retried at every offset: 32K of spaces cost ~800ms and a body
  // at the 1MB cap blocked the event loop for minutes, reachable through
  // POST /api/tts/prepare. One character class (no alternation, no nested
  // quantifier) plus a lookahead that bails the instant a line holds no pipe
  // keeps it linear — 1MB now costs ~3ms. The class is `\s` minus the line
  // breaks, because a separator row never spans lines.
  text = text.replace(/^(?=[^\r\n]*\|)[ \t\f\v   -   　﻿|:-]*$/gm, "");
  ...
  text = text.replace(
    /\p{Extended_Pictographic}|\p{Variation_Selector}|[\u{1F000}-\u{1FAFF}\u{2600}-\u{27BF}\u{2190}-\u{21FF}\u{2B00}-\u{2BFF}]/gu,
    "",
  );
