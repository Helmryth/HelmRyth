...
const API = process.env.HELMRYTH_ELEVENLABS_API || "https://api.elevenlabs.io/v1";
...
const voiceApiBodySchema = z.object({
  detail: z.union([
    z.string(),
  ...
  voices: z.array(z.object({
    ...
    labels: z.object({ accent: z.string().optional(), description: z.string().optional() }).optional(),
  })).optional(),
}).passthrough();
type VoiceApiBody = z.infer<typeof voiceApiBodySchema>;

async function safeJson(res: Response): Promise<VoiceApiBody | null> {
  try {
    const parsed = voiceApiBodySchema.safeParse(await res.json());
...
function message(status: number, what: string, body: VoiceApiBody | null): string {
  const theirs =
    (body?.detail instanceof Object ? body.detail.message?.trim() : body?.detail?.trim()) ||
    body?.message?.trim() ||
    "";
...
export async function listVoices(key: string): Promise<Voice[]> {
  ...
  return (body?.voices ?? [])
    .map((voice): Voice => ({
      ...
      description: [voice.labels?.accent, voice.labels?.description].filter(Boolean).join(" · ") || undefined,
    }))
    .filter((v: Voice) => v.id);
}
