...
posixOnly("conversation branching e2e (fake ACP fleet)", () => {
  ...
  beforeAll(async () => {
    ...
    home = mkdtempSync(join(tmpdir(), "hry-branch-test-"));
  ...
  it(
    ...
    async () => {
      ...
      const bot = await getBot(created.id);
      const log = readFileSync(join(home, ".helmryth", "native", `${bot.threadId}.ndjson`), "utf8");
      const prompts = log
        .split("\n")
        .filter(Boolean)
      ...
      expect(prompts[0]).not.toContain("joining this workstream");
      // second engine: joined mid-thread with the earlier exchange inline
      expect(prompts[1]).toMatch(/joining this workstream[\s\S]*User: my dog is named Biscuit[\s\S]*what is my dog called\?/);
      ...
      expect(prompts[2]).toMatch(/joining this workstream[\s\S]*User: what is my dog called\?[\s\S]*and again\?/);
    },
    40_000,
  );
});
