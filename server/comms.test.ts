...
describe("mentionedBots", () => {
  ...
  it("matches a tag at a word start, case-insensitively", () => {
    expect(mentionedBots("hey @divyam, look", peers).map((b) => b.id)).toEqual(["3"]);
    expect(mentionedBots("@Divyam first thing", peers).map((b) => b.id)).toEqual(["3"]);
  });
  ...
  it("requires a word boundary at the end of the name", () => {
    ...
    expect(mentionedBots("@Divyamo is someone else", peers)).toEqual([]);
  });
});
...
describe("roomResponders", () => {
  ...
  it("lets explicit mentions override the configured lead", () => {
    expect(roomResponders("@Divyam take this", members, { kind: "member", botId: "atlas" })).toEqual([members[1]]);
  });
...
describe("comms e2e (fake ACP fleet)", () => {
  ...
  beforeAll(async () => {
    ...
    home = mkdtempSync(join(tmpdir(), "hry-comms-test-"));
  ...
  it(
    ...
    async () => {
      ...
      expect(askerBot.activity).toBe("waiting-on-you");
      expect(askerBot.busy).toBe(true);
      ...
      const resumedAsker = (await api("GET", "/api/bots")).body.bots.find((b: any) => b.id === asker.id);
      expect(resumedAsker.activity).toBe("working");
      expect(resumedAsker.busy).toBe(true);
