...
/** Node refuses an argv entry containing a NUL and throws before the child
 * exists, so one null byte anywhere in a composed prompt kills the whole turn
 * with a message about an argv index. A profile that already holds one (stored
 * before the input guard existed) must still be able to run — drop the byte
 * rather than leaving the operator permanently broken. */
function withoutNulBytes(args: string[]): string[] {
  if (!args.some((arg) => arg.includes("\u0000"))) return args;
  console.error("helmryth: stripped NUL bytes from CLI arguments; check the operator profile for pasted control characters");
...
export function spawnCli(
  ...
  const resolved = resolveCli(cli, withoutNulBytes(args));
  ...
  // SAFETY: every driver supplies `stdio: "pipe"` or the equivalent
  // three-pipe tuple; this helper's return contract deliberately exposes
  // those non-null streams to its callers.
  const pipedChild = child as ChildProcessByStdio<Writable, Readable, Readable>;
 ...
 * and bind(2) answers a longer path with EINVAL rather than truncating. Keep
 * one byte of headroom for the shortest of the two. */
...
export function brokerSocketPath(dataDir: string, tag: string): string {
  if (process.platform === "win32") {
    ...
    return `\\\\.\\pipe\\helmryth-perm-${process.pid}-${tag}`;
  }
  const preferred = join(dataDir, `perm-${tag}.sock`);
  if (Buffer.byteLength(preferred, "utf8") <= SUN_PATH_MAX) return preferred;

  // A deep data dir (a synced Documents folder, a nested test lane) pushes the
  // socket past the limit, listen() fails EINVAL, and every permission gate
  // then degrades to a deny nobody can explain. Fall back to a short path —
  // but keep it 0700 and owner-only, because whoever binds this socket decides
  // what the operator is allowed to do.
  const room = createHash("sha256").update(dataDir).digest("hex").slice(0, 8);
  const dir = join(tmpdir(), `helmryth-perm-${process.getuid?.() ?? 0}-${room}`);
  ensurePrivateDirectory(dir);
  return join(dir, `${tag}.sock`);
}
