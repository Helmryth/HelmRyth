...
import { parseCrewManifest } from "./team-manifest.ts";
import { scoutProject, suggestTeam, type ProjectProfile } from "./project-scout.ts";

let dirs: string[] = [];

function project(files: Record<string, string>): string {
  const dir = mkdtempSync(join(tmpdir(), "hry-scout-"));
  dirs.push(dir);
...
describe("scoutProject", () => {
  it("names the project from the README and summarizes its first paragraph", () => {
    const dir = project({
      "README.md": "[![ci](x)](y)\n# Sigil Tracker\n\nTracks every sigil in the house.\n\nMore prose.",
      "package.json": JSON.stringify({ name: "not-this", description: "not this either" }),
    });
    const profile = scoutProject(dir);
    expect(profile.name).toBe("Sigil Tracker");
    expect(profile.summary).toBe("Tracks every sigil in the house.");
  });
...
describe("suggestTeam", () => {
  const profile: ProjectProfile = {
    name: "Sigil Tracker",
    summary: "Tracks every sigil in the house.",
  ...
  it("always leads with a lead, then one member per signal, each with a reason", () => {
    ...
    expect(suggestion.roomName).toBe("Sigil Tracker");
    expect(suggestion.manifest.crew.operators.map((operator) => operator.key)).toEqual(["lead", "frontend", "testing"]);
    expect(suggestion.reasons.frontend).toContain("react");
    expect(suggestion.manifest.crew.description).toBe(profile.summary);
    const frontend = suggestion.manifest.crew.operators[1]!;
    expect(frontend.description).toContain("Sigil Tracker");
  ...
  it("adds a generalist when nothing was detected", () => {
    ...
    expect(suggestion.manifest.crew.operators.map((operator) => operator.key)).toEqual(["lead", "builder"]);
  });
  ...
  it("caps the lineup at a lead plus five specialists", () => {
    ...
    expect(suggestTeam(wide).manifest.crew.operators).toHaveLength(6);
  });
  ...
  it("emits a manifest the importer accepts verbatim", () => {
    ...
    expect(() => parseCrewManifest(JSON.parse(JSON.stringify(suggestion.manifest)))).not.toThrow();
  });
});
