...
import type { CrewManifestOperator } from "./team-manifest.ts";
...
const packageSchema = z.object({
  format: z.literal(BOT_PACKAGE_FORMAT, { error: "This is not a Helmryth package" }),
  ...
  package: z.object({
    ...
    tags: z.array(requiredText(80)).max(30, { message: "must have at most 30 entries" }).optional(),
    outcomes: z.array(requiredText(240)).min(1, { message: "must have at least one entry" }).max(12, { message: "must have at most 12 entries" }),
    setupMinutes: z.number().int().min(1, { message: "must be at least 1" }).max(240, { message: "must be at most 240" }),
    requirements: z.object({
      ...
      })).max(30, { message: "must have at most 30 entries" }),
      capabilities: z.array(requiredText(80)).max(20, { message: "must have at most 20 entries" }),
      platforms: z.array(requiredText(80)).max(10, { message: "must have at most 10 entries" }).optional(),
    }),
    agents: z.array(z.object({
      ...
      playbooks: z.array(key).max(40, { message: "must have at most 40 entries" }).optional(),
    })).min(1, { message: "must have at least one entry" }).max(200, { message: "must have at most 200 entries" }),
    chiefOfStaff: key.optional(),
    rooms: z.array(z.object({
      key,
      name: requiredText(100),
      members: z.array(key).min(1, { message: "must have at least one entry" }).max(200, { message: "must have at most 200 entries" }),
    ...
    })).max(30, { message: "must have at most 30 entries" }).optional(),
    routines: z.array(z.object({
      ...
      schedule: z.discriminatedUnion("type", [
        ...
        z.object({
          ...
          weekdays: z.array(z.number().int().min(0, { message: "must be at least 0" }).max(6, { message: "must be at most 6" })).min(1, { message: "must be at least 1" }).max(7, { message: "must be at most 7" }),
        }),
      ]),
      durationMinutes: z.number().int().min(15, { message: "must be at least 15" }).max(240, { message: "must be at most 240" }),
      enabledAfterInstall: z.literal(false),
    })).max(50, { message: "must have at most 50 entries" }).optional(),
    playbooks: z.array(z.object({
      key,
      name: requiredText(100),
      summary: requiredText(300),
      triggers: z.array(requiredText(100)).min(1, { message: "must have at least one entry" }).max(30, { message: "must have at most 30 entries" }),
      instructions: requiredText(24_000),
    })).max(80, { message: "must have at most 80 entries" }).optional(),
    examples: z.array(z.object({
      title: requiredText(120),
      input: requiredText(4_000),
      output: requiredText(8_000),
    })).max(12, { message: "must have at most 12 entries" }).optional(),
  }),
});
...
function markdownDocument(markdown: string): JsonValue {
  if (Buffer.byteLength(markdown) > 1_000_000) throw new Error("The operator playbook is too large");
  ...
  const metadataResult = z.record(z.string(), z.json()).safeParse(metadata);
...
export function parseBotPackage(value: JsonValue | ParsedBotPackage): ParsedBotPackage {
  const markdown = z.string().safeParse(value);
  const source = markdown.success ? markdownDocument(markdown.data) : value;
  const parsed = packageSchema.safeParse(source);
  if (!parsed.success) throw new Error(schemaIssue(parsed.error, "This is not an operator package"));
  ...
  if (pkg.chiefOfStaff && !agents.has(pkg.chiefOfStaff)) {
    throw new Error(`Unknown lead operator: ${pkg.chiefOfStaff}`);
  }
 ...
 * run without Helmryth or another proprietary parser. */
export function renderBotPackageMarkdown(document: ParsedBotPackage): string {
  ...
  const frontmatter = stringifyYaml(
    { [CREW_MARKDOWN_VERSION_KEY]: CREW_MARKDOWN_VERSION, ...pkg },
    { lineWidth: 0 },
  ).trim();
  ...
  return `---\n${frontmatter}\n---\n\n# ${pkg.name}\n\n${pkg.tagline}\n\n> **Give this file to your lead operator.** It is the complete crew blueprint. Any operator system can run it; Helmryth can also install it directly.\n\n## Activation\n\nYou are the lead operator for this blueprint. Read the whole document before acting. Confirm the user's goal and any missing inputs, then create or delegate to the specialist roles below. Preserve their names, ownership, boundaries, shared-room rules, and playbooks. If your platform cannot literally spawn operators, perform the roles one at a time and keep their outputs clearly separated.\n\nNever request pasted passwords or secret keys. Use the platform's normal connection flow. Do not send messages, publish content, spend money, delete data, or enable a schedule without the user's explicit approval. All routines start paused.\n\n## Mission\n\n${pkg.summary}\n\n## Outcomes\n\n${list(pkg.outcomes)}\n\n## Connections\n\n${connections}\n\n## Team\n\n${agents}\n\n## Lead operator\n\nThe lead operator role is \`${pkg.chiefOfStaff ?? pkg.agents[0].key}\`. This role owns delegation, synthesis, conflict resolution, and the final answer to the user.\n${rooms ? `\n## Shared rooms\n\n${rooms}\n` : ""}${routines ? `\n## Suggested routines\n\n${routines}\n` : ""}${playbooks ? `\n## Playbooks\n\n${playbooks}\n` : ""}${examples ? `\n## Example job\n\n${examples}\n` : ""}\n## Completion rule\n\nReturn one clear result to the user, distinguish evidence from inference, cite source links when the work uses external material, and state what still needs human approval or a connected app.\n`;
}

export function packageAgentAsOperator(agent: BotPackageAgent): CrewManifestOperator {
  const appearance: CrewManifestOperator["appearance"] = {
    color: agent.appearance.color,
  };
  if (agent.appearance.sigilExpression) appearance.sigilExpression = agent.appearance.sigilExpression;
  return {
    key: agent.key,
    name: agent.name,
    title: agent.title ?? "",
