...
function jsonResponse(body: JsonValue, options: { ok?: boolean; status?: number; statusText?: string } = {}): Response {
...
afterEach(() => {
  ...
  delete process.env.HELMRYTH_TOKEN;
...
describe("MCP JSON-RPC protocol", () => {
  it("negotiates supported and newer protocol versions", async () => {
    ...
    expect(supported.result).toMatchObject({
      ...
      serverInfo: { name: "helmryth-mcp", version: "1.1.0" },
      capabilities: { tools: {} },
    });
  ...
  it("lists a closed, annotated orchestration surface", async () => {
    ...
    expect(names).toContain("create_operator");
    expect(names).toContain("create_crew");
  ...
  it("returns structured and text tool results", async () => {
    const handler = vi.fn<typeof handleToolCall>(async () => ({ bots: [{ id: "bot-1" }] }));
    const response = JSON.parse((await processMcpMessage(JSON.stringify({
      jsonrpc: "2.0",
      id: 3,
      method: "tools/call",
      params: { name: "list_operators", arguments: {} },
    }), handler))!);
  ...
  it("rejects unknown tools and malformed arguments as invalid params", async () => {
    for (const params of [
      ...
      { name: "send_operator_message", arguments: { operator_id: "bot-1", text: { not: "text" } } },
      { name: "list_operators", arguments: { unexpected: true } },
    ]) {
      const response = JSON.parse((await processMcpMessage(JSON.stringify({
  ...
  it("cancels an in-flight tool call with the MCP cancellation notification", async () => {
    const handler = vi.fn<typeof handleToolCall>(async (_name, _args, _fetcher, signal) => {
      if (!signal) throw new Error("test call did not receive an abort signal");
    ...
    const pending = processMcpMessage(JSON.stringify({
      ...
      params: {
        ...
        arguments: { target_type: "operator", target_id: "bot-1" },
      },
    }), handler);
...
describe("MCP tool execution", () => {
  it("lists bots without fetching transcripts", async () => {
    ...
    const result: any = await handleToolCall("list_operators", {}, fetcher);
    expect(result.operators[0]).toMatchObject({ id: "bot-1", activeTaskId: "task-1", activity: "idle" });
    expect(result.operators[0].tasks[0]).toMatchObject({ taskId: "task-1", active: true });
    expect(result.operators[0]).not.toHaveProperty("messages");
  });
  ...
  it("reads a bounded bot task and removes pixels and approval grant keys", async () => {
    ...
    const result: any = await handleToolCall("get_operator_messages", { operator_id: "bot-1", limit: 200 }, fetcher);
  ...
  it("pins bot and channel sends to the active task", async () => {
    ...
    await expect(handleToolCall("send_operator_message", {
      operator_id: "bot-1", run_id: "bot-old", text: "Wrong task",
    }, fetcher)).rejects.toThrow("not active");
    await expect(handleToolCall("send_crew_message", {
      crew_id: "channel-1", run_id: "channel-old", text: "Wrong task",
    }, fetcher)).rejects.toThrow("not active");

    await expect(handleToolCall("send_operator_message", {
      operator_id: "bot-1", text: "Investigate",
    }, fetcher)).resolves.toMatchObject({ success: true, runId: "bot-task" });
    await expect(handleToolCall("send_crew_message", {
      crew_id: "channel-1", text: "Ship it",
    }, fetcher)).resolves.toMatchObject({ success: true, runId: "channel-task" });
  });
  ...
  it("creates a bot through the safe profile boundary", async () => {
    ...
    const result: any = await handleToolCall("create_operator", { name: "Mira", title: "Researcher", section: "Work" }, fetcher);
    expect(result.operator).toMatchObject({ id: "bot-new", name: "Mira", section: "Work" });
  });
  ...
  it("leaves single-request bot-create failures to the server without destructive rollback", async () => {
    ...
    await expect(handleToolCall("create_operator", { name: "Mira" }, fetcher)).rejects.toThrow("profile rejected");
  ...
  it("routes bot and channel task mutations", async () => {
    ...
    const created: any = await handleToolCall("create_run", { target_type: "crew", target_id: "channel-1", title: "Fresh" }, fetcher);
    expect(fetcher).toHaveBeenLastCalledWith("/api/groups/channel-1/tasks", expect.objectContaining({ method: "POST" }));
    expect(created.run.taskId).toBe("task-2");
    const switched: any = await handleToolCall("switch_run", {
      target_type: "operator", target_id: "bot-1", run_id: "task-2",
    }, fetcher);
  ...
  it("rejects incomplete mutation responses before projecting them", async () => {
    ...
    await expect(handleToolCall("update_operator_profile", {
      operator_id: "bot-1", name: "Mira",
    }, fetcher)).rejects.toThrow("Helmryth did not return the updated operator");
    await expect(handleToolCall("update_crew", {
      crew_id: "channel-1", name: "Launch",
    }, fetcher)).rejects.toThrow("Helmryth did not return the updated crew");
    await expect(handleToolCall("create_run", {
      target_type: "operator", target_id: "bot-1", title: "Fresh",
    }, fetcher)).rejects.toThrow("Helmryth did not return the created run");
    await expect(handleToolCall("rename_run", {
      target_type: "operator", target_id: "bot-1", run_id: "task-1", title: "Renamed",
    }, fetcher)).rejects.toThrow("Helmryth did not return the renamed run");
  });

  it("searches with encoded, bounded parameters", async () => {
    const fetcher = vi.fn(async () => ({ hits: [{ messageId: "m1" }] }));
    const result: any = await handleToolCall("search_messages", { query: "release notes", run_id: "task-1", limit: 100 }, fetcher);
  ...
  it("requires an exact available model and refuses changes while busy", async () => {
    ...
    await expect(handleToolCall("set_operator_model", {
      operator_id: "bot-1", instance_id: "codex", model: "gpt-5.6-sol",
    ...
    await expect(handleToolCall("set_operator_model", {
      operator_id: "bot-1", instance_id: "codex", model: "made-up",
    }, fetcher)).rejects.toThrow("not offered");
    const result: any = await handleToolCall("set_operator_model", {
      operator_id: "bot-1", instance_id: "codex", model: "gpt-5.6-sol", effort: "high",
    }, fetcher);
  ...
  it("waits on a bot conversation and returns a compact attention tail", async () => {
    ...
    const result: any = await handleToolCall("wait_for_conversation", {
      target_type: "operator", target_id: "bot-1", timeout_seconds: 1,
    }, fetcher);
  ...
  it("reports no-signal as stalled and keeps the frozen task tail", async () => {
    ...
    const historical: any = await handleToolCall("wait_for_conversation", {
      target_type: "operator", target_id: "bot-1", run_id: "task-old", timeout_seconds: 1,
    }, fetcher);
    expect(historical).toMatchObject({ status: "settled", runId: "task-old", messages: [{ text: "Old task" }] });
  ...
  it("reports asynchronous bot and channel dispatch failures", async () => {
    ...
    const channelResult: any = await handleToolCall("wait_for_conversation", {
      target_type: "crew", target_id: "channel-1", timeout_seconds: 1,
    }, channelFetcher);
  ...
  it("detects durable channel blockers and interrupts the exact target thread", async () => {
    ...
    const interrupted: any = await handleToolCall("interrupt_conversation", {
      target_type: "crew", target_id: "channel-1",
    }, fetcher);
    expect(interrupted).toEqual({
      success: true, targetType: "crew", targetId: "channel-1", runId: "task-1",
    });
  });
...
describe("connection security and discovery", () => {
  it("accepts loopback HTTP and HTTPS origins, but rejects unsafe URL shapes", () => {
    ...
    expect(validateBaseUrl("https://sigil.example.com")).toBe("https://sigil.example.com");
    expect(() => validateBaseUrl("ftp://sigil.example.com")).toThrow("http:// or https://");
    expect(() => validateBaseUrl("https://sigil.example.com/api")).toThrow("origin without a path");
    expect(() => validateBaseUrl("https://user:pass@sigil.example.com")).toThrow("must not contain credentials");
  ...
  it("skips a foreign process and discovers the real fallback port", async () => {
    globalThis.fetch = vi.fn(async (url: string | URL | Request) => {
      if (String(url).includes(":8799")) return jsonResponse({ app: "not-helmryth" });
      if (String(url).includes(":18799")) return jsonResponse({ app: "helmryth" });
  ...
  it("rejects successful non-JSON responses and sends an optional bearer token", async () => {
    process.env.HELMRYTH_TOKEN = "proxy-token";
    ...
    await expect(request("/api/health", {}, "https://sigil.example.com")).rejects.toThrow("non-JSON response");
  });

  it("requires an explicit destination before sending a bearer token", async () => {
    process.env.HELMRYTH_TOKEN = "proxy-token";
    await expect(resolveBaseUrl()).rejects.toThrow("HELMRYTH_URL or HELMRYTH_PORT");
  });

  it("validates direct tool arguments", () => {
    expect(() => validateToolArguments("send_operator_message", { operator_id: "bot-1", text: "hello" })).not.toThrow();
    expect(() => validateToolArguments("send_operator_message", { operator_id: "bot-1", text: "hello", extra: true })).toThrow("not supported");
  });
});
