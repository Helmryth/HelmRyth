...
function fakeStore(bots: BotRecord[]): SteerStore & { messages: Message[] } {
  ...
  return {
    ...
    ownsThread: (botId, threadId) => bots.some((bot) =>
      bot.id === botId && (bot.threadId === threadId || bot.tasks?.some((task) => task.threadId === threadId)),
    ),
    messageByQueueId: (threadId, queueId) =>
      messages.find((message) => message.queueId === queueId && message.id.endsWith(`-${threadId}`)) ?? null,
...
describe("steer-queue module", () => {
  it("does not append a queued user message until drain", async () => {
    ...
    bot.busy = false;
    const run = vi.fn();
    await drainSteeredMessages(store, run);
  ...
  it("keeps a stable client receipt while a send waits to drain", async () => {
    ...
    bot.busy = false;
    await drainSteeredMessages(store, vi.fn());
    expect(store.messages[0]).toMatchObject({
      text: "retry safely",
      sendId: "send_1234567890123456",
      queueId: queued.id,
  ...
  it("holds the queue while the bot is busy and drains it once when idle", async () => {
  ...
  it("drops a cancelled message so drain does not send it", async () => {
  ...
  it("keeps reply metadata and the provider-facing reply prompt while queued", async () => {
  ...
  it("fires nothing when nothing is queued", async () => {
    const run = vi.fn();
    await drainSteeredMessages(fakeStore([fakeBot("bot-c", "thread-c", false)]), run);
    expect(run).not.toHaveBeenCalled();
  });

  it("drops the queue of a deleted bot without running it", async () => {
    const bot = fakeBot("bot-d", "thread-d", true);
    queueSteeredMessage(bot.id, bot.threadId, "orphaned");
    const run = vi.fn();
    await drainSteeredMessages(fakeStore([]), run);
    expect(run).not.toHaveBeenCalled();
    expect(_queuedCount("thread-d")).toBe(0);
  });

  it("reloads durable directions in order and commits one idempotent drain", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "hry-steer-unit-"));
    try {
      initializeSteeredMessageQueue(dataDir);
      const bot = fakeBot("bot-durable", "thread-durable", true);
      const first = queueSteeredMessage(bot.id, bot.threadId, "first after restart", {
        sendId: "send_durable_1234567890",
      });
      const second = queueSteeredMessage(bot.id, bot.threadId, "second after restart", {
        sendId: "send_durable_2234567890",
      });

      // A fresh initialization stands in for a new process reading the last
      // atomic snapshot. Stable receipts survive with their exact ownership.
      initializeSteeredMessageQueue(dataDir);
      expect(_queuedCount(bot.threadId)).toBe(2);
      expect(queuedSteeredMessage(bot.id, bot.threadId, "send_durable_1234567890")?.id).toBe(first.id);
      ...
      await drainSteeredMessages(store, run);
      expect(run).toHaveBeenCalledTimes(1);
      expect(run.mock.calls[0][2]).toBe("first after restart\nsecond after restart");
      expect(store.messages.map((message) => message.queueId)).toEqual([first.id, second.id]);
  ...
  it("persists cancellation and drops deleted ownership before recovery dispatch", async () => {
    const dataDir = mkdtempSync(join(tmpdir(), "hry-steer-owner-"));
    try {
      initializeSteeredMessageQueue(dataDir);
      const bot = fakeBot("bot-owner", "thread-owner", true);
      const cancelled = queueSteeredMessage(bot.id, bot.threadId, "cancel durably");
      expect(cancelSteeredMessage(bot.id, cancelled.id)).toBe(true);
      initializeSteeredMessageQueue(dataDir);
      expect(_queuedCount(bot.threadId)).toBe(0);

      queueSteeredMessage(bot.id, bot.threadId, "owner deleted");
      expect(discardSteeredMessagesForBot(bot.id)).toBe(1);
      initializeSteeredMessageQueue(dataDir);
      expect(_queuedCount(bot.threadId)).toBe(0);

      queueSteeredMessage(bot.id, bot.threadId, "task deleted");
      bot.threadId = "another-thread";
      const run = vi.fn();
      await drainSteeredMessages(fakeStore([bot]), run);
      expect(run).not.toHaveBeenCalled();
      initializeSteeredMessageQueue(dataDir);
      expect(_queuedCount("thread-owner")).toBe(0);
    } finally {
      rmSync(dataDir, { recursive: true, force: true });
    }
  });
...
describe("steer-queue e2e (fake ACP fleet)", () => {
  ...
  const stopHarness = async (signal: NodeJS.Signals = "SIGTERM") => {
  ...
  beforeAll(async () => {
    ...
    home = mkdtempSync(join(tmpdir(), "hry-steer-test-"));
  ...
  it(
    "recovers queued directions once after a hard restart and preserves cancellation",
    async () => {
      rmSync(drainGate, { force: true });
      const bot = await newBot("steer", "Restart durable");
      expect((await api("POST", `/api/bots/${bot.id}/messages`, { text: "work before crash" })).status).toBe(202);
      await until(async () => (await botById(bot.id)).busy === true, "the pre-crash turn");

      const firstSendId = "send_restart_1234567890";
      const secondSendId = "send_restart_2234567890";
      const first = await api("POST", `/api/bots/${bot.id}/messages`, {
        text: "first recovered direction",
        sendId: firstSendId,
      });
      const second = await api("POST", `/api/bots/${bot.id}/messages`, {
        text: "second recovered direction",
        sendId: secondSendId,
      });
      expect(first.body).toMatchObject({ queued: true, threadId: bot.threadId });
      expect(second.body).toMatchObject({ queued: true, threadId: bot.threadId });
      expect(readFileSync(join(home, ".helmryth", "steer-queue.json"), "utf8")).toContain(firstSendId);

      // SIGKILL is intentional: graceful shutdown could settle the provider
      // and drain before exit. The next process must recover the atomic file.
      await stopHarness("SIGKILL");
      await startHarness();
      await until(async () => {
        const snapshot = await botById(bot.id);
        const texts = snapshot.messages.filter((message: any) => message.role === "user").map((message: any) => message.text);
        return texts.includes("first recovered direction") && texts.includes("second recovered direction") && snapshot.busy;
      }, "the recovered queued turn");

      const retry = await api("POST", `/api/bots/${bot.id}/messages`, {
        text: "first recovered direction",
        sendId: firstSendId,
      });
      expect(retry.body.queued).toBeUndefined();
      expect(retry.body).toMatchObject({
        message: { text: "first recovered direction", sendId: firstSendId, queueId: first.body.queueId },
      });

      writeFileSync(drainGate, "open");
      let recovered: any;
      await until(async () => {
        recovered = await botById(bot.id);
        return !recovered.busy && echoes(recovered).some((message) =>
          message.text.includes("first recovered direction\nsecond recovered direction"),
        );
      }, "the recovered direction reply");
      const recoveredTexts = recovered.messages
        .filter((message: any) => message.role === "user")
        .map((message: any) => message.text);
      expect(recoveredTexts.filter((text: string) => text === "first recovered direction")).toHaveLength(1);
      expect(recoveredTexts.filter((text: string) => text === "second recovered direction")).toHaveLength(1);
      expect(recoveredTexts.indexOf("first recovered direction")).toBeLessThan(
        recoveredTexts.indexOf("second recovered direction"),
      );

      // Cancellation is a durable mutation too: a cancelled queue id cannot
      // reappear merely because the process dies immediately afterward.
      rmSync(drainGate, { force: true });
      expect((await api("POST", `/api/bots/${bot.id}/messages`, { text: "second crash window" })).status).toBe(202);
      await until(async () => (await botById(bot.id)).busy === true, "the cancellation crash window");
      const cancelled = await api("POST", `/api/bots/${bot.id}/messages`, {
        text: "must stay cancelled",
        sendId: "send_cancelled_1234567890",
      });
      expect(cancelled.body.queued).toBe(true);
      expect((await api("DELETE", `/api/bots/${bot.id}/queue/${cancelled.body.queueId}`)).status).toBe(200);

      await stopHarness("SIGKILL");
      await startHarness();
      const afterCancellation = await botById(bot.id);
      expect(afterCancellation.messages.some((message: any) => message.text === "must stay cancelled")).toBe(false);
      expect(readFileSync(join(home, ".helmryth", "steer-queue.json"), "utf8")).not.toContain("send_cancelled_1234567890");
    },
    90_000,
  );
});
