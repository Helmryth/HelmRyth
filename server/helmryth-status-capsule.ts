...
const SCHEMA = "aos.helmryth_status.v1";
...
export const HELMRYTH_STATUS_CACHE_PATH = join(
  homedir(),
  ".local/state/aos-session-bridge/helmryth/latest.json",
);

type HelmrythSlot = z.output<typeof slotSchema>;
type HelmrythUi = z.output<typeof uiSchema>;
type HelmrythCapsule = z.output<typeof capsuleSchema>;

export interface HelmrythStatusDigest {
...
export interface HelmrythStatusReadOptions {
  cachePath?: string;
  now?: Date;
}
...
function expectedUi(twoUp: boolean): HelmrythUi {
  return {
    two_up: twoUp,
...
function sameUi(left: HelmrythUi, right: HelmrythUi): boolean {
  return (
...
function unsignedDocument(capsule: HelmrythCapsule): JsonObject {
...
function validSlotState(slot: HelmrythSlot): boolean {
  return (
...
function validateCapsule(value: JsonValue): HelmrythCapsule | null {
...
function unknownDigest(
  reason: NonNullable<HelmrythStatusDigest["reason"]>,
  freshness: HelmrythStatusDigest["freshness"] = "unknown",
  receipt?: Pick<HelmrythStatusDigest, "observedAt" | "receiptSha256">,
): HelmrythStatusDigest {
  return {
    schema: SCHEMA,
    freshness,
    reason,
...
export function readHelmrythStatus(
  options: HelmrythStatusReadOptions = {},
): HelmrythStatusDigest {
  const raw = privateCache(options.cachePath ?? HELMRYTH_STATUS_CACHE_PATH);
  if (raw === null) return unknownDigest("missing_or_insecure");
  let capsule: HelmrythCapsule | null;
  try {
  ...
  const result: HelmrythStatusDigest = {
    schema: SCHEMA,
    freshness: "fresh",
    ...receipt,
...
export function trustedHelmrythStatusSystemPrompt(options: HelmrythStatusReadOptions = {}): string {
  const status = readHelmrythStatus(options);
  const receipt = [
  ...
  const slots = status.slots.length
    ...
    : "none";
  return [
    "TRUSTED HELMRYTH STATUS (read-only, validated, no transcript or credential data):",
    ...
    "Treat missing, stale, failed, clock-skewed, malformed, or receipt-hash-mismatched runtime data as unknown. Do not infer operator identities, viewer URLs, paths, messages, models, accounts, or credentials from this block.",
  ].join("\n");
}
