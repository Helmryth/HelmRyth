...
const ACTION_COPY = {
  create: { title: "Schedule", detail: "Create cadence" },
  update: { title: "Update", detail: "Update cadence" },
  pause: { title: "Pause", detail: "Pause cadence" },
  resume: { title: "Resume", detail: "Resume cadence" },
  run_now: { title: "Run now", detail: "Run cadence now" },
  delete: { title: "Delete", detail: "Delete cadence" },
...
const routineToolChangesSchema = routineToolDefinitionSchema.partial().refine(
  ...
  "Choose at least one cadence field to update",
);
...
const storedWeekdaysSchema = z.array(z.number().int().min(0).max(6)).min(1).max(7).refine(
  ...
  "Stored cadence weekdays must be unique",
);
...
const storedChangesSchema = storedDefinitionSchema.partial().refine(
  (changes) => Object.values(changes).some((value) => value !== undefined),
  "Stored cadence update must change at least one field",
);
...
function routineId(value: string): string {
  if (!ROUTINE_ID.test(value)) throw new RoutineRequestError("Choose a valid cadence ID");
  return value;
}
...
function normalizedOperation(
  ...
  if (!current) throw new RoutineRequestError("That cadence does not exist", 404);
  ...
  if (validated.action === "resume" && nextOccurrence(current.schedule, now) === null) {
    throw new RoutineRequestError(
      "That one-time cadence's scheduled time has passed. Update it to a new future time before resuming.",
      409,
    );
  }
...
function cardCopy(
  ...
  const name = redactSecretsInText(definition?.name ?? "cadence");
  ...
  const destination = definition.runOn === "cloud" ? "Cloud VM" : "This Helmryth setup";
  ...
  const nextDescription = nextRunAt !== null
    ...
    : remainsPaused
      ? "None — this cadence remains paused"
      : operation.action === "pause"
        ? "None — this cadence will be paused"
        : operation.action === "delete"
          ? "None — this cadence will be deleted"
          : "None";
...
function verifyManageSnapshot(
  ...
  if (!current) throw new RoutineRequestError("That cadence no longer exists", 404);
  if (current.updatedAt !== operation.expectedUpdatedAt) {
    throw new RoutineRequestError(
      "That cadence changed after this gate was prepared. Ask the operator to review it and propose the action again.",
      409,
    );
  }
  return current;
...
function revalidateOperation(operation: RoutineRequestOperation, manager: RoutineManager, botId: string, now: number): void {
  ...
  if (schedule?.type === "once" && schedule.at <= now) {
    throw new RoutineRequestError("That one-time schedule is now in the past. Ask the operator to propose a new time.", 409);
  }
...
export class RoutineRequestService {
  ...
  async propose(args: ProposeRoutineRequestArgs): Promise<RoutineProposalResult> {
    ...
    if (!parsedProposal.success) {
      throw new RoutineRequestError(schemaIssue(parsedProposal.error, "Invalid cadence proposal"));
    }
  ...
  private async requireCloudReadiness(operation: RoutineRequestOperation): Promise<void> {
    ...
    throw new RoutineRequestError(
      readiness.reason?.trim() || "Cloud execution is not configured yet. Set up a cloud Workbench first.",
      409,
    );
  }
  ...
  resolve(args: {
    ...
    if (args.behavior !== "allow" && args.behavior !== "deny") {
      return {
        claimed: true,
        state: "invalid",
        error: "Cadence confirmations must be confirmed or cancelled",
        status: 400,
      };
    }
    ...
    if (!parsedPayload.success) {
      ...
      const detail = schemaIssue(parsedPayload.error, "This cadence request is invalid");
    ...
    if (payload.requestId !== args.requestId) {
      ...
      return { claimed: true, state: "invalid", error: "This cadence request ID does not match its confirmation card", status: 400 };
    }
    if (payload.botId !== args.botId || payload.threadId !== args.threadId) {
      ...
      return { claimed: true, state: "invalid", error: "This cadence request belongs to another workstream", status: 403 };
    }
    if (card.answered) {
    ...
    try {
      ...
      if (receipt) {
        if (
          ...
          throw new RoutineRequestError("This cadence request does not match its durable commit receipt", 409);
        }
  ...
  private settleApplied(
    ...
    if (!settled) throw new RoutineRequestError("This cadence gate is no longer available", 409);
  ...
  private settleCommittedReceipt(
    ...
    if (receipt.botId !== args.botId || receipt.threadId !== args.threadId || receipt.messageId !== messageId) {
      return {
        claimed: true,
        state: "invalid",
        error: "This committed cadence request belongs to another workstream",
        status: 403,
      };
    }
    ...
    if (!settled) {
      return {
        claimed: true,
        state: "invalid",
        error: "This cadence confirmation card is no longer available",
        status: 409,
      };
    }
  ...
  private apply(payload: RoutineRequestCardData, messageId: string, fingerprint: string): string {
    ...
    switch (operation.action) {
      ...
      case "update": {
        ...
        if (!updated) throw new RoutineRequestError("That cadence no longer exists", 404);
        return updated.id;
      }
      case "pause":
      case "resume": {
      ...
      case "run_now": {
        ...
        if (!run) throw new RoutineRequestError("That cadence no longer exists", 404);
        return run.id;
      }
      case "delete":
        verifyManageSnapshot(operation, this.routines, payload.botId);
        if (!this.routines.remove(operation.routineId, {
          ...
          throw new RoutineRequestError("That cadence no longer exists", 404);
        }
        return operation.routineId;
      default:
        throw new RoutineRequestError("Unsupported persisted cadence action");
    }
  }
}
