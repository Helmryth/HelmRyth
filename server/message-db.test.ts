...
describe("message-db", () => {
  ...
  it("stores transcripts with owner-only permissions", () => {
    ...
    if (process.platform !== "win32") {
      for (const suffix of ["", "-wal", "-shm"]) {
        const file = join(DATA_DIR, `messages.db${suffix}`);
        if (existsSync(file)) expect(statSync(file).mode & 0o777).toBe(0o600);
      }
    }
  });

  it.skipIf(process.platform === "win32")("repairs a permissive legacy database and imported transcript", () => {
    insertMessage("legacy-mode", msg("m1", "kept"));
    closeMessageDb();
    const database = join(DATA_DIR, "messages.db");
    chmodSync(database, 0o644);
    writeFileSync(legacy("private-import"), JSON.stringify([msg("old", "private legacy")]), { mode: 0o644 });

    expect(readThread("legacy-mode", legacy("legacy-mode")).messages[0]?.text).toBe("kept");
    expect(readThread("private-import", legacy("private-import")).messages[0]?.text).toBe("private legacy");
    expect(statSync(database).mode & 0o777).toBe(0o600);
    expect(statSync(`${legacy("private-import")}.imported`).mode & 0o777).toBe(0o600);
  });
  ...
  it("search reports the match offset for highlighting, and finds activity chips by tool name", () => {
    ...
    const activity: Message = {
      ...
      kind: "activity",
      role: "bot",
      tool: { name: "Bash: alembic upgrade head", ok: true },
    };
    const attributed: Message = {
      ...msg("m3", "we spoke about it"),
      from: { botId: "b2", name: "Scout", color: "green" },
    };
    insertMessage("t7", activity);
    insertMessage("t7", attributed);
