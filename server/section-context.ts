...
function loadFile(): SectionContextFile {
  ...
  try {
    ...
    // Same reasoning as routines/webhooks: a missing file is a first run and
    // must stay silent; unreadable bytes that DO exist are preserved.
    quarantineCorruptFile(SECTION_CONTEXTS_FILE, "", cause);
    return emptyFile();
  }
}
...
export function sectionContextSystemPrompt(section?: string | null): string {
  ...
  return (
    `\n\nShared context for the ${JSON.stringify(label)} section follows. The user manages this reference for every operator in the crew; you cannot edit it.` +
