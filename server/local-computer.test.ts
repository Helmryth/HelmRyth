...
function linuxDescriptor(userData: string, { session = "x11" }: { session?: "x11" | "wayland" } = {}) {
  ...
  if (session === "wayland") {
    Object.assign(descriptor, { compositor: "gnome-mutter" });
    Object.assign(descriptor.mcp.env, { CUA_DRIVER_RS_ENABLE_WAYLAND: "1" });
  }
  return descriptor;
}
...
function privateUserData(name: string) {
  ...
  const root = mkdtempSync(join(base, "hry-local-computer-"));
