...
// inside the VM and Helmryth never exposes another inbound port.

export const REMOTE_CUA_VERSION = "0.20.0";
export const REMOTE_CUA_EXECUTABLE = "/opt/helmryth/cua-driver";
export const REMOTE_CUA_SOCKET = "/opt/helmryth/run/cua.sock";
export const REMOTE_CUA_SESSION = "helmryth";
export const REMOTE_CDP_HELPER = "/opt/helmryth/helmryth-cdp.mjs";
...
export function ensureRemoteCuaCommand(): string {
  return [
    ...
    '    display=${display:-:0}',
    `    nohup env HOME="$HOME" DISPLAY="$display" CUA_DRIVER_INSTALL_CHANNEL=python_package CUA_DRIVER_RS_TELEMETRY_ENABLED=0 ${REMOTE_CUA_EXECUTABLE} serve --socket ${REMOTE_CUA_SOCKET} --permission-mode standard > /tmp/helmryth-cua-driver.log 2>&1 &`,
...
export function remoteComputerBootstrapCommand(botName: string): string {
  ...
  const installer = [
    "set -eu",
    "trap 'rm -f /tmp/helmryth-cua-installing' EXIT",
    "sudo mkdir -p /opt/helmryth/run",
    'sudo chown -R "$(id -u):$(id -g)" /opt/helmryth',
    'arch="$(uname -m)"',
  ...
  return [
    ...
    'pkill -f "^/opt/helmryth/venv/bin/python -m computer_server( |$)" >/dev/null 2>&1 || true',
    `[ -f /opt/helmryth/cua-${REMOTE_CUA_VERSION}-ready ] || [ -f /tmp/helmryth-cua-installing ] || { touch /tmp/helmryth-cua-installing; nohup bash -c ${shellQuote(installer)} > /tmp/helmryth-cua-install.log 2>&1 & }`,
    ensureRemoteCuaCommand(),
    `tmux has-session -t work 2>/dev/null || tmux new-session -d -s work 'echo; echo "  ▦ ${safeName}'"'"'s computer — Helmryth"; echo; exec bash -i'`,
    "echo bootstrapped",
  ].join("\n");
}

export interface SemanticBrowserInput {
  ref?: string;
  text?: string;
  url?: string;
}
