...
export const VPS_MANAGED_LABEL = "com.helmryth.vps";
export const VPS_CONTAINER_LABEL = "com.helmryth.container";
export const VPS_VIEWER_LABEL = "com.helmryth.vps-viewer";
export const VPS_CONTAINER_PREFIX = "helmryth-vps";
...
const tcpAddressSchema = z.object({ port: z.number().int().min(0).max(65_535) }).passthrough();

const CONTAINER_NAME = /^[a-zA-Z0-9][a-zA-Z0-9_.-]+$/;
const CONTAINER_ID = /^[a-f0-9]{12,64}$/i;
const IMAGE_ID = /^sha256:[a-f0-9]{64}$/i;
const PIDS_LIMIT = 512;
const SCREENSHOT_PATH = "/tmp/helmryth-vps-preview.png";
...
function unusedLoopbackPort(): Promise<number> {
  return new Promise((resolve, reject) => {
    ...
    probe.listen(0, "127.0.0.1", () => {
      const address = tcpAddressSchema.safeParse(probe.address());
      if (!address.success) {
        probe.close(() => reject(new Error("could not read the reserved VPS viewer port")));
        return;
      }
...
export interface VpsSpawnedProcess {
...
export function createVpsCommandRunner(spawnProcess: VpsProcessSpawner): VpsCommandRunner {
  return (args: string[], options: VpsCommandOptions = {}) => new Promise((resolve, reject) => {
...
const spawnVpsProcess: VpsProcessSpawner = (command, args, options) => spawn(command, args, options);
export const defaultRunner = createVpsCommandRunner(spawnVpsProcess);
...
type VpsPublishedPortBindings = Record<string, Array<{ HostIp?: string; HostPort?: string }> | null>;
type VpsNetworks = Record<string, { IPAddress?: string }>;

function hasNoPublishedPorts(config: {
  NetworkMode?: string;
  PortBindings?: VpsPublishedPortBindings | null;
  PublishAllPorts?: boolean;
} | undefined, networks?: VpsNetworks | null): boolean {
...
const vpsPublishedPortsSchema = z.record(z.string(), z.array(z.object({
  HostIp: z.string().optional(),
  HostPort: z.string().optional(),
}).passthrough()).nullable());
const vpsNetworksSchema = z.record(z.string(), z.object({ IPAddress: z.string().optional() }).passthrough());
const vpsHardeningSchema = z.object({
...
const vpsImageInspectSchema = z.array(z.object({
...
const vpsContainerInspectSchema = z.array(z.object({
  Config: z.object({
    Image: z.string().optional(),
    Labels: stringMapSchema.optional(),
    Env: z.array(z.string()).optional(),
  }).passthrough().optional(),
  HostConfig: vpsHardeningSchema.extend({
    Binds: z.array(z.string()).nullable().optional(),
    VolumesFrom: z.array(z.string()).nullable().optional(),
    NetworkMode: z.string().optional(),
    PortBindings: vpsPublishedPortsSchema.nullable().optional(),
    PublishAllPorts: z.boolean().optional(),
  }).passthrough().optional(),
  Id: z.string().optional(),
  id: z.string().optional(),
  Image: z.string().optional(),
  NetworkSettings: z.object({ Networks: vpsNetworksSchema.nullable().optional() }).passthrough().optional(),
  Mounts: z.array(z.json()).optional(),
  State: z.object({ Running: z.boolean().optional() }).passthrough().optional(),
}).passthrough());
const vpsHealthReportSchema = z.object({
...
function statusProblem(status: VpsComputerStatus): string | null {
  ...
  if (!status.image) return `Prepare the pinned Helmryth Cua image on the VPS (Driver ${CUA_DRIVER_VERSION})`;
  if (status.container === "missing") return "No Helmryth container exists for this operator on the VPS";
  if (!status.imageMatches) return "The VPS container uses an incompatible or untrusted Helmryth image";
  if (!status.managed) return "The VPS container name is occupied by a container Helmryth did not create";
  ...
  if (status.security === "unsafe") return "The VPS container is missing Helmryth safety limits";
  if (status.container === "stopped") return "The Helmryth VPS container is stopped";
...
async function computeVpsComputerStatus(
  ...
  try {
    const inspected = vpsImageInspectSchema.parse(parseJson((await run(["image", "inspect", VPS_IMAGE])).stdout));
  ...
  try {
    const inspected = vpsContainerInspectSchema.parse(parseJson((await run(["inspect", status.container_name])).stdout));
    ...
    if (canProbe && containerRef) {
      try {
        ...
        const report = vpsHealthReportSchema.parse(parseJson(health.stdout));
        if (
  ...
  const operation = async () => {
    ...
    try {
      ...
      if (action === "provision") {
        ...
        if (before.container === "missing") throw Object.assign(new Error("No VPS container exists for this operator"), { status: 409 });
        ...
        // never docker-rm a container Helmryth did not create, even one
        // squatting on our name.
        if (before.container === "missing") return before;
        if (!before.managed) {
          throw Object.assign(
            new Error("The VPS container name is occupied by a container Helmryth did not create — remove it on the VPS yourself"),
            { status: 409 },
          );
        }
...
export interface VpsComputerMcpLaunch {
  command: string;
  args: string[];
  env: Record<string, string>;
}

export function vpsComputerMcp(cfg: AppConfig, botId: string, containerRef?: string): VpsComputerMcpLaunch {
  const alias = vpsSshAlias(cfg);
