...
export function appendNative(threadId: string, entry: { dir: "in" | "out"; source: string; msg: unknown }) {
  try {
    ...
    // an MCP header can carry a connected-app token. These owner-only trace
    // files are still commonly pasted into bug reports, so values are masked
    // while the shape stays intact.
    appendFileSync(
