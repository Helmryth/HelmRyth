...
export function claudeSignedIn(
  ...
  return new Promise((resolve) => {
    run(cli, ["auth", "status", "--json"], { timeout: 8000, env }, (_error, stdout) => {
      try {
        const status = authStatusSchema.safeParse(JSON.parse(stdout));
        resolve(status.success && status.data.loggedIn);
      } catch {
        resolve(false);
      }
    });
...
const optionalSettingStringSchema = z.string().optional().catch(undefined);
const modelSettingSchema = z.union([
  z.string().transform((id) => (CLAUDE_MODEL_ID.test(id) ? { id, label: id } : null)),
  z.object({
    id: optionalSettingStringSchema,
    model: optionalSettingStringSchema,
    slug: optionalSettingStringSchema,
    name: optionalSettingStringSchema,
    displayName: optionalSettingStringSchema,
    label: optionalSettingStringSchema,
  }).catchall(z.json()).transform((row) => {
    const id = row.id ?? row.model ?? row.slug;
    if (!id || !CLAUDE_MODEL_ID.test(id)) return null;
    return { id, label: row.name ?? row.displayName ?? row.label ?? id };
  }),
  z.json().transform(() => null),
]);
const modelSettingsListSchema = z.array(modelSettingSchema).catch([]).transform((rows) =>
  rows.flatMap((row) => row === null ? [] : [row]),
);
const claudeSettingsSchema = z.object({
  availableModels: modelSettingsListSchema.default([]),
  customModels: modelSettingsListSchema.default([]),
  extraModels: modelSettingsListSchema.default([]),
  env: z.object({ ANTHROPIC_MODEL: optionalSettingStringSchema }).catchall(z.json()).optional().catch(undefined),
}).catchall(z.json());
...
export function readClaudeModelCatalog(env: Record<string, string | undefined> = process.env) {
  ...
  try {
    settings = claudeSettingsSchema.parse(
      JSON.parse(readFileSync(join(claudeConfigDir(env), "settings.json"), "utf8")),
    );
  } catch {
  ...
  if (envModel && CLAUDE_MODEL_ID.test(envModel)) extras.push({ id: envModel, label: envModel });
...
const askInputSchema = z.object({
  ...
  choices: z.array(z.string()).optional().catch(undefined),
}).catchall(z.json());
const permissionAskSchema = z.object({
  t: z.literal("ask"),
  id: z.json().optional(),
  kind: z.literal("question").optional().catch(undefined),
  tool: z.string().optional().catch(undefined),
  input: askInputSchema.default({}),
}).catchall(z.json());

const DENY_TIMEOUT_NOTE =
  "Helmryth: nobody answered this permission request in time. Skip this action and finish what you can without it.";
const QUESTION_TIMEOUT_NOTE = "Helmryth: nobody answered in time. Use your best judgment and continue.";
const DUPLICATE_ASK_ID_NOTE = "Helmryth: duplicate ask id — skipping this request.";
...
function systemEndedReply(kind: Ask["kind"]): { behavior: AskBehavior; message: string } {
  return kind === "question"
    ? { behavior: "answer", message: "Helmryth: the turn is ending — wrap up." }
    : { behavior: "deny", message: "Helmryth: the turn ended" };
}

/** One human-readable line for an ask — what the card subtitle shows. */
function askSummary(ask: Ask): string {
  if (ask.input.question) return ask.input.question.slice(0, 300);
  if (ask.input.command) return ask.input.command.slice(0, 200);
  if (ask.input.url) return ask.input.url.slice(0, 200);
  const text = JSON.stringify(ask.input);
  ...
  const server = createNetServer((conn) => {
    ...
    conn.on("data", (chunk) => {
      ...
      while ((nl = buf.indexOf("\n")) !== -1) {
        ...
        let decoded;
        try {
          decoded = permissionAskSchema.safeParse(JSON.parse(line));
        } catch {
          continue;
        }
...
const toolListSchema = z.array(z.string().trim().min(1)).transform((tools) => Array.from(new Set(tools)));
const claudeConfigSchema = z.object({
  cli: z.string().optional().catch(undefined),
  permissionMode: z.enum(["acceptEdits", "auto", "bypassPermissions"]).optional(),
  tools: toolListSchema.optional(),
  disallowedTools: toolListSchema.optional(),
});

const decodeConfig: ProviderDriver<ClaudeConfig>["decodeConfig"] = (raw) => {
  const decoded = claudeConfigSchema.safeParse(raw ?? {});
  if (!decoded.success) {
    const field = String(decoded.error.issues[0]?.path[0] ?? "config");
    throw new Error(`claude: invalid ${field}`);
  }
  const result: ClaudeConfig = {
    cli: decoded.data.cli ?? "claude",
    permissionMode: decoded.data.permissionMode ?? "acceptEdits",
  };
  if (decoded.data.tools !== undefined) result.tools = decoded.data.tools;
  if (decoded.data.disallowedTools !== undefined) result.disallowedTools = decoded.data.disallowedTools;
  return result;
};
...
const protocolEnvelopeSchema = z.object({ type: z.string() }).catchall(z.json());
const systemMessageSchema = z.object({
  type: z.literal("system"),
  subtype: z.string(),
  session_id: z.string().optional().catch(undefined),
  model: z.string().nullable().optional().catch(undefined),
  estimated_tokens: z.number().nullable().optional().catch(undefined),
}).catchall(z.json());
const streamEventMessageSchema = z.object({
  type: z.literal("stream_event"),
  parent_tool_use_id: z.json().optional(),
  event: z.object({
    type: z.string(),
    delta: z.object({
      type: z.string(),
    ...
    }).catchall(z.json()).optional().catch(undefined),
  }).catchall(z.json()).optional().catch(undefined),
}).catchall(z.json());
const assistantMessageSchema = z.object({
  type: z.literal("assistant"),
  message: z.object({
    content: claudeContentSchema.default([]),
    usage: claudeUsageSchema.optional().catch(undefined),
  }).catchall(z.json()).default({ content: [] }),
}).catchall(z.json());
...
const resultMessageSchema = z.object({
  ...
  terminal_reason: z.string().nullable().optional().catch(undefined),
  total_cost_usd: z.number().nullable().optional().catch(undefined),
  usage: claudeUsageSchema.optional().catch(undefined),
}).catchall(z.json());

function firstText(content: z.infer<typeof claudeContentSchema>): string {
  return content
...
export const ClaudeDriver: ProviderDriver<ClaudeConfig> = {
  ...
  async create(input: DriverCreateInput<ClaudeConfig>): Promise<ProviderInstance> {
    ...
    const configuredIdleMinimum = Number(process.env.HELMRYTH_CLAUDE_SESSION_IDLE_MIN_MS);
    const sessionIdleMinimum = Number.isFinite(configuredIdleMinimum) && configuredIdleMinimum > 0
      ? configuredIdleMinimum
      : 10_000;
    const SESSION_IDLE_MS = Math.max(sessionIdleMinimum, Number(process.env.HELMRYTH_CLAUDE_SESSION_IDLE_MS) || 10 * 60_000);
    ...
    const sendTurn = async (turn: SendTurnInput) => {
      ...
      const decodedResumeCursor = z.string().safeParse(turn.resumeCursor);
      const sessionId = decodedResumeCursor.success ? decodedResumeCursor.data : null;
      ...
      if (turn.integrations?.computer) {
        ...
        // Host tools always route through Helmryth's permission broker.
      ...
      if (config.permissionMode !== "bypassPermissions") {
        ...
        args.push("--permission-prompt-tool", "mcp__helmryth__approve");
        mcpServers.helmryth = { command: process.execPath, args: [PERM_PROXY_PATH, socketPath], env: { ...NODE_ENV_FLAG } };
        allowed.push("mcp__helmryth");
      }
      ...
      if (Object.keys(mcpServers).length) {
        mcpConfigPath = join(mkdtempSync(join(tmpdir(), "hry-mcp-")), "mcp.json");
        ...
        broker = createPermissionBroker({
          ...
          onAsk: (ask) => {
            ...
            emit({
              ...
              approvalScope:
                controlsHost && ask.tool.startsWith("mcp__computer")
                  ? "local-computer"
                  : undefined,
              choices: ask.input.choices?.slice(0, 5),
            });
          },
          onResolve: (resolved) => {
            const eventTurnId = sessions.get(threadId)?.turn?.turnId ?? turnId;
            const askTool = askTools.get(resolved.id);
            emit({
              ...base(threadId, eventTurnId),
              ...
              approvalScope:
                controlsHost && askTool?.startsWith("mcp__computer") ? "local-computer" : undefined,
            });
      ...
      const handleLine = (line: string) => {
        let decoded;
        try {
          decoded = protocolEnvelopeSchema.safeParse(JSON.parse(line));
        } catch {
          return;
        }
        if (!decoded.success) return;
        appendNative(threadId, { dir: "in", source: "claude.sdk.message", msg: decoded.data });
        switch (decoded.data.type) {
          case "system": {
            const message = systemMessageSchema.safeParse(decoded.data);
            if (!message.success) break;
            const o = message.data;
            if (o.subtype === "init") {
              if (o.session_id) session.sessionId = o.session_id;
              emit({
          ...
          case "stream_event": {
            const message = streamEventMessageSchema.safeParse(decoded.data);
            if (!message.success) break;
            const o = message.data;
          ...
          case "assistant": {
            const message = assistantMessageSchema.safeParse(decoded.data);
            if (!message.success) break;
            ...
            for (const b of msg.content) {
              const toolUse = claudeToolUseBlockSchema.safeParse(b);
              if (!toolUse.success) continue;
              emit({
                ...base(threadId, currentTurnId()),
                type: "item.started",
                itemType: "tool",
                itemId: toolUse.data.id,
                title: toolUse.data.name,
              });
            }
            if (msg.usage) {
              ...
              if (msg.usage.cache_read_input_tokens !== undefined) {
                usageEvent.cachedInput = msg.usage.cache_read_input_tokens;
              }
              emit(usageEvent);
            }
            break;
          ...
          case "user": {
            const message = userMessageSchema.safeParse(decoded.data);
            if (!message.success) break;
            for (const b of message.data.message.content) {
          ...
          case "result": {
            const message = resultMessageSchema.safeParse(decoded.data);
            if (!message.success) break;
            const o = message.data;
            ...
            if (o.usage) {
              ...
              if (o.usage.cache_read_input_tokens !== undefined) {
                usage.cachedInput = o.usage.cache_read_input_tokens;
              }
            }
            settle(
              o.is_error !== true,
