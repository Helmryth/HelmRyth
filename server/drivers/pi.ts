...
interface PiMcpServers {
  [name: string]: PiMcpServer;
}
...
export function buildMcpServers(turn: SendTurnInput): PiMcpServers | null {
  const servers: PiMcpServers = {};
  ...
  if (turn.integrations?.computer) {
    ...
    if (local.scope) computer.scope = local.scope;
    servers.computer = computer;
  }
...
const PI_MODELS_RESPONSE_SCHEMA = z.object({
  type: z.literal("response"),
  command: z.literal("get_available_models"),
  success: z.boolean(),
  data: z.object({
    models: z.array(z.object({ provider: z.string(), id: z.string(), name: z.string().optional() })).optional(),
  }).optional(),
});
...
export function parsePiCatalog(stdout: string, fallbackDefault = ""): ModelCatalog {
  ...
  for (const line of stdout.split("\n")) {
    ...
    const decoded = PI_MODELS_RESPONSE_SCHEMA.safeParse(msg);
    if (!decoded.success || !decoded.data.success) continue;
    const res = decoded.data;
...
export function ensurePiInjectModel(
  ...
  const decodedProviders = jsonObjectSchema.safeParse(root.providers);
  const providers: JsonObject = decodedProviders.success ? { ...decodedProviders.data } : {};
  const previous = providers[inject.host];
  const decodedExisting = jsonObjectSchema.safeParse(previous);
  const existing: JsonObject = decodedExisting.success
      ? { ...decodedExisting.data }
      : {
  ...
  const decodedApi = z.string().min(1).safeParse(existing.api);
  existing.api = decodedApi.success ? decodedApi.data : "openai-completions";
  existing.apiKey = hostApiKey(host, env);
  if (!existing.compat) {
    existing.compat = { supportsDeveloperRole: false, supportsReasoningEffort: true };
  }
  const decodedModels = z.array(jsonObjectSchema).safeParse(existing.models);
  const models: JsonObject[] = decodedModels.success ? [...decodedModels.data] : [];
...
function readPiDefaultModel(env: Record<string, string | undefined>): string {
  ...
  try {
    const s = z.object({ defaultProvider: z.string().optional(), defaultModel: z.string().optional() })
      .parse(parseJson(readFileSync(`${home}/.pi/agent/settings.json`, "utf8")));
...
type PiConfigWireInput = Parameters<ProviderDriver<PiConfig>["decodeConfig"]>[0];

const PI_CONFIG_SCHEMA = z.object({
  cli: z.string().optional(),
  fullAuto: z.boolean().optional(),
});

function decodeConfig(raw: PiConfigWireInput): PiConfig {
  const obj = PI_CONFIG_SCHEMA.parse(raw ?? {});
  return {
...
const PI_EVENT_SCHEMA: z.ZodType<PiEvent> = z.object({
  type: z.string(),
  ...
  message: z.object({
    ...
    usage: z.object({ input: z.number().optional(), output: z.number().optional() }).passthrough().optional(),
  }).passthrough().optional(),
  usage: z.object({ input: z.number().optional(), output: z.number().optional() }).passthrough().optional(),
  id: z.string().optional(),
  method: z.string().optional(),
  options: z.array(z.custom<JsonValue>()).optional(),
  title: z.string().optional(),
}).passthrough();

const PI_SESSION_RESPONSE_SCHEMA = z.object({
  sessionFile: z.string().optional(),
  sessionId: z.string().optional(),
});

type PiOutboundFrame =
  | { type: "abort" }
  | { type: "extension_ui_response"; id: string; cancelled: true }
  | { type: "extension_ui_response"; id: string; value: string }
  | { type: "extension_ui_response"; id: string; confirmed: true }
  | { type: "switch_session"; sessionPath: string }
  | { type: "new_session" }
  | { type: "set_model"; provider: string; modelId: string }
  | { type: "set_thinking_level"; level: (typeof EFFORT_LEVELS)[number] | "off" }
  | { type: "prompt"; message: string };

type PiResponseData = JsonValue | undefined;

function piEnvironment(source: Record<string, string | undefined>) {
  const env = { ...source };
...
export const PiDriver: ProviderDriver<PiConfig> = {
  ...
  async create(input: DriverCreateInput<PiConfig>): Promise<ProviderInstance> {
    ...
    const sendTurn = async (turn: SendTurnInput) => {
      ...
      if (mcpServers) {
        mcpTempDir = mkdtempSync(join(tmpdir(), "hry-pi-mcp-"));
        try {
      ...
      const child = (() => {
        try {
          const childEnvironment = { ...process.env, ...input.environment };
          if (mcpServers && mcpTempDir) {
            childEnvironment.HELMRYTH_MCP_CONFIG = join(mcpTempDir, "mcp.json");
          }
          return spawnCli(config.cli, childArgs, {
            stdio: ["pipe", "pipe", "pipe"],
            cwd: turn.cwd,
            env: piEnvironment(childEnvironment),
          });
        } catch (err) {
          if (mcpTempDir) {
            try {
      ...
      const responseWaiters = new Map<string, {
        resolve: (data: PiResponseData) => void;
      ...
      const awaitResponse = (command: string, timeoutMs = 20_000) =>
        new Promise<PiResponseData>((resolve, reject) => {
      ...
      const send = (frame: PiOutboundFrame) => {
        appendNative(threadId, { dir: "out", source: "pi.rpc", msg: frame });
      ...
      child.stdout.on("data", (chunk: string) => {
        ...
        while ((nl = buf.indexOf("\n")) !== -1) {
          ...
          if (!line.trim()) continue;
          try {
            const event = PI_EVENT_SCHEMA.safeParse(parseJson(line));
      ...
      child.on("error", (err) => {
        const fail = describeSpawnFailure(err, config.cli);
      ...
      const decodedSessionPath = z.string().safeParse(turn.resumeCursor);
      const sessionPath = decodedSessionPath.success ? decodedSessionPath.data : null;
      ...
      try {
        ...
        const decodedHandshake = PI_SESSION_RESPONSE_SCHEMA.safeParse(await hsPromise);
        const hs = decodedHandshake.success ? decodedHandshake.data : undefined;
      ...
      const chosen = turn.model ? splitPiModel(turn.model) : null;
      if (chosen) {
        try {
