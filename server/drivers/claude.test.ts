...
const brokerAnswerSchema = z.object({
  id: z.string().optional(),
  behavior: z.string(),
  message: z.string().optional(),
  updatedInput: z.unknown().optional(),
});
type BrokerAnswer = z.infer<typeof brokerAnswerSchema>;

const mcpServerDumpSchema = z.object({
  command: z.string(),
  args: z.array(z.string()),
  env: z.record(z.string(), z.string()),
});

const fakeCliDumpSchema = z.object({
  pid: z.number(),
  argv: z.array(z.string()),
  env: z.record(z.string(), z.string()),
  prompt: z.unknown(),
  mcpConfig: z
    .object({ mcpServers: z.record(z.string(), mcpServerDumpSchema) })
    .nullable(),
});

function readFakeCliDump(path: string) {
  return fakeCliDumpSchema.parse(JSON.parse(readFileSync(path, "utf8")));
}

function readMcpCliDump(path: string) {
  const dump = readFakeCliDump(path);
  if (dump.mcpConfig === null) {
    throw new Error(`fake Claude CLI did not receive an MCP config: ${path}`);
  }
  return { ...dump, mcpConfig: dump.mcpConfig };
}

function argumentAfter(argv: string[], flag: string): string {
  const index = argv.indexOf(flag);
  const value = index === -1 ? undefined : argv[index + 1];
  if (value === undefined) throw new Error(`fake Claude CLI argv is missing ${flag}`);
  return value;
}

function requireSessionId(events: RuntimeEvent[]): string {
  for (const event of events) {
    if (event.type === "session.started" && event.sessionId !== null) return event.sessionId;
  }
  throw new Error("Claude test transcript contains no non-null session id");
}

function requireAssistantReply(events: RuntimeEvent[], predicate: (text: string) => boolean): string {
  for (const event of events) {
    if (event.type === "item.completed" && event.itemType === "assistant_text" && predicate(event.text)) {
      return event.text;
    }
  }
  throw new Error("Claude test transcript contains no matching assistant reply");
}
...
function answerQueue(conn: ReturnType<typeof connect>) {
  const waiters: Array<(message: BrokerAnswer) => void> = [];
  let buf = "";
  conn.on("data", (chunk) => {
    buf += chunk;
    let nl;
    while ((nl = buf.indexOf("\n")) !== -1) {
      ...
      buf = buf.slice(nl + 1);
      waiters.shift()?.(brokerAnswerSchema.parse(JSON.parse(line)));
    }
  });
  return () => new Promise<BrokerAnswer>((resolve) => waiters.push(resolve));
}
...
describe("ClaudeDriver.decodeConfig", () => {
  ...
  it.skipIf(process.platform !== "win32")("names permission pipes per harness process", () => {
    expect(permissionSocketPath("thread-abc")).toMatch(
      new RegExp(`^\\\\\\\\\\.\\\\pipe\\\\helmryth-perm-${process.pid}-thre[0-9a-f]{4}$`),
    );
  });
...
describe("ClaudeDriver turns (fake CLI)", () => {
  ...
  beforeEach(() => {
    ...
    scratch = mkdtempSync(join(tmpdir(), "hry-claude-test-"));
  });
  ...
  afterEach(async () => {
    ...
    delete process.env.HELMRYTH_TTS_KEY;
    delete process.env.HELMRYTH_CLAUDE_SESSION_IDLE_MS;
    delete process.env.HELMRYTH_CLAUDE_SESSION_IDLE_MIN_MS;
    recorder?.stop();
  ...
  it("streams partial-message text deltas without re-emitting the whole message", async () => {
    ...
    const text = deltas.filter((delta) => delta.streamKind === "assistant_text");
    // two streamed chunks, and NO third full-text fallback delta after them
    expect(text.map((delta) => delta.delta)).toEqual(["hello from ", "fake claude"]);
    // subagent narration (parent_tool_use_id) never surfaces
    expect(text.some((delta) => delta.delta.includes("SUBAGENT"))).toBe(false);
    // reasoning streams on its own kind
    expect(deltas.some((delta) => delta.streamKind === "reasoning_text" && delta.delta === "hmm")).toBe(true);
    // the settled message still lands exactly once
    const settled = recorder.events.filter((event) => event.type === "item.completed" && event.itemType === "assistant_text");
    expect(settled).toHaveLength(1);
    expect(settled[0]?.text).toBe("hello from fake claude");
  });
  ...
  it("sends the prompt over stdin, never argv, and strips identity env vars", async () => {
    ...
    expect(seen.env.HELMRYTH_TTS_KEY).toBeUndefined();
  });
    ...
    await instance.adapter.sendTurn({
      ...
      integrations: {
        agents: {
          ...
          env: { HELMRYTH_HARNESS_URL: "http://127.0.0.1:1", HELMRYTH_BOT_ID: "b1", HELMRYTH_COMMS_TOKEN: "tok", HELMRYTH_TURN_DEPTH: "0" },
        },
      },
    });
    await recorder.until((e) => e.type === "turn.completed");

    const seen = readMcpCliDump(dump);
    expect(seen.mcpConfig.mcpServers.agents).toMatchObject({
      args: ["/fake/agents-proxy.js"],
      env: { HELMRYTH_BOT_ID: "b1", HELMRYTH_COMMS_TOKEN: "tok" },
    });
    ...
    await instance.adapter.sendTurn({
      ...
      integrations: {
        composio: {
          ...
          env: { HELMRYTH_CONNECTOR_UPSTREAM_URL: "https://example.test/mcp" },
        },
      },
    });
    await recorder.until((e) => e.type === "turn.completed");
  ...
  it.each([
    ...
    const configPath = (() => {
      const seen = readFakeCliDump(dump);
      return argumentAfter(seen.argv, "--mcp-config");
    })();
    expect(configPath).toMatch(/hry-mcp-/);
  ...
  it("a message sent mid-turn is steered into the running turn", async () => {
    ...
    const reply = requireAssistantReply(recorder.events, (text) => text.startsWith("reply to:"));
    expect(reply).toContain("steered: and also this");
  ...
  it("reuses the live process for the next compatible turn", async () => {
    ...
    const announced = requireSessionId(recorder.events);
  ...
  it("denies late broker asks between retained turns without opening a zombie card", async () => {
    ...
    await expect(answer).resolves.toMatchObject({
      id: "ask-between",
      behavior: "deny",
      message: "Helmryth: the turn ended",
    });
  ...
  it("replaces and resumes a live process when its spawn contract changes", async () => {
    ...
    await recorder.until((e) => e.type === "turn.completed" && e.turnId === second.turnId);
    const dump = readFakeCliDump(dumpPath);
  ...
  it("closes an idle session after the configured window", async () => {
    process.env.HELMRYTH_CLAUDE_SESSION_IDLE_MIN_MS = "10";
    process.env.HELMRYTH_CLAUDE_SESSION_IDLE_MS = "50";
    await create();
    ...
    await recorder.until((e) => e.type === "turn.completed" && e.turnId === second.turnId);
    expect(readFakeCliDump(join(scratch, "idle-dump.json")).argv).toContain("--resume");
  });
  ...
  it("auto-retries transient exits, then completes with exactly one final message", async () => {
    ...
    expect(retries.every((event) => event.delayMs > 0 && event.reason.length > 0)).toBe(true);
  ...
  it("brokers a permission ask into request.opened and answers over the socket", async () => {
    ...
    const answered = new Promise<BrokerAnswer>((resolve) => {
      let buf = "";
      conn.on("data", (c) => {
        buf += c;
        const nl = buf.indexOf("\n");
        if (nl !== -1) resolve(brokerAnswerSchema.parse(JSON.parse(buf.slice(0, nl))));
      });
    });
    ...
    // one at a time
    const answered2 = new Promise<BrokerAnswer>((resolve) => {
      let buf = "";
      conn.on("data", (c) => {
        buf += c;
        const nl = buf.indexOf("\n");
  ...
  it("denies a colliding ask id on the same connection without orphaning the original", async () => {
    ...
    expect(await nextAnswer()).toMatchObject({
      id: "dup-1",
      behavior: "deny",
      message: "Helmryth: duplicate ask id — skipping this request.",
    });
  ...
  it("drops a late ask on an already-closed broker instead of a dead card (#211)", async () => {
    ...
    const opensBefore = recorder.events.filter((e) => e.type === "request.opened").length;
    const reply = new Promise<BrokerAnswer>((resolve) => {
      let buf = "";
      conn.on("data", (c) => {
        buf += c;
        const nl = buf.indexOf("\n");
  ...
  it("drops a late question on an already-closed broker with an answer, not a deny (#211)", async () => {
    ...
    expect(await reply).toMatchObject({
      id: "q-late",
      behavior: "answer",
      message: "Helmryth: the turn is ending — wrap up.",
    });
    expect(recorder.events.filter((e) => e.type === "request.opened")).toHaveLength(opensBefore);
    await expect(
  ...
  it("strips workspace credentials from generateText helper children", async () => {
    ...
    process.env.FAKE_CLAUDE_DUMP = dump;
    const names = ["XAI_API_KEY", "COMPOSIO_API_KEY", "BOX_TOKEN", "OPENCODE_API_KEY", "HELMRYTH_TTS_KEY"] as const;
