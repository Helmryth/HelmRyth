...
let repoStatus = 200;
let runBody: JsonValue;
let runResponse = { status: "ok", output: "model output", error: "" };
interface RpcReply {
  id: number;
  result: { content: Array<{ text: string }>; tools: Array<{ name: string }>; serverInfo: { name: string }; isError?: boolean };
}
...
beforeAll(async () => {
  stub = createServer((req, res) => {
    ...
    if (req.method === "GET" && req.url === "/api/repo/status") {
      ...
      return res.end(
        JSON.stringify(repoStatus === 200 ? { repo: "Helmryth", branch: "main", commit: "abc123" } : { error: "offline" }),
      );
    }
...
describe("dweb-proxy MCP surface", () => {
  ...
  it("forwards opencode runs with the selected model", async () => {
    runResponse = { status: "ok", output: "model output", error: "" };
  ...
  it("adds the daemon URL to errors from every tool", async () => {
    ...
    runResponse = { status: "error", output: "", error: "model unavailable" };
