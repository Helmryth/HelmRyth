...
describe("appendNative", () => {
  it("masks the tokens an ACP session/new hands the agent", () => {
          ...
          mcpServers: [
            {
              name: "computer",
              env: [
                { name: "HELMRYTH_BOX_ID", value: "box-7" },
                { name: "HELMRYTH_BOX_TOKEN", value: "box_live_dontlogme" },
              ],
            },
          ],
        },
    ...
    expect(log).toContain("HELMRYTH_BOX_TOKEN");
  ...
  it("never throws, whatever it is handed", () => {
    ...
    interface RecursiveRecord { [key: string]: string | RecursiveRecord }
    const cyclic: RecursiveRecord = {};
    cyclic.self = cyclic;
