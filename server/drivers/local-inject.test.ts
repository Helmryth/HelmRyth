...
interface TestEnvironment extends Record<string, string | undefined> {}

const fakeAcpDumpSchema = z.object({
  argv: z.array(z.string()).default([]),
  env: z.record(z.string(), z.string()).default({}),
});
const configCallsSchema = z.array(z.object({
  method: z.string(),
  params: z.object({
...
const droidSettingsSchema = z.object({
  hooks: z.object({ Stop: z.array(z.string()) }).optional(),
...
const qwenSettingsSchema = z.object({
  env: z.record(z.string(), z.string()).default({}),
  modelProviders: z.object({
    openai: z.array(z.object({ id: z.string() })),
  }),
});
...
describe("mergeLocalInject", () => {
  it("marks oMLX /v1/models/status loaded rows, not the default_model", async () => {
    const catalog = await mergeLocalInject(
      ...
      { VITEST: "true", HELMRYTH_PROBE_LOCAL_INJECT: "1" },
      async (url) => {
  ...
  it("keeps a valid model id when unrelated host metadata is malformed", async () => {
    const catalog = await mergeLocalInject(
      { default: "keep", options: [{ id: "keep", label: "Keep" }] },
      { VITEST: "true", HELMRYTH_PROBE_LOCAL_INJECT: "1" },
      async (url) => {
        if (String(url).includes(":8080/v1/models")) {
          return new Response(JSON.stringify({ data: [{ id: "GLM-5.2-fp8", context_length: "invalid" }] }), {
            status: 200,
          });
        }
        return new Response("nope", { status: 500 });
    ...
    expect(catalog.options.some((option) => option.id === "omlx::GLM-5.2-fp8")).toBe(true);
  });
...
describe("codexLocalProviderArgs", () => {
  it("configures custom providers through env keys without putting credentials on argv", () => {
    const env: TestEnvironment = { UNSLOTH_STUDIO_AUTH_TOKEN: "unsloth-secret" };
    ...
    expect(rendered).toContain("HELMRYTH_LOCAL_UNSLOTH_API_KEY");
    ...
    expect(env.HELMRYTH_LOCAL_UNSLOTH_API_KEY).toBe("unsloth-secret");
  });
});

describe("ensureGrokInjectSlug", () => {
  it("writes a config block the first time and reuses it after", () => {
    const home = mkdtempSync(join(tmpdir(), "hry-grok-inject-"));
  ...
  it("writes the resolved Unsloth credential instead of a placeholder", () => {
    const home = mkdtempSync(join(tmpdir(), "hry-grok-unsloth-"));
    scratchDirs.push(home);
    mkdirSync(join(home, ".grok"), { recursive: true });
  ...
  it("puts the resolved oMLX slug after agent on the Grok child argv", async () => {
    const home = mkdtempSync(join(tmpdir(), "hry-grok-argv-"));
    scratchDirs.push(home);
    mkdirSync(join(home, ".grok"), { recursive: true });
    ...
    try {
      ...
      const argv = fakeAcpDumpSchema.parse(JSON.parse(readFileSync(dump, "utf8"))).argv;
      ...
      const configCalls = configCallsSchema.parse(JSON.parse(readFileSync(`${dump}.config.json`, "utf8")));
...
describe("ensureKimiInjectAlias", () => {
  it("writes an openai_legacy provider and model alias without touching hooks", () => {
    const home = mkdtempSync(join(tmpdir(), "hry-kimi-inject-"));
    scratchDirs.push(home);
  ...
  it("amends an existing alias with protocol and context size and leaves user keys", () => {
    const home = mkdtempSync(join(tmpdir(), "hry-kimi-patch-"));
    scratchDirs.push(home);
    const root = join(home, ".kimi-code");
    mkdirSync(root, { recursive: true });
    writeFileSync(
  ...
  it("does not overwrite a user's protocol or context size", () => {
    const home = mkdtempSync(join(tmpdir(), "hry-kimi-keep-"));
    scratchDirs.push(home);
    const root = join(home, ".kimi-code");
    mkdirSync(root, { recursive: true });
    writeFileSync(
  ...
  it("treats a quoted protocol key as already set and does not duplicate it", () => {
    const home = mkdtempSync(join(tmpdir(), "hry-kimi-quoted-"));
    scratchDirs.push(home);
    const root = join(home, ".kimi-code");
    mkdirSync(root, { recursive: true });
    writeFileSync(
  ...
  it("finds a heading with a trailing comment and does not append a second table", () => {
    const home = mkdtempSync(join(tmpdir(), "hry-kimi-heading-"));
    scratchDirs.push(home);
    const root = join(home, ".kimi-code");
    mkdirSync(root, { recursive: true });
    writeFileSync(
  ...
  it("does not hide a model table behind an apostrophe in a preceding comment", () => {
    const home = mkdtempSync(join(tmpdir(), "hry-kimi-apos-"));
    scratchDirs.push(home);
    const root = join(home, ".kimi-code");
    mkdirSync(root, { recursive: true });
    writeFileSync(
  ...
  it("stops a model table before a following array-of-tables heading", () => {
    const home = mkdtempSync(join(tmpdir(), "hry-kimi-aot-"));
    scratchDirs.push(home);
    const root = join(home, ".kimi-code");
    mkdirSync(root, { recursive: true });
    writeFileSync(
  ...
  it("treats a unicode-escaped model key as the same table as the literal alias", () => {
    const home = mkdtempSync(join(tmpdir(), "hry-kimi-unicode-"));
    scratchDirs.push(home);
    const root = join(home, ".kimi-code");
    mkdirSync(root, { recursive: true });
    writeFileSync(
  ...
  it("does not treat a malformed escape as a canonical alias", () => {
    const home = mkdtempSync(join(tmpdir(), "hry-kimi-badesc-"));
    scratchDirs.push(home);
    const root = join(home, ".kimi-code");
    mkdirSync(root, { recursive: true });
    writeFileSync(
  ...
  it("treats whitespace around dotted heading keys as the same table", () => {
    const home = mkdtempSync(join(tmpdir(), "hry-kimi-dots-"));
    scratchDirs.push(home);
    const root = join(home, ".kimi-code");
    mkdirSync(root, { recursive: true });
    writeFileSync(
  ...
  it("does not treat a triple-quote inside a single-line string as multiline", () => {
    const home = mkdtempSync(join(tmpdir(), "hry-kimi-squote-"));
    scratchDirs.push(home);
    const root = join(home, ".kimi-code");
    mkdirSync(root, { recursive: true });
    writeFileSync(
  ...
  it("does not treat a triple-quote inside a comment as multiline", () => {
    const home = mkdtempSync(join(tmpdir(), "hry-kimi-hash-"));
    scratchDirs.push(home);
    const root = join(home, ".kimi-code");
    mkdirSync(root, { recursive: true });
    writeFileSync(
  ...
  it("does not treat a bracket line inside a multiline string as a table", () => {
    const home = mkdtempSync(join(tmpdir(), "hry-kimi-ml-"));
    scratchDirs.push(home);
    const root = join(home, ".kimi-code");
    mkdirSync(root, { recursive: true });
    writeFileSync(
  ...
  it("treats USERPROFILE as the same home for credentials and config", async () => {
    const home = mkdtempSync(join(tmpdir(), "hry-kimi-userprofile-"));
    scratchDirs.push(home);
...
describe("applyKimiLocalModelEnv", () => {
  ...
  it("leaves subscription slugs and already-resolved aliases alone", () => {
    const env: TestEnvironment = { KIMI_MODEL_NAME: "keep-me" };
  ...
  it("puts the overlay on the Kimi child only for a local inject pick", async () => {
    const home = mkdtempSync(join(tmpdir(), "hry-kimi-overlay-"));
    scratchDirs.push(home);
    ...
    try {
      ...
      await recorder.until((e) => e.type === "turn.completed");
      const injectDump = fakeAcpDumpSchema.parse(JSON.parse(readFileSync(dump, "utf8")));
      ...
      const cloudDump = fakeAcpDumpSchema.parse(JSON.parse(readFileSync(dump, "utf8")));
...
describe("ensureDroidInjectModel", () => {
  it("upserts a generic-chat-completion BYOK row and reuses it", () => {
    const home = mkdtempSync(join(tmpdir(), "hry-droid-inject-"));
    scratchDirs.push(home);
    ...
    expect(first).toBe("custom:helmryth-omlx-MiniMax-M3-4bit");
    expect(again).toBe(first);
    const settings = droidSettingsSchema.parse(
      JSON.parse(readFileSync(join(home, ".factory", "settings.json"), "utf8")),
    );
  ...
  it("persists a generated id onto a matching customModels row that has none", () => {
    const home = mkdtempSync(join(tmpdir(), "hry-droid-inject-noid-"));
    scratchDirs.push(home);
    mkdirSync(join(home, ".factory"), { recursive: true });
    writeFileSync(
    ...
    expect(id).toBe("custom:helmryth-omlx-MiniMax-M3-4bit");
    expect(settings.customModels).toHaveLength(1);
...
describe("applyDroidLocalAuthEnv", () => {
  it("fills a placeholder Factory key only for a local inject pick", () => {
    ...
    expect(env.FACTORY_API_KEY).toBe("helmryth-local");
    applyDroidLocalAuthEnv(env, "ollama::ornith:35b-bf16");
    expect(env.FACTORY_API_KEY).toBe("helmryth-local");
  });

  it("leaves a real Factory key and cloud slugs alone", () => {
    const kept: TestEnvironment = { FACTORY_API_KEY: "fk-real" };
  ...
  it("does not invent a Factory key when a Droid auth file already exists", () => {
    const home = mkdtempSync(join(tmpdir(), "hry-droid-authfile-"));
    scratchDirs.push(home);
    mkdirSync(join(home, ".factory"), { recursive: true });
    writeFileSync(join(home, ".factory", "auth.v2.file"), "signed-in");
    const env: TestEnvironment = { FACTORY_HOME_OVERRIDE: home };
    applyDroidLocalAuthEnv(env, "ollama::ornith:35b-bf16");
  ...
  it("puts the placeholder on the Droid child only for a local inject pick", async () => {
    const home = mkdtempSync(join(tmpdir(), "hry-droid-overlay-"));
    scratchDirs.push(home);
    mkdirSync(join(home, ".factory"), { recursive: true });
    const dump = join(home, "dump.json");
    ...
    try {
      ...
      await recorder.until((e) => e.type === "turn.completed");
      expect(fakeAcpDumpSchema.parse(JSON.parse(readFileSync(dump, "utf8"))).env.FACTORY_API_KEY).toBe("helmryth-local");
      ...
      await recorder.until((e) => e.type === "turn.completed" && e.threadId === "t-cloud");
      expect(fakeAcpDumpSchema.parse(JSON.parse(readFileSync(dump, "utf8"))).env.FACTORY_API_KEY).not.toBe(
        "helmryth-local",
      );
    } finally {
      await instance.dispose();
...
describe("ensureOpenCodeInjectModel", () => {
  it("merges a host provider into opencode.json without dropping existing models", () => {
    const home = mkdtempSync(join(tmpdir(), "hry-opencode-inject-"));
    scratchDirs.push(home);
    ...
    const config = openCodeConfigSchema.parse(JSON.parse(readFileSync(join(dir, "opencode.json"), "utf8")));
  ...
  it("injects into a default object when opencode.json is malformed", () => {
    const home = mkdtempSync(join(tmpdir(), "hry-opencode-bad-json-"));
    scratchDirs.push(home);
    const dir = join(home, ".config", "opencode");
    mkdirSync(dir, { recursive: true });
...
describe("ensureHermesInjectProvider", () => {
  it("writes a named custom provider and returns the ACP model id", () => {
    const home = mkdtempSync(join(tmpdir(), "hry-hermes-inject-"));
    scratchDirs.push(home);
  ...
  it("replaces a nested provider block without leaving orphan YAML lines", () => {
    const home = mkdtempSync(join(tmpdir(), "hry-hermes-nested-"));
    scratchDirs.push(home);
    mkdirSync(join(home, ".hermes"), { recursive: true });
    writeFileSync(
...
describe("ensureQwenInjectModel", () => {
  it("upserts an OpenAI-compatible provider without dropping existing rows", () => {
    const home = mkdtempSync(join(tmpdir(), "hry-qwen-inject-"));
    scratchDirs.push(home);
    ...
    const settings = qwenSettingsSchema.parse(
      JSON.parse(readFileSync(join(home, ".qwen", "settings.json"), "utf8")),
    );
    expect(settings.modelProviders.openai.map((row) => row.id)).toEqual(["keep-me", "GLM-5.2-fp8"]);
    expect(settings.env.HELMRYTH_QWEN_OMLX_API_KEY).toBe("omlx");
  ...
  it("injects into a default object when settings.json is malformed", () => {
    const home = mkdtempSync(join(tmpdir(), "hry-qwen-bad-json-"));
    scratchDirs.push(home);
    mkdirSync(join(home, ".qwen"), { recursive: true });
...
describe("live Custom lists on every local CLI harness", () => {
  it("merges probed host models onto Kimi, Droid, and Antigravity", async () => {
    ...
    const home = mkdtempSync(join(tmpdir(), "hry-all-inject-"));
    scratchDirs.push(home);
    const environment = { HOME: home, HELMRYTH_PROBE_LOCAL_INJECT: "1" };
