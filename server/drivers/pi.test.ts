...
const thinkingDumpSchema = z.object({ thinkingLevel: z.string().optional() });
const environmentDumpSchema = z.object({ argv: z.array(z.string()), envConfigured: z.array(z.string()) });
const mcpServerDumpSchema = z.object({
  command: z.string(),
  args: z.array(z.string()),
  env: z.record(z.string(), z.string()),
  scope: z.literal("local-computer").optional(),
});
const mcpDumpSchema = z.object({
  argv: z.array(z.string()),
  mcpConfig: z.object({ mcpServers: z.record(z.string(), mcpServerDumpSchema).optional() }).nullable().optional(),
});
const modelSelectionDumpSchema = z.object({
  setModel: z.object({ provider: z.string(), modelId: z.string() }).optional(),
});
const turnModelsFileSchema = z.object({
  providers: z.object({
    omlx: z.object({ baseUrl: z.string(), models: z.array(z.object({ id: z.string() })) }),
  }),
});
const injectedModelsFileSchema = z.object({
  providers: z.object({
    omlx: z.object({
      baseUrl: z.string(),
      api: z.string(),
      apiKey: z.string(),
      models: z.array(z.object({ id: z.string(), contextWindow: z.number().optional() })),
    }),
  }),
});
const unslothModelsFileSchema = z.object({
  providers: z.object({ unsloth: z.object({ apiKey: z.string(), baseUrl: z.string() }) }),
});

function parseJsonLines<T>(source: string, schema: z.ZodType<T>): T[] {
...
describe("buildMcpServers", () => {
  ...
  it("wraps the cloud computer in the computer-proxy spawn contract", () => {
    ...
    expect(servers?.computer).toMatchObject({
      ...
      env: expect.objectContaining({ HELMRYTH_BOX_ID: "b1", HELMRYTH_BOX_TOKEN: "tok" }),
    });
  });
...
describe("PiDriver catalog (fake CLI)", () => {
  ...
  it("probes the live catalog and flags every option custom", async () => {
    const catalog = await fetchPiModels(FAKE_CLI, { PATH: process.env.PATH ?? "", HOME: join(tmpdir(), "hry-pi-no-settings") });
    expect(catalog.options).toEqual([
      { id: "ollama-cloud/glm-5.2", label: "glm-5.2", custom: true },
      { id: "openai/gpt-4o", label: "gpt-4o", custom: true },
    ]);
...
describe("PiDriver turns (fake CLI)", () => {
  ...
  const create = async (mode?: string, environment: Record<string, string> = {}) => {
    ...
    if (mode) childEnvironment.FAKE_PI_MODE = mode;
  ...
  it("normalizes a full turn into the canonical event sequence", async () => {
    ...
    expect(session?.type === "session.started" ? session.sessionId : null).toMatch(/\/fake\/pi-session-\d+\.json/);

    const text = recorder.events.find(
      (e) => e.type === "item.completed" && "itemType" in e && e.itemType === "assistant_text",
    );
    expect(text?.type === "item.completed" && "text" in text ? text.text : null).toBe("Hello from pi");
  ...
  it("resumes a prior pi session using the sessionFile resume cursor", async () => {
    ...
    const firstSession = recorder.events.find((e) => e.type === "session.started" && e.turnId === first.turnId);
    const firstSessionId = firstSession?.type === "session.started" ? firstSession.sessionId : null;
    expect(firstSessionId).toMatch(/\/fake\/pi-session-\d+\.json/);

    const second = await instance.adapter.sendTurn({
      threadId: "t-resume",
      text: "second",
      resumeCursor: firstSessionId,
    });
    await recorder.until((e) => e.type === "turn.completed" && e.turnId === second.turnId);
    const secondSession = recorder.events.find((e) => e.type === "session.started" && e.turnId === second.turnId);
    const secondSessionId = secondSession?.type === "session.started" ? secondSession.sessionId : null;
  ...
  it("pins reasoning effort via set_thinking_level after the model", async () => {
    const dir = mkdtempSync(join(tmpdir(), "hry-pi-effort-"));
    ...
    const levels = parseJsonLines(readFileSync(dump, "utf8"), thinkingDumpSchema)
  ...
  it("scrubs provider and workspace credentials from every pi child env", async () => {
    const dir = mkdtempSync(join(tmpdir(), "hry-pi-dump-"));
    const dump = join(dir, "dump.jsonl");
    ...
    const rows = parseJsonLines(readFileSync(dump, "utf8"), environmentDumpSchema);
  ...
  it("mounts integrations as stdio MCP servers and loads the pi-mcp-extension", async () => {
    const dir = mkdtempSync(join(tmpdir(), "hry-pi-mcp-dump-"));
    const dump = join(dir, "dump.jsonl");
    await create(undefined, { FAKE_PI_DUMP: dump });
    const { turnId } = await instance.adapter.sendTurn({
      threadId: "t-mcp",
    ...
    const rows = parseJsonLines(readFileSync(dump, "utf8"), mcpDumpSchema);
    ...
    expect(servers.computer.env).toMatchObject({ HELMRYTH_BOX_ID: "b1", HELMRYTH_BOX_TOKEN: "bt" });
  ...
  it("rides the toolUse auto-continue and only settles on the final end_turn", async () => {
    ...
    expect(recorder.events.filter((e) => e.type === "item.completed" && "itemType" in e && e.itemType === "tool").length).toBe(1);
    expect(done).toMatchObject({ ok: true, stopReason: "end_turn" });
    expect(done).toMatchObject({ usage: { input: 12, output: 2 } });
    const text = recorder.events.find(
      (e) => e.type === "item.completed" && "itemType" in e && e.itemType === "assistant_text",
    );
    expect(text?.type === "item.completed" && "text" in text ? text.text : null).toBe("done");
  ...
  it("emits each assistant text block before the tool that follows it", async () => {
    ...
    const texts = recorder.events
      .filter((e) => e.type === "item.completed" && "itemType" in e && e.itemType === "assistant_text")
      .map((e) => e.type === "item.completed" && "text" in e ? e.text : "");
  ...
  it("writes models.json and set_model for a host::model inject pick", async () => {
    const home = mkdtempSync(join(tmpdir(), "hry-pi-turn-inject-"));
    ...
    await recorder.until((e) => e.type === "turn.completed" && e.turnId === turnId);
    const dumps = parseJsonLines(readFileSync(dump, "utf8"), modelSelectionDumpSchema);
    expect(dumps.some((row) => row.setModel?.provider === "omlx" && row.setModel?.modelId === "MiniMax-M3-4bit")).toBe(
      true,
    );
    const written = turnModelsFileSchema.parse(JSON.parse(readFileSync(join(home, ".pi", "agent", "models.json"), "utf8")));
...
describe("ensurePiInjectModel", () => {
  it("upserts a provider into ~/.pi/agent/models.json without dropping existing models", () => {
    const home = mkdtempSync(join(tmpdir(), "hry-pi-inject-"));
    ...
    const written = injectedModelsFileSchema.parse(
      JSON.parse(readFileSync(join(home, ".pi", "agent", "models.json"), "utf8")),
    );
    expect(written.providers.omlx.baseUrl).toBe("http://127.0.0.1:8080/v1");
  ...
  it("writes Unsloth's studio token, not the placeholder", () => {
    const home = mkdtempSync(join(tmpdir(), "hry-pi-unsloth-"));
    ...
    const written = unslothModelsFileSchema.parse(
      JSON.parse(readFileSync(join(home, ".pi", "agent", "models.json"), "utf8")),
    );
  ...
  it("leaves official slugs and the models.json file untouched", () => {
    const home = mkdtempSync(join(tmpdir(), "hry-pi-cloud-"));
  ...
  it("does not destroy a malformed models.json", () => {
    const home = mkdtempSync(join(tmpdir(), "hry-pi-badjson-"));
    mkdirSync(join(home, ".pi", "agent"), { recursive: true });
