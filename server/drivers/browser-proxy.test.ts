...
let held = false;
let helpOpen = false;
const helpRequests: Array<{ method: string; body: JsonObject }> = [];
const hits: Array<{ path: string; auth: string | undefined; body: JsonObject }> = [];
...
const text = (res: RpcReply) => String(res.result.content[0].text);
...
beforeAll(async () => {
  ...
  child = spawn(process.execPath, ["--experimental-strip-types", PROXY], {
    env: {
      ...process.env,
      HELMRYTH_BROWSER_URL: `http://127.0.0.1:${stubPort}`,
      HELMRYTH_BROWSER_TOKEN: TOKEN,
      HELMRYTH_BOT_ID: "bot-1",
      HELMRYTH_BROWSER_PROFILE: "work",
      HELMRYTH_CONTROL_URL: `http://127.0.0.1:${stubPort}/control`,
      HELMRYTH_CONTROL_TOKEN: CONTROL_TOKEN,
    },
