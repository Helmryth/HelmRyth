...
const upstreamUsageSchema = z.object({
  prompt_tokens: z.number().catch(0),
  completion_tokens: z.number().catch(0),
}).transform((usage): TokenUsage => ({
...
const completionResponseSchema = z.object({
  choices: z.array(z.object({
    message: z.object({ content: z.string().nullish().catch(undefined) }).catch({}),
  }).catch({ message: {} })).catch([]),
...
const streamChunkSchema = z.object({
  choices: z.array(z.object({
    delta: z.object({ content: z.string().nullish().catch(undefined) }).catch({}),
  }).catch({ delta: {} })).catch([]),
  usage: upstreamUsageSchema.nullish().catch(undefined),
}).catch({ choices: [] });
...
const externalErrorSchema = z.object({
  ...
  message: z.string().catch("Unknown xAI failure"),
});
...
const decodeConfig: ProviderDriver<GrokConfig>["decodeConfig"] = (raw) => {
  const payload = configPayloadSchema.parse(raw ?? {});
  return {
    url: payload.url ?? DEFAULT_URL,
    apiKeyEnv: payload.apiKeyEnv ?? "XAI_API_KEY",
  };
};
...
export const GrokDriver: ProviderDriver<GrokConfig> = {
  ...
  async create(input: DriverCreateInput<GrokConfig>): Promise<ProviderInstance> {
    ...
    const complete = async (
      ...
      if (!res.body) throw new Error("xAI returned no response body");
    ...
    const snapshot = async (): Promise<ProviderSnapshot> => {
      if (!apiKey) {
        return {
          state: "unavailable",
          reason: `no xAI API key — add {"xai":{"key":"xai-…"}} to ~/.helmryth/config.json or set ${config.apiKeyEnv}`,
        };
      }
