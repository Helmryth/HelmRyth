...
function scratchConfig(toml: string): string {
  const dir = mkdtempSync(join(tmpdir(), "hry-grok-catalog-"));
...
describe("readGrokModelCatalog", () => {
  it("returns the static cloud pair when there is no config", () => {
    expect(readGrokModelCatalog({ HOME: join(tmpdir(), "hry-grok-missing-home") })).toEqual(STATIC_GROK_MODELS);
  });
...
`);
  ...
  it("honors GROK_HOME over HOME", () => {
    ...
    const grokHomeParent = mkdtempSync(join(tmpdir(), "hry-grok-home-"));
