...
const trimmedYamlStringSchema = z.string().transform((value) => value.trim());
const hermesNestedModelSchema = z.looseObject({
  provider: trimmedYamlStringSchema.optional().catch(undefined),
  model: trimmedYamlStringSchema.optional().catch(undefined),
  default: trimmedYamlStringSchema.optional().catch(undefined),
});
const hermesModelCandidateSchema = z.union([trimmedYamlStringSchema, hermesNestedModelSchema]);
const hermesModelConfigSchema = z.looseObject({
  provider: trimmedYamlStringSchema.optional().catch(undefined),
  default: hermesModelCandidateSchema.optional().catch(undefined),
  model: hermesModelCandidateSchema.optional().catch(undefined),
  name: hermesModelCandidateSchema.optional().catch(undefined),
});
const hermesRootConfigSchema = z.looseObject({
  provider: trimmedYamlStringSchema.optional().catch(undefined),
  model: z.union([trimmedYamlStringSchema, hermesModelConfigSchema]).optional().catch(undefined),
});
const hermesAvailableModelSchema = z.looseObject({
  modelId: z.string().min(1),
  name: z.string().optional(),
});
const hermesProbeMessageSchema = z.looseObject({
  id: z.number(),
  result: z.looseObject({
    models: z.looseObject({
      availableModels: z.array(z.json()).optional().catch(undefined),
    }).optional().catch(undefined),
  }).optional().catch(undefined),
});

interface HermesInitializeParams {
  protocolVersion: number;
  clientCapabilities: { fs: { readTextFile: boolean; writeTextFile: boolean } };
}

interface HermesSessionNewParams {
  cwd: string;
  mcpServers: never[];
}

type HermesProbeParams = HermesInitializeParams | HermesSessionNewParams;
type AcpConfigureSessionContext = Parameters<NonNullable<AcpSupport["configureSession"]>>[0];
type AcpRequest = AcpConfigureSessionContext["request"];

interface HermesModelSetting {
  sessionId: string;
  modelId: string;
}

export const HERMES_HELMRYTH_SCREENSHOT_COMPAT = "HERMES_HELMRYTH_SCREENSHOT_COMPAT";
export const HERMES_HELMRYTH_SCREENSHOT_COMPAT_MODEL = "HERMES_HELMRYTH_SCREENSHOT_COMPAT_MODEL";
...
export function bindHermesScreenshotCompat(
  ...
  delete env[HERMES_HELMRYTH_SCREENSHOT_COMPAT];
  delete env[HERMES_HELMRYTH_SCREENSHOT_COMPAT_MODEL];
  const inject = decodeInjectId(modelId);
  if (!inject) return;
  env[HERMES_HELMRYTH_SCREENSHOT_COMPAT] = "1";
  env[HERMES_HELMRYTH_SCREENSHOT_COMPAT_MODEL] = inject.model;
}
...
function hermesConfigDefault(text: string): { model: string; provider: string } | null {
  let config: z.infer<typeof hermesRootConfigSchema>;
  try {
    const parsed = hermesRootConfigSchema.safeParse(parseYaml(text));
  ...
  const scalarModel = trimmedYamlStringSchema.safeParse(config.model);
  if (scalarModel.success) {
    const model = scalarModel.data;
    return model ? { model, provider: rootProvider } : null;
  }
  const parsedModelConfig = hermesModelConfigSchema.safeParse(config.model);
  if (!parsedModelConfig.success) return null;

  const modelConfig = parsedModelConfig.data;
  const outerProvider = modelConfig.provider || rootProvider;
  for (const key of ["default", "model", "name"] as const) {
    const candidate = modelConfig[key];
    const scalar = trimmedYamlStringSchema.safeParse(candidate);
    if (scalar.success && scalar.data) return { model: scalar.data, provider: outerProvider };
    const parsedNested = hermesNestedModelSchema.safeParse(candidate);
    if (!parsedNested.success) continue;
    const nested = parsedNested.data;
    const nestedModel = nested.model || nested.default || "";
...
async function fetchHermesAcpModels(
  ...
  return await new Promise((resolve) => {
    ...
    try {
      child = spawn(cli, ["acp"], { stdio: ["pipe", "pipe", "ignore"], env });
    } catch {
      return resolve([]);
    }
    let settled = false;
    ...
    let buf = "";
    let id = 0;
    const send = (method: "initialize" | "session/new", params: HermesProbeParams) => {
      id += 1;
      try {
    ...
    child.stdout?.on("data", (chunk) => {
      ...
      while ((nl = buf.indexOf("\n")) >= 0) {
        ...
        buf = buf.slice(nl + 1);
        let msg: z.infer<typeof hermesProbeMessageSchema>;
        try {
          const parsed = hermesProbeMessageSchema.safeParse(parseJson(line));
        ...
        if (msg.id === initId) {
          ...
          const models: { id: string; label: string; custom: true }[] = [];
          for (const candidate of msg.result?.models?.availableModels ?? []) {
            const parsed = hermesAvailableModelSchema.safeParse(candidate);
            if (!parsed.success) continue;
...
async function applySetting(
  ...
  try {
    ...
    throw new Error(`Hermes rejected ${what} via ${method}: ${message}`);
  }
}
...
const support: AcpSupport = {
  ...
  access: "custom",
  models: EMPTY,
  resolveModels: (env, config) => resolveModels(env, config),
  resolveTurnModel: (model, env) => {
    // Never inherit a broad or stale compatibility grant from the parent.
    // Only this Helmryth driver binds one concrete local model; Hermes still
