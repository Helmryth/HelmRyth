...
// models discovered from that harness rather than separate Helmryth drivers.
...
const openCodeModelRecordSchema = z.object({
  name: z.string().optional().catch(undefined),
  status: z.string().optional().catch(undefined),
  limit: z.object({ context: z.number().optional().catch(undefined) }).optional().catch(undefined),
  api: z.object({ url: z.string().optional().catch(undefined) }).optional().catch(undefined),
});
const openCodeAuthEntrySchema = z.object({
  key: z.json().optional(),
  access: z.json().optional(),
  refresh: z.json().optional(),
});
const openCodeFailureSchema = z.object({
  code: z.union([z.number(), z.string()]).optional(),
});

type OpenCodeModelRecord = z.infer<typeof openCodeModelRecordSchema>;
type OpenCodeFailure = z.infer<typeof openCodeFailureSchema>;
...
function localModelRecord(record: OpenCodeModelRecord): boolean {
...
function parseJsonObject(text: string): JsonObject | null {
  try {
    const parsed = jsonObjectSchema.safeParse(parseJson(text));
...
export function parseOpenCodeModelsOutput(stdout: string): ModelCatalog | null {
  ...
  const flush = () => {
    ...
    let record: OpenCodeModelRecord = {};
    const raw = jsonLines.join("\n").trim();
    if (raw) {
      try {
        const parsed = openCodeModelRecordSchema.safeParse(parseJson(raw));
    ...
    const name = record.name?.trim() || labelForModel(model);
    const context = record.limit?.context;
    const contextWindow = context !== undefined && Number.isFinite(context) && context > 0
      ? Math.floor(context)
      : undefined;
    seen.add(slug);
    ...
    if (localModelRecord(record)) {
      option.custom = true;
      option.loaded = true;
    }
    if (contextWindow) option.contextWindow = contextWindow;
    options.push(option);
  };
  ...
  const selected = options.find((option) => option.id === STATIC_MODELS.default) ?? options[0];
  if (!selected) return null;
  return { default: selected.id, options };
}
...
export function ensureOpenCodeInjectModel(
  ...
  let config: JsonObject = { $schema: "https://opencode.ai/config.json" };
  if (existsSync(path)) {
    try {
      config = parseJsonObject(readFileSync(path, "utf8")) ?? config;
    } catch {
      // An unreadable user config is equivalent to a malformed one here.
    }
  }
  const providers: JsonObject = { ...jsonObject(config.provider) };
  const previous = providers[inject.host];
  const existing: JsonObject = jsonObject(previous) ?? {
    npm: "@ai-sdk/openai-compatible",
    name: host.label,
    options: {},
    models: {},
  };
  const options: JsonObject = { ...jsonObject(existing.options) };
  options.baseURL = host.baseUrl;
  if (!options.apiKey) options.apiKey = hostApiKey(host, env);
  const models: JsonObject = { ...jsonObject(existing.models) };
...
function usableAuthEntry(parsed: JsonObject): boolean {
  return Object.values(parsed).some((auth) => {
    const result = openCodeAuthEntrySchema.safeParse(auth);
...
function hasStoredOpenCodeAuth(env: Record<string, string | undefined>) {
  ...
  return candidates.some((raw) => {
    ...
    return parsed ? usableAuthEntry(parsed) : false;
  });
}
...
const support = (loadCatalog: OpenCodeCatalogLoader): AcpSupport => ({
  ...
  classifyError: (error) => {
    const parsed = openCodeFailureSchema.safeParse(error);
    return classifyOpenCodeError(parsed.success ? parsed.data : {});
  },
...
export function classifyOpenCodeError(error: OpenCodeFailure): ProviderErrorCode | undefined {
