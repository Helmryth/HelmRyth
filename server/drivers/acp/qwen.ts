// Qwen Code — Alibaba's `qwen --acp` CLI. Custom-only in Helmryth:
...
const qwenProviderRowSchema = z
  .object({ id: z.string(), baseUrl: z.string() })
  .catchall(z.json());
...
function envKeyFor(hostId: string): string {
  return `HELMRYTH_QWEN_${hostId.toUpperCase().replace(/[^A-Z0-9]+/g, "_")}_API_KEY`;
}
...
export function ensureQwenInjectModel(
  ...
  const parsedEnv = jsonObjectSchema.safeParse(settings.env);
  const envMap: JsonObject = parsedEnv.success ? { ...parsedEnv.data } : {};
  envMap[keyName] = key;
  settings.env = envMap;

  const parsedProviders = jsonObjectSchema.safeParse(settings.modelProviders);
  const providers: JsonObject = parsedProviders.success ? { ...parsedProviders.data } : {};
  const parsedOpenAi = z.array(z.json()).safeParse(providers.openai);
  const openai: JsonValue[] = parsedOpenAi.success ? [...parsedOpenAi.data] : [];
  const match = openai.find((row) => {
    const parsed = qwenProviderRowSchema.safeParse(row);
    return parsed.success && parsed.data.id === inject.model && parsed.data.baseUrl === host.baseUrl;
  });
  if (!match) {
    openai.push({
      id: inject.model,
