...
export interface AcpSupport {
  ...
  classifyError?(cause: unknown): ProviderErrorCode | undefined;
...
/** Outgoing ACP parameters keep their caller-owned shape until JSON encoding. */
export interface AcpRequest {
  <Params>(method: string, params: Params, timeoutMs?: number): Promise<JsonValue>;
}
...
const acpConfigSchema = z
  .object({
    cli: optionalStringSchema,
    fullAuto: z.literal(true).optional().catch(undefined),
    workspace: optionalStringSchema,
  })
  .catchall(z.json())
  .catch({});

const acpRpcIdSchema = z.union([z.string(), z.number(), z.null()]);
const acpRpcErrorSchema = z
  .object({
    code: z.union([z.string(), z.number()]).optional().catch(undefined),
    data: z.json().optional().catch(undefined),
    message: optionalStringSchema,
  })
  .catchall(z.json());
const acpWireMessageSchema = z
  .object({
    id: acpRpcIdSchema.optional(),
    method: optionalStringSchema,
    params: z.json().optional(),
    result: z.json().optional(),
    error: acpRpcErrorSchema.optional(),
  })
  .catchall(z.json());

const permissionOptionSchema = z
  .object({ optionId: optionalStringSchema, kind: optionalStringSchema })
  .catchall(z.json());
const permissionToolCallSchema = z
  .object({
    ...
    rawInput: z
      .object({ command: z.json().optional() })
      .catchall(z.json())
      .optional()
      .catch(undefined),
  })
...
const permissionParamsSchema = z
  .object({
    options: z.array(permissionOptionSchema).optional().catch([]),
    toolCall: permissionToolCallSchema.optional().catch(undefined),
  })
  .catchall(z.json());
...
const sessionUpdateParamsSchema = z
  .object({
    _meta: z
      .object({ isReplay: z.boolean().optional().catch(undefined) })
      .catchall(z.json())
      .optional()
      .catch(undefined),
    update: z
      .object({
        sessionUpdate: optionalStringSchema,
        status: optionalStringSchema,
        toolCallId: optionalStringSchema,
        title: z.json().optional(),
        content: z
          .object({ text: optionalStringSchema })
          .catchall(z.json())
          .optional()
          .catch(undefined),
        rawInput: z
...
const initializeResultSchema = z
  .object({
    authMethods: z
      .array(z.object({ id: optionalStringSchema }).catchall(z.json()))
      .optional()
      .catch([]),
    _meta: z
      .object({
        modelState: z
          .object({ currentModelId: optionalStringSchema })
          .catchall(z.json())
          .optional()
          .catch(undefined),
      })
...
const sessionResultSchema = z
  .object({
    ...
    configOptions: z
      .array(
        z
          .object({ id: optionalStringSchema, currentValue: optionalStringSchema })
          .catchall(z.json()),
      )
      .optional()
      .catch([]),
    models: z
      .object({
        availableModels: z
          .array(
            z
              .object({ modelId: optionalStringSchema, name: optionalStringSchema })
              .catchall(z.json()),
          )
          .optional()
          .catch([]),
...
const promptResultSchema = z
  .object({
    ...
    _meta: usageSchema.optional().catch(undefined),
  })
  .catchall(z.json())
  .catch({});

type AcpRpcErrorPayload = z.infer<typeof acpRpcErrorSchema>;
type AcpWireMessage = z.infer<typeof acpWireMessageSchema>;
type AcpSessionResult = z.infer<typeof sessionResultSchema>;

class AcpRpcError extends Error {
  readonly code: string | number | undefined;
  readonly data: JsonValue | undefined;

  constructor(payload: AcpRpcErrorPayload) {
    super(payload.message ?? JSON.stringify(payload));
...
function errorFromCause(cause: unknown): Error {
  ...
  const parsed = acpRpcErrorSchema.safeParse(cause);
  return parsed.success ? new AcpRpcError(parsed.data) : new Error(String(cause));
}

function decodeAcpConfig(defaultCli: string): ProviderDriver<AcpConfig>["decodeConfig"] {
  return (raw): AcpConfig => {
    const config = acpConfigSchema.parse(raw ?? {});
    return {
    ...
    async create(input: DriverCreateInput<AcpConfig>): Promise<ProviderInstance> {
      ...
      const sendTurn = async (turn: SendTurnInput) => {
        ...
        const rpcPending = new Map<
          number,
          { resolve: (value: JsonValue) => void; reject: (error: Error) => void; timer: ReturnType<typeof setTimeout> | null }
        >();

        const send = <Message>(obj: Message) => {
          try {
            child.stdin.write(JSON.stringify(obj) + "\n");
          } catch {}
          appendNative(threadId, { dir: "out", source: SOURCE, msg: obj });
        };
        const request: AcpRequest = <Params>(method: string, params: Params, timeoutMs?: number) =>
        ...
        const handleServerRequest = (msg: AcpWireMessage) => {
          ...
          const parsedParams = permissionParamsSchema.safeParse(msg.params ?? {});
          const params = parsedParams.success ? parsedParams.data : permissionParamsSchema.parse({});
          flushAssistantText();
          const options = params.options ?? [];
          const optionFor = (want: "allow" | "reject") =>
            options.find((option) => option.kind?.startsWith(want) && option.optionId)?.optionId ?? null;
        ...
        const handleNotification = (msg: AcpWireMessage) => {
          ...
          const parsedParams = sessionUpdateParamsSchema.safeParse(msg.params ?? {});
        ...
        child.stdout.on("data", (chunk) => {
          ...
          while ((nl = buf.indexOf("\n")) !== -1) {
            ...
            try {
              const parsed = acpWireMessageSchema.safeParse(parseJson(line));
            ...
            if (msg.id !== undefined && (msg.result !== undefined || msg.error !== undefined)) {
              const responseId = z.number().safeParse(msg.id);
              if (!responseId.success) continue;
              const pend = rpcPending.get(responseId.data);
              if (pend) {
                rpcPending.delete(responseId.data);
                if (pend.timer) clearTimeout(pend.timer);
                if (msg.error) {
                  pend.reject(new AcpRpcError(msg.error));
        ...
        (async () => {
          try {
            ...
            const parsedCursor = z.string().safeParse(turn.resumeCursor);
            const cursor = parsedCursor.success ? parsedCursor.data : null;
            let sessionResult: AcpSessionResult = {};
            if (cursor) {
              try {
                sessionResult = sessionResultSchema.parse(
                  await request(
                    "session/load",
            ...
            if (!sessionId) {
              sessionResult = sessionResultSchema.parse(
                await request("session/new", { cwd, mcpServers }, NEW_SESSION_TIMEOUT),
              );
            ...
            try {
              if (support.selectModel) {
                ...
                const currentOf = (result: AcpSessionResult) =>
                  (result.configOptions ?? []).find((option) => option.id === configId)?.currentValue ?? null;
              ...
              if (support.configureSession) {
                await support.configureSession({
                  ...
                  config,
                  turn: cliTurn,
                  sessionModels: sessionResult.models?.availableModels ?? [],
                });
            ...
            if (!state.settled) {
              ...
              if (needsAuth) runtimeError.setup = true;
              emit(runtimeError);
