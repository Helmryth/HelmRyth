...
const factoryCustomModelSchema = z.looseObject({
...
const factorySettingsSchema = z.looseObject({
  customModels: z.array(factoryCustomModelSchema).optional().catch(undefined),
  modelFavorites: z.array(z.string()).optional().catch(undefined),
  sessionDefaultSettings: z
    .looseObject({ model: z.string().optional().catch(undefined) })
    .optional()
    .catch(undefined),
});

function parseFactorySettings(text: string): FactorySettings {
  return factorySettingsSchema.parse(parseJson(text));
}

const INJECT_ID_PREFIX = "custom:helmryth-";
...
export function ensureDroidInjectModel(
  ...
  try {
    settings = parseFactorySettings(readFileSync(path, "utf8"));
  } catch (error) {
...
export function applyDroidLocalAuthEnv(
  ...
  env.FACTORY_API_KEY = "helmryth-local";
}

function readSettings(env: Record<string, string | undefined>): FactorySettings {
  return parseFactorySettings(readFileSync(join(factoryHome(env), ".factory", "settings.json"), "utf8"));
}
...
type AcpConfigureSession = NonNullable<AcpSupport["configureSession"]>;
type AcpSessionRequest = Parameters<AcpConfigureSession>[0]["request"];

interface DroidModeParams {
  sessionId: string;
  modeId: string;
}

interface DroidModelParams {
  sessionId: string;
  modelId: string;
}
...
async function applySetting(
  ...
  method: string,
  params: DroidModeParams | DroidModelParams,
  what: string,
): Promise<void> {
  try {
    ...
    throw new Error(
      `Droid rejected ${what} via ${method}: ${message}. ` +
