...
const classifiedErrorSchema = z.object({ code: z.literal(-32_000) }).passthrough();
const acpArgvDumpSchema = z.object({ argv: z.array(z.string()) }).passthrough();

type AssistantTextEvent = Extract<RuntimeEvent, { type: "item.completed"; itemType: "assistant_text" }>;

function isAssistantTextEvent(event: RuntimeEvent): event is AssistantTextEvent {
  return event.type === "item.completed" && event.itemType === "assistant_text";
}
...
const ClassifiedErrorDriver = createAcpDriver({
  ...
  selectModel: undefined,
  classifyError: (cause) => classifiedErrorSchema.safeParse(cause).success ? "invalid_credentials" : undefined,
});
...
describe("ACP turns (fake CLI)", () => {
  ...
  beforeEach(() => {
    ...
    scratch = mkdtempSync(join(tmpdir(), "hry-acp-test-"));
  });
  ...
  it("normalizes a full turn into the canonical event sequence", async () => {
    ...
    const text = recorder.events.find(isAssistantTextEvent);
    expect(text?.text).toBe("hello from fake acp");
  ...
  it("mounts connected apps as a stdio MCP server", async () => {
    ...
    await instance.adapter.sendTurn({
      ...
      integrations: {
        composio: {
          ...
          env: { HELMRYTH_CONNECTOR_UPSTREAM_URL: "http://127.0.0.1:8799/api/internal/connectors/mcp" },
        },
      },
    });
    ...
    expect(JSON.parse(readFileSync(`${dump}.mcp.json`, "utf8"))).toContainEqual({
      ...
      command: process.execPath,
      args: ["/tmp/connector-proxy.js"],
      env: [{ name: "HELMRYTH_CONNECTOR_UPSTREAM_URL", value: "http://127.0.0.1:8799/api/internal/connectors/mcp" }],
    });
  });
  ...
  it("surfaces a permission ask as request.opened and completes once allowed", async () => {
    ...
    if (opened.type !== "request.opened" || !opened.requestId) {
      throw new Error("expected a permission request with an id");
    }
  ...
  it("puts Grok -m after agent so ACP stdio binds the local slug", async () => {
    ...
    const argv = acpArgvDumpSchema.parse(parseJson(readFileSync(dump, "utf8"))).argv;
...
describe("ACP snapshot", () => {
  ...
  it("kimi checks KIMI_CODE_HOME before the child HOME", async () => {
    const scratch = mkdtempSync(join(tmpdir(), "hry-kimi-auth-"));
  ...
  it("droid resolves the signed-in CLI before falling back to FACTORY_API_KEY", async () => {
    const scratch = mkdtempSync(join(tmpdir(), "hry-droid-auth-"));
  ...
  it("droid reads custom models, favourites order, and the configured default", async () => {
    const scratch = mkdtempSync(join(tmpdir(), "hry-droid-models-"));
  ...
  it("droid falls back to the built-in catalog when settings.json is unreadable", async () => {
    const scratch = mkdtempSync(join(tmpdir(), "hry-droid-nosettings-"));
    mkdirSync(join(scratch, ".factory"), { recursive: true });
  ...
  it("kimi resolves default credentials from the child HOME", async () => {
    const scratch = mkdtempSync(join(tmpdir(), "hry-kimi-home-"));
