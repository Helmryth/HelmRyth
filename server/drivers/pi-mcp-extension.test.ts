...
const originalMcpConfig = process.env.HELMRYTH_MCP_CONFIG;
...
function tempDir(): string {
  const dir = mkdtempSync(join(tmpdir(), "hry-pi-mcp-ext-"));
  tempDirs.push(dir);
  return dir;
}
...
afterEach(() => {
  ...
  if (originalMcpConfig === undefined) delete process.env.HELMRYTH_MCP_CONFIG;
  else process.env.HELMRYTH_MCP_CONFIG = originalMcpConfig;
});
...
describe("Pi MCP extension registration", () => {
  it("keeps earlier tools alive when a later registration fails", async () => {
    ...
    process.env.HELMRYTH_MCP_CONFIG = config;
