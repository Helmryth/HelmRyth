...
const fakeCodexDumpSchema = z.object({
...
const turnStartParamsSchema = z.object({
  input: z.array(z.object({ text: z.string() })).optional(),
  effort: z.string().optional(),
});

type FakeCodexDump = z.infer<typeof fakeCodexDumpSchema>;

function readFakeCodexDump(path: string): FakeCodexDump {
  return fakeCodexDumpSchema.parse(JSON.parse(readFileSync(path, "utf8")));
}

function parseCallParams<T>(dump: FakeCodexDump, method: string, schema: z.ZodType<T>): T {
  const call = dump.calls.find((candidate) => candidate.method === method);
  if (!call) throw new Error(`Expected fake Codex call ${method}`);
  return schema.parse(call.params);
}
...
describe("CodexDriver turns (fake app-server)", () => {
  ...
  beforeEach(() => {
    ...
    scratch = mkdtempSync(join(tmpdir(), "hry-codex-test-"));
  });
  ...
  it("runs the handshake and normalizes a full turn", async () => {
    ...
    expect(types).toEqual([
      ...
      "item.started", // webSearch Helmryth
    ...
    const seen = readFakeCodexDump(dump);
    expect(seen.env.OPENAI_API_KEY).toBeUndefined();
    expect(seen.env.BOX_TOKEN).toBeUndefined();
    expect(seen.env.HELMRYTH_TTS_KEY).toBeUndefined();
    const methods = seen.calls.map((call) => call.method);
    ...
    const turnStart = parseCallParams(seen, "turn/start", turnStartParamsSchema);
    expect(turnStart.input?.[0]?.text).toBe("You are Testy.\n\nlist files");
    const threadStart = parseCallParams(seen, "thread/start", threadStartParamsSchema);
    expect(threadStart).toMatchObject({ model: "gpt-5.6-sol", modelProvider: "openai" });
  });
  ...
  it("uses the instance environment for the Codex process", async () => {
    ...
    expect(readFakeCodexDump(dump).env.CODEX_HOME).toBe(codexHome);
  });
  ...
  it("mounts connected apps without placing credential values in argv", async () => {
      ...
      integrations: {
        composio: {
          ...
          env: {
            ...
            HELMRYTH_COMMS_TOKEN: "per-boot-token",
          },
        },
      },
    });
    await recorder.until((event) => event.type === "turn.completed");
    const seen = readFakeCodexDump(dump);
    expect(seen.argv.join(" ")).toContain("mcp_servers.helmryth_connectors.command");
    expect(seen.argv.join(" ")).toContain("HELMRYTH_COMMS_TOKEN");
    expect(seen.argv.join(" ")).not.toContain("per-boot-token");
    expect(seen.env.HELMRYTH_COMMS_TOKEN).toBe("per-boot-token");
  });
  ...
  it("mounts peer-agent comms without placing the comms token in argv", async () => {
      ...
      integrations: {
        agents: {
          ...
          env: {
            ...
            HELMRYTH_BOT_ID: "captain",
            HELMRYTH_THREAD_ID: "t-agents",
            HELMRYTH_COMMS_TOKEN: "peer-comms-secret",
    ...
    expect(seen.env.HELMRYTH_COMMS_TOKEN).toBe("peer-comms-secret");
  ...
  it("mounts the Local VM computer MCP server without placing credentials in argv", async () => {
    ...
    await instance.adapter.sendTurn({
      ...
      integrations: {
        localComputer: {
          command: process.execPath,
          args: ["/tmp/container-mcp.js", "podman", "helmryth-computer", "/run/cua.sock"],
          env: { ELECTRON_RUN_AS_NODE: "1", HELMRYTH_VM_TOKEN: "vm-secret" },
        },
      },
    });
    await recorder.until((event) => event.type === "turn.completed");
    ...
    expect(seen.argv.join(" ")).toContain("HELMRYTH_VM_TOKEN");
    expect(seen.argv.join(" ")).not.toContain("vm-secret");
    expect(seen.env.HELMRYTH_VM_TOKEN).toBe("vm-secret");
  });
  ...
  it("mounts the remote computer proxy without placing its token in argv", async () => {
    ...
    expect(seen.argv.join(" ")).toContain("HELMRYTH_BOX_TOKEN");
    expect(seen.argv.join(" ")).not.toContain("remote-secret");
    expect(seen.env.HELMRYTH_BOX_ID).toBe("box-123");
    expect(seen.env.HELMRYTH_BOX_TOKEN).toBe("remote-secret");
  });
  ...
  it("sends the local provider when the picker id is custom-encoded", async () => {
    ...
    await recorder.until((e) => e.type === "turn.completed");
    const localDump = readFakeCodexDump(dump);
    const threadStart = parseCallParams(localDump, "thread/start", threadStartParamsSchema);
    expect(threadStart).toMatchObject({
      model: "Qwen3.6-35B-A3B-bf16:qwen3-5-6-n-r-reasoning",
      modelProvider: "unsloth",
    });
    const seen = localDump;
    ...
    expect(seen.env.HELMRYTH_LOCAL_UNSLOTH_API_KEY).toBe("unsloth-secret");
  });
  ...
  it("streams agentMessage deltas without re-emitting the settled text", async () => {
    ...
    const text = recorder.events.flatMap((event) =>
      event.type === "content.delta" && event.streamKind === "assistant_text" ? [event.delta] : []
    );
    // the two streamed chunks only — no third whole-message fallback delta
    expect(text).toEqual(["done from ", "fake codex"]);
    const settled = recorder.events.flatMap((event) =>
      event.type === "item.completed" && event.itemType === "assistant_text" ? [event.text] : []
    );
    expect(settled).toHaveLength(1);
    expect(settled[0]).toBe("done from fake codex");
  });
  ...
  it("tries thread/resume with a cursor and reuses the thread id", async () => {
    ...
    const methods = readFakeCodexDump(dump).calls.map((call) => call.method);
  ...
  it("surfaces an approval request and forwards the user's decision", async () => {
    ...
    expect(readFakeCodexDump(dump).decision).toEqual({ decision: "approved" });
  });
  ...
  it("answers Codex 0.149 MCP elicitation with the MCP result shape", async () => {
    ...
    await recorder.until((e) => e.type === "turn.completed");
    expect(readFakeCodexDump(dump).decision).toEqual({ action: "accept", content: {} });
  });
  ...
  it("stamps approvalScope on cards only when the turn controls this Mac", async () => {
    ...
    if (vm.type !== "request.opened") throw new Error("Expected the Local VM permission request");
    expect(vm.approvalScope).toBeUndefined();
  ...
  it("auto-retries a transient turn/start failure, then completes with one final message", async () => {
    ...
    expect(retries.every((e) => e.delayMs > 0 && e.reason.length > 0)).toBe(true);
