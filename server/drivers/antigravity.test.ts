...
describe("readAntigravityModelCatalog", () => {
  it("returns the official list when settings are missing", () => {
    expect(readAntigravityModelCatalog({ HOME: join(tmpdir(), "hry-agy-missing-home") })).toEqual(
      STATIC_ANTIGRAVITY_MODELS,
    );
  });

  it("tags extra settings models as custom", () => {
    const home = mkdtempSync(join(tmpdir(), "hry-agy-catalog-"));
  ...
  it("keeps valid custom models when settings contain future keys and malformed siblings", () => {
    const home = mkdtempSync(join(tmpdir(), "hry-agy-catalog-drift-"));
    mkdirSync(join(home, ".gemini", "antigravity-cli"), { recursive: true });
    writeFileSync(
      join(home, ".gemini", "antigravity-cli", "settings.json"),
      JSON.stringify({
        customModels: [null, 42, { futureModelField: true }, "valid-direct", { model: "valid-row", name: "Valid Row" }],
        availableModels: "future-non-array-shape",
        futureTopLevelKey: { keepParsing: true },
      }),
    );
    try {
      const catalog = readAntigravityModelCatalog({ HOME: home });
      expect(catalog.options.slice(-2)).toEqual([
        { id: "valid-direct", label: "valid-direct", custom: true },
        { id: "valid-row", label: "Valid Row", custom: true },
      ]);
    } finally {
      rmSync(home, { recursive: true, force: true });
    }
...
describe("Antigravity turns (fake CLI)", () => {
  ...
  it("normalizes a full print-mode turn into the canonical event sequence", async () => {
    ...
    const session = recorder.events.find((event) => event.type === "session.started");
    expect(session?.sessionId).toBe("conv-fake-123");

    const completedItems = recorder.events.filter((event) => event.type === "item.completed");
    const tool = completedItems.find((event) => event.itemType === "tool");
    expect(tool?.ok).toBe(true);

    const usage = recorder.events.find((e) => e.type === "thread.token-usage.updated")!;
    expect(usage).toMatchObject({ input: 105, output: 20 });

    const text = completedItems.find((event) => event.itemType === "assistant_text");
    expect(text?.text).toBe("done from fake agy");
  ...
  it("ignores future events and defaults malformed optional fields without dropping a successful result", async () => {
    const scratch = mkdtempSync(join(tmpdir(), "hry-agy-stream-drift-"));
    const cli = join(scratch, "agy-drift.mjs");
    writeFileSync(
      cli,
      `#!/usr/bin/env node
...
out({ event: "future_event", future_event: { newProtocolField: true } });
out({ event: "init", conversation_id: 42, init: { futureInitField: true } });
out({ event: "step_update", step_update: { step_type: "agent_response", usage: { input_tokens: 4, cache_read_tokens: "drift", output_tokens: 2 } } });
out({ event: "result", result: { status: "SUCCESS", response: { futureContent: true }, usage: { input_tokens: 7, output_tokens: 3 } } });
`,
    );
    chmodSync(cli, 0o755);
    instance = await AntigravityDriver.create({
      instanceId: "agy-drift",
    ...
    recorder = recordEvents(instance.adapter);
    try {
      await instance.adapter.sendTurn({ threadId: "t-drift", text: "continue through protocol drift" });
      await recorder.until((event) => event.type === "turn.completed");

      const session = recorder.events.find((event) => event.type === "session.started");
      const usageEvents = recorder.events.filter((event) => event.type === "thread.token-usage.updated");
      const completed = recorder.events.find((event) => event.type === "turn.completed");
      expect(session?.sessionId).toBeNull();
      expect(usageEvents).toEqual([
        expect.objectContaining({ input: 4, output: 2 }),
        expect.objectContaining({ input: 7, output: 3 }),
      ]);
      expect(completed).toMatchObject({ ok: true, stopReason: "SUCCESS", usage: { input: 7, output: 3 } });
...
describe("Antigravity snapshot", () => {
  ...
  it("strips workspace credentials from snapshot and helper children", async () => {
    const scratch = mkdtempSync(join(tmpdir(), "hry-agy-env-"));
...
describe("Antigravity computer MCP config", () => {
  ...
  it("builds the cloud-box spec on the shared computer proxy (never path-resolved locally)", () => {
    expect(antigravityComputerMcpServer(boxIntegrations)).toEqual({
      ...
      env: {
        ...
        HELMRYTH_BOX_ID: "bx_1",
        HELMRYTH_BOX_TOKEN: "box-tok",
        HELMRYTH_CONTROL_URL: "http://127.0.0.1:9/control",
        HELMRYTH_CONTROL_TOKEN: "ctl-tok",
      },
    });
  });
  ...
  it("upserts only its own key — the user's servers and unknown top-level keys survive", () => {
    const home = mkdtempSync(join(tmpdir(), "hry-agy-mcpcfg-"));
    try {
  ...
  it("starts fresh from malformed JSON instead of failing the turn", () => {
    const home = mkdtempSync(join(tmpdir(), "hry-agy-mcpbad-"));
    try {
      mkdirSync(join(home, ".gemini", "config"), { recursive: true });
  ...
  it("restricts the token-bearing config directory and file to the current user", () => {
    ...
    const home = mkdtempSync(join(tmpdir(), "hry-agy-mcpperms-"));
    try {
  ...
  it("preserves concurrent config edits while restoring only its own MCP entry", () => {
    const home = mkdtempSync(join(tmpdir(), "hry-agy-mcpconcurrent-"));
    try {
  ...
  it("a computer-less turn removes only its own key, and never creates the file just to remove", () => {
    const home = mkdtempSync(join(tmpdir(), "hry-agy-mcprm-"));
    try {
  ...
  it("uses the spawned CLI's HOME and restores the prior config when the turn exits", async () => {
    ensureDirs();
    chmodSync(FAKE_CLI, 0o755);
    const home = mkdtempSync(join(tmpdir(), "hry-agy-mcpturn-"));
  ...
  it("serializes overlapping turns so each child sees only its own computer mount", async () => {
    ensureDirs();
    chmodSync(FAKE_CLI, 0o755);
    const home = mkdtempSync(join(tmpdir(), "hry-agy-mcplease-"));
  ...
  it("reaps a child that hangs after result, restores the mount, and unblocks the next turn", async () => {
    ensureDirs();
    chmodSync(FAKE_CLI, 0o755);
    const home = mkdtempSync(join(tmpdir(), "hry-agy-mcpreaper-"));
    const firstDump = join(home, "first.json");
    const secondDump = join(home, "second.json");
    const first = await AntigravityDriver.create({
  ...
  it("force-reaps an interrupted child that ignores SIGTERM before result", async () => {
    ensureDirs();
    chmodSync(FAKE_CLI, 0o755);
    const home = mkdtempSync(join(tmpdir(), "hry-agy-mcpinterrupt-"));
