...
const keySchema = z.enum(["back", "delete", "down", "enter", "escape", "home", "left", "recent", "return", "right", "space", "tab", "up"]);
const KEYCODES = {
  back: "KEYCODE_BACK",
  delete: "KEYCODE_DEL",
...
const toolArgsSchema = z.object({
  ...
  index: z.json().optional(),
  x: z.json().optional(),
  y: z.json().optional(),
  direction: z.json().optional(),
  key: z.json().optional(),
});
...
const rpcMessageSchema = z.object({
  ...
  params: z.object({
    ...
    arguments: toolArgsSchema.optional(),
  }).optional(),
});
...
export function resolveAdbPath(env: NodeJS.ProcessEnv = process.env, platform = process.platform): string | null {
  ...
  const home = homedir();
  const candidates = [
    env.HELMRYTH_ADB_PATH,
    env.HELMRYTH_RESOURCES_PATH && join(env.HELMRYTH_RESOURCES_PATH, "android-platform-tools", platform, executable),
...
async function runAdb(args: string[], options: { binary?: boolean; timeoutMs?: number } = {}): Promise<Buffer> {
  ...
  if (!adb) throw new Error("Android platform tools are unavailable. Reopen Helmryth or install adb.");
...
async function readySerial(requested?: string): Promise<string> {
  const devices = await usbDevices();
  const serial = requested || process.env.PH_ANDROID_SERIAL;
  ...
  if (device.state === "unauthorized") throw new Error("Wake and authenticate the phone, then accept Allow USB debugging");
...
export function findPackages(name: string, packages: string[]) {
  ...
  const known = KNOWN_PACKAGES.get(needle);
...
async function readNodes(serial: string) {
  ...
  const remotePath = "/data/local/tmp/helmryth-window.xml";
...
async function callTool(name: string, args: ToolArgs): Promise<ToolResult> {
  ...
  if (name === "swipe") {
    ...
    const points = new Map<string, [number, number, number, number]>([
    ...
    const point = points.get(direction);
  ...
  if (name === "press") {
    const key = keySchema.safeParse(String(args.key ?? "").toLowerCase());
    if (!key.success) throw new Error("Unsupported Android key");
    await onDevice(serial, ["shell", "input", "keyevent", KEYCODES[key.data]]);
    return textResult(`Pressed ${key.data}`);
  }
...
async function handle(message: z.infer<typeof rpcMessageSchema>) {
  const id = message.id;
  const method = message.method;
  const params = message.params ?? {};
  if (method === "initialize") return ok(id, { protocolVersion: params.protocolVersion ?? "2024-11-05", capabilities: { tools: {} }, serverInfo: { name: "helmryth-phone", version: "1" } });
...
if (process.argv[1] && existsSync(process.argv[1]) && /phone-proxy\.(?:ts|js)$/.test(process.argv[1])) {
  ...
  lines.on("line", (line) => {
    try {
      const message = rpcMessageSchema.parse(parseJson(line));
