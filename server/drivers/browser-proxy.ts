...
//   HELMRYTH_BROWSER_URL    loopback host, e.g. http://127.0.0.1:52144
//   HELMRYTH_BROWSER_TOKEN  per-boot bearer secret from browser-connection.json
//   HELMRYTH_BOT_ID         which bot's tab to drive (one view per bot)
//   HELMRYTH_BROWSER_PROFILE named shared session the bot is pointed at ("" = own)
//   HELMRYTH_CONTROL_URL / HELMRYTH_CONTROL_TOKEN  who-is-driving endpoint: while the
...
const HOST = (process.env.HELMRYTH_BROWSER_URL ?? "").replace(/\/$/, "");
const TOKEN = process.env.HELMRYTH_BROWSER_TOKEN ?? "";
const BOT_ID = process.env.HELMRYTH_BOT_ID ?? "";
const PROFILE = process.env.HELMRYTH_BROWSER_PROFILE ?? "";
...
const rpcMessageSchema = z.object({
  ...
  params: z.object({ name: z.string().optional(), arguments: z.json().optional(), protocolVersion: z.string().optional() }).optional(),
});
...
function wallNote(kind: WallKind): string {
  return kind === "verification"
    ? "This looks like an automated-traffic check or verification page. Do not try to solve it: call browser_request_takeover so the user can complete it in the Browser panel, then continue from the page you get back."
...
export type HostRequest = (operation: string, body?: HostBody) => Promise<JsonValue>;
...
export async function hostRequest(operation: string, body: HostBody = {}, fetchImpl: typeof fetch = fetch): Promise<JsonValue> {
  if (!HOST || !TOKEN || !BOT_ID) throw new Error("The built-in browser is not connected for this operator");
  ...
  const parsed = z.json().parse(await res.json().catch(() => ({})));
  if (!res.ok) {
...
export const TOOLS = [
  {
    ...
    description:
      "Open a web address in this operator's built-in browser tab so the user can watch it in the Browser panel. Returns the page's interactive elements with refs; do not follow it with browser_snapshot.",
...
async function observed(request: HostRequest, operation: string, body?: HostBody): Promise<ToolResult> {
  return textResult(formatObserved(pageSchema.parse(await request(operation, body))));
}

export async function callTool(name: string, args: JsonValue, request: HostRequest = hostRequest): Promise<ToolResult> {
...
interface WaitRequestBody extends HostBody {
  text?: string;
  url?: string;
  timeoutMs?: number;
}
...
const rpcError = (id: JsonRpcId | undefined, code: number, message: string) => send({ jsonrpc: "2.0", id, error: { code, message } });
...
async function handle(line: string) {
  ...
  if (method === "initialize") {
    return ok(id, {
      ...
      capabilities: { tools: {} },
      serverInfo: { name: "helmryth-browser", version: "1" },
    });
  }
