...
// HELMRYTH_DATA_DIR at import time — so the env var must be set before the import
// is evaluated (same pattern as attachments.test.ts).
const DATA_ROOT = mkdtempSync(join(tmpdir(), "hry-checkpoints-"));
process.env.HELMRYTH_DATA_DIR = join(DATA_ROOT, "data");
// The shared Vitest setup may already have loaded config.ts for message-db;
// use its effective path so this fixture always precedes checkpoints.ts.
const { DATA_DIR: CHECKPOINT_DATA_DIR } = await import("./config.ts");
process.env.HELMRYTH_DATA_DIR = CHECKPOINT_DATA_DIR;
const LEGACY_SHADOW = join(CHECKPOINT_DATA_DIR, "checkpoints", "legacy-bot", "legacy-shadow");
const LEGACY_INFO = join(LEGACY_SHADOW, ".git", "info");
const LEGACY_OBJECTS = join(LEGACY_SHADOW, ".git", "objects", "ab");
mkdirSync(LEGACY_INFO, { recursive: true, mode: 0o755 });
mkdirSync(LEGACY_OBJECTS, { recursive: true, mode: 0o755 });
writeFileSync(join(LEGACY_SHADOW, "gitconfig"), "legacy config", { mode: 0o644 });
writeFileSync(join(LEGACY_SHADOW, "gitconfig_empty"), "", { mode: 0o644 });
writeFileSync(join(LEGACY_INFO, "exclude"), "legacy exclude", { mode: 0o644 });
writeFileSync(join(LEGACY_OBJECTS, "object"), "legacy object", { mode: 0o644 });
chmodSync(join(CHECKPOINT_DATA_DIR, "checkpoints"), 0o755);
chmodSync(join(CHECKPOINT_DATA_DIR, "checkpoints", "legacy-bot"), 0o755);
chmodSync(LEGACY_SHADOW, 0o755);
chmodSync(join(LEGACY_SHADOW, ".git"), 0o755);
chmodSync(LEGACY_INFO, 0o755);
chmodSync(join(LEGACY_SHADOW, ".git", "objects"), 0o755);
chmodSync(LEGACY_OBJECTS, 0o755);

const {
  CHECKPOINTS_DIR,
  acquireCheckpointWorkspaceLease,
  checkpointsEnabled,
  listCheckpoints,
  refusalReason,
  restore,
  snapshot,
} = await import("./checkpoints.ts");
...
let seq = 0;
function workspace() {
  const cwd = mkdtempSync(join(tmpdir(), "hry-ckpt-ws-"));
...
function chmodCheckpointTree(path: string, directoryMode: number, fileMode: number): void {
  const stat = lstatSync(path);
  if (stat.isSymbolicLink()) return;
  chmodSync(path, stat.isDirectory() ? directoryMode : fileMode);
  if (!stat.isDirectory()) return;
  for (const entry of readdirSync(path)) chmodCheckpointTree(join(path, entry), directoryMode, fileMode);
}

function expectPrivateCheckpointTree(path: string): void {
  const stat = lstatSync(path);
  if (stat.isSymbolicLink()) return;
  expect(stat.mode & 0o777).toBe(stat.isDirectory() ? 0o700 : 0o600);
  if (!stat.isDirectory()) return;
  for (const entry of readdirSync(path)) expectPrivateCheckpointTree(join(path, entry));
}

describe("snapshot", () => {
  it.skipIf(process.platform === "win32")("creates and repairs legacy checkpoint storage with owner-only modes", async () => {
    expectPrivateCheckpointTree(LEGACY_SHADOW);

    const { bot, cwd } = workspace();
    writeFileSync(join(cwd, "a.txt"), "one");
    const checkpoint = await snapshot(bot, cwd, "turn 1");
    if (checkpoint === null) throw new Error("expected the initial checkpoint to succeed");

    chmodSync(CHECKPOINTS_DIR, 0o755);
    chmodCheckpointTree(join(CHECKPOINTS_DIR, bot), 0o755, 0o644);
    expect(lstatSync(CHECKPOINTS_DIR).mode & 0o777).toBe(0o755);

    const unchanged = await snapshot(bot, cwd, "turn 2");
    expect(unchanged).toBe(checkpoint);
    expectPrivateCheckpointTree(CHECKPOINTS_DIR);
    expect((await listCheckpoints(bot, cwd)).map((item) => item.hash)).toEqual([checkpoint]);
  });
...
describe("restore", () => {
  it("blocks same-operator and cross-operator restores until every canonical-workspace owner releases", async () => {
    const { bot, cwd } = workspace();
    const otherBot = `${bot}-other`;
    writeFileSync(join(cwd, "a.txt"), "one");
    const checkpoint = await snapshot(bot, cwd, "turn 1");
    if (checkpoint === null) throw new Error("expected the initial checkpoint to succeed");
    writeFileSync(join(cwd, "a.txt"), "two");

    const aliasRoot = mkdtempSync(join(tmpdir(), "hry-ckpt-alias-"));
    scratchDirs.push(aliasRoot);
    const alias = join(aliasRoot, "workspace");
    symlinkSync(cwd, alias, process.platform === "win32" ? "junction" : "dir");

    const owner = acquireCheckpointWorkspaceLease(bot, cwd);
    expect(owner.ok).toBe(true);
    if (!owner.ok) throw new Error("expected the active workspace lease to be acquired");

    expect(await restore(bot, alias, checkpoint)).toEqual({
      ok: false,
      error: "restore blocked because this project folder is in use by an active operator",
    });
    expect(readFileSync(join(cwd, "a.txt"), "utf8")).toBe("two");

    const secondOwner = acquireCheckpointWorkspaceLease(otherBot, alias);
    expect(secondOwner.ok).toBe(true);
    if (!secondOwner.ok) throw new Error("expected the second active workspace lease to be acquired");
    owner.lease.release();
    expect(await restore(bot, cwd, checkpoint)).toEqual({
      ok: false,
      error: "restore blocked because this project folder is in use by an active operator",
    });

    secondOwner.lease.release();
    expect(await restore(bot, cwd, checkpoint)).toEqual({ ok: true });
    expect(readFileSync(join(cwd, "a.txt"), "utf8")).toBe("one");

    const safety = (await listCheckpoints(bot, cwd)).find((item) => item.label === "before restore");
    expect(safety).toBeDefined();
    if (!safety) throw new Error("expected restore to create an undo checkpoint");
    expect(await restore(bot, cwd, safety.hash)).toEqual({ ok: true });
    expect(readFileSync(join(cwd, "a.txt"), "utf8")).toBe("two");
  });
...
describe("refusals", () => {
  it("refuses the home folder, protected folders, and missing paths", async () => {
    ...
    expect(refusalReason(join(tmpdir(), "hry-ckpt-definitely-missing-xyz"))).not.toBeNull();
  ...
  it("lists nothing (and creates nothing) for a folder never snapshotted", async () => {
    ...
    const shadow = join(process.env.HELMRYTH_DATA_DIR!, "checkpoints", bot);
