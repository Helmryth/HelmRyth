...
export function parseSkillMd(raw: string): ParsedSkill | { error: string } {
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    // `description: >` / `description: |` (with optional chomping indicator).
    // Without this the indicator itself was captured as the whole value and the
    // indented body below it was skipped, so the description became ">".
    const blockScalar = inline.match(/^([|>])([+-]?)$/);
    if (!blockScalar) {
      fields[key] = inline.replace(/^["']|["']$/g, "").trim();
      continue;
    }
    const body: string[] = [];
    let end = i + 1;
    for (; end < lines.length; end += 1) {
      ...
      if (!/^[ \t]/.test(line)) break; // a dedent to column 0 closes the block
      body.push(line);
    }
    ...
    const stripped = body.map((line) => (line.trim() === "" ? "" : line.slice(indent)));
    fields[key] = blockScalar[1] === "|"
      // literal: newlines are content
      ? stripped.join("\n").trim()
      // folded: a line break is a space, a blank line is a paragraph break
      : stripped.reduce((out, line) => (
        line === "" ? `${out}\n` : out === "" || out.endsWith("\n") ? out + line : `${out} ${line}`
      ), "").trim();
    i = end - 1;
  }
...
export interface SkillListing {
  ...
  warnings: string[];
  skippedFiles: string[];
  /** Hash of provenance, scan results, inventory, and current Markdown. */
  reviewRevision: string;
  reviewedRevision?: string;
  reviewedAt?: string;
}

export interface SkillReviewProof {
  revision: string;
  acknowledged: true;
}

interface CurrentSkillRevision {
  revision: string;
  reviewable: boolean;
}

function currentSkillRevision(botId: string, name: string, entry: SkillManifestEntry): CurrentSkillRevision {
  ...
  try {
    const directory = join(skillsDir(botId), name);
    const files = readdirSync(directory, { withFileTypes: true })
      .filter((file) => file.isFile() && file.name.toLowerCase().endsWith(".md"))
      .map((file) => file.name)
      .sort();
    const hasPrimaryMethod = files.includes("SKILL.md");
    ...
    reviewable = hasPrimaryMethod;
  } catch {
    hash.update("unreadable-method-content");
  }
  return { revision: hash.digest("hex"), reviewable };
}

function listingFor(botId: string, name: string, entry: SkillManifestEntry): SkillListing {
  return { name, ...entry, reviewRevision: currentSkillRevision(botId, name, entry).revision };
}

/** One index line — the exact text the prompt carries, so the budget a method
 * is measured against is the budget it actually spends. */
function indexLine(skill: { name: string; description: string }): string {
  return `- ${skill.name}: ${skill.description}`;
}

/** Split enabled methods, in prompt order, into the ones the index has room
 * for and the ones it does not. Everything in `dropped` cannot reach the
 * operator at all, so nothing may leave it looking enabled. */
function indexBudget<T extends { name: string; description: string }>(enabled: T[]) {
  const fits: T[] = [];
  const dropped: T[] = [];
  let bytes = 0;
  for (const skill of enabled) {
    bytes += Buffer.byteLength(indexLine(skill), "utf8");
    if (fits.length >= INDEX_MAX_SKILLS || bytes > INDEX_MAX_BYTES) dropped.push(skill);
    else fits.push(skill);
  }
  return { fits, dropped } satisfies { fits: T[]; dropped: T[] };
}

export function listSkills(botId: string): SkillListing[] {
  const manifest = readManifest(botId);
  let invalidated = false;
  const listings = Object.entries(manifest).map(([name, entry]) => {
    const current = currentSkillRevision(botId, name, entry);
    if (entry.enabled && entry.reviewedRevision !== current.revision) {
      entry.enabled = false;
      delete entry.reviewedRevision;
      delete entry.reviewedAt;
      invalidated = true;
    }
    return { name, ...entry, reviewRevision: current.revision };
  });
  listings.sort((a, b) => a.name.localeCompare(b.name));
  // A manifest written before the index budget was enforced can carry more
  // enabled methods than the prompt has room for. Those were dropped silently
  // while the panel still drew them as active — an operator reading the list
  // could not tell which of their methods the bot had actually been told
  // about. Turning them off is the only honest listing.
  for (const skill of indexBudget(listings.filter((listing) => listing.enabled)).dropped) {
    const entry = manifest[skill.name]!;
    entry.enabled = false;
    delete entry.reviewedRevision;
    delete entry.reviewedAt;
    skill.enabled = false;
    skill.reviewedRevision = undefined;
    skill.reviewedAt = undefined;
    invalidated = true;
  }
  if (invalidated) {
...
export function readSkillFile(botId: string, name: string): string | null {
  ...
  try {
    ...
    if (files[0] !== "SKILL.md") return null;
...
export function installSkill(
  ...
  writeManifest(botId, manifest);
  return listingFor(botId, parsed.name, entry);
}
...
export function setSkillEnabled(
  ...
  name: string,
  enabled: boolean,
  review?: SkillReviewProof,
): SkillListing | { error: string; status: 404 | 409 } {
  if (!isSkillName(name)) return { error: "invalid skill name", status: 404 };
  const manifest = readManifest(botId);
  const entry = manifest[name];
  if (!entry) return { error: `no imported skill named "${name}"`, status: 404 };
  const current = currentSkillRevision(botId, name, entry);
  if (enabled && (!current.reviewable || review?.acknowledged !== true || review.revision !== current.revision)) {
    entry.enabled = false;
    delete entry.reviewedRevision;
    delete entry.reviewedAt;
    writeManifest(botId, manifest);
    syncSkillLinks(botId);
    return {
      error: current.reviewable
        ? "Read and acknowledge the current complete method before enabling it"
        : "The current complete method could not be read for review",
      status: 409,
    };
  }
  // The prompt index is a hard budget: past it a method never reaches the
  // operator. Enabling anyway used to succeed with a 200 and an "on" toggle
  // for a method the bot was never told about, so refuse instead and name the
  // limit — the operator picks which method to give up.
  if (enabled) {
    const next = Object.entries(manifest)
      .filter(([key, candidate]) => key === name || candidate.enabled)
      .map(([key, candidate]) => ({ name: key, description: candidate.description }))
      .sort((a, b) => a.name.localeCompare(b.name));
    if (indexBudget(next).dropped.length) {
      return {
        error: next.length > INDEX_MAX_SKILLS
          ? `Only ${INDEX_MAX_SKILLS} methods can reach the operator at once — turn one off first`
          : `Enabled method descriptions are limited to ${INDEX_MAX_BYTES} characters — turn one off first`,
        status: 409,
      };
    }
  }
  ...
  if (enabled) {
    entry.reviewedRevision = current.revision;
    entry.reviewedAt = new Date().toISOString();
  }
  writeManifest(botId, manifest);
  syncSkillLinks(botId);
  return listingFor(botId, name, entry);
}
...
export function skillsSystemPrompt(botId: string): string {
  // listSkills has already disabled anything past the budget, so `fits` is the
  // whole enabled set; running the same split here keeps the prompt's hard
  // budget true even for a manifest nothing has listed yet this process.
  const enabled = listSkills(botId).filter((skill) => skill.enabled);
  if (!enabled.length) return "";
  const dir = skillsDir(botId);
  const lines = indexBudget(enabled).fits.map(indexLine);
