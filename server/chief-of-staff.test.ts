...
describe("leadOperatorSystemPrompt roster caps", () => {
  it("clips oversized persona fields instead of interpolating them whole", () => {
    const prompt = leadOperatorSystemPrompt(
      "chief",
      [
        { id: "chief", name: "Atlas" },
        {
  ...
  it("caps the roster length and says how many were left out", () => {
    ...
    const prompt = leadOperatorSystemPrompt("chief", [{ id: "chief", name: "Atlas" }, ...team], true);
...
describe("leadOperatorSystemPrompt", () => {
  const bots = [
  ...
  it("describes visible teammates, roles, and availability", () => {
    const prompt = leadOperatorSystemPrompt("chief", bots, true);

    expect(prompt).toContain("lead operator for the Work section");
    ...
    expect(prompt).toContain("Use ask_operator");
    expect(prompt).toContain("use create_operator");
  });

  it("does not promise delegation when the engine cannot mount agent tools", () => {
    const prompt = leadOperatorSystemPrompt("chief", bots, false);

    expect(prompt).toContain("cannot contact teammates");
    expect(prompt).not.toContain("Use ask_operator");
  });

  it("includes trusted Helmryth status only when the Chief caller supplies it", () => {
    const status = "TRUSTED HELMRYTH STATUS\nfreshness=fresh; runtime_state=degraded";

    const chiefPrompt = leadOperatorSystemPrompt("chief", bots, true, status);
    const ordinaryPrompt = leadOperatorSystemPrompt("writer", bots, true);

    expect(chiefPrompt).toContain(status);
    expect(ordinaryPrompt).not.toContain("TRUSTED HELMRYTH STATUS");
  });
});
