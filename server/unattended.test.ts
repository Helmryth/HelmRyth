...
async function deliverWebhook(endpointUrl: string, secret: string, payload: JsonValue) {
...
async function waitForRunThread(runId: string, ms = 20_000) {
  ...
  while (Date.now() < deadline) {
    ...
    const run = (body.runs ?? []).find((candidate: { id: string; threadId?: string }) => candidate.id === runId);
...
posixOnly("unattended turns keep asking", () => {
  beforeAll(async () => {
    ...
    home = mkdtempSync(join(tmpdir(), "hry-unattended-"));
  ...
  it(
    ...
    async () => {
      ...
      const delivered = await deliverWebhook(
        hook.body.credential.endpointUrl,
        hook.body.credential.secret,
        { status: "failed" },
      );
      expect(delivered.status).toBe(202);
      const { runId } = z.object({ runId: z.string() }).parse(await delivered.json());
