...
// Helmryth server and waits for the human's answer.
//
...
const dead = () => {
  for (const resolve of waiting.values()) {
    resolve({ behavior: "deny", message: "Helmryth: permission broker unavailable — skip this action" });
  }
  waiting.clear();
};
conn.on("error", dead);
...
const send = <Message>(value: Message) => process.stdout.write(JSON.stringify(value) + "\n");

const TOOLS = [
  {
    name: "approve",
    description: "Ask the Helmryth user whether a tool use is allowed",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    name: "ask_user",
    description:
      "Ask the person who owns this operator a question and wait for their answer. Use it for a decision, preference, missing information, or sign-off before consequential work. Do not guess on choices the owner should make. Returns their answer as text.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  if (msg.method === "initialize") {
    return send({
      ...
      result: {
        ...
        capabilities: { tools: {} },
        serverInfo: { name: "helmryth-permissions", version: "1" },
      },
    });
  }
  ...
  if (msg.method === "tools/call") {
    ...
    if (!isQuestion) {
      if (answer.behavior === "allow") {
        ...
        text = JSON.stringify({ behavior: "deny", message: answer.message || "Denied from Helmryth" });
      }
    }
