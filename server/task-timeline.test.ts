...
describe("execution timeline", () => {
  it("derives only persisted, observable events and preserves failures", () => {
    ...
    expect(events).toMatchObject([
      ...
      { label: "Workbench observed", kind: "screen", state: "observed" },
  ...
  it("does not call every later user direction a new run", () => {
    const events = timelineEvents([
    ...
    expect(events.map((event) => event.label)).toEqual(["Run started", "Response recorded", "Direction added"]);
  });
});
