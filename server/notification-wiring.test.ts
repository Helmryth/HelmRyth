...
posixOnly("routine failure notification wiring", () => {
  beforeAll(async () => {
    ...
    home = mkdtempSync(join(tmpdir(), "hry-notifications-e2e-"));
  ...
  it(
    ...
    async () => {
      ...
      try {
        ...
        if (!notification) throw new Error("Expected the routine failure notification frame");
        expect(notification).toMatchObject({
          kind: "routine-failed",
          botId: bot.id,
          title: "Routine Scout's cadence failed",
        });
        expect(notification.threadId).not.toBe(bot.threadId);
        expect(notification.body).toContain("Broken nightly report");
        ...
        expect(
          stream.frames.flatMap((candidate) => {
            const candidateNotification = candidate.notification;
            if (candidate.kind !== "notify" || !candidateNotification) return [];
            return candidateNotification.threadId === notification.threadId ? [candidateNotification.kind] : [];
          }),
