...
posixOnly("mid-turn steering e2e", () => {
  ...
  beforeAll(async () => {
    ...
    home = mkdtempSync(join(tmpdir(), "hry-steer-"));
    ...
    const childEnv: NodeJS.ProcessEnv = { HOME: home, USERPROFILE: home, HELMRYTH_PORT: String(PORT) };
  ...
  it("rejects a delayed steer acknowledgement after the bot is deleted", async () => {
    ...
    expect(rejected.body.error).toMatch(/no such operator/i);
  }, 40_000);
