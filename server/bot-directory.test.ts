...
const entry = (over: JsonObject = {}): JsonObject => ({
  slug: "release-scribe",
  name: "Release Scribe",
  category: "Docs",
  integrations: ["GitHub"],
...
describe("fetchBotDirectory", () => {
  it("fetches, validates, and passes errors through", async () => {
    const ok = vi.fn<typeof fetch>(async () => new Response(JSON.stringify({ version: 1, bots: [entry()] })));
    await expect(fetchBotDirectory(ok)).resolves.toHaveLength(1);
    expect(ok).toHaveBeenCalledWith(BOT_DIRECTORY_API_URL, expect.objectContaining({ redirect: "error" }));

    const down = vi.fn<typeof fetch>(async () => new Response("nope", { status: 503 }));
    await expect(fetchBotDirectory(down)).rejects.toThrow("HTTP 503");
  });
  ...
  it("rejects an oversized response while reading, not after buffering it whole", async () => {
    ...
    const big = vi.fn<typeof fetch>(async () => new Response(endless));
    await expect(fetchBotDirectory(big)).rejects.toThrow("too large");
    expect(cancelled).toBe(true);
    // barely past the 1 MB cap — nowhere near what an unbounded read would take
    expect(sent).toBeLessThan(2_000_000);

    const announced = vi.fn<typeof fetch>(async () =>
      new Response("{}", { headers: { "content-length": String(50_000_000) } }),
    );
    await expect(fetchBotDirectory(announced)).rejects.toThrow("too large");
  });
});
...
describe("matchDirectoryBots", () => {
  ...
  const bot = (slug: string, name: string, category: string, integrations: string[]): DirectoryBot => ({
    slug,
    name,
    category,
    integrations,
    prompt: "Set up a specialist operator.",
    detailUrl: `https://botdirectory.ai/bots/${slug}/`,
  });
  const bots: DirectoryBot[] = [
    bot("react-reviewer", "React Reviewer", "Engineering", ["GitHub"]),
    bot("payments-auditor", "Payments Auditor", "Finance", ["Stripe"]),
    bot("gig-closer", "Gig Closer", "Ops", ["QuickBooks"]),
  ];
  ...
  it("ignores stack names too short to mean anything as substrings", () => {
    ...
    const google = bot("google-helper", "Google Helper", "Ops", []);
