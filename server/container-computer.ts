...
//
// Helmryth owns only the sandbox boundary: image preparation, container
...
export const IMAGE_REPOSITORY = "localhost/helmryth/cua-local-vm";
export const IMAGE_LAYER_VERSION = "4";
export const IMAGE_LAYER_LABEL = "com.helmryth.image-layer";
export const IMAGE = `${IMAGE_REPOSITORY}:driver-${CUA_DRIVER_VERSION}-v${IMAGE_LAYER_VERSION}`;
export const CONTAINER = "helmryth-computer";
export const MANAGED_LABEL = "com.helmryth.local-vm";
export const DRIVER_LABEL = "com.helmryth.cua-driver";
export const BASE_IMAGE_LABEL = "com.helmryth.cua-base";
export const WORKSPACE_LABEL = "com.helmryth.workspace";
export const TARGET_LABEL = "com.helmryth.local-vm-target";
...
export const CUA_SOCKET = "/run/user/1000/helmryth-cua.sock";
export const CUA_EXECUTABLE = "/usr/local/libexec/helmryth/cua-driver";
...
RUN printf '%s\\n' \\
      ...
      > /usr/local/bin/prepare-helmryth-workspace.sh \\
    && chmod 0755 /usr/local/bin/prepare-helmryth-workspace.sh
RUN printf '%s\\n' \\
      '#!/bin/sh' \\
      '/usr/local/bin/prepare-helmryth-workspace.sh' \\
      'attempt=0' \\
      ...
      > /usr/local/bin/start-helmryth-cua-driver.sh \\
    && chmod 0755 /usr/local/bin/start-helmryth-cua-driver.sh
RUN printf '%s\\n' \\
      '' \\
      '[program:helmryth-cua-driver]' \\
      'command=/usr/local/bin/start-helmryth-cua-driver.sh' \\
      'user=cua' \\
...
function statusProblem(status: ContainerComputerStatus): string | null {
  ...
  if (status.container === "missing" && !status.create_supported) {
    return "Per-operator Local VMs require Docker or Podman because Apple container requires a fixed host port";
  }
  ...
  if (!status.managed) return "The existing container was not created by Helmryth; recreate it";
...
const publishedPortsSchema = z.record(z.string(), z.array(publishedPortSchema).nullable());
const dockerHardeningSchema = z.object({
  Memory: z.number().optional(),
  MemorySwap: z.number().optional(),
  NanoCpus: z.number().optional(),
  PidsLimit: z.number().nullable().optional(),
  ...
  Devices: z.array(z.json()).nullable().optional(),
  DeviceRequests: z.array(z.json()).nullable().optional(),
  SecurityOpt: z.array(z.string()).nullable().optional(),
  UsernsMode: z.string().optional(),
  CgroupnsMode: z.string().nullable().optional().transform((value) => value ?? undefined),
  OomKillDisable: z.boolean().nullable().optional(),
  AutoRemove: z.boolean().optional(),
  RestartPolicy: z.object({ Name: z.string().optional(), MaximumRetryCount: z.number().optional() }).passthrough().optional(),
}).passthrough();

const imageInspectSchema = z.array(z.object({
  Id: z.string().optional(),
  id: z.string().optional(),
  Config: z.object({ Labels: stringMapSchema.optional() }).passthrough().optional(),
  config: z.object({ Labels: stringMapSchema.optional(), labels: stringMapSchema.optional() }).passthrough().optional(),
  configuration: z.object({
    labels: stringMapSchema.optional(),
    descriptor: z.object({ digest: z.string().optional() }).passthrough().optional(),
  }).passthrough().optional(),
}).passthrough());

const appleImageReferenceSchema = z.union([
  z.string(),
  z.object({
...
const appleInspectSchema = z.array(z.object({
  configuration: z.object({
    image: appleImageReferenceSchema.optional(),
    imageReference: z.string().optional(),
    resources: z.object({ cpus: z.number().optional(), memoryInBytes: z.number().optional() }).passthrough().optional(),
    publishedPorts: z.array(z.object({
      hostAddress: z.string().optional(),
      hostPort: z.number().optional(),
      containerPort: z.number().optional(),
    }).passthrough()).optional(),
    environment: z.union([z.array(z.string()), stringMapSchema]).optional(),
    labels: stringMapSchema.optional(),
  ...
  status: z.object({ state: z.string().optional() }).passthrough().optional(),
}).passthrough());
...
const dockerInspectSchema = z.array(z.object({
  ...
  HostConfig: dockerHardeningSchema.extend({ PortBindings: publishedPortsSchema.optional() }).passthrough().optional(),
  NetworkSettings: z.object({ Ports: publishedPortsSchema.optional() }).passthrough().optional(),
  Mounts: z.array(z.object({
  ...
  EffectiveCaps: z.array(z.string()).optional(),
  BoundingCaps: z.array(z.string()).optional(),
  State: z.object({ Running: z.boolean().optional() }).passthrough().optional(),
  Image: z.string().optional(),
}).passthrough());
...
function inspectedImage(stdout: string): InspectedImage {
  const parsed = imageInspectSchema.parse(parseJson(stdout));
...
export async function containerComputerStatus(
  ...
  try {
    ...
    // The prepared Helmryth derivative has not been built yet.
  }

  try {
    const { stdout } = await runner(status.runtime, ["inspect", target.containerName]);
    if (status.runtime === "container") {
      const inspected = appleInspectSchema.parse(parseJson(stdout));
      ...
      const imageString = z.string().safeParse(detail?.configuration?.image);
      const imageObject = appleImageReferenceSchema.options[1].safeParse(detail?.configuration?.image);
      const appleImage = imageString.success
        ? imageString.data
        : imageObject.success
          ? imageObject.data.reference ?? detail?.configuration?.imageReference
          : detail?.configuration?.imageReference;
      const appleImageId = imageObject.success
        ? normalizeImageId(imageObject.data.descriptor?.digest)
        : null;
      status.imageMatches =
      ...
      const inspected = dockerInspectSchema.parse(parseJson(stdout));
      const detail = inspected[0];
  ...
  if (canProbe) {
    try {
      ...
      const report = healthReportSchema.parse(parseJson(health.stdout));
      if (
      ...
      const readinessShot = "/tmp/helmryth-readiness.png";
      await runner(
        status.runtime,
        cuaExecArgs([
          "call",
...
export interface DockerHardeningConfig {
  ...
  UTSMode?: string;
  ShmSize?: number;
  Devices?: JsonValue[] | null;
  DeviceRequests?: JsonValue[] | null;
...
export function containerRunArgs(
  ...
  if (runtime === "container" && target.key !== SHARED_LOCAL_VM_TARGET.key) {
    throw new Error("Per-operator Local VMs require Docker or Podman because Apple container requires a fixed host port");
  }
...
async function prepareManagedImage(runtime: Runtime, runner: CommandRunner): Promise<void> {
  ...
  const context = await mkdtemp(join(tmpdir(), "helmryth-cua-image-"));
  try {
...
export async function containerComputerAction(
  ...
  // A fixed container name is not proof of ownership. Lifecycle mutations
  // must never stop or delete a foreign container that happens to use it.
  // Pull is image-only and remains safe; a Helmryth-owned stale/unsafe VM is
  // still removable so the user can repair it.
  if (action !== "pull" && before.container !== "missing" && !before.managed) {
    throw Object.assign(new Error("The existing container was not created by Helmryth; manage it outside Helmryth"), {
      status: 409,
    });
  }
  ...
  if (action === "run" && !before.create_supported) {
    throw Object.assign(new Error(before.problem ?? "This runtime cannot create a per-operator Local VM"), { status: 409 });
  }
  if (action === "start") {
...
export async function containerComputerScreenshot(
  ...
  try {
    const screenshot = "/tmp/helmryth-preview.png";
    await runner(
      status.runtime,
      cuaExecArgs([
        "call",
...
export function containerComputerMcp(
  ...
  const launch: ContainerMcpLaunch = {
  ...
  if (control) {
    launch.env.HELMRYTH_CONTROL_URL = control.url;
    launch.env.HELMRYTH_CONTROL_TOKEN = control.token;
  }
  return launch;
}
...
/** Cloud boxes still use Helmryth's high-latency REST adapter. Local VMs
...
export function computerProxyEnv(
  ...
  const environment: NodeJS.ProcessEnv = {
    HELMRYTH_BOX_ID: computer.boxId ?? "",
    HELMRYTH_BOX_TOKEN: computer.token ?? "",
  };
  if (computer.control) {
    environment.HELMRYTH_CONTROL_URL = computer.control.url;
    environment.HELMRYTH_CONTROL_TOKEN = computer.control.token;
  }
  return environment;
}
