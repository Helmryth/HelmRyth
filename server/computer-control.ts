...
export interface HelpRequestLeaseResult {
  snapshot: ControlSnapshot;
  requestId: string;
}
...
const helpReasonSchema = z.string();
const helpRequestIdSchema = z.string();
...
export class ComputerControl {
  ...
  requestHelp(botId: string, reason: JsonValue | undefined): ControlSnapshot {
  ...
  requestHelpLease(botId: string, reason: JsonValue | undefined): HelpRequestLeaseResult {
    const parsedReason = helpReasonSchema.safeParse(reason);
    const text = parsedReason.success ? parsedReason.data.trim().slice(0, MAX_REASON_CHARS) : "";
    const entry = this.entries.get(botId) ?? {
      heldSinceMs: null,
      helpReason: null,
      helpRequestId: null,
    ...
    let requestId = entry.helpRequestId;
    if (entry.helpReason === null || requestId === null) {
      entry.helpReason = text || "the operator asked you to take over";
      requestId = `${botId}-${++this.requestSequence}`;
      entry.helpRequestId = requestId;
    }
    this.entries.set(botId, entry);
    return { snapshot: this.changed(botId), requestId };
  }
  ...
  expireHelp(botId: string, requestId: JsonValue | undefined): ControlSnapshot {
    const entry = this.entries.get(botId);
    const parsedRequestId = helpRequestIdSchema.safeParse(requestId);
    if (!entry || !parsedRequestId.success || entry.helpRequestId !== parsedRequestId.data) return this.snapshot(botId);
    entry.helpReason = null;
    entry.helpRequestId = null;
    if (entry.heldSinceMs === null) this.entries.delete(botId);
    return this.changed(botId);
