...
const fakeDriverConfigSchema = z
  .union([z.record(z.string(), z.custom<JsonValue>()), z.null(), z.undefined()])
  .transform((value): JsonObject => value ?? {});
type FakeDriverConfigInput = z.input<typeof fakeDriverConfigSchema>;
...
export interface FakeDriverHandle {
  driver: ProviderDriver<JsonObject>;
...
export function makeFakeDriver(opts: FakeDriverOptions = {}): FakeDriverHandle {
  ...
  const handle: FakeDriverHandle = {
    ...
    driver: {
      ...
      decodeConfig(raw: FakeDriverConfigInput) {
        const config = fakeDriverConfigSchema.parse(raw);
        if (config.bad === true) {
          throw new Error(`${kind}: bad config`);
        }
        handle.decodedConfigs.push(config);
        return config;
      },
      defaultConfig: () => ({ isDefault: true }),
      async create(input: DriverCreateInput<JsonObject>): Promise<ProviderInstance> {
