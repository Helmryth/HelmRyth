...
// DATA_DIR (~/.helmryth) never touches the real one. os.homedir()
...
const home = mkdtempSync(join(tmpdir(), "hry-test-home-"));
process.env.HOME = home;
process.env.USERPROFILE = home;
// HELMRYTH_DATA_DIR is an intentional production override, but tests must never
// let it escape the throwaway home they are about to delete.
delete process.env.HELMRYTH_DATA_DIR;
...
process.env.HELMRYTH_COMPANION_DIR = join(home, ".helmryth-companion");
