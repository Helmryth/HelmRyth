...
//                     session/new's mcpServers, call list_operators + ask_operator on a
//                     peer, and reply with what the peer said — the comms e2e)
//                   | delegate-peer (same as ask-peer but uses delegate_operator —
...
const dumpState: DumpState = { argv, env: dumpEnv };
...
const out = <Message>(message: Message) => process.stdout.write(JSON.stringify(message) + "\n");
const result = <Response>(id: JsonValue | undefined, response: Response) => out({ jsonrpc: "2.0", id, result: response });
...
const configCalls: Array<{ method: string; params: object }> = [];
...
function driveMcp(entry: McpEntry, calls: Array<{ name: string; args: (prev: string) => object }>): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    let last = "";
    const write = <Message>(message: Message) => child.stdin.write(JSON.stringify(message) + "\n");
    const next = () => {
      step += 1;
...
interface RpcParams {
  ...
  configId?: JsonValue;
  value?: JsonValue;
  prompt?: Array<{ text?: JsonValue }>;
  [key: string]: JsonValue | McpEntry[] | Array<{ text?: JsonValue }> | undefined;
}
...
interface RpcMessage {
  ...
  method?: string;
  params?: RpcParams;
  result?: { content?: Array<{ text?: JsonValue }> };
  error?: JsonValue;
}

interface AcpSessionResponse {
  sessionId?: string;
  configOptions?: NonNullable<ReturnType<typeof configOptions>>;
  models?: NonNullable<ReturnType<typeof sessionModels>>;
}

function rpcString(value: JsonValue | undefined): string | null {
  return String(value) === value ? String(value) : null;
}
...
function handle(msg: RpcMessage) {
  ...
  switch (msg.method) {
    ...
    case "session/new": {
      ...
      agentsMcp = servers.find((server) => server.name === "agents") ?? null;
      if (process.env.FAKE_ACP_DUMP) {
        writeFileSync(`${process.env.FAKE_ACP_DUMP}.mcp.json`, JSON.stringify(servers, null, 2));
      }
      const opts = configOptions();
      const mdls = sessionModels();
      const session: AcpSessionResponse = {
        sessionId: "fake-acp-session",
      };
      if (opts) session.configOptions = opts;
      if (mdls) session.models = mdls;
      result(msg.id, session);
      break;
    }
    case "session/load": {
      const opts = configOptions();
    ...
    case "session/set_model": {
      ...
      const settingValue = settingId === "modeId" ? msg.params?.modeId : msg.params?.modelId;
      if (!rpcString(msg.params?.sessionId) || !rpcString(settingValue)) {
        out({
          jsonrpc: "2.0",
          id: msg.id,
    ...
    case "session/set_config_option": {
      ...
      const modelValue = rpcString(value);
      if (configId !== "model" || !modelValue || !models.includes(modelValue)) {
        out({
          jsonrpc: "2.0",
          id: msg.id,
      ...
      if (!process.env.FAKE_ACP_MODEL_STICKS) currentModel = modelValue;
    ...
    case "session/prompt": {
      ...
      if (mode === "ask-peer" && agentsMcp) {
        // the comms e2e: reach a peer operator through the injected agents proxy
        ...
        void driveMcp(agentsMcp, [
          { name: "list_operators", args: () => ({}) },
          {
            name: "ask_operator",
            args: (list) => ({ operator_id: /id: ([\w-]+)/.exec(list)?.[1] ?? "", message: "ping from fake" }),
          },
        ])
          .then((reply) => {
          ...
          .catch((error) => {
            ...
            out({ jsonrpc: "2.0", method: "session/update", params: { update: { sessionUpdate: "agent_message_chunk", content: { text: `peer error: ${detail}` } } } });
            complete();
          });
        return;
      }
      if (mode === "create-peer" && agentsMcp) {
        void driveMcp(agentsMcp, [
          ...
          },
          {
            ...
            args: (created) => ({
              operator_id: /id: ([\w-]+)/.exec(created)?.[1] ?? "",
      ...
      if (mode === "delegate-peer" && agentsMcp) {
        ...
        void driveMcp(agentsMcp, [
          ...
          .catch((error) => {
            const detail = error instanceof Error ? error.message : String(error);
            out({ jsonrpc: "2.0", method: "session/update", params: { update: { sessionUpdate: "agent_message_chunk", content: { text: `delegate error: ${detail}` } } } });
            complete();
          });
        return;
      }
