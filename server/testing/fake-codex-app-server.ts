...
type ApprovalDecision = JsonValue | { error: JsonValue | undefined };
...
const calls: Array<{ method: string; params: RpcParams | null }> = [];
let decision: ApprovalDecision = null;

const out = <Message>(message: Message) => process.stdout.write(JSON.stringify(message) + "\n");
const notify = <Params>(method: string, params: Params) => out({ jsonrpc: "2.0", method, params });
...
const finishTurn = () => {
  ...
  dump();
  // Real codex-cli sends an explicit `error: null` on a SUCCESSFUL turn. The
  // fixture used to omit the key entirely, so the driver's schema was never
  // exercised against the shape the CLI actually emits — and a schema that
  // rejected null silently marked every successful run as failed.
  ...
  while ((nl = buf.indexOf("\n")) !== -1) {
    ...
    switch (msg.method) {
      ...
      case "turn/start": {
        ...
        notify("item/started", { item: { id: "w1", type: "webSearch", query: "Helmryth" } });
