...
const promptText = (prompt: JsonValue): string => {
  if (!prompt || Array.isArray(prompt) || Object(prompt) !== prompt) return "";
  // SAFETY: JSON objects are the only non-null, non-array JsonValue members
  // for which Object(value) preserves identity.
  const envelope = prompt as JsonObject;
  const message = envelope.message;
  if (!message || Array.isArray(message) || Object(message) !== message) return "";
  // SAFETY: the same JSON-object invariant was checked for `message` above.
  const content = (message as JsonObject).content;
  return String(content) === content ? String(content) : "";
};
...
const playTurn = (prompt: JsonValue) => {
  ...
  if (mode === "stream") {
    const delta = <Payload>(payload: Payload) => out({ type: "stream_event", event: { type: "content_block_delta", delta: payload } });
