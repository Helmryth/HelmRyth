...
const frameEnvelopeSchema = z.object({ content: z.string().min(1).optional() });
const automationResultSchema = z.object({
  effect: z.string().optional(),
  route: z.string().optional(),
  escalation: z.string().optional(),
});
const semanticBrowserSnapshotSchema = z.object({
  title: z.string(),
  url: z.string(),
...
const boxId = process.env.HELMRYTH_BOX_ID ?? "";
const token = process.env.HELMRYTH_BOX_TOKEN ?? "";
...
const CONTROL_POLL_MS = Math.max(Number(process.env.HELMRYTH_CONTROL_POLL_MS) || 1_500, 25);
const CONTROL_WAIT_MS = Math.max(Number(process.env.HELMRYTH_CONTROL_WAIT_MS) || 600_000, 100);
...
const SHOT_PATH = "/tmp/helmryth-shot.jpg";
...
const CHROME_PROFILE = "$HOME/.helmryth/chrome-profile";
...
const CHROME_PROFILE_SETUP = [
  ...
  "      exit 1",
  "    fi",
  '    mv "$browser_dir" "$browser_dir.pre-helmryth-$(date +%s)-$$"',
  "  fi",
...
function cuaOrX11(tool: string, argumentsShell: string, fallback: string): string {
  return [
    ...
    `then if CUA_OUT=$(env ${CUA_ENV} ${REMOTE_CUA_EXECUTABLE} call ${tool} ${argumentsShell} --socket ${REMOTE_CUA_SOCKET} 2>/tmp/helmryth-cua-call.error);`,
...
async function fetchFrame(expectedBytes?: number): Promise<string | null> {
  ...
  try {
    ...
    const body = frameEnvelopeSchema.safeParse(await res.json().catch(() => null));
    const content = body.success ? body.data.content : undefined;
...
function automationSummary(stdout: string): string {
  if (/^BACKEND CUA$/m.test(stdout)) {
    ...
    try {
      const result = automationResultSchema.parse(parseJson(Buffer.from(encoded, "base64").toString("utf8")));
      const details = [result.effect, result.route, result.escalation].filter(Boolean).slice(0, 3);
...
const send = <Message>(value: Message): void => {
  process.stdout.write(JSON.stringify(value) + "\n");
};
const text = (id: JsonRpcId, t: string, isError = false): void => {
  const result: TextContentResult = {
...
const TOOLS = [
  {
    name: "screenshot",
    description:
      "See the operator's cloud Workbench when visual state is needed. Prefer browser_state for Chrome title and URL checks. The frame is captured fresh; byte-identical pixels are not resent.",
    inputSchema: {
      type: "object",
      properties: {
        region: {
  ...
  },
  {
    name: "computer_exec",
    description:
      "Run a shell command on the operator's cloud Workbench (Linux, passwordless sudo, X11 desktop). Returns stdout, stderr, and exit code; unlike the visual tools, it does not capture a screen unless requested.",
    inputSchema: {
      type: "object",
      properties: {
        command: { type: "string" },
...
async function actAndObserve(
  ...
  for (const a of actions) {
    ...
    const commandPart = z.string().safeParse(shell);
    if (!commandPart.success) return text(id, actionErrorSchema.parse(shell).error, true);
...
async function call(id: JsonRpcId, name: string, args: any) {
  ...
  if (name === "browser_snapshot") {
    ...
    try {
      const snapshot = semanticBrowserSnapshotSchema.parse(parseJson(out.stdout));
  ...
  if (msg.method === "initialize") {
    return send({
      ...
      result: {
        ...
        capabilities: { tools: {} },
        serverInfo: { name: "helmryth-computer", version: "3" },
      },
    });
  }
