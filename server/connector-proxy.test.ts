...
async function listen(handler: RequestListener) {
  ...
  // SAFETY: a TCP server listening on an IP host returns AddressInfo, never a pipe-name string.
  const tcpAddress = address as AddressInfo;
  return `http://127.0.0.1:${tcpAddress.port}`;
}
...
function nextJson(lines: readline.Interface) {
  return new Promise<ConnectorReply>((resolve, reject) => {
...
describe("connector MCP bridge", () => {
  it("turns agent connection requests into authenticated chat-card requests", async () => {
    ...
    const lines = start({
      HELMRYTH_HARNESS_URL: harness,
      HELMRYTH_COMMS_TOKEN: "bridge-secret",
    ...
    expect(reply.result.content[0].text).toMatch(/secure connection gate/i);
  ...
  it("answers initialize after a bounded wait when the upstream stalls", async () => {
    ...
    const lines = start({ HELMRYTH_CONNECTOR_UPSTREAM_URL: upstream });
    child!.stdin.write(`${JSON.stringify({
      jsonrpc: "2.0",
      id: 11,
      method: "initialize",
  ...
  it("still opens the upstream MCP session on initialize without echoing secrets", async () => {
    ...
    const lines = start({
      HELMRYTH_CONNECTOR_UPSTREAM_URL: upstream,
      HELMRYTH_CONNECTOR_UPSTREAM_HEADERS: JSON.stringify({ authorization: "Bearer upstream-secret" }),
    });
    ...
    expect(reply.result.serverInfo).toEqual({ name: "helmryth-connectors", version: "1" });
