...
const controlStateSchema = z.object({ held: z.boolean().optional(), helpOpen: z.boolean().optional() });
const helpRequestSchema = z.object({ requestId: z.string().min(1) });
...
export function createControlClient(options?: {
  ...
  const url = options?.url ?? process.env.HELMRYTH_CONTROL_URL ?? "";
  const token = options?.token ?? process.env.HELMRYTH_CONTROL_TOKEN ?? "";
  ...
  async function read(): Promise<ControlState> {
    try {
      ...
      const body = controlStateSchema.safeParse(await res.json().catch(() => null));
      if (!body.success) return DISENGAGED;
      return { held: body.data.held === true, helpOpen: body.data.helpOpen === true };
    } catch {
      return DISENGAGED;
    }
  }
  ...
  return {
    ...
    async requestHelp(reason: string): Promise<string | null> {
      ...
      try {
        ...
        const body = helpRequestSchema.safeParse(await res.json().catch(() => null));
        return body.success ? body.data.requestId : null;
      } catch {
        return null;
      }
    },
