...
export function createBotPackageExport(input: {
  ...
  if (!bots.length) throw new Error("Create an operator before exporting your package");
  ...
  const id = portableKey(input.name, "helmryth-package", new Set());
  const agents: BotPackageDefinition["agents"] = bots.map((bot) => {
    ...
    if (bot.sigilExpression) appearance.sigilExpression = bot.sigilExpression;
  ...
  const definition: BotPackageDefinition = {
    ...
    release: "1.0.0",
    name: input.name,
    tagline: `A portable Helmryth setup with ${bots.length} ${bots.length === 1 ? "operator" : "operators"}.`,
    summary: "Exported from Helmryth. Review the roles, rooms, playbooks, connector requirements, and paused routines before sharing or publishing.",
    category: "Community",
    author: { name: input.authorName?.trim() || "Helmryth user" },
    license: "Unspecified",
    outcomes: ["Recreate this operator setup without copying private runtime state."],
