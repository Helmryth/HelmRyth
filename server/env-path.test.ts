...
type ProbeCallback = (err: Error | null, stdout: string | Buffer) => void;

describe("augmentedPath", () => {
  afterEach(() => {
    delete process.env.HELMRYTH_EXTRA_PATH;
    setLoginShellProbeForTests(execFile);
    resetPathCacheForTests();
  });
  ...
  it("keeps the existing PATH entries first", () => {
    ...
    // HELMRYTH_EXTRA_PATH is unset here, so the inherited PATH leads
    expect(path.split(delimiter)[0]).toBe(firstExisting);
  });

  it("prepends HELMRYTH_EXTRA_PATH and dedupes", () => {
    process.env.HELMRYTH_EXTRA_PATH = ["/tmp/hry-extra", "/tmp/hry-extra"].join(delimiter);
    resetPathCacheForTests();
    const parts = augmentedPath().split(delimiter);
    expect(parts[0]).toBe("/tmp/hry-extra");
    expect(parts.filter((p) => p === "/tmp/hry-extra")).toHaveLength(1);
  });
  ...
  posixIt("makes a CLI in a known install dir spawnable despite a bare PATH", async () => {
    ...
    const fake = join(bin, "hry-fake-cli");
  ...
  posixIt("keeps the last login-shell PATH available during a rescan", () => {
    const rcOnlyBin = join(homedir(), "rc-only", "bin");
    const callbacks: ProbeCallback[] = [];

    const previousVitest = process.env.VITEST;
    try {
      delete process.env.VITEST;
      resetPathCacheForTests();
      setLoginShellProbeForTests((_shell, _args, _options, done) => {
        callbacks.push(done);
      });

      augmentedPath();
      const done = callbacks[0];
      if (!done) throw new Error("expected login-shell probe callback");
      done(null, `__HELMRYTH_PATH__${rcOnlyBin}`);
  ...
  posixIt("ignores stale login-shell probes after a cache reset", () => {
    const staleBin = join(homedir(), "stale", "bin");
    const freshBin = join(homedir(), "fresh", "bin");
    const callbacks: ProbeCallback[] = [];
    ...
    try {
      ...
      expect(freshCallback).toBeTypeOf("function");

      staleCallback?.(null, `__HELMRYTH_PATH__${staleBin}`);
      expect(augmentedPath().split(delimiter)).not.toContain(staleBin);

      freshCallback?.(null, `__HELMRYTH_PATH__${freshBin}`);
      expect(augmentedPath().split(delimiter)).toContain(freshBin);
    } finally {
      if (previousVitest === undefined) delete process.env.VITEST;
      else process.env.VITEST = previousVitest;
      resetPathCacheForTests();
  ...
  it.skipIf(process.platform !== "win32")("finds Antigravity installed after launch", () => {
    ...
    const localAppData = mkdtempSync(join(tmpdir(), "hry-localappdata-"));
    try {
...
winOnly("resolveCli (Windows)", () => {
  let dir: string;
  const onPath = () => {
    process.env.HELMRYTH_EXTRA_PATH = dir;
    resetPathCacheForTests();
  };
  ...
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), "hry-shim-"));
  });
  afterEach(async () => {
    delete process.env.HELMRYTH_EXTRA_PATH;
    resetPathCacheForTests();
