...
describe("computer proxy (fake box)", () => {
  ...
  beforeAll(async () => {
      ...
      if (url.pathname.endsWith("/commands")) {
        ...
        req.on("end", () => {
          ...
          const stdout = command.includes("127.0.0.1:9222/json/list")
            ...
            : command.includes("helmryth-cdp.mjs snapshot")
              ? JSON.stringify({
                  title: "Account",
              ...
              : command.includes("helmryth-cdp.mjs click") || command.includes("helmryth-cdp.mjs fill")
                ? `GEOM 1920 1080\nHASH ${hash}\nSIZE ${size}\nB64 ${JPEG}\nSEM ok\n`
    ...
    port = (box.address() as AddressInfo).port;

    proxy = spawn(process.execPath, ["--experimental-strip-types", PROXY], {
      env: {
        ...process.env,
        HELMRYTH_BOX_API: `http://127.0.0.1:${port}`,
        HELMRYTH_BOX_ID: "box-1",
  ...
  it("clicks and returns the frame in ONE round trip, scaled box-side", async () => {
    ...
    expect(command).toContain("/opt/helmryth/cua-driver call click");
      ...
      params: {
        ...
        arguments: {
          actions: [
            ...
            { action: "type_text", text: "divyam@example.com" },
  ...
  it("uses fresh semantic browser refs without exposing URL secrets", async () => {
    ...
    expect(commands.length - before).toBe(1);
    expect(commands.at(-1)).toContain("helmryth-cdp.mjs fill");
  ...
  it("uses a private Chrome profile, strips URL credentials, and reports redirects", async () => {
    ...
    expect(issued[0]).toContain('profile="$HOME/.helmryth/chrome-profile"');
    ...
    expect(issued[0]).toContain('--user-data-dir="$HOME/.helmryth/chrome-profile"');
...
describe("computer proxy control gate (fake box + fake control)", () => {
  ...
  beforeAll(async () => {
    ...
    await new Promise<void>((r) => box.listen(0, "127.0.0.1", r));
    // SAFETY: this HTTP server listens on an IP host, so Node returns AddressInfo.
    const boxPort = (box.address() as AddressInfo).port;
    ...
    proxy = spawn(process.execPath, ["--experimental-strip-types", PROXY], {
      env: {
        ...process.env,
        HELMRYTH_BOX_API: `http://127.0.0.1:${boxPort}`,
        HELMRYTH_BOX_ID: "box-1",
        HELMRYTH_BOX_TOKEN: "t",
        HELMRYTH_CONTROL_URL: `http://127.0.0.1:${controlPort}/api/internal/computer-control?botId=b1`,
        HELMRYTH_CONTROL_TOKEN: "control-secret",
        // fast cadence so the wait tests measure logic, not wall-clock
        HELMRYTH_CONTROL_POLL_MS: "25",
        HELMRYTH_CONTROL_WAIT_MS: "1500",
      },
      stdio: ["pipe", "pipe", "pipe"],
    });
    let buf = "";
