...
Cursor is an optional Helmryth engine. Helmryth runs the official
...
Operators on this engine consume the user's Cursor subscription (or a
...
3. Confirm `cursor-agent --version` works. The binary installs to `~/.local/bin` by
   default; Helmryth already looks there when launched from a GUI.
...
CLI also implements ACP `session/set_model`, Helmryth pins the same id over
...
switch). Helmryth still answers ACP `session/request_permission` itself:
