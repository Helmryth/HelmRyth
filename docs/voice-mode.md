...
Decision doc, 2026-08-14. How operators speak, and how you hold a conversation with
one.
...
through playback transcribes the operator's own voice back into the conversation and
the two of them talk forever. So the mic is live only when the operator is not
...
feel conversational is to put the operator you call on a fast model and let it
...
- **Crews do not speak yet**, though per-operator voices already exist (`operator.voice`).
...
- Call phases are updated synchronously alongside React state, so a helper exit
  ...
  `listening` phase.
- Leaving the voice view owns and ends its call. A hidden overlay cannot leave a
