...
Helmryth has an Ubuntu 24.04 LTS x86_64 desktop beta. The Electron package embeds the harness server, so
installed builds do not require Node, pnpm, Swift, or a terminal at runtime. For giving an operator the same kind
...
- Explicit opt-in local workstation control on GNOME Xorg using the bundled, pinned Cua Driver without its
...
- `Helmryth-amd64.deb` — recommended; APT installs its desktop dependencies.
- `Helmryth.AppImage` — does not install system files.
- `SHA256SUMS-ubuntu-x64.txt` — verify before install.
...
- `release/Helmryth-<version>-amd64.deb`
- `release/Helmryth-<version>-x86_64.AppImage`
...
```sh
sudo apt install ./Helmryth-amd64.deb
```

Then open **Helmryth** from the GNOME application launcher. To remove it:

```sh
sudo apt remove helmryth
```
...
./release/Helmryth-*-x86_64.AppImage
```

For a downloaded release AppImage, use `Helmryth.AppImage` in place of the versioned path above.
...
These defaults share `http://127.0.0.1:5199` as the exact trusted renderer
origin. A custom UI port must use the same `HELMRYTH_UI_PORT` and
`HELMRYTH_UI_ORIGIN` in all three terminals; see
[Local control-plane request boundary](local-control-plane-security.md).

For a package-shaped build without creating `.deb` or AppImage artifacts:

```sh
pnpm package:linux:dir
./release/linux-unpacked/helmryth
```

## Agent CLI discovery

Applications launched from GNOME do not inherit the same interactive shell `PATH` as a terminal. Helmryth
...
```sh
HELMRYTH_EXTRA_PATH=/your/custom/bin ./release/Helmryth-*-x86_64.AppImage
```

Restart Helmryth after installing or signing in to a CLI.
...
Open the Workbench panel and use the separate **Preview this workstation** card. Capture never starts when the app
or panel opens.
...
automatically. Helmryth does not capture screen audio, remember the selected monitor after restart, or
...
On GNOME Wayland, **This workstation** remains unavailable and an older persisted opt-in is reset to off with private
...
The Xorg runtime uses private sockets, standard permission mode, per-action Helmryth gates,
...
**Auto** never routes to the user's desktop, and no Cloud or Local VM gate can authorize it.
...
### An operator needs workstation tools

Choose **Cloud box** and add a Box token in App Settings, or use Local VM. Linux **This workstation** remains disabled
