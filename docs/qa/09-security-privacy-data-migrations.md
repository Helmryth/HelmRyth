...
This document defines the production security/privacy QA plan for Helmryth's local harness, Electron shell, Reach companion, webhook ingress, credential migrations, and release chain. It follows the QA case contract in [TEST-CASE-TEMPLATE.md](./TEST-CASE-TEMPLATE.md) and the threat model in [SECURITY.md](../../SECURITY.md).

## Trust boundaries under test

- `Electron main ↔ renderer preload bridge`: renderer sees only `window.helmryth`, never raw Node or `ipcRenderer` ([electron/preload.cjs:1](../../electron/preload.cjs)).
- `Renderer / phone / browser ↔ local harness`: the harness trusts loopback only, rejects non-loopback `Host`, and rejects non-loopback browser `Origin` ([server/index.ts:4200](../../server/index.ts)).
- `Phone ↔ Reach companion`: device tokens are required for the network listener, the route surface is allowlisted, and cloud desktop join is separately capability-gated ([companion/src/proxy.ts:264](../../companion/src/proxy.ts), [companion/src/routes.ts:47](../../companion/src/routes.ts)).
- `Electron ↔ OS secret store`: credentials are migrated out of plaintext config and stored in `credentials.bin` through `safeStorage`, with fail-closed behavior when the store cannot be read ([electron/main.mjs:335](../../electron/main.mjs), [electron/workspace-credentials.mjs:54](../../electron/workspace-credentials.mjs)).
- `External webhook sender ↔ webhook ingress`: only valid capability URLs or bearer secrets are accepted, payloads are size-bounded, and duplicate deliveries are deduplicated ([server/webhook-ingress.ts:128](../../server/webhook-ingress.ts), [server/webhooks.ts:527](../../server/webhooks.ts)).
- `Packaged app ↔ release repository / third-party binaries`: update target, codesign, notarization, cloudflared provenance, and CUA SBOM are all release gates ([.github/workflows/release.yml:1](../../.github/workflows/release.yml), [scripts/verify-linux-package.mjs:156](../../scripts/verify-linux-package.mjs)).
...
- Result: `9` files passed, `147` tests passed. This is the August 30, 2026 figure and is left as recorded; the same
  nine files now report `160` tests as regression coverage was added, so re-run the command above for a current count
