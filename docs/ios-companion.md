...
The iOS app is a thin, native client for the Helmryth instance running on
your Mac. The Mac remains the only machine that owns agent processes,
credentials, SQLite data, transcripts, and workstations. The iPhone trusts a Mac
by scanning the QR code shown in desktop **Settings → Mobile**; it does not need
a Helmryth account of its own.

## Current status

The first version includes:

- QR-first pairing from desktop **Settings → Mobile**, with the workstation name
...
- Operator and crew lists, paged transcripts, sending, interruption, and unread
  state.
- Gates and questions, including narrow “always allow” grants.
- Resumable SSE, streamed reply text, reconnect hydration, and an opt-in live
  Box workstation view. The loopback-only VPS SSH viewer remains desktop-only.
...
to the user's own workstation; it is not a cloud transcript store and cannot wake
a terminated iOS app.

The Mac must be running Helmryth and must not be asleep. Desktop **Settings → Mobile** offers an off-by-default **Keep this workstation awake** switch that prevents system sleep while mobile access is on; the display may still turn off. A sleeping or powered-off workstation cannot receive phone requests or run its local cadences, including through the optional hosted transport.
...
The sidecar keeps its device registry in `~/.helmryth/devices.json`. That is
...
Tailscale** in the desktop setup alternatives. Helmryth then places the
...
Tailscale is optional. The direct path does not use a Helmryth-operated
...
The Helmryth control plane stores account and installation metadata plus
opaque tunnel/DNS identifiers in D1, but not operators, transcripts, gates,
...
1. On the Mac, open **Settings → Mobile**, turn on mobile access, and choose
   **Set up a phone**.
2. On the iPhone, choose **Connect my workstation** and scan the QR.
3. Confirm the workstation name and the displayed transport — **HTTPS connection**,
...
4. If scanning is unavailable, open **Other ways to connect** for a nearby workstation, manual address, or six-digit code.
5. Revoking the phone on the Mac removes its access and lets it pair again.

The Mac must remain awake with Helmryth running for chats, gates, and cadences to work, including through hosted HTTPS or Tailscale.
...
route never reaches the harness and returns only the workstation name plus a
bounded list of connection origins.

An Helmryth account is not required for nearby, manual, or Tailscale
...
- Request a fresh interactive cloud-desktop viewer only when the workstation
  owner has enabled that capability for this specific paired phone.
- Send messages, interrupt operators, answer gates/questions, and mark chats
  read.
- Create a basic operator.

The write surface uses purpose-built `read` and `always-allow` endpoints. The
general operator and crew `PATCH` endpoints are not reachable through the sidecar.
An always-allow request succeeds only when its server-issued key is still on a
pending gate for that operator, so possession of a device token is not enough
...
cd ios
swift test
../scripts/prepare-xcodegen.sh --run generate
xcodebuild -project HelmrythCompanion.xcodeproj \
  -scheme HelmrythCompanion \
  -sdk iphonesimulator \
