...
const CargoPackageSchema = z
  ...
  .passthrough();
const CargoAboutReportSchema = z
  .object({
    crates: z.array(
      z
        .object({
    ...
    licenses: z.array(
      z
        .object({
          ...
          text: z.string(),
          used_by: z
            .array(z.object({ crate: CargoPackageSchema }).passthrough())
            .optional(),
        })
        .passthrough(),
    ),
...
function isRegistryPackage(pkg) {
  return pkg.source?.startsWith(REGISTRY_SOURCE_PREFIX) === true;
}
...
function registryComponent(pkg, cargoLockPackages) {
  ...
  const externalReferences = [{ type: "distribution", url: distribution }];
  if (pkg.repository && /^https?:\/\//.test(pkg.repository)) {
    externalReferences.push({ type: "vcs", url: pkg.repository });
  }
  const component = {
    type: "library",
  ...
  component.properties = [
    { name: "helmryth:cargo:package-id", value: pkg.id },
    { name: "helmryth:cargo:crate-sha256", value: locked.checksum },
  ];
  return component;
}
...
function interComponent() {
  return {
    ...
    properties: [
      {
        name: "helmryth:font:name-table-version",
        value: "Version 4.001;git-66647c0bb",
      },
      { name: "helmryth:font:embedded-sha256", value: INTER_FONT_SHA256 },
    ],
  };
}
...
<head>
  ...
  <title>Helmryth — Cua Driver third-party licenses</title>
...
const driverReport = CargoAboutReportSchema.safeParse(
  JSON.parse(driverBytes.toString("utf8")),
);
if (!driverReport.success) {
  throw new Error("cua-driver report is not cargo-about JSON output");
}
const cursorThemeReport = CargoAboutReportSchema.safeParse(
  JSON.parse(cursorThemeBytes.toString("utf8")),
);
if (!cursorThemeReport.success) {
  throw new Error("cursor-theme report is not cargo-about JSON output");
}
...
const bom = {
  ...
  metadata: {
    component: {
      ...
      properties: [
        { name: "helmryth:archive:sha256", value: ARCHIVE_SHA256 },
        { name: "helmryth:cargo-lock:sha256", value: CARGO_LOCK_SHA256 },
        { name: "helmryth:target", value: "x86_64-unknown-linux-gnu" },
        { name: "helmryth:feature", value: "cua-driver/portal-input" },
        {
          name: "helmryth:registry-component-count",
          value: String(EXPECTED_REGISTRY_UNION_COUNT),
        },
        {
          name: "helmryth:trycua-component-count",
          value: String(TRYCUA_COMPONENTS.length),
        },
        {
          name: "helmryth:file:cua-driver:sha256",
          value:
            "ed5844fadf07b9b72c4a3b3802e1c47233c166d66d6198608d5991f807aab4ac",
        },
        {
          name: "helmryth:file:cua-cursor-theme:sha256",
          value:
...
Helmryth redistributes two executables from the official Cua Driver ${RELEASE_VERSION} Linux x64 release: \`cua-driver\` and \`cua-cursor-theme\`.
...
The complete MPL-2.0 text and package copyright notices appear in \`THIRD_PARTY_LICENSES.html\`. These files remain under their original licenses; Helmryth's and Cua's MIT licenses do not replace them.
