...
const prefixName = "hry-linux-smoke-runtime-";
...
const executables = [
  path.join(root, "release", "linux-unpacked", "helmryth"),
  path.join(root, "release", appImage),
];
if (process.env.HELMRYTH_SMOKE_INSTALLED_DEB === "1") {
  executables.push("/opt/Helmryth/helmryth");
}
  ...
  const bundled = spawnSync(
    ...
    ["--", "xvfb-run", "-a", process.execPath, path.join(root, "scripts", "smoke-linux-package.mjs")],
    {
      ...
      env: {
        ...
        HELMRYTH_SMOKE_BUNDLED_CUA: "1",
        HELMRYTH_SMOKE_EXECUTABLE: executable,
      },
      stdio: "inherit",
    },
  );
  ...
  const signalShutdown = spawnSync(
    ...
    ["--", "xvfb-run", "-a", process.execPath, path.join(root, "scripts", "smoke-linux-package.mjs")],
    {
      ...
      env: {
        ...
        XDG_RUNTIME_DIR: runtimeDirectory,
        HELMRYTH_SMOKE_BUNDLED_CUA: "1",
        HELMRYTH_SMOKE_SIGNAL_SHUTDOWN: "1",
        HELMRYTH_SMOKE_EXECUTABLE: path.join(root, "release", appImage),
      },
      stdio: "inherit",
    },
  );
  ...
  const result = spawnSync(
    ...
    ["--", "xvfb-run", "-a", process.execPath, path.join(root, "scripts", "smoke-linux-package.mjs")],
    {
      ...
      env: {
        ...process.env,
        XDG_RUNTIME_DIR: runtimeDirectory,
        HELMRYTH_SMOKE_WAYLAND: lane.wayland ? "1" : "0",
        HELMRYTH_SMOKE_LINUX_CUA_BLOCKED: lane.blocked ? "1" : "0",
      },
      stdio: "inherit",
    },
  );
