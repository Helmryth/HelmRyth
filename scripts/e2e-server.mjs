// End-to-end check of a running Helmryth harness server — the exact
...
// codex turn, and — with --with-box + HELMRYTH_E2E_BOX_TOKEN — box provisioning,
...
const PORT = Number(opt("--port", process.env.HELMRYTH_PORT ?? 8799));
const BASE = `http://127.0.0.1:${PORT}`;
const WITH_BOX = flag("--with-box");
const KEEP_BOTS = flag("--keep-bots");
const BOX_TOKEN = process.env.HELMRYTH_E2E_BOX_TOKEN ?? "";

const tag = Date.now().toString(36).slice(-6);
const marker = (s) => `hry-e2e-${s}-${tag}`;
...
async function waitTurnDone(botId, budgetMs) {
  return until(
    `turn on operator ${botId.slice(0, 8)} to settle`,
    async () => {
...
async function waitAskOrSettle(botId, budgetMs) {
  ...
  for (;;) {
    ...
    if (Date.now() - t0 > budgetMs) fail(`neither ask nor settle within ${budgetMs / 1000}s on operator ${botId.slice(0, 8)}`);
    await sleep(2000);
  }
}
...
async function makeBot(name, instanceId, model) {
  ...
  log(`operator "${name}" created (${patched.bot.id.slice(0, 8)}) on ${instanceId}/${model}`);
  return patched.bot;
}
...
async function expectReply(bot, want, budgetMs) {
  ...
  if (!hit) {
    ...
    fail(`operator ${bot.name} settled but never said ${want}. Last message: ${JSON.stringify(last)?.slice(0, 300)}`);
  }
...
async function main() {
  ...
  try {
    ...
    if (byKind.claudeAgent?.snapshot.state === "available") {
      ...
      await send(bot.id, `Run exactly one shell command: curl -sS https://example.com -o hry_${tag}_allow.html — then tell me the first word of the downloaded file.`);
      ...
      if (allowTry.ask) {
        ...
        if (!afterAllow.messages.find((m) => m.role === "bot" && m.kind === "text"))
          fail("no operator text after allow");
      ...
      await send(denyBot.id, `Run the shell command "curl -sS https://example.org -o hry_${tag}_deny.html" — nothing else.`);
    ...
    // ── box: cloud computer ──
    if (WITH_BOX) {
      if (!BOX_TOKEN) fail("--with-box needs HELMRYTH_E2E_BOX_TOKEN");
    ...
    console.error(`\nFAIL: ${e.message} — ${created.length} test operator(s) kept for postmortem; delete them through the operator API`);
    process.exit(1);
  }
  if (!KEEP_BOTS) {
    for (const id of created) await api(`/api/bots/${id}`, { method: "DELETE" }).catch(() => {});
    if (created.length) log("  ✓ test operators deleted");
  }
