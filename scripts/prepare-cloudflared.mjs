...
// Helmryth updates this dependency with an ordinary reviewed app release.
...
async function releaseBytes(asset) {
  const cacheDirectory = process.env.HELMRYTH_CLOUDFLARED_ARCHIVE_DIR;
...
async function stageTarget(root, target) {
  ...
  const scratch = mkdtempSync(join(tmpdir(), `helmryth-cloudflared-${target}-`));
  try {
