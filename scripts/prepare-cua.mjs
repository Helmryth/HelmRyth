...
async function officialBinary() {
  const cache = join(root, "node_modules", ".cache", "helmryth", `cua-driver-${release.version}`);
  ...
  const response = await fetch(url, { headers: { "user-agent": "Helmryth-packager" } });
...
await build({
  stdin: {
    ...
    resolveDir: root,
    sourcefile: "helmryth-cua-entry.mjs",
    loader: "js",
  },
  bundle: true,
  platform: "node",
  ...
  format: "esm",
  banner: {
    js: 'import { createRequire as __helmrythCreateRequire } from "node:module"; const require = __helmrythCreateRequire(import.meta.url);',
  },
  outfile: bundle,
  logLevel: "silent",
});
...
await writeFile(
  ...
  bundledSource.replace(
    resolverPattern,
    `${resolvers[0]}\n      if (process.env.HELMRYTH_CUA_SDK_LIBRARY) return resolveOverride(opts.crateName, process.env.HELMRYTH_CUA_SDK_LIBRARY);`,
  ),
);
