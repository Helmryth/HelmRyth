...
const resources = process.env.HELMRYTH_CUA_RESOURCES ?? join(root, "dist-native", hostArch);
process.env.HELMRYTH_CUA_SDK_LIBRARY = join(resources, "cua-sdk/native/libcua_driver_sdk.dylib");
...
const host = new EmbeddedCuaDriverHost(binary, "com.helmryth.app");
let proxy;
...
try {
  ...
  proxy = spawn(connection.mcp.command, connection.mcp.args, {
    env: {
      ...
      CUA_DRIVER_HOST_BUNDLE_ID: "com.helmryth.app",
  ...
  await rpc("initialize", {
    ...
    capabilities: {},
    clientInfo: { name: "helmryth-package-smoke", version: "1" },
  });
