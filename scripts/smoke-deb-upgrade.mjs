...
try {
  const status = execFileSync("dpkg-query", ["-W", "-f=${db:Status-Abbrev}", "helmryth"], {
    encoding: "utf8",
    stdio: ["ignore", "pipe", "ignore"],
  }).trim();
  if (status.startsWith("ii")) fail("refusing to replace a pre-existing Helmryth installation");
} catch (error) {
  if (String(error?.message ?? error).includes("refusing to replace")) throw error;
}

const temporary = fs.mkdtempSync(path.join(path.resolve(runnerTemp), "hry-deb-upgrade-"));
...
const legacyApp = path.join(legacyRoot, "opt", "Helmryth");
const legacyResources = path.join(legacyApp, "resources");
const fixtureVersion = "0.0.9";
const legacyDeb = path.join(temporary, `helmryth_${fixtureVersion}_amd64.deb`);
...
try {
  ...
  fs.writeFileSync(
    ...
    [
      ...
      "Architecture: amd64",
      "Maintainer: Helmryth CI <ci@helmryth.invalid>",
      "Description: Pre-release directory-mode upgrade fixture",
      "",
    ].join("\n"),
    { mode: 0o644 },
  );
  fs.writeFileSync(path.join(legacyResources, "legacy-upgrade-fixture"), `${fixtureVersion}\n`, { mode: 0o644 });

  execFileSync("dpkg-deb", ["--build", "--root-owner-group", legacyRoot, legacyDeb], {
    stdio: "inherit",
  });
  execFileSync("dpkg", ["--install", legacyDeb], { stdio: "inherit" });
  for (const directory of ["/opt/Helmryth", "/opt/Helmryth/resources"]) {
  ...
  for (const directory of [
    "/opt/Helmryth",
    "/opt/Helmryth/resources",
    "/opt/Helmryth/resources/cua-linux-x64",
  ]) {
  ...
  for (const executable of ["cua-driver", "cua-cursor-theme"]) {
    const file = path.join("/opt/Helmryth/resources/cua-linux-x64", executable);
  ...
  const chromiumSandbox = "/opt/Helmryth/chrome-sandbox";
  ...
  const installedVersion = execFileSync("dpkg-query", ["-W", "-f=${Version}", "helmryth"], {
    encoding: "utf8",
  }).trim();
  console.log(
    `[smoke-deb-upgrade] OK: ${fixtureVersion} fixture modes repaired by ${installedVersion} without weakening the runtime path`,
  );
} finally {
