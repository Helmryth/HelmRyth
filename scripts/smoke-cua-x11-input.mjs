...
const prefix = "hry-cua-x11-input-";
...
const title = `Helmryth CUA input safety ${process.pid}`;
const xev = spawn(
  "xev",
...
try {
  ...
  await rpc("initialize", {
    ...
    capabilities: {},
    clientInfo: { name: "helmryth-x11-input-smoke", version: "1" },
  });
  ...
  const session = `helmryth-x11-smoke-${process.pid}`;
  assertToolResult(
