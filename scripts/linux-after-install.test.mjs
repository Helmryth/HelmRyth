...
function fixture() {
  const appRoot = fs.mkdtempSync(path.join(fs.realpathSync(os.tmpdir()), "hry-deb-upgrade-"));
...
function runHook(appRoot) {
  return spawnSync("/bin/sh", [hook], {
    encoding: "utf8",
    env: { ...process.env, HELMRYTH_POSTINSTALL_TEST_ROOT: appRoot },
  });
}
...
describe.skipIf(process.platform !== "linux")("Linux DEB upgrade hook", () => {
  ...
  it("refuses to follow a replaced package directory symlink", () => {
    ...
    const external = fs.mkdtempSync(path.join(fs.realpathSync(os.tmpdir()), "hry-deb-external-"));
