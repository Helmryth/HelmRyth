...
} from "../electron/managed-service-config.mjs";

const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");
const wayland = process.env.HELMRYTH_SMOKE_WAYLAND === "1";
const hardDeath = process.env.HELMRYTH_SMOKE_HARD_DEATH === "1";
const bundled = process.env.HELMRYTH_SMOKE_BUNDLED_CUA === "1";
const sessionBlocked = process.env.HELMRYTH_SMOKE_LINUX_CUA_BLOCKED === "1";
const signalShutdown = process.env.HELMRYTH_SMOKE_SIGNAL_SHUTDOWN === "1";
...
const executable = path.resolve(
  process.env.HELMRYTH_SMOKE_EXECUTABLE ?? path.join(root, "release", "linux-unpacked", "helmryth"),
);
if (!existsSync(executable)) throw new Error(`[smoke-linux-package] missing executable: ${executable}`);

// Official release targets in one build receive the same signed resource.
// Local unsigned package smoke has no release overlay and must fail closed as
// "missing". In either lane compare the expected contract with what the real
// packaged main process handed to its embedded server and renderer.
const packagedServiceConfigPath = path.join(
  root,
  "release",
  "linux-unpacked",
  "resources",
  "helmryth-service-config.json",
);
const expectedManagedServices = existsSync(packagedServiceConfigPath)
  ? managedServiceConfigStatus(
      parseManagedServiceConfig(readFileSync(packagedServiceConfigPath, "utf8")),
    )
  : {
      source: "packaged",
      state: "missing",
      registry: { configured: false, origin: "" },
      conduit: { configured: false, origin: "" },
    };

const sandbox = mkdtempSync(path.join(tmpdir(), "hry-linux-smoke-"));
...
mkdirSync(path.join(home, ".helmryth"), { recursive: true });
mkdirSync(xdgConfig, { recursive: true });
mkdirSync(xdgRuntime, { recursive: true, mode: 0o700 });
chmodSync(xdgRuntime, 0o700);
writeFileSync(
  path.join(home, ".helmryth", "config.json"),
  JSON.stringify({ instances: { ghost: { driver: "not-a-real-driver", displayName: "Ghost" } } }),
);
for (const appName of ["helmryth", "Helmryth"]) {
...
const metadata = {
  ...
  pid: process.pid,
  embedded: true,
  host_bundle_id: "com.helmryth.app",
};
...
const desktopEnv = {
  ...
  // A packaged launch must ignore both spoofed legacy origin variables. The
  // expected values come only from the signed resource read above.
  HELMRYTH_REGISTRY_ORIGIN: "https://spoofed-registry.invalid",
  HELMRYTH_CONDUIT_URL: "https://spoofed-conduit.invalid",
  HELMRYTH_SMOKE_TEST: "1",
  HELMRYTH_SMOKE_CUA: hardDeath || bundled || sessionBlocked ? "0" : "1",
  HELMRYTH_SMOKE_BUNDLED_CUA: bundled ? "1" : "0",
};
if (hardDeath || signalShutdown) desktopEnv.HELMRYTH_SMOKE_KEEP_OPEN = "1";
...
try {
  ...
  if (health?.app !== "helmryth" || health.static !== true) {
    throw new Error(`unexpected embedded health response: ${JSON.stringify(health)}`);
  }
  if (!String(title).includes("Helmryth")) throw new Error(`unexpected renderer title: ${title}`);
  ...
  if (JSON.stringify(result.config?.managedServices) !== JSON.stringify(expectedManagedServices)) {
    throw new Error(
      `packaged managed-service status did not match the signed resource: ${JSON.stringify(result.config?.managedServices)}`,
    );
  }
  if (
    result.config.managedServices.registry.origin.includes("spoofed") ||
    result.config.managedServices.conduit.origin.includes("spoofed")
  ) {
    throw new Error("legacy launch environment overrode the signed managed-service resource");
  }
  if (sessionBlocked) {
    await waitForExit();
    if (existsSync(marker)) throw new Error("release safety block still invoked a CUA executable");
    const activeUserData = ["helmryth", "Helmryth"]
    ...
    console.log(
      `[smoke-linux-package] OK (${wayland ? "GNOME/Wayland" : path.basename(executable)}): signed managed-service topology loaded and Wayland CUA failed closed`,
    );
  } else if (bundled) {
    ...
    const userData = ["helmryth", "Helmryth"]
      .map((name) => path.join(xdgConfig, name))
      .find((directory) => existsSync(path.join(directory, "cua-connection.json")));
  ...
  if (hardDeath) {
    ...
    const restart = spawn(executable, wayland ? ["--ozone-platform=x11"] : [], {
      cwd: root,
      detached: true,
      env: { ...desktopEnv, HELMRYTH_SMOKE_KEEP_OPEN: "0" },
      stdio: ["ignore", "pipe", "pipe"],
    });
    try {
  ...
  await stopProcess();
  if (process.env.HELMRYTH_KEEP_SMOKE_DIR !== "1") rmSync(sandbox, { recursive: true, force: true });
