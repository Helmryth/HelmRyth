...
// ~/.helmryth and no real name, key or token can end up in a fixture. The
...
async function main() {
  ...
  writeFileSync(join(home, ".helmryth", "config.json"), JSON.stringify({ profile: PROFILE }));
  ...
  const harness = start("harness", [join(ROOT, "server", "index.ts")], {
    ...
    HELMRYTH_WEBHOOK_PORT: String(base + 1),
  });
  ...
  const sidecar = start("companion", [join(ROOT, "companion", "src", "index.ts")], {
    ...
    HELMRYTH_COMPANION_DIR: join(home, "companion"),
  });
  ...
  write("pair-response", { ...paired.body, token: "hry_REDACTED" });

  // ── a bot, and a few messages for it to have said ──────────────────────
  const created = await json(`${SIDECAR}/api/bots`, asDevice({ method: "POST" }));
  const bot = created.body.bot;
  if (!bot) throw new Error(`could not create an operator: ${JSON.stringify(created.body)}`);
