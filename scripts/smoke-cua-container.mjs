...
const runtime = process.env.HELMRYTH_CUA_RUNTIME || "docker";
const container = process.env.HELMRYTH_CUA_CONTAINER;
const socket = process.env.HELMRYTH_CUA_SOCKET || "/run/user/1000/helmryth-cua.sock";
if (!container || !/^[a-zA-Z0-9_.-]+$/.test(container)) {
  throw new Error("set HELMRYTH_CUA_CONTAINER to the explicitly created smoke container name");
}
...
const child = spawn(
  ...
  [
    ...
    container,
    "/usr/local/libexec/helmryth/cua-driver",
    "mcp",
    "--socket",
    socket,
  ],
...
try {
  await rpc("initialize", {
    ...
    capabilities: {},
    clientInfo: { name: "helmryth-container-smoke", version: "1" },
  });
