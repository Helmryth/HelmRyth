...
//
// A compiled server can die only after packaging when `tsc` leaves a bare
// dependency import and the packaged app carries no node_modules. Unit tests
// run inside the repo, where that import resolves, and an existence check is
// not execution proof.
//
...
const staging = mkdtempSync(join(tmpdir(), "hry-smoke-"));
const home = mkdtempSync(join(tmpdir(), "hry-smoke-home-"));
const port = 21000 + Math.floor(Math.random() * 9000);

const smokeEnvironment = (profileHome, serverPort) => {
  const environment = {
    HOME: profileHome,
    USERPROFILE: profileHome,
    HELMRYTH_PORT: String(serverPort),
  };
...
// HELMRYTH_SMOKE_DIST lets the release workflow aim this at a packaged app's
// Resources/server tree instead of the repo build.
cpSync(process.env.HELMRYTH_SMOKE_DIST ?? join(root, "dist-server"), join(staging, "server"), { recursive: true });

const child = spawn(process.execPath, [join(staging, "server", "index.js")], {
  cwd: staging,
  env: smokeEnvironment(home, port),
...
// A server can answer /api/health while its spawned proxies point outside
// Resources/server. Check the paths the server ACTUALLY
...
if (
  ...
  mcpReport.responses?.find((response) => response.id === 1)?.result?.serverInfo?.name !== "helmryth-mcp" ||
