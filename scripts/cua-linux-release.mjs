...
// Pin the complete upstream archive shape, even though Helmryth extracts
...
async function validateBinaryManifest(binary) {
  ...
  const manifestResult = CuaManifestSchema.safeParse(JSON.parse(stdout));
  if (!manifestResult.success) {
    throw new Error("staged CUA Driver returned an incompatible manifest");
  }
  const invocationPath = await realpath(
    manifestResult.data.mcp_invocation.command,
  ).catch(() => null);
  if (invocationPath !== (await realpath(binary))) {
    throw new Error("staged CUA Driver returned an incompatible manifest");
  }
}
...
async function readVerifiedArchive({
  ...
  try {
    const response = await fetchImpl(LINUX_CUA_RELEASE.url, {
      headers: { "user-agent": "Helmryth-packager" },
