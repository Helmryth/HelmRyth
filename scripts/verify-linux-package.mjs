...
const CycloneComponentSchema = z
  .object({
    ...
    hashes: z
      .array(
        z
          .object({ alg: z.string().min(1), content: z.string().min(1) })
          .passthrough(),
      )
      .optional(),
...
const CycloneDxSchema = z
  .object({
    components: z.array(CycloneComponentSchema),
    dependencies: z
      .array(
        z
          .object({
  ...
  .passthrough();
const CuaManifestSchema = z
  .object({
    schema_version: z.literal("1"),
    binary_version: z.literal("0.19.3"),
    mcp_invocation: z
      .object({
...
function readSbom(licenses, label) {
  const result = CycloneDxSchema.safeParse(
    JSON.parse(readFileSync(path.join(licenses, "SBOM.cdx.json"), "utf8")),
  );
  if (!result.success) fail(`${label} SBOM does not match the CycloneDX package contract`);
  return result.data;
}
...
function verifyCompliance(licenses, label) {
  ...
  const sbom = readSbom(licenses, label);
  ...
  for (const component of registry) {
    const packageId = component.properties?.find(
      (property) => property.name === "helmryth:cargo:package-id",
    )?.value;
    if (packageId?.startsWith("registry+") !== true) {
...
function verifyCuaResources(resources, label, {
  ...
  const manifestResult = CuaManifestSchema.safeParse(
    JSON.parse(
  ...
  try {
    invocationPath = realpathSync(manifestResult.data.mcp_invocation.command);
  } catch {}
  if (invocationPath !== realpathSync(driver)) {
    fail(`${label} CUA manifest does not match the packaged executable`);
  }
...
const executable = path.join(unpacked, "helmryth");
...
const extracted = mkdtempSync(path.join(tmpdir(), "hry-deb-verify-"));
try {
  execFileSync("dpkg-deb", ["--extract", deb, extracted]);
  const debAppRoot = path.join(extracted, "opt", "Helmryth");
  ...
  for (const expected of [
    "Name=Helmryth",
    "Exec=/opt/Helmryth/helmryth %U",
    "Icon=helmryth",
    "StartupWMClass=com.helmryth.app",
    "Categories=Utility;",
  ]) {
...
const appImageExtracted = mkdtempSync(path.join(tmpdir(), "hry-appimage-verify-"));
try {
