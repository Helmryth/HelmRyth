...
const override = process.env.HELMRYTH_ANDROID_PLATFORM_TOOLS_SOURCE;
const temporary = mkdtempSync(join(tmpdir(), "helmryth-android-tools-"));
const staged = join(temporary, platform);

try {
  if (override) {
    if (!existsSync(join(override, process.platform === "win32" ? "adb.exe" : "adb"))) {
      throw new Error(`HELMRYTH_ANDROID_PLATFORM_TOOLS_SOURCE has no adb: ${override}`);
    }
