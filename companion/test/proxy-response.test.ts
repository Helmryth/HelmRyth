...
import { serverPort } from "./helpers.ts";
...
const listen = (server: Server): Promise<number> =>
  new Promise((resolve) => server.listen(0, "127.0.0.1", () => resolve(serverPort(server))));
...
beforeAll(async () => {
  harness = createServer((req, res) => {
    companionMarker = String(req.headers["x-helmryth-companion"] ?? "");
    respond(res);
  });
...
describe("preparing a harness response for a device", () => {
  ...
  it("requires the Mac to enable cloud desktop for this phone", async () => {
    ...
    try {
      ...
      expect(text).toContain("enable it in Helmryth");
      expect(text).toContain("Settings → Reach");
    } finally {
