...
class FailingPersistRegistry extends DeviceRegistry {
  protected override persist(): void {
...
describe("DeviceRegistry", () => {
  ...
  it("issues a token that authenticates, and never stores it", () => {
    ...
    expect(token.startsWith("hry_")).toBe(true);
  ...
  it("completes a record that is missing its labels", () => {
    ...
    expect(listed.name).toBe("Helmryth Mobile");
  ...
  it("refuses unknown, empty, and near-miss tokens", () => {
    ...
    expect(registry.authenticate("hry_nope")).toBeNull();
  ...
  it("points an out-of-window pairing attempt to Reach settings", () => {
    expect(new DeviceRegistry().redeem("000000", "iPhone")).toEqual({
      error: "no pairing is in progress — open Reach settings on your workbench",
    });
  });
  ...
  it("replays one logical redemption without creating an orphan device", () => {
    ...
    expect(registry.redeem("hry_pair_wrong", "iPhone", requestId)).toMatchObject({
      error: expect.stringContaining("no pairing"),
    });
  });
  ...
  it("rolls cloud desktop access back when it cannot be saved", () => {
    const registry = new SwitchablePersistRegistry();
    const { token, device } = pair(registry);
  ...
  it("uses a high-entropy QR credential and burns the manual fallback with it", () => {
    ...
    expect(token).toMatch(/^hry_pair_[A-Za-z0-9_-]{43}$/);
...
describe("authenticate under a failing disk", () => {
  it("still authenticates when the lastSeenAt write throws", () => {
    ...
    registry.failWrites = true;
    registry.writeAttempts = 0;

    expect(registry.authenticate(token)?.id).toBe(device.id);
    expect(registry.writeAttempts).toBe(1);
...
describe("a pairing that cannot be saved", () => {
  ...
  it("is not left live in memory", () => {
    const registry = new FailingPersistRegistry();
...
describe("cleanDeviceName", () => {
  it("clamps, trims, and strips control characters", () => {
    expect(cleanDeviceName("  Divyam's iPhone  ")).toBe("Divyam's iPhone");
  ...
  it("falls back rather than allowing an empty label", () => {
    expect(cleanDeviceName("")).toBe("Helmryth Mobile");
    expect(cleanDeviceName(undefined)).toBe("Helmryth Mobile");
    expect(cleanDeviceName("   ")).toBe("Helmryth Mobile");
  });
});
...
describe("bearerToken", () => {
  it("reads only a well-formed Bearer header", () => {
    expect(bearerToken("Bearer hry_abc")).toBe("hry_abc");
    expect(bearerToken("  Bearer hry_abc  ")).toBe("hry_abc");
    expect(bearerToken("hry_abc")).toBeUndefined();
    expect(bearerToken("Basic hry_abc")).toBeUndefined();
  ...
  it("matches the scheme however it is cased", () => {
    ...
    expect(bearerToken("Bearer   ")).toBeUndefined();
    expect(bearerToken("Bearerhry_abc")).toBeUndefined();
  });
});
