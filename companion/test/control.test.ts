...
import { jsonObject, parseJson, type JsonObject } from "../src/json.ts";
import { requiredArray, requiredObject, requiredString, serverPort } from "./helpers.ts";

class SwitchablePersistRegistry extends DeviceRegistry {
  failWrites = false;
...
const ask = async (
  ...
  const responseBody = jsonObject(parseJson(await res.text()));
  if (!responseBody) throw new Error(`expected ${method} ${path} to return a JSON object`);
  return { status: res.status, body: responseBody };
};

const pairingIn = (body: JsonObject): JsonObject | null =>
  body.pairing === null ? null : requiredObject(body.pairing, "pairing");

const endpointsIn = (body: JsonObject): JsonObject[] =>
  requiredArray(body.endpoints, "endpoints")
    .map((endpoint, index) => requiredObject(endpoint, `endpoints[${index}]`));

beforeAll(async () => {
  devices = new SwitchablePersistRegistry();
  ...
  port = await new Promise<number>((resolve) =>
    control.listen(0, "127.0.0.1", () => resolve(serverPort(control))),
  );
});
...
describe("origins the control server will change state for", () => {
  ...
  it("reports a permission write failure without dropping the control server", async () => {
    ...
    if (!(devices instanceof SwitchablePersistRegistry)) throw new Error("expected the test registry");
    devices.failWrites = true;
    try {
  ...
  it("refuses a state change from a foreign page", async () => {
    ...
    expect(requiredString(body.error, "error")).toContain("cross-origin");
    // and it did not happen anyway
    expect(pairingIn((await ask("GET", "/state")).body)).toBeNull();
  });
  ...
  it("allows the page it serves, and no other loopback origin", async () => {
    ...
    expect(pairingIn((await ask("GET", "/state")).body)).not.toBeNull();
  ...
  it("does not let a stale conditional close cancel a replacement code", async () => {
    ...
    const firstToken = requiredString(pairingIn(first.body)?.token, "first pairing token");
    const secondToken = requiredString(pairingIn(second.body)?.token, "second pairing token");
    await ask("DELETE", `/pairing?expectedToken=${encodeURIComponent(firstToken)}`);
    expect(requiredString(pairingIn((await ask("GET", "/state")).body)?.token, "pairing token")).toBe(secondToken);
    await ask("DELETE", `/pairing?expectedToken=${encodeURIComponent(secondToken)}`);
    expect(pairingIn((await ask("GET", "/state")).body)).toBeNull();
  });
...
describe("hostCandidates", () => {
  it("orders by reachability: tailnet name, then LAN, then the mDNS name last", () => {
    ...
    expect(hosts.at(-1)).toMatch(/^helmryth-[0-9a-f]{8}\.local$/);
  ...
  it("is what /state hands the pairing panel", async () => {
    ...
    const hosts = requiredArray(body.hosts, "hosts");
    const endpoints = endpointsIn(body);
    // Whatever this machine's interfaces are, the mDNS fallback is always
    // present and always last.
    expect(hosts.at(-1)).toMatch(/^helmryth-[0-9a-f]{8}\.local$/);
    expect(endpoints.at(-1)).toMatchObject({ kind: "bonjour", priority: 300 });
    expect(requiredString(endpoints.at(-1)?.url, "last endpoint URL")).toMatch(/^http:\/\/helmryth-[0-9a-f]{8}\.local:8810$/);
  });

  it("reports only the device ids backed by live authenticated streams", async () => {
    connectedDeviceIds = ["phone-live"];
    try {
      expect(requiredArray((await ask("GET", "/state")).body.connectedDeviceIds, "connectedDeviceIds")).toEqual(["phone-live"]);
    } finally {
      connectedDeviceIds = [];
    }
  });
  ...
  it("disconnects streams only after a device is successfully revoked", async () => {
    ...
    expect(requiredArray(revoked.body.connectedDeviceIds, "connectedDeviceIds")).toEqual([]);
  });
});
...
describe("hosted endpoint advertisement", () => {
  it("publishes and withdraws only a complete HTTPS origin", async () => {
    ...
    const published = await ask(
      ...
      "/hosted-endpoint",
      headers,
      JSON.stringify({ url: "https://C-Opaque.Helmryth.Test/" }),
    );
    expect(published.status).toBe(200);
    expect(endpointsIn(published.body)[0]).toEqual({
      kind: "hosted",
      priority: 0,
      url: "https://c-opaque.helmryth.test",
    });

    expect(
      (await ask("PUT", "/hosted-endpoint", headers, JSON.stringify({ url: "http://unsafe.test" }))).status,
    ).toBe(400);
    expect(endpointsIn((await ask("GET", "/state")).body)[0]).toMatchObject({ kind: "hosted" });
    ...
    expect(endpointsIn(withdrawn.body).some((endpoint) => endpoint.kind === "hosted")).toBe(false);
  });
  ...
  it("refuses a hosted-endpoint body larger than 4096 bytes", async () => {
    ...
    expect(endpointsIn((await ask("GET", "/state")).body).some(
      (endpoint) => endpoint.kind === "hosted",
    )).toBe(false);
  });
});
