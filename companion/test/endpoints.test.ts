...
describe("hostedCompanionUrl", () => {
  it("normalizes one explicit HTTPS origin", () => {
    expect(hostedCompanionUrl("  https://Sigil.Example/  ")).toBe("https://sigil.example");
  ...
  it("refuses insecure or ambiguous hosted routes", () => {
    for (const value of [
      "http://sigil.example",
      "https://user:secret@sigil.example",
      "https://sigil.example/companion",
      "https://sigil.example?device=one",
      "https://sigil.example#pair",
      "not a URL",
    ]) {
      expect(() => hostedCompanionUrl(value)).toThrow(/HELMRYTH_COMPANION_HOSTED_URL/);
    }
  });
});
...
describe("companionEndpointCandidates", () => {
  ...
  it("keeps direct routes when no hosted route exists", () => {
    expect(
      companionEndpointCandidates(8810, ["192.168.1.42"], null, null, "helmryth-abcd1234.local"),
    ).toEqual([
  ...
  it("caps pathological interface lists without losing the Bonjour fallback", () => {
    ...
    expect(endpoints.at(-1)).toEqual({
      url: "http://helmryth-abcd1234.local:8810",
      kind: "bonjour",
      priority: 300,
    });
  });
