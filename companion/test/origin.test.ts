...
describe("private managed origin", () => {
  it("accepts only the one-generation UDS or named-pipe shape", () => {
    ...
    expect(
      companionOriginSocket("/tmp/hry-reach-origin-test/origin.sock", "linux"),
    ).toBe("/tmp/hry-reach-origin-test/origin.sock");
    expect(
      companionOriginSocket(
        "\\\\.\\pipe\\helmryth-reach-origin-42-12345678-1234-1234-1234-123456789abc",
        "win32",
      ),
    ).toContain("helmryth-reach-origin-42");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const directory = fs.mkdtempSync(path.join(os.tmpdir(), "hry-reach-origin-test-"));
      directories.push(directory);
      fs.chmodSync(directory, 0o700);
      const socketPath = path.join(directory, "origin.sock");
      const server = createServer((_incoming, response) => {
        response.writeHead(200, { "content-type": "application/json" });
        response.end(JSON.stringify({ app: "helmryth" }));
      });
      ...
      expect(JSON.parse(body)).toEqual({ app: "helmryth" });
