...
import { jsonObject, parseJson, type JsonObject, type JsonValue } from "../src/json.ts";
import {
  requiredArray,
  requiredObject,
  requiredString,
...
const device = async (
  method: string,
  path: string,
  opts: { token?: string | null; body?: JsonValue; headers?: Record<string, string> } = {},
): Promise<{ status: number; body: JsonObject; headers: Headers }> => {
  const token = opts.token === undefined ? TOKEN : opts.token;
  const headers: DeviceHeaders = { ...opts.headers };
  ...
  if (!body) throw new Error(`expected ${method} ${path} to return a JSON object`);
  return { status: res.status, body, headers: res.headers };
};

const botsIn = (body: JsonObject): JsonObject[] =>
  requiredArray(body.bots, "bots").map((value, index) => requiredObject(value, `bots[${index}]`));
...
beforeAll(async () => {
  ...
  const harnessEnv: NodeJS.ProcessEnv = {
    HOME: home,
    USERPROFILE: home,
    HELMRYTH_PORT: String(HARNESS_PORT),
  };
  if (process.env.PATH) harnessEnv.PATH = process.env.PATH;
  if (process.env.SystemRoot) harnessEnv.SystemRoot = process.env.SystemRoot;
...
describe("the sidecar in front of an unmodified harness", () => {
  ...
  it("rejects an event stream revoked while upstream headers are pending", async () => {
    ...
    const delayedHarnessPort = serverPort(delayedHarness);
    ...
    const delayedProxyPort = serverPort(delayedProxy);
  ...
  it("requires a paired token", async () => {
    ...
    expect((await device("GET", "/api/bots", { token: "hry_wrong" })).status).toBe(401);
  ...
  it("serves a minimal, non-cacheable companion health identity", async () => {
    ...
    expect(health.body).toEqual({ app: "helmryth" });
  ...
  it("refuses what a device has no business doing, by default", async () => {
    ...
    const botId = requiredString(botsIn(body)[0]?.id, "bots[0].id");
  ...
  it("lets a device answer an approval, and manage its own chats", async () => {
    ...
    const { body } = await device("GET", "/api/bots");
    const bot = botsIn(body)[0];
    if (!bot) throw new Error("expected a bot fixture");
    const botId = requiredString(bot.id, "bot.id");
    const threadId = requiredString(bot.threadId, "bot.threadId");

    for (const [method, path, payload] of [
      ["POST", `/api/threads/${threadId}/respond`, { requestId: "nope", behavior: "allow" }],
      ["POST", `/api/bots/${botId}/messages`, { text: "hello from a test" }],
      ["POST", `/api/bots/${botId}/interrupt`, undefined],
      ["POST", `/api/bots/${botId}/read`, undefined],
    ] as const) {
  ...
  it("only remembers an always-allow key carried by a pending card", async () => {
    ...
    if (!bot) throw new Error("expected a bot fixture");
    const botId = requiredString(bot.id, "bot.id");
    const attempt = await device("POST", `/api/bots/${botId}/always-allow`, {
      body: { allowKey: "Bash" },
    });
    expect(attempt.status).toBe(409);

    const refreshed = await device("GET", "/api/bots");
    const refreshedBot = botsIn(refreshed.body).find((candidate) => candidate.id === botId);
    expect(refreshedBot?.alwaysAllow ?? []).not.toContain("Bash");
  });
  ...
  it("never passes the provider session cursors through", async () => {
    ...
    for (const bot of botsIn(listed.body)) {
      expect(bot).not.toHaveProperty("resumeCursors");
      const tasks = bot.tasks === undefined ? [] : requiredArray(bot.tasks, "bot.tasks");
      for (const task of tasks) expect(task).not.toHaveProperty("resumeCursors");
    }
  ...
  it("says the harness is down rather than hanging, when it is", async () => {
    ...
    const port = serverPort(orphan);
    try {
      const res = await fetch(`http://127.0.0.1:${port}/api/bots`, {
        headers: { authorization: `Bearer ${TOKEN}` },
      });
      expect(res.status).toBe(502);
      expect(requiredString((await responseObject(res)).error, "error")).toContain("not running");
    } finally {
  ...
  it("gives up on a harness that accepts a connection and then goes quiet", async () => {
    ...
    const mutePort = serverPort(mute);
    ...
    try {
      ...
      expect(requiredString((await responseObject(res)).error, "error")).toContain("did not respond");
  ...
  it("hangs up on the harness when the device disappears", async () => {
    ...
    const slowPort = serverPort(slow);
  ...
  it("ends a stream whose event never terminates, rather than buffering it", async () => {
    ...
    const floodPort = serverPort(flood);
    const relay = createServer(
      createProxyHandler({
...
describe("live companion endpoint refresh", () => {
  it("requires a valid paired bearer and reflects hosted add/remove without restart", async () => {
    ...
    const port = serverPort(endpointServer);
    ...
    try {
      ...
      endpoints = [
        { kind: "hosted", priority: 0, url: "https://c-opaque.helmryth.test" },
      ...
      const bounded = await responseObject(await load(TOKEN));
      const boundedEndpoints = requiredArray(bounded.endpoints, "endpoints")
        .map((endpoint, index) => requiredObject(endpoint, `endpoints[${index}]`));
      expect(boundedEndpoints).toHaveLength(8);
      expect(
        boundedEndpoints.every(
  ...
  it("turns a QR credential into a working device token", async () => {
    ...
    const paired = createServer(
      createProxyHandler({
        ...
        hosts: () => ["macbook.tail1234.ts.net", "192.168.1.42", "helmryth-abcd1234.local"],
        endpoints: () => [
    ...
    const port = serverPort(paired);
    const control = createControlServer({
      devices: registry,
      companionPort: port,
      discovery: () => ({ advertising: false, name: "Helmryth" }),
    ...
    const controlPort = serverPort(control);
    ...
    try {
      ...
      const opened = await responseObject(await fetch(`${ctl}/pairing`, { method: "POST" }));
      const pairingCode = requiredString(opened.code, "code");
      const pairingToken = requiredString(opened.token, "token");
      expect(pairingCode).toMatch(/^\d{6}$/);
      expect(pairingToken).toMatch(/^hry_pair_[A-Za-z0-9_-]{43}$/);
      ...
      const deviceToken = requiredString(body.token, "token");
      expect(requiredString(body.serverName, "serverName")).toBe("Ada's computer");
      expect(deviceToken).toMatch(/^hry_/);
      ...
      expect(requiredArray(body.hosts, "hosts")).toEqual(["macbook.tail1234.ts.net", "192.168.1.42", "helmryth-abcd1234.local"]);
      expect(requiredArray(body.endpoints, "endpoints")).toEqual([
        { url: "https://device-123.companion.example", kind: "hosted", priority: 0 },
        { url: "http://192.168.1.42:8810", kind: "lan", priority: 200 },
      ]);
      ...
      expect(requiredString((await responseObject(replay)).token, "token")).toBe(deviceToken);

      // and the token works on the real API, through the real proxy
      const bots = await fetch(`${base}/api/bots`, { headers: { authorization: `Bearer ${deviceToken}` } });
      ...
      const state = await responseObject(await fetch(`${ctl}/state`));
      const stateDevices = requiredArray(state.devices, "devices")
        .map((device, index) => requiredObject(device, `devices[${index}]`));
      expect(stateDevices.map((device) => requiredString(device.name, "device.name"))).toContain("Ada's iPhone");
      ...
      const ada = stateDevices.find((device) => device.name === "Ada's iPhone");
      if (!ada) throw new Error("expected Ada's paired device");
      const adaId = requiredString(ada.id, "device.id");
      expect(requiredArray(state.connectedDeviceIds, "connectedDeviceIds")).toContain(adaId);
      const revoked = await fetch(`${ctl}/devices/${adaId}`, { method: "DELETE" });
      expect(revoked.status).toBe(200);
      expect(requiredArray((await responseObject(revoked)).connectedDeviceIds, "connectedDeviceIds")).not.toContain(adaId);
      await eventStreamClosed;
      expect((await fetch(`${base}/api/bots`, { headers: { authorization: `Bearer ${deviceToken}` } })).status).toBe(401);
    } finally {
