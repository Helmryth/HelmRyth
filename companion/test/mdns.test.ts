...
const service: ServiceInfo = {
  name: "Divyam's computer",
  type: "_helmryth._tcp",
  port: 8800,
  host: "helmryth-1a2b3c4d.local",
  addresses: ["192.168.1.42"],
  txt: ["v=1", "name=Divyam's computer"],
};

const INSTANCE = "Divyam's computer._helmryth._tcp.local";
const SERVICE_NAME = "_helmryth._tcp.local";
...
describe("naming", () => {
  it("keeps an instance name to a single valid label", () => {
    ...
    expect(dnsLabel("")).toBe("Helmryth");
    expect(dnsLabel("   ")).toBe("Helmryth");
  });

  it("claims a host name the system responder will not fight us for", () => {
    const name = defaultHostName("Divyams-MacBook-Pro");
    expect(name).toMatch(/^helmryth-[0-9a-f]{8}\.local$/);
    // stable across restarts, distinct per machine
    expect(defaultHostName("Divyams-MacBook-Pro")).toBe(name);
    expect(defaultHostName("another-machine")).not.toBe(name);
    expect(name).not.toContain("Divyams-MacBook-Pro");
  });
...
describe("isOnLink", () => {
  ...
  const interfaces = {
    ...
    en0: [{ family: "IPv4", address: "192.168.1.42", netmask: "255.255.255.0" }],
    ts0: [{ family: "IPv4", address: "100.102.178.88", netmask: "255.192.0.0" }],
  } satisfies NetworkInterfaceTable;
