...
import { responseObject, serverPort } from "./helpers.ts";

const listen = (server: Server): Promise<number> =>
  new Promise((resolve) =>
    server.listen(0, "127.0.0.1", () => resolve(serverPort(server))),
  );
...
describe("an upstream that fails mid-stream", () => {
  it("does not take the sidecar down with it", async () => {
    ...
    const escaped = await watchingForCrashes(async () => {
      const res = await fetch(`${base}/api/events`, { headers: { authorization: "Bearer hry_x" } });
      ...
      try {
        ...
        while (!(await reader.read()).done) {
          // Drain until the deliberately destroyed stream rejects or ends.
        }
      } catch {
    ...
    const after = await fetch(`${base}/api/health`, { headers: { authorization: "Bearer hry_x" } });
  ...
  it("ends the phone's request when an image stops half way", async () => {
    ...
    const escaped = await watchingForCrashes(async () => {
      const res = await fetch(`${base}/api/threads/t1/messages/m1/image`, {
        headers: { authorization: "Bearer hry_x" },
      });
      expect(res.status).toBe(200);
  ...
  it("still says 502 when the harness was never there to begin with", async () => {
    ...
    const escaped = await watchingForCrashes(async () => {
      const res = await fetch(`${base}/api/health`, { headers: { authorization: "Bearer hry_x" } });
      expect(res.status).toBe(502);
      expect(await responseObject(res)).toEqual({
