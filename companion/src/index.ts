...
import { jsonObject, jsonString, parseJson } from "./json.ts";
...
const HARNESS_PORT = num(process.env.HELMRYTH_PORT, 8799);
const WEBHOOK_PORT = num(process.env.HELMRYTH_WEBHOOK_PORT, HARNESS_PORT + 1);
const COMPANION_PORT = num(process.env.HELMRYTH_COMPANION_PORT, 8810);
const CONTROL_PORT = num(process.env.HELMRYTH_CONTROL_PORT, 8811);
const SERVICE_TYPE = "_helmryth._tcp";
let hostedUrl = hostedCompanionUrl(process.env.HELMRYTH_COMPANION_HOSTED_URL);
const PRIVATE_ORIGIN = companionOriginSocket(process.env.HELMRYTH_COMPANION_INTERNAL_ORIGIN);
...
let cachedName = process.env.HELMRYTH_COMPANION_NAME?.trim() || "";

/** What this computer is called on the phone. Never empty. */
const machineName = (): string => cachedName || "Helmryth";
...
async function refreshMachineName(): Promise<void> {
  ...
  try {
    ...
    if (!res.ok) return;
    const configValue = parseJson(await res.text());
    const config = jsonObject(configValue);
    const profile = config ? jsonObject(config.profile) : null;
    const owner = jsonString(profile?.name)?.trim();
    if (owner) cachedName = `${owner}'s computer`;
  } catch {
    /* not up, or no profile — "Helmryth" is a fine thing to be called */
  }
}
  ...
  new Promise((resolve, reject) => {
    const onError = (error: NodeJS.ErrnoException) => {
      ...
      const hint = ` — another copy of the companion may already be running; ${
        port === COMPANION_PORT ? "HELMRYTH_COMPANION_PORT" : "HELMRYTH_CONTROL_PORT"
...
async function main(): Promise<void> {
  const clash =
    conflict("HELMRYTH_COMPANION_PORT", COMPANION_PORT) ?? conflict("HELMRYTH_CONTROL_PORT", CONTROL_PORT);
  ...
  if (COMPANION_PORT === CONTROL_PORT) {
    throw new Error(
      `HELMRYTH_COMPANION_PORT and HELMRYTH_CONTROL_PORT are both port ${COMPANION_PORT}, and they cannot share one: ` +
...
const shutdown = async (signal: string): Promise<void> => {
  ...
  if (managedOrigin) closers.push(new Promise<void>((r) => managedOrigin.close(() => r())));
