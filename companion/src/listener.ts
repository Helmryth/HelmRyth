    ...
    const name = await new Promise<string | null>((resolve) => {
      execFile(
        ...
        (error, stdout) => {
          ...
          try {
            const status = jsonObject(parseJson(stdout));
            const self = status ? jsonObject(status.Self) : null;
            const dns = jsonString(self?.DNSName);
            // MagicDNS names are fully qualified, trailing dot and all
            const trimmed = dns ? dns.replace(/\.$/, "") : null;
