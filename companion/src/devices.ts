...
import { jsonNumber, jsonObject, jsonString, parseJson, type JsonValue } from "./json.ts";
...
export function cleanDeviceName(raw: JsonValue | undefined): string {
...
const timestamp = (value: JsonValue | undefined, fallback: number): number => {
  const parsed = jsonNumber(value, fallback);
...
function parseDevice(value: JsonValue): DeviceRecord | null {
  const record = jsonObject(value);
  if (!record) return null;
  const id = jsonString(record.id);
  const tokenHash = jsonString(record.tokenHash);
...
export class DeviceRegistry {
  ...
  constructor() {
    try {
      const parsed = jsonObject(parseJson(readFileSync(DEVICES_FILE, "utf8")));
      const storedDevices = parsed?.devices;
      if (Array.isArray(storedDevices)) {
        this.devices = storedDevices.flatMap((value) => {
  ...
  openPairing(): PairingWindow {
    ...
    this.window = {
      ...
      token: `hry_pair_${randomBytes(32).toString("base64url")}`,
  ...
  redeem(
    ...
    const proposedRequestId = jsonString(pairRequestId);
    const requestId = proposedRequestId && /^[A-Za-z0-9._-]{16,128}$/.test(proposedRequestId)
      ? proposedRequestId
      : null;
    ...
    if (!window) return { error: "no pairing is in progress — open Reach settings on your workbench" };
    ...
    const token = `hry_${randomBytes(32).toString("base64url")}`;
    ...
    try {
      ...
      return { error: `could not save the pairing: ${message}` };
    }
    const pub: PublicDevice = {
      id: device.id,
      name: device.name,
      createdAt: device.createdAt,
      lastSeenAt: device.lastSeenAt,
