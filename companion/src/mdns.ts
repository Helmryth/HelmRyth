...
export interface NetworkInterfaceTable {
  [name: string]: readonly NetworkInterfaceAddress[] | undefined;
}
 ...
 * itself.
 */
export function isOnLink(from: string, interfaces: NetworkInterfaceTable = networkInterfaces()): boolean {
 ...
 * record here:
 *
 * - **Shared records (§10.2).** The PTR of `_helmryth._tcp.local` is
...
export interface ServiceInfo {
  ...
  type: string;
  port: number;
  /** the name our A records claim, e.g. "helmryth-1a2b3c4d.local" */
  host: string;
  addresses: string[];
...
interface ServiceRecords {
  serviceName: string;
  instance: string;
  ptr: Extract<ResourceRecord, { type: 12 }>;
  srv: Extract<ResourceRecord, { type: 33 }>;
  txt: Extract<ResourceRecord, { type: 16 }>;
  addresses: Array<Extract<ResourceRecord, { type: 1 }>>;
}
...
export function serviceRecords(service: ServiceInfo): ServiceRecords {
  ...
  const ptr: Extract<ResourceRecord, { type: 12 }> = { name: serviceName, type: TYPE.PTR, data: instance };
  const srv: Extract<ResourceRecord, { type: 33 }> = {
    name: instance,
    type: TYPE.SRV,
    data: { port: service.port, target: service.host },
  };
  const txt: Extract<ResourceRecord, { type: 16 }> = {
    name: instance,
    type: TYPE.TXT,
    data: service.txt,
  };
  const addresses: Array<Extract<ResourceRecord, { type: 1 }>> = service.addresses.map((address) => ({
    name: service.host,
    type: TYPE.A,
    data: address,
  }));
...
export function dnsLabel(text: string, fallback = "Helmryth"): string {
  const printable = text.split("").filter((character) => {
