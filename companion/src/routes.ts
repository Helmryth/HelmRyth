...
const EXPLAINED: ReadonlyArray<{ path: RegExp; error: string }> = [
  {
    ...
    error: "Reach settings are managed on your workbench",
  },
  { path: /^\/api\/config$/, error: "API keys can only be changed on your workbench" },
  { path: /^\/api\/local-computer(\/|$)/, error: "the Local VM is set up on your workbench" },
  {
    ...
    path: /^\/api\/webhooks(\/|$)/,
    error: "webhooks are set up on your workbench",
  },
  { path: /^\/api\/connectors(\/|$)/, error: "connected apps are set up on your workbench" },
  {
    path: /^\/api\/routines(\/|$)/,
    error: "this routine operation is only available on your workbench",
  },
  { path: /^\/api\/teams(\/|$)/, error: "teams are imported and exported on your workbench" },
];
...
export function denyReason({ path, method, authenticated }: RouteRequest): Denial | null {
  ...
  if (!authenticated) {
    return { status: 401, error: "pair this device from Reach settings in Helmryth on your workbench" };
  }
