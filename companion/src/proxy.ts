...
import { bearerToken, type PublicDevice } from "./devices.ts";
import {
...
export interface ProxyOptions {
  ...
  redeem: (
    ...
    pairRequestId?: JsonValue,
  ) => { token: string; device: PublicDevice } | { error: string };
...
const readJson = (req: IncomingMessage, limit = 64 * 1024): Promise<JsonObject> =>
  new Promise((resolve, reject) => {
    ...
    req.on("end", () => {
      ...
      try {
        resolve(jsonObject(parseJson(text)) ?? {});
      } catch {
...
const sendJson = (res: ServerResponse, status: number, body: JsonResponseBody): void => {
  if (res.headersSent) {
    res.destroy();
    return;
  }
...
const endpointSnapshot = (options: ProxyOptions): CompanionEndpointSnapshot => {
  ...
  for (const candidate of options.endpoints?.() ?? []) {
    ...
    const candidateUrl = jsonString(candidate.url);
    if (
      ...
      Buffer.byteLength(candidateUrl) > 2_048
    ) {
      continue;
    }
    let parsed: URL;
...
interface ForwardHeaders extends OutgoingHttpHeaders {
  accept: string;
  "x-helmryth-companion": string;
  "content-type"?: string;
  "last-event-id"?: string;
}

const forwardHeaders = (req: IncomingMessage): ForwardHeaders => {
  const out: ForwardHeaders = {
    ...
    "x-helmryth-companion": "1",
  };
  const contentType = req.headers["content-type"];
  if (contentType) {
    out["content-type"] = String(contentType);
  } else {
    const method = String(req.method ?? "GET").toUpperCase();
    const path = new URL(req.url ?? "/", "http://companion.invalid").pathname;
    // The native companion has historically omitted Content-Type for
    // bodyless actions such as read/run. The core now gives every public JSON
    // mutation one media contract, so the trusted native gateway supplies
    // that declaration. Raw image upload keeps the device's actual type.
    if (["POST", "PUT", "PATCH", "DELETE"].includes(method) && path !== "/api/attachments") {
  ...
  return function handle(req: IncomingMessage, res: ServerResponse): void {
    ...
    if (isCloudDesktopJoin(method, path) && !device?.cloudDesktopAccess) {
      return sendJson(res, 403, {
        error: "cloud desktop access is off for this phone — enable it in Helmryth → Settings → Reach",
      });
    }
      ...
      (harness) => {
        ...
        if (method === "GET" && path === "/api/health") {
          ...
          const fail = () => {
            if (finished) return;
            finished = true;
            sendJson(res, 502, { error: "Helmryth is not ready on this computer" });
          };
          ...
          harness.on("end", () => {
            ...
            try {
              identity = parseJson(Buffer.concat(chunks).toString("utf8"));
            } catch {
              fail();
              return;
            }
            ...
            if (
              ...
              identityObject?.app !== "helmryth"
            ) {
              fail();
              return;
            }
            finished = true;
            sendJson(res, 200, { app: "helmryth" });
          });
          return;
        }
        ...
        if (isStream) {
          ...
          if (tracksDeviceConnection && currentDevice?.id !== device?.id) {
            ...
            return sendJson(res, 401, {
              error: "pair this device from Reach settings in Helmryth on your workbench",
            });
          }
          const disconnect = () => {
        ...
        harness.on("data", (chunk: Buffer) => {
          ...
          if (size > MAX_JSON_BODY_BYTES) {
            ...
            else sendJson(res, 502, { error: "the response from Helmryth was too large" });
            return;
          }
          chunks.push(chunk);
        });
    ...
    upstream.on("error", () => {
      ...
      sendJson(
        ...
        timedOut ? 504 : 502,
        timedOut
          ? { error: "Helmryth did not respond" }
          : { error: "Helmryth is not running on this computer" },
      );
    });
    req.pipe(upstream);
  };
