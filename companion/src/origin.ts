...
const RUNTIME_PREFIX = "hry-reach-origin-";
...
export function companionOriginSocket(
  ...
  if (platform === "win32") {
    return /^\\\\\.\\pipe\\helmryth-reach-origin-[1-9][0-9]*-[0-9a-f-]{36}$/i.test(value)
      ? value
      : null;
  }
  if (
  ...
  return new Promise((resolve, reject) => {
    ...
    const onListening = () => {
      ...
      server.on("error", (error: Error) => {
        console.warn(`companion: Helmryth Reach origin error — ${error.message}`);
      });
      resolve();
    };
