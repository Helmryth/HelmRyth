...
import { jsonObject, jsonString, parseJson, type JsonResponseBody, type JsonValue } from "./json.ts";
...
const json = (res: ServerResponse, status: number, body: JsonResponseBody) => {
...
const parseHostedEndpointPayload = (value: JsonValue): HostedEndpointPayload | null => {
  const object = jsonObject(value);
  if (!object) return null;
  const keys = Object.keys(object);
  if (keys.length !== 1 || keys[0] !== "url") return null;
  const valueUrl = object.url;
  if (valueUrl === null) return { url: null };
  const url = jsonString(valueUrl);
...
const readHostedEndpoint = (
  ...
  new Promise((resolve, reject) => {
    ...
    req.on("end", () => {
      try {
        const parsed = parseHostedEndpointPayload(parseJson(Buffer.concat(chunks).toString("utf8")));
...
export function companionState(options: ControlOptions) {
  ...
  const network: CompanionNetwork = {};
  if (tailscale) network.tailscale = tailscale;
  if (tailscale && name) network.tailnetName = name;
  return {
...
<title>Helmryth Reach</title>
<style>
  :root { color-scheme: light; --fg: #22303a; --dim: #5f6b72; --line: #d7d1c8; --bg: #f7f2ea; --card: #fffdfa; --accent: #b85633; --accent-soft: #f3d9cf; }
  @media
...
<main>
  <h1>Helmryth Reach</h1>
  <p class="sub">Your phone reaches this workbench through here. Only pair a device you trust.</p>
...
function render(s) {
  ...
  el("where").innerHTML =
    ...
    (s.tailscale && !s.tailnetName
      ? "<p class=dim>On a tailnet, but this workbench's MagicDNS name could not be read from Tailscale — either MagicDNS is off, or its command line tool is not where we looked. iPhones cannot connect to a bare tailnet address; the console output lists what was tried.</p>"
      : "") +
    (!s.tailscale
      ? "<p class=dim>Reachable on this network only. Install Tailscale on both this workbench and your phone to reach it from anywhere — including networks that stop devices from seeing each other.</p>"
      : "") +
    (s.discovery.advertising
      ? "<p class=dim>Your phone can also find this workbench as \\u201c" + esc(s.discovery.name) + "\\u201d.</p>"
      : "");
  ...
  el("devices").innerHTML =
    ...
    (s.devices.length
      ? "<ul>" + s.devices.map((d) =>
          ...
          (d.cloudDesktopAccess ? "Cloud workbench on" : "Allow cloud workbench") + "</button></div>" +
