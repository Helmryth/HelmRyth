...
/** The sigil is a first-class avatar choice; the other values crop an image. */
export const BOT_AVATAR_CROPS = ["sigil", "circle", "rounded", "square"] as const;
 ...
 * this prevents an operator profile from becoming an external tracking pixel or a
 * script-capable SVG.
 */
...
export function botAvatarProfile(value: BotAvatarProfileInput): BotAvatarProfile {
  const profile: BotAvatarProfile = {
    avatarCrop: botAvatarCropSchema.safeParse(value.avatarCrop).data ?? "sigil",
  };
