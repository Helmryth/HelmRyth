...
const FLASH_CLASSES = ["ring-2", "ring-accent/70", "rounded-md"];
...
export async function landOnSearchHit(
  ...
  const bot = hit.botId
    ? state.bots.find((candidate) => candidate.id === hit.botId)
    : undefined;
  const group = hit.groupId
    ? state.groups.find((candidate) => candidate.id === hit.groupId)
    : undefined;
  if (!ownerId || (!bot && !group))
    throw new Error("That workstream record is no longer available.");

  dispatch({ type: "select", id: ownerId });
  if (bot && bot.threadId !== hit.threadId) {
    const result = await api(`/api/bots/${bot.id}/tasks/${hit.threadId}`, {
      method: "POST",
    });
    if (result?.bot) dispatch({ type: "taskSwitched", bot: result.bot });
  }
  if (group && group.threadId !== hit.threadId) {
    const result = await api(`/api/groups/${group.id}/tasks/${hit.threadId}`, {
      method: "POST",
    });
...
export function useFocusMessage(threadId: string, ready: boolean) {
  ...
  useEffect(() => {
    if (!focus || focus.consumed || focus.threadId !== threadId || !ready)
      return;
    ...
    const attempt = () => {
      ...
      const wrapper = document.querySelector<HTMLElement>(
        `[data-mid="${CSS.escape(focus.messageId)}"]`,
      );
      const candidate = wrapper?.lastElementChild;
      target = candidate instanceof HTMLElement ? candidate : null;
      if (!target) {
