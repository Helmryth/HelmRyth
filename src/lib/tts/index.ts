...
export class Speaker {
  ...
  async speak(text: string, opts: SpeakOptions = {}): Promise<void> {
    ...
    type Rendered = { blob: Blob; error?: never } | { blob?: never; error: Error };
  ...
  private async prepare(text: string, voiceId: string | undefined, signal: AbortSignal): Promise<string[]> {
    ...
    if (!body.ready) {
      throw new Error("Add the shared ElevenLabs key in an operator profile on this host, then choose a voice for the operator.");
    }
