...
import { isAcceptedPairingToken, type CompanionEndpoint } from "./companion-pairing";
...
const PAIRING_OPEN_FAILURE_MESSAGE =
  "Helmryth Mobile pairing did not open. Open Pairing trace & recovery, confirm Mobile access is on, then try again.";
...
export function companionPairingOpenFailure(
  ...
  if (
    ...
    || !isAcceptedPairingToken(pairing.token)
...
const START_FAILURE_MESSAGE =
  "Helmryth Mobile could not start. Open Pairing trace & recovery, then turn Mobile access on again.";
...
const PUBLIC_ACCOUNT_MESSAGES = [
  ...
  /^Helmryth could not reach its secure connection service\./,
