...
import { z } from "zod";
import type { JsonObject } from "../../server/schema.js";
import { isAttachment, type Attachment } from "./composer-attachments.js";

const KEY = "hry-drafts";
const ATTACHMENTS_KEY = "hry-draft-attachments";
const SEND_IDS_KEY = "hry-draft-send-ids";
...
const storedValuesSchema = z.record(z.string(), z.json());
...
function read(store: Store, key: string): JsonObject {
  try {
    const raw = store?.getItem(key);
    if (!raw) return {};
    const parsed = storedValuesSchema.safeParse(JSON.parse(raw));
...
export function restoredSendId(draftId: string): string | undefined {
  ...
  restoredSendIds.set(draftId, parsed.data);
  return parsed.data;
}
