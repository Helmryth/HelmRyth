...
describe("intakeFiles", () => {
  ...
  it("names an oversized image once, not twice", async () => {
    const out = await intakeFiles([file("shot.png", "image/png", 20_000_000)], {
      allowImages: true,
      getPath: onDisk,
      // The real uploader refuses with a sentence that already opens with
      // the filename, exactly as imageAttachmentFromFile does over 10 MB.
      uploadImage: async (f) => {
        throw new Error(`${f.name} exceeds 10 MB`);
      },
    });
    expect(out.notice).toBe("shot.png exceeds 10 MB");
  });
});
