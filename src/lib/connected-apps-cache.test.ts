...
import type { JsonValue } from "../../server/schema.js";

import {
  CONNECTED_APPS_CACHE_KEY,
  patchCachedInventoryService,
  readCachedInventory,
  writeCachedInventory,
} from "./connected-apps-cache";
...
describe("connected apps cache", () => {
  ...
  it("rejects malformed service and account records before the panel paints them", () => {
    const cached = (services: JsonValue) => fakeStorage({
  ...
  it("patches an inline connection immediately without persisting its authorization URL", () => {
    const storage = fakeStorage();
    writeCachedInventory({ gmail: { connected: false } }, 1, storage);
    patchCachedInventoryService("gmail", { connected: true, pending: false, status: "ACTIVE" }, 2, storage);

    expect(readCachedInventory(storage)).toEqual({
      at: 2,
      services: { gmail: { connected: true, pending: false, status: "ACTIVE" } },
    });
    expect(storage.getItem(CONNECTED_APPS_CACHE_KEY)).not.toMatch(/authorize|https?:\/\//i);
  });
