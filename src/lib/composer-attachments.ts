...
export interface PasteAttachment extends JsonObject {
  kind: "paste";
  id: string;
  text: string;
  size: number;
...
export interface FileAttachment extends JsonObject {
  kind: "file";
  id: string;
  path: string;
  name: string;
...
export interface ImageAttachment extends JsonObject {
  kind: "image";
  id: string;
  path: string;
  name: string;
...
const attachmentSizeSchema = z.number().finite().nonnegative();
...
export function isAttachment(value: JsonValue): value is Attachment {
 ...
 * The server writes ~/.helmryth/attachments/<uuid>.<ext> and answers
...
export async function imageAttachmentFromFile(file: File): Promise<ImageAttachment | null> {
  ...
  if (!response.ok) {
    const detail = z.object({ error: z.string().optional() }).safeParse(
      await response.json().catch(() => ({ error: response.statusText })),
    );
    throw Object.assign(new Error(detail.success ? (detail.data.error ?? "upload failed") : "upload failed"), {
...
export interface AttachedImageSplit {
  display: string;
  images: string[];
}

export function splitAttachedImages(text: string): AttachedImageSplit {
  const images: string[] = [];
  ...
  for (const file of files) {
    if (allowImages && isImageFile(file)) {
      try {
        ...
        // The uploader's own refusals already open with the filename
        // ("shot.png exceeds 10 MB"); naming it again reads as
        // "shot.png: shot.png exceeds 10 MB". Only prefix when it doesn't.
        const named = reason.startsWith(`${file.name} `) || reason.startsWith(`${file.name}:`);
        imageErrors.push(named ? reason : `${file.name}: ${reason}`);
      }
      continue;
    }
