...
describe("bot patch queue", () => {
  ...
  it("coalesces upload then remove so an older avatar can never resurrect", async () => {
    ...
    queue.enqueue("bot-1", { avatarUrl: null, avatarCrop: "sigil" }, bot());
    await vi.advanceTimersByTimeAsync(399);
    expect(sent).toHaveLength(0);
    await vi.advanceTimersByTimeAsync(1);

    expect(sent).toEqual([{ avatarUrl: null, avatarCrop: "sigil" }]);
    expect(authoritative).toHaveBeenLastCalledWith(
      expect.objectContaining({ avatarUrl: null, avatarCrop: "sigil" }),
      {},
    );
  });
