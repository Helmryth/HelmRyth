...
import type { SigilColor, SigilMotion } from "@/lib/sigil";
...
import { invalidateSharedInstances, loadSharedInstances } from "@/lib/instances-loader";
...
export type { SigilColor } from "@/lib/sigil";
...
export interface Message {
  ...
  from?: { botId: string; name: string; color: SigilColor };
  /** emoji reactions; by = "user" or a member botId. */
  reactions?: Array<{ emoji: string; by: string }>;
  /** comm chips: "Messaged @X" linking to the bot⇄bot channel. */
  comm?: { groupId: string; withBotId: string; withName: string; withColor: SigilColor };
...
export interface ConfigStatus {
  ...
  managedServices?: {
    ...
    registry: { configured: boolean; origin: string };
    conduit: { configured: boolean; origin: string };
  };
...
export type ConfigStatusFrame = Pick<
  ConfigStatus,
  "xai" | "openaiCompat" | "managedServices" | "composio" | "box" | "vps" | "rooms" | "localVm" | "opencodeGo" | "tts" | "imageGen" | "profile" | "features" | "browserProfiles"
>;
...
export function configStatusFromFrame(frame: ConfigStatusFrame): ConfigStatus {
  return {
    ...
    managedServices: frame.managedServices,
...
export interface AppState {
  ...
  connected: boolean;
  error: string | null;
  notice: { level: "info" | "warning"; message: string } | null;
  sigilMotion: {
    botId: string;
    nonce: number;
    kind: Exclude<SigilMotion, "none">;
  } | null;
...
export type GroupPatch = Partial<
  Pick<Group, "name" | "bulletin" | "memberIds" | "defaultResponder" | "pinnedMessageId" | "section">
>;
...
export type Action =
  ...
  | { type: "groupPatchRollback"; groupId: string; patch: GroupPatch }
  ...
  | { type: "reactionRollback"; threadId: string; messageId: string; emoji: string; selected: boolean }
  | { type: "interruptGroup"; groupId: string; threadId: string }
  ...
  | { type: "branchReconciled"; botId: string; activeLeafId: string | null | undefined }
  | { type: "threadActive"; threadId: string; activeLeafId: string }
  | { type: "answerCard"; botId: string; messageId: string; answer: string }
  | { type: "restoreOpeningCard"; botId: string; threadId: string; message: Message }
...
export interface InterruptApiRequest {
  path: string;
  init: RequestInit;
}

export function interruptRequest(
  action: Extract<Action, { type: "interrupt" | "interruptGroup" }>,
): InterruptApiRequest {
  return {
    path: action.type === "interrupt"
      ? `/api/bots/${action.botId}/interrupt`
      : `/api/groups/${action.groupId}/interrupt`,
    init: {
      method: "POST",
...
/** The roster PATCHes that put a duplicated operator in every crew its source
 * belongs to. Without them the copy looks identical in the sidebar but sits
 * outside every room the original answers in — a silent difference the person
 * only discovers when the duplicate never speaks. DM channels are the pair
 * they were opened for and reject roster edits, so they are left alone. */
export function crewJoinRequests(
  ...
  return groups
    .filter(
      (group) =>
        !group.dm && group.memberIds.includes(sourceBotId) && !group.memberIds.includes(duplicateBotId),
    )
    .map((group) => ({
      path: `/api/groups/${group.id}`,
      init: {
        method: "PATCH",
        body: JSON.stringify({ memberIds: [...group.memberIds, duplicateBotId] }),
      },
    }));
}

export function previousGroupPatch(group: Group | undefined, patch: GroupPatch): GroupPatch {
  if (!group) return {};
  const previous: GroupPatch = {};
  if (patch.name !== undefined) previous.name = group.name;
  if (patch.bulletin !== undefined) previous.bulletin = group.bulletin;
  if (patch.memberIds !== undefined) previous.memberIds = [...group.memberIds];
  if (patch.defaultResponder !== undefined) previous.defaultResponder = group.defaultResponder;
  if (patch.pinnedMessageId !== undefined) previous.pinnedMessageId = group.pinnedMessageId;
  if (patch.section !== undefined) previous.section = group.section;
  return previous;
}
...
function withSigilMotion(
  ...
  return {
    ...
    sigilMotion: {
      botId,
      nonce: (state.sigilMotion?.nonce ?? 0) + 1,
      kind,
    },
  };
}
...
export function reducer(state: AppState, action: Action): AppState {
  switch (action.type) {
    ...
    case "groupPatched": {
      ...
      // SAFETY: a previously unseen group is emitted as a complete Group by
      // the create/import API; subsequent events are the partial patch form.
      const incomingGroup = action.group as Group;
      const groups = exists
        ? state.groups.map((g) => (g.id === action.group.id ? { ...g, ...action.group, messages: action.group.messages ?? g.messages } : g))
        : [{ ...incomingGroup, messages: action.group.messages ?? [] }, ...state.groups];
      return { ...state, groups };
    }
    case "groupDeleted": {
    ...
    case "select": {
      ...
      return updateBot(
        withSigilMotion({ ...state, activeView: "chat", selectedId: action.id }, action.id, "switch"),
        action.id,
        (b) => ({ ...b, unread: false }),
      );
    }
    ...
    case "answerCard": {
      ...
      if (!card?.requestId) patch.dismissed = true;
      return withSigilMotion(
        patchCard(state, action.botId, action.messageId, patch),
        action.botId,
        "working",
      );
    }
    case "restoreOpeningCard": {
      const bot = state.bots.find((candidate) => candidate.id === action.botId);
      if (!bot || bot.threadId !== action.threadId) return state;
    ...
    case "markUnread":
      return updateBot(withSigilMotion(state, action.botId, "surprise"), action.botId, (b) => ({ ...b, unread: true }));
    case "botPatched": {
        ...
        action.bot.unread && !before?.unread
          ...
          : action.bot.busy === true && !before?.busy
            ...
            : action.bot.busy === false && before?.busy
              ? "celebrate"
              : null;
      const animated = kind ? withSigilMotion(state, action.bot.id, kind) : state;
    ...
    case "messageAdded": {
      ...
      if (!bot) {
        // room thread — plain linear append, no branching/sigil machinery
        ...
        action.message.kind === "options"
          ...
          : action.message.kind === "activity"
            ...
            : action.message.role === "bot" && action.message.kind === "text"
              ? "blink"
              : null;
      const animated = motion ? withSigilMotion(next, bot.id, motion) : next;
      return animated;
    }
    case "messagePatched": {
      const bot = state.bots.find((b) => b.threadId === action.threadId);
        ...
        action.message.kind === "activity"
          ? action.message.tool?.ok === false
            ...
            : action.message.tool?.ok === true
              ...
              : "working"
          : null;
      const next = motion ? withSigilMotion(state, bot.id, motion) : state;
    ...
    case "error":
      return {
        ...(action.message && state.selectedId
          ? withSigilMotion(state, state.selectedId, "alert")
          : state),
        error: action.message,
      };
    case "notice":
      return { ...state, notice: action.notice };
    // Operator settings, Workbench, and Trace share the right slot. System is
    // a full-screen modal: keep an existing Workbench mounted beneath it so
    // BrowserPanel retains its selected surface and can restore the native
    // view immediately when the modal closes.
    case "toggleSettings": {
    ...
    case "updateBot": {
      const sigilChanged =
        ...
        Object.prototype.hasOwnProperty.call(action.patch, "sigilExpression");
      const animated = sigilChanged
        ? withSigilMotion(state, action.botId, "customize")
        : state;
    ...
    case "reactionRollback": {
      const reconcile = (message: Message): Message => {
        if (message.id !== action.messageId) return message;
        const withoutUser = (message.reactions ?? []).filter(
          (reaction) => !(reaction.emoji === action.emoji && reaction.by === "user"),
        );
        const reactions = action.selected
          ? [...withoutUser, { emoji: action.emoji, by: "user" }]
          : withoutUser;
      ...
      return {
        ...
        bots: state.bots.map((bot) =>
          bot.threadId === action.threadId
            ? { ...bot, messages: bot.messages.map(reconcile) }
            : bot,
        ),
        groups: state.groups.map((group) =>
          group.threadId === action.threadId
            ? { ...group, messages: group.messages.map(reconcile) }
            : group,
        ),
      };
    }
    ...
    case "send":
      return withSigilMotion(dismissOnboardingCard(state, action.botId), action.botId, "working");
    case "editMessage":
      return withSigilMotion(state, action.botId, "working");
    case "newTask":
    case "switchTask":
    case "deleteTask":
    case "newGroupTask":
...
type ThreadErasureSummary = {
  state: "complete" | "pending";
  summary: string;
};

const threadErasureSummarySchema = z.object({
  state: z.enum(["complete", "pending"]),
  summary: z.string(),
});
const erasureNoticeSourceSchema = z.object({
  erasure: threadErasureSummarySchema.optional(),
  erasures: z.array(threadErasureSummarySchema).optional(),
  workspaceErasure: threadErasureSummarySchema.optional(),
});

export function erasureNotice(value: z.input<typeof erasureNoticeSourceSchema>): AppState["notice"] {
  const parsed = erasureNoticeSourceSchema.safeParse(value);
  if (!parsed.success) return null;
  const summaries: ThreadErasureSummary[] = [
    ...(parsed.data.erasures ?? (parsed.data.erasure ? [parsed.data.erasure] : [])),
    ...(parsed.data.workspaceErasure ? [parsed.data.workspaceErasure] : []),
  ];
  if (summaries.length === 0) return null;
  const unique = [...new Set(summaries.map((entry) => entry.summary.trim()).filter(Boolean))];
  if (unique.length === 0) return null;
  return {
    level: summaries.some((entry) => entry.state === "pending") ? "warning" : "info",
    message: unique.join(" "),
  };
}
...
export async function api(path: string, init?: RequestInit): Promise<any> {
  // Default the content-type instead of letting `init` replace the whole header
  // set. A caller that adds one header of its own — an idempotency key, say —
  // would otherwise drop the JSON type, and fetch would label the string body
  // text/plain, which every mutating core route rejects with 415.
  const headers = new Headers(init?.headers);
  if (!headers.has("content-type")) headers.set("content-type", "application/json");
  const res = await fetch(path, { ...init, headers });
  const body = await res.json().catch(() => ({}));
  if (!res.ok) throw new Error(body.error ?? statusMessage(res.status, res.statusText));
  return body;
}

/** The core names its own failures, and that message is always preferred. This
 * is the other case: a reply with no JSON error body — the local service down,
 * a proxy in between, a crash mid-request — where the fallback used to surface
 * the raw status line. "502 Bad Gateway" tells someone running a desktop app
 * nothing about what happened or what to do next. */
function statusMessage(status: number, statusText: string): string {
  if (status === 502 || status === 503 || status === 504) {
    return "Helmryth can’t reach its local service right now. It may still be starting — try again in a moment.";
  }
  if (status >= 500) return "Helmryth’s local service hit an unexpected error. Try again, or check the service log.";
  if (status === 404) return "That item no longer exists. It may have been deleted in another window.";
  if (status === 413) return "That is too large for Helmryth to accept.";
 ...
 * useStore consumer — sidebar, sigils, pickers, the settled transcript —
...
export function StoreProvider({ children }: { children: ReactNode }) {
  ...
  const dispatch = useMemo(() => {
    const showError = (cause: unknown) => {
      const error = normalizeSnapshotFailure(cause);
      rawDispatch({ type: "error", message: error.message });
      setTimeout(() => rawDispatch({ type: "error", message: null }), 6000);
    };
    const showNotice = (value: z.input<typeof erasureNoticeSourceSchema>) => {
      const notice = erasureNotice(value);
      if (!notice) return;
      rawDispatch({ type: "notice", notice });
      setTimeout(() => rawDispatch({ type: "notice", notice: null }), 8000);
    };
    ...
    const reconcileGroup = async (groupId: string) => {
      const snapshot: { groups?: Group[] } = await api("/api/bots");
      const group = snapshot.groups?.find((candidate) => candidate.id === groupId);
      if (group) rawDispatch({ type: "groupPatched", group });
    };
    const reconcileBot = async (botId: string) => {
      const snapshot: { bots?: Bot[] } = await api("/api/bots");
      const bot = snapshot.bots?.find((candidate) => candidate.id === botId);
      if (bot) rawDispatch({ type: "botPatched", bot });
    };
    const reconcileMessage = async (threadId: string, messageId: string) => {
      const snapshot: { bots?: Bot[]; groups?: Group[] } = await api("/api/bots");
      const conversation = snapshot.bots?.find((candidate) => candidate.threadId === threadId)
        ?? snapshot.groups?.find((candidate) => candidate.threadId === threadId);
      const message = conversation?.messages.find((candidate) => candidate.id === messageId);
      if (message) rawDispatch({ type: "messagePatched", threadId, message });
    };
    const branchGenerations = new Map<string, number>();
    const openingBriefSendIds = new Map<string, string>();
    ...
    const wrapped: React.Dispatch<Action> = (action) => {
      ...
      const cardBeforeAnswer = (() => {
        if (action.type !== "answerCard") return undefined;
        const bot = stateRef.current.bots.find((candidate) => candidate.id === action.botId);
        const message = bot?.messages.find((candidate) => candidate.id === action.messageId);
        return bot && message ? { threadId: bot.threadId, message } : undefined;
      })();
      const branchBefore = action.type === "switchBranch"
        ? stateRef.current.bots.find((candidate) => candidate.id === action.botId)?.activeLeafId
        : undefined;
      const groupPatchBefore = action.type === "patchGroup"
        ? previousGroupPatch(
            stateRef.current.groups.find((candidate) => candidate.id === action.groupId),
            action.patch,
          )
        : undefined;
      const reactionWasSelected = (() => {
        if (action.type !== "toggleReaction") return undefined;
        const conversation = stateRef.current.bots.find((candidate) => candidate.threadId === action.threadId)
          ?? stateRef.current.groups.find((candidate) => candidate.threadId === action.threadId);
        const message = conversation?.messages.find((candidate) => candidate.id === action.messageId);
      ...
      switch (action.type) {
        ...
        case "send": {
          ...
          void api(`/api/bots/${action.botId}/messages`, {
            ...
            .then((body) => {
              // Persist only after the direction was accepted. On failure the
              // exact opening brief is restored below so the person can retry.
              if (quizBeforeSend) persistCard(action.botId, quizBeforeSend.id, { dismissed: true });
              if (body?.message && body.threadId?.constructor === String) {
                rawDispatch({ type: "messageAdded", threadId: body.threadId, message: body.message });
              }
              if (
                body?.queued &&
                body.threadId?.constructor === String &&
                body.queueId?.constructor === String
              ) {
                rawDispatch({
                  type: "pendingQueued",
            ...
            .catch((error) => {
              if (quizBeforeSend && threadId) {
                rawDispatch({
                  type: "restoreOpeningCard",
                  botId: action.botId,
                  threadId,
                  message: quizBeforeSend,
                });
              }
              showError(error);
              action.onError?.();
        ...
        case "editMessage":
          ...
          break;
        case "switchBranch": {
          const generation = (branchGenerations.get(action.botId) ?? 0) + 1;
          branchGenerations.set(action.botId, generation);
          api(`/api/bots/${action.botId}/active-branch`, {
            method: "POST",
            body: JSON.stringify({ messageId: action.messageId }),
          })
            .then((body) => {
              if (branchGenerations.get(action.botId) !== generation) return;
              const activeLeafId = body?.activeLeafId;
              const reconciledLeaf = String(activeLeafId);
              if (activeLeafId === reconciledLeaf) {
                rawDispatch({ type: "branchReconciled", botId: action.botId, activeLeafId: reconciledLeaf });
              }
            })
            .catch((error) => {
              if (branchGenerations.get(action.botId) !== generation) return;
              rawDispatch({ type: "branchReconciled", botId: action.botId, activeLeafId: branchBefore });
              showError(error);
              void reconcileBot(action.botId).catch(() => {});
            });
          break;
        }
        case "decideRequest": {
        ...
        case "answerCard": {
          ...
          if (card?.requestId) {
            ...
            const threadId = cardBeforeAnswer?.threadId;
            const sendKey = `${action.botId}:${threadId ?? ""}:${action.messageId}`;
            const sendId = openingBriefSendIds.get(sendKey) ?? crypto.randomUUID();
            openingBriefSendIds.set(sendKey, sendId);
            api(`/api/bots/${action.botId}/messages`, {
              ...
              .then(() => {
                openingBriefSendIds.delete(sendKey);
                persistCard(action.botId, action.messageId, {
        ...
        case "duplicateBot": {
          ...
          api("/api/bots", { method: "POST" })
            .then(({ bot }) =>
              ...
              }).then(async ({ bot: patched }) => {
                rawDispatch({ type: "botAdded", bot: { ...bot, ...patched, messages: bot.messages } });
                // Crew membership is part of who the source is, so the copy
                // joins the same rooms. One at a time: each PATCH replaces a
                // whole roster, and a refused join must stop the rest and
                // surface rather than leave the copy half-placed in silence.
                for (const join of crewJoinRequests(stateRef.current.groups, action.botId, bot.id)) {
                  await api(join.path, join.init);
                }
              }),
            )
            .catch(showError);
        ...
        case "deleteBot":
          api(`/api/bots/${action.botId}`, { method: "DELETE" })
            .then(showNotice)
            .catch(showError);
          break;
        case "markUnread":
            ...
            body: JSON.stringify(action.patch),
              ...
              if (groupPatchBefore) {
                rawDispatch({
                  ...
                  patch: groupPatchBefore,
                });
              }
              showError(error);
              void reconcileGroup(action.groupId).catch(() => {});
            });
          break;
        case "deleteGroup":
          api(`/api/groups/${action.groupId}`, { method: "DELETE" })
            .then(showNotice)
            .catch(showError);
          break;
        case "toggleReaction":
          api(`/api/threads/${action.threadId}/messages/${action.messageId}/reactions`, {
            ...
            body: JSON.stringify({ emoji: action.emoji, by: "user" }),
              ...
              rawDispatch({
                ...
                messageId: action.messageId,
                emoji: action.emoji,
                selected: reactionWasSelected === true,
              });
              showError(error);
              void reconcileMessage(action.threadId, action.messageId).catch(() => {});
            });
          break;
        case "setModel":
        ...
        case "interrupt":
          {
            const request = interruptRequest(action);
            api(request.path, request.init).catch(showError);
          }
          break;
        ...
        case "deleteTask":
          api(`/api/bots/${action.botId}/tasks/${action.threadId}`, { method: "DELETE" })
            .then((r: any) => {
              showNotice(r);
              if (r?.bot) dispatch({ type: "taskSwitched", bot: r.bot });
            })
            .catch(showError);
          break;
        ...
        case "deleteGroupTask":
          api(`/api/groups/${action.groupId}/tasks/${action.threadId}`, { method: "DELETE" })
            .then((r: any) => {
              showNotice(r);
              if (r?.group) dispatch({ type: "groupPatched", group: r.group });
            })
            .catch(showError);
          break;
        case "interruptGroup":
    ...
    const peripheralParts: PeripheralPart[] = [
      {
        key: "instances",
        request: async () => {
          const instances = await loadSharedInstances();
    ...
    handleFrame = (frame) => {
      ...
      switch (frame.kind) {
        case "message": {
          ...
          if (frame.message?.role === "user" && frame.message.queueId?.constructor === String) {
            rawDispatch({
        ...
        case "thread":
          ...
          break;
        case "bot": {
          const bot: BotAnnouncement = frame.bot;
        ...
        case "group": {
          const group: Partial<Group> & { id: string } = frame.group;
        ...
        case "computer-control":
          rawDispatch({
            ...
            helpReason: frame.helpReason?.constructor === String ? frame.helpReason : null,
          });
          break;
        case "bot.deleted":
  ...
  const refreshInstances = useCallback(async () => {
    invalidateSharedInstances();
    const instances = await loadSharedInstances({ force: true });
  ...
  useEffect(() => {
    const onFocus = () => {
      ...
      void loadSharedInstances().then((instances) => {
        rawDispatch({ type: "instances", instances });
      });
    };
