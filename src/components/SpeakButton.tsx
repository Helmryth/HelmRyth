...
export function SpeakButton({
  ...
  const label = !configured
    ? "Add an ElevenLabs key in an operator profile to read messages aloud"
    : !ready
      ? "Pick a voice in this operator's profile to read messages aloud"
    : mine
      ? "Stop speaking"
      : "Read this aloud";
  return (
    <button
      ...
      title={label}
      className={cn(
        "flex size-9 items-center justify-center rounded-md text-ink-secondary transition-opacity hover:bg-raised hover:text-ink focus-visible:opacity-100 group-focus-within:opacity-100",
        // stays visible while speaking — a stop button you have to hunt for
        // is not a stop button
        mine ? "text-accent opacity-100" : ready ? "opacity-70 hover:opacity-100" : "cursor-not-allowed opacity-60",
        className,
      )}
    >
      {preparing ? <Loader2 size={14} className="motion-safe:animate-spin" /> : mine ? <Square size={14} className="fill-current" /> : <Volume2 size={14} />}
    </button>
  );
}
