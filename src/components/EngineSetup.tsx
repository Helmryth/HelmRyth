...
function hostPlatform(): Platform {
  const platform = window.helmryth?.platform;
...
/** The engine command itself is absent. Local-model injection needs the CLI but
...
function CommandRow({ command, actionLabel }: { command: string; actionLabel: string }) {
  ...
  const canOpen = Boolean(window.helmryth?.openInstallTerminal);
  ...
  const openTerminal = async () => {
    const opened = await window.helmryth!.openInstallTerminal!(command);
      ...
      {canOpen ? (
        <>
          <button
            ...
            className="mt-2 flex w-full items-center justify-center gap-2 whitespace-nowrap rounded-md bg-accent px-3 py-2 text-[12.5px] font-semibold text-accent-ink hover:brightness-95"
          >
...
export function EngineSetup({
  ...
  const description = signInOnly
    ...
    : instance.driverKind === "openai-compat"
      ? "Add your OpenAI-compatible API key in System → Connections. For source or headless runs, set OPENAI_COMPAT_API_KEY in the host environment instead."
    : intent === "inject"
      ? "Install the engine command once, then run local models without a cloud sign-in."
      : `Install the command-line app once. Models will appear here as soon as it’s ready${signInCommand ? "; sign-in may follow" : ""}.`;

  // Some engines are configured elsewhere (for example, a hosted Workbench
  // token) and intentionally have no install descriptor.
  if (!install) {
    return (
      <div className={cn("border-l-2 border-warning bg-warning/5 px-3 py-2.5", className)}>
  ...
  return (
    <div className={cn("border-l-2 border-signal bg-signal/5 px-3 py-2.5", className)}>
      <div className="flex items-start gap-2.5">
        <span className="mt-0.5 flex size-7 shrink-0 items-center justify-center rounded-md bg-inset text-ink-secondary">
