...
type ModelPane = "main" | "custom";

/** Choose the first useful pane for an engine. BYOK/custom access alone does
 * not imply a local catalog: remote API models are normal selectable rows.
 * The local pane wins only for an explicitly selected injected model or a
 * catalog that has no remote rows at all. */
export function modelPickerPane(
  ...
  const selectedIsCustom = Boolean(
    selectedModel && options.some((option) => option.id === selectedModel && option.custom === true),
  );
  const hasOfficial = options.some((option) => option.custom !== true);
  return selectedIsCustom || isCustomOnly(instance) || !hasOfficial ? "custom" : "main";
}
...
export function ModelPicker({
   ...
   * narrow parent (the operator profile sidebar). */
  contained?: boolean;
  label?: ReactNode;
}) {
  ...
  const [pane, setPane] = useState<ModelPane>("main");
  ...
  const openFor = (instance: InstanceInfo | undefined) => {
    setPane(modelPickerPane(instance, selection.model));
    resetList();
  };

  const selectRail = (instance: InstanceInfo) => {
    setRailId(instance.instanceId);
    setPane(modelPickerPane(instance));
    resetList();
  };
  ...
  const trigger = (
    <button
      ...
      className={cn(
        "flex items-center gap-1.5 rounded-md border border-hairline/60 bg-raised py-1 pl-2 pr-2.5 text-[13px] text-ink hover:bg-control",
        // in a narrow chat header fold to a rounded square with just the
        // provider mark; the model name rides the tooltip (an operator with no
    ...
    <div ref={rootRef} className={cn(contained ? "w-full" : "relative", className)}>
      ...
      {open && (
        <div
          ...
          className={cn(
            "flex overflow-hidden rounded-lg border border-hairline/70 bg-raised",
            contained
              ? "relative mt-3 w-full max-h-[min(420px,50dvh)]"
              : "absolute right-0 top-full z-30 mt-2 w-[380px] max-h-[min(480px,calc(100dvh-7rem))] shadow-xl",
          )}
        >
              ...
              const railButton = (instance: InstanceInfo) => {
                ...
                return (
                  <button
                    ...
                    aria-label={`${instance.displayName}: ${engineStatus(instance)}`}
              ...
              <>
                <div className="shrink-0 px-4 pb-2 pt-3.5">
                  ...
                  <div className="mt-0.5 text-[11.5px] text-ink-secondary">
                    {pane === "custom"
                      ? "Run this operator with a model already on your machine."
                      : "Choose the model assigned to this operator."}
                  </div>
                </div>
                  ...
                  <div className="min-h-0 flex-1 overflow-y-auto px-3 pb-3 pt-1">
                    ...
                    <p className="mt-2 text-center text-[11.5px] text-ink-secondary/70">
                      {pane === "main" && official.length > 0
                        ...
                        : "Local models appear as soon as the engine command is installed."}
                    </p>
                  </div>
                ) : (
                  <>
                    ...
                    <div className="min-h-0 flex-1 overflow-y-auto px-2 pb-2">
                      {pane === "main" ? (
                        ...
                        <>
                          ...
                          {rest.map(renderRow)}
                          {custom.length === 0 && (
                            <div className="mx-1 border-l-2 border-signal px-3 py-4">
                          ...
                          {custom.length > 0 && filteredCustom.length === 0 && (
                            <div className="px-2 py-5 text-center text-[12.5px] text-ink-secondary">
                              No local model matches &ldquo;{query.trim()}&rdquo;.
                            </div>
                          )}
                        </>
                      )}
