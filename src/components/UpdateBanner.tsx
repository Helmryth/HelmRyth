...
import { ArrowDownToLine, Loader2, Radio, RefreshCw, X } from "lucide-react";
...
// gets going.
const primaryAction =
  "flex flex-1 items-center justify-center gap-1.5 rounded-md bg-accent py-1.5 text-[13px] font-medium text-accent-ink disabled:cursor-default disabled:bg-control disabled:text-ink-secondary";
...
export function UpdateBanner() {
  ...
  const title =
    s.status === "available"
      ? `Helmryth ${s.version} is available`
    ...
    s.status === "available"
      ...
      : s.status === "downloading"
        ...
        : s.status === "downloaded"
          ...
          : installing
            ? "Helmryth will reopen in a moment."
            : friendlyError(s.message);

  return (
    <div role="status" aria-live="polite" className="animate-panel-in fixed bottom-4 left-4 z-50 w-[320px] rounded-lg border border-hairline/70 bg-panel p-3.5 shadow-xl">
      <div className="flex items-start gap-2.5">
        <span className="mt-0.5 flex size-7 shrink-0 items-center justify-center rounded-md border border-signal/30 bg-signal/10 text-signal">
          <Radio size={14} />
        </span>
        ...
        {!busy && (
          <button
            ...
            title="Dismiss"
            aria-label="Dismiss Helmryth update"
          >
            <X size={14} />
          </button>
        )}
      ...
      {s.status === "downloading" && (
        <div className="mt-2.5 h-1 overflow-hidden rounded-full bg-control">
          <div
            className={cn(
              "h-full w-full origin-left rounded-full bg-accent transition-transform",
              // before the first progress report, a sliver that breathes beats
              // a zero-width bar that looks stalled
              s.percent == null && "scale-x-25 animate-pulse",
            )}
            style={s.percent == null ? undefined : { transform: `scaleX(${Math.min(100, Math.max(0, s.percent)) / 100})` }}
          />
        </div>
      )}
