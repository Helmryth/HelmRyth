...
const COPY = {
  ...
  waiting: { label: "Waiting at gate", tone: "text-warning", border: "border-warning/35" },
...
export function RoutineRunCard({
  ...
  if (!run) {
    ...
    return fallback ? (
      <div className="w-fit max-w-[min(42rem,88%)] border-l-2 border-hairline bg-panel px-4 py-2.5 text-[14px] leading-relaxed text-ink">
        {fallback}
      </div>
    ) : null;
  }
  ...
  const actionLabel = run.status === "waiting" ? "Review gate" : "Open run";

  return (
    <section
      aria-label={`${run.routineName} cadence run: ${copy.label}`}
      className={cn(
        "w-full max-w-[680px] rounded-md border bg-panel px-3.5 py-3",
        copy.border,
      )}
    >
      <div className="flex items-start gap-3">
        <div className="mt-0.5 flex size-8 shrink-0 items-center justify-center rounded-sm border border-hairline bg-inset">
