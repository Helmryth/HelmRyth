...
describe("RoutineRunCard", () => {
  it("shows a compact completion receipt and a path to the isolated run", () => {
    ...
    expect(markup).toContain('aria-label="Morning brief cadence run: Completed"');
  });
  ...
  it("makes a waiting question or approval an explicit Review action", () => {
    ...
    expect(markup).toContain("Waiting at gate");
    expect(markup).toContain("Review gate");
    expect(markup).toContain('aria-label="Review gate for Morning brief"');
  });
