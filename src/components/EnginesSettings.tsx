...
import { installCommandFor } from "./EngineSetup";
...
interface ProbeResult {
  ...
  version?: string;
  message?: string;
  /** POST /api/cli-test attaches this when the path could not be spawned and
   * the driver knows how to install itself. */
  install?: EngineInstall;
}

/** The one-line recipe for the binary the probe could not find. The endpoint
 * has always returned it; the picker used to drop it, leaving the user with
 * "command not found" and nowhere to go. Null when the driver ships no
 * installer for this platform — a hint we cannot honour is worse than none. */
export function probeInstallHint(probe: ProbeResult | null | undefined): { command: string; needsNode: boolean } | null {
  if (!probe || probe.ok) return null;
  const command = installCommandFor(probe.install);
  return command ? { command, needsNode: probe.install?.needsNode === true } : null;
}
...
function CustomPicker({ instance, cliDefault, onClose, onSaved }: {
  ...
  const installHint = probeInstallHint(probe);
      ...
      {probe && !probe.ok && probe.message && (
        <div role="alert" className="flex gap-1.5 rounded-lg border border-warning/25 bg-warning/10 px-2.5 py-2 text-[12px] leading-relaxed text-warning">
          ...
          <span>
            ...
            {installHint && (
              <>
                {" "}Install it with <code className="font-mono">{installHint.command}</code>
                {installHint.needsNode ? " (needs Node.js and npm)" : ""}.
              </>
            )}
  ...
  return (
    <div>
      <div className="flex items-center gap-2 text-[13px]">
        <span aria-hidden="true" className={cn("size-1.5 shrink-0 rounded-full", instance.cli ? "bg-accent" : "bg-raised-hover")} />
        ...
        {instance.cli && (
          <button
            ...
            onClick={reset}
            disabled={switching}
            aria-label={`Reset ${instance.displayName} engine command`}
        ...
        <button
          ...
          onClick={() => setOpen((v) => !v)}
          aria-expanded={open}
          aria-label={`Set ${instance.displayName} engine command`}
          className={cn(
    ...
    <div className="flex flex-col gap-5">
      {rows.length === 0 && (
        <div className="border-l-2 border-warning pl-3 text-[13px] leading-relaxed text-ink-secondary">
          No engine commands were detected. Install one from the setup screen, then return here to verify its path.
        </div>
      )}
      {(() => {
      ...
      <div className="border-l-2 border-signal pl-3 text-[12px] leading-relaxed text-ink-secondary">
        An engine command can point to a versioned build, wrapper script, or absolute path. Saving reloads providers and interrupts active runs.
      </div>
    </div>
  );
}
