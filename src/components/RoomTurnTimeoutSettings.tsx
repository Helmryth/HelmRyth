...
export function RoomTurnTimeoutSettings() {
  ...
  return (
    <div className="flex flex-col gap-2">
      <label htmlFor="crew-step-timeout" className="text-[13px] font-medium text-ink">
        Maximum operator step
      </label>
      <div
      ...
      >
        <input
          ...
          aria-describedby={error ? "crew-step-timeout-error crew-step-timeout-help" : "crew-step-timeout-help"}
          onChange={(event) => {
      ...
      <p id="crew-step-timeout-help" className="text-[12px] leading-relaxed text-ink-secondary">
        Applies to every operator contribution in a crew. Direct workstreams use the inactivity watchdog instead.
      </p>
      {error ? (
        <p id="crew-step-timeout-error" role="alert" className="text-[12px] text-danger">
          {error}
        </p>
      ) : null}
    </div>
