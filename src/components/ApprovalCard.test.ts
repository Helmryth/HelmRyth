...
describe("ApprovalCard cadence gates", () => {
  it("states the requested action, resource, consequence, and requester in Helmryth language", () => {
    ...
    expect(markup).toContain("Gate · Decision required");
    expect(markup).toContain("Schedule a cadence");
    ...
    expect(markup).toContain("Method · cadence_schedule");
  });
  ...
  it("records the exact routine action after confirmation", () => {
    ...
    expect(markup).toContain("Cadence deleted");
  });
  ...
  it("does not imply a run-now request has already started", () => {
    ...
    const markup = renderToStaticMarkup(createElement(ApprovalCard, { message }));
    expect(markup).toContain("Cadence run queued");
    expect(markup).not.toContain("Cadence started");
  });
  ...
  it("speaks a routine's concise title instead of narrating all instructions", () => {
    ...
    expect(spoken).toContain("Schedule cadence “Backlog review”?");
