// Per-operator voice profile. The key is shared; the voice and autoplay choice
// belong to the selected operator.
//
...
const SAMPLE = "Morning. Overnight the tests went green, and I left two notes for you in the workstream.";

export function VoiceKeyEntryForm({
  configured,
  value,
  saving,
  onChange,
...
export function VoiceSettings({
  ...
  const saveKey = () => {
    ...
    const request = window.helmryth?.setCredential
      ? window.helmryth.setCredential("ttsKey", nextKey)
  ...
  return (
    <section className="border-b border-hairline/50 py-4">
      <div className="text-[15px] font-medium text-ink">Voice channel</div>
      <div className="mt-0.5 text-[13px] text-ink-secondary">
        Assign a voice for calls and spoken replies. The choice belongs to this operator;
        {provider === "system"
          ...
          <div className="inline-flex rounded-xl bg-inset p-1" role="radiogroup" aria-label="Voice engine">
            {([
              ...
              <button
                ...
                className={cn(
                  "rounded-md px-3.5 py-1.5 text-[12.5px] disabled:opacity-50",
          ...
          <div className="flex gap-2">
            ...
            >
              ...
              {selectedVoice && !voices.some((voice) => voice.id === selectedVoice) && (
                <option value={selectedVoice}>Current operator voice</option>
              )}
              {voices.map((v) => (
      ...
      <div className="mt-4 flex items-center justify-between gap-4 border-t border-hairline/40 pt-4">
        <div>
          ...
          <div className="mt-0.5 text-[11.5px] leading-relaxed text-ink-secondary">
            Speak this operator&rsquo;s replies as they arrive, even from another workstream.
          </div>
        </div>
        <button
          role="switch"
          aria-checked={Boolean(bot.speakReplies)}
          aria-label="Read this operator's replies aloud"
        ...
        >
          <span
            className={cn(
              ...
              bot.speakReplies && "translate-x-[18px]",
            )}
          />
        </button>
      </div>
