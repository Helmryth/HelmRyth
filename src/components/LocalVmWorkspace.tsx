...
function statusLabel(status: LocalVmWorkspaceStatus | null, nativeStatus: DesktopWorkspaceState["status"]) {
  if (!status) return "Checking workbench";
  if (status.container === "missing") return "Workbench not created";
  if (status.container === "stopped") return "Workbench stopped";
  if (!status.ready) return "Workbench unavailable";
...
function LocalVmPane({
  ...
  const botName = bot?.name ?? "Isolated Workbench";
  ...
  useEffect(() => {
    const bridge = window.helmryth?.desktopWorkspace;
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      if (!bridge) {
        setError("Split Workbench requires Helmryth Desktop.");
        return;
      }
      try {
        ...
        const next = await bridge.open({
          contextId,
          url: viewerUrl,
          title: `${botName} · Isolated Workbench`,
          bounds,
        });
        if (!alive) {
        ...
        setError(
          cause instanceof Error && cause.message === "layout-unavailable"
            ...
            : "Helmryth could not connect this Isolated Workbench viewer.",
        );
      }
    };
  ...
  const updateLayout = useCallback(() => {
    ...
    void bridge
      ...
      .catch(() => setError("Helmryth could not position this Isolated Workbench viewer."));
  ...
  const paneFailed = Boolean(error || (status && !status.ready));

  return (
    <section
      aria-label={`Pane ${index + 1}: ${botName}`}
      className={cn(
        "flex min-h-0 min-w-0 flex-col overflow-hidden rounded-md border bg-panel",
        active ? "border-accent" : "border-hairline/60",
      )}
    >
      <div className="flex min-h-[68px] items-center gap-3 border-b border-hairline/40 px-3 py-2.5">
        <div className="min-w-0 flex-1">
          <label className="sr-only" htmlFor={`local-vm-slot-${index}`}>
            Isolated Workbench operator for pane {index + 1}
          </label>
          <select
            ...
            className="w-full truncate rounded-md border border-hairline/60 bg-card px-2.5 py-1.5 text-[13px] font-medium text-ink outline-none focus-visible:border-accent focus-visible:ring-2 focus-visible:ring-accent/30"
          >
          ...
          <div className="mt-1 flex items-center gap-1.5 text-[12px] text-ink-secondary" aria-live="polite" aria-atomic="true">
            <span
              aria-hidden="true"
              className={cn(
            ...
            />
            {active ? "You hold the controls" : heldElsewhere ? "Controls held elsewhere" : label}
          </div>
        </div>
        {bot && active ? (
          <button
            ...
            onClick={onRelease}
            disabled={controlPending}
            className="flex shrink-0 items-center gap-1.5 rounded-md bg-accent px-2.5 py-2 text-[12px] font-medium text-accent-ink hover:bg-accent-border disabled:opacity-50"
          >
            {controlPending ? <Loader2 size={13} className="motion-safe:animate-spin" /> : <Hand size={13} />}
            Return controls
          </button>
        ) : bot ? (
          <button
            ...
            className="flex min-h-9 shrink-0 items-center gap-1.5 rounded-md bg-raised px-2.5 text-[12px] text-ink hover:bg-raised-hover disabled:opacity-45"
            title={`Pause only ${bot.name} and enable keyboard and pointer input in this pane`}
          >
            {controlPending ? <Loader2 size={13} className="motion-safe:animate-spin" /> : <Hand size={13} />}
            {heldElsewhere ? "Held elsewhere" : "Take controls"}
          </button>
        ) : null}
      </div>
      ...
      <div
        ...
        className="relative min-h-0 flex-1 bg-inset"
      >
        <div role={paneFailed ? "alert" : "status"} className="absolute inset-0 flex items-center justify-center p-6 text-center">
          {!bot ? (
            <div className="flex max-w-[260px] flex-col items-center gap-2 text-ink-secondary">
              <Monitor size={22} />
              <span className="text-[12px]">Choose an operator assigned to an Isolated Workbench.</span>
            </div>
          ) : !status && !error ? (
            <div className="flex items-center gap-2 text-[12px] text-ink-secondary">
              <Loader2 size={15} className="motion-safe:animate-spin" /> Checking {bot.name}'s workbench…
            </div>
          ) : status?.ready && nativeState.status !== "error" && !error ? (
            <div className="flex items-center gap-2 text-[12px] text-ink-secondary">
              <Loader2 size={15} className="motion-safe:animate-spin" /> Connecting live view…
            </div>
          ) : (
              ...
              <div className="text-[12px] leading-relaxed">
                {error ??
                  (status?.container === "missing"
                    ? `${bot.name}'s Isolated Workbench has not been created.`
                    : status?.container === "stopped"
                      ? `${bot.name}'s Isolated Workbench is stopped.`
                      : `${bot.name}'s Isolated Workbench is not ready for a live view.`)}
              </div>
              <div className="flex gap-2">
                <button
                  type="button"
                  onClick={() => setRetry((value) => value + 1)}
                  className="flex min-h-9 items-center gap-1.5 rounded-md bg-raised px-3 text-[12px] text-ink hover:bg-raised-hover"
                >
                ...
                <button
                  ...
                  className="min-h-9 rounded-md bg-raised px-3 text-[12px] text-ink hover:bg-raised-hover"
                >
                  Open Workbench
                </button>
              </div>
...
export function LocalVmWorkspace({
  ...
  useEffect(() => {
    ...
    return () => {
      window.requestAnimationFrame(() => {
        document.querySelector<HTMLElement>(`[data-split-workbench-launcher="${CSS.escape(primaryBotId)}"]`)?.focus();
      });
    };
  }, [primaryBotId]);
  ...
  const handBack = useCallback(async () => {
    ...
    try {
      ...
      setControlError("Helmryth could not hand control back. The workspace stayed open.");
      return false;
    } finally {
    ...
    async (botId: string) => {
      ...
      try {
        const alignedPort: LocalVmWorkspaceControlPort = {
          ...
          async setInteractive(nextContextId) {
            if (nextContextId && contextForBot(botId) !== nextContextId) {
              throw new Error("The Isolated Workbench pane changed while acquiring controls");
            }
        ...
        if (result.status === "held-elsewhere") {
          setControlError("This workbench is controlled in another viewer. Return it there first.");
          return;
        }
  ...
  return (
    <main ref={mainRef} tabIndex={-1} className="flex h-full min-w-0 flex-1 flex-col bg-app outline-none" aria-label="Split Workbench">
      <header className="flex min-h-[64px] items-center gap-3 border-b border-hairline/60 px-5 py-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="text-[11px] font-semibold uppercase tracking-[0.09em] text-ink-secondary">Workbench</div>
          <h1 className="truncate font-display text-[20px] font-semibold leading-tight text-ink">Split Workbench</h1>
          <p className="truncate text-[13px] text-ink-secondary">
            Two isolated desktops · one control lease · view-only by default
          </p>
        </div>
        <button
          type="button"
          ...
          disabled={controlPending}
          className="flex size-9 items-center justify-center rounded-md text-ink-secondary hover:bg-raised hover:text-ink disabled:opacity-50"
          aria-label="Close Split Workbench"
        >
          <X size={18} />
        </button>
      </header>
      ...
      {controlError && (
        <div role="alert" className="mx-3 mt-3 flex items-center gap-2 border-l-2 border-danger bg-danger/10 px-3 py-2 text-[13px] text-danger">
          <AlertTriangle size={14} className="shrink-0" /> {controlError}
        </div>
      )}

      <div className="grid min-h-0 flex-1 grid-cols-1 auto-rows-[minmax(320px,1fr)] gap-3 overflow-y-auto p-3 lg:grid-cols-2 lg:auto-rows-auto lg:overflow-hidden">
