...
// from the member sigiles in the room header and pre-ticked with who is
...
export function ManageMembersPanel({
  ...
  const save = () => {
    ...
    if (rosterChanged) {
      setSaveError("This crew’s roster changed while the panel was open. Close it and try again.");
      return;
    }
    if (changed) {
  ...
  return (
    ...
    >
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-label={`Manage roster for ${group.name}`}
        className="w-[340px] rounded-md border border-hairline/60 bg-panel p-4 shadow-xl"
      >
        <div className="mb-1 text-[15px] font-semibold text-ink">Crew roster</div>
        <div className="mb-3 truncate text-[13px] text-ink-secondary">{group.name}</div>
        <BotPickerList bots={bots} picked={picked} onToggle={toggle} emptyHint="Create an operator before assembling a crew." />
        {!memberIds.length && <div className="mt-2 text-[12px] text-ink-secondary">A crew needs at least one operator.</div>}
        {saveError && (
        ...
        <div className="mt-3 flex gap-2">
          <button
            onClick={onClose}
            className="flex-1 rounded-lg bg-raised py-2 text-[14px] font-medium text-ink hover:bg-raised-hover"
          >
            Cancel
          </button>
          <button
            ...
            className="flex-1 rounded-lg bg-accent py-2 text-[14px] font-medium text-white hover:bg-accent/90 disabled:opacity-40"
          >
            Save{memberIds.length ? ` · ${memberIds.length} ${memberIds.length === 1 ? "operator" : "operators"}` : ""}
          </button>
        </div>
      </div>
    </div>
