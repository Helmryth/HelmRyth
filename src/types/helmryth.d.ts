...
type SkillRecordingPayload = {
  ...
  interface Window {
    helmryth?: {
      ...
      /** Receives a GitHub package URL opened through helmryth://install. */
      onPackageInstall?(cb: (url: string) => void): () => void;
      /** Updates the native Dock/taskbar unread indicator. */
      setUnreadCount?(count: number): void;
      /** Opens a live desktop as a sandboxed window owned by Helmryth. */
      desktopViewer?: {
      ...
      /** Asks where to save a bot-created file (inside ~/.helmryth), copies
      ...
      setCredential?(
        name: "composioApiKey" | "xaiApiKey" | "openaiCompatApiKey" | "boxToken" | "opencodeGoApiKey" | "ttsKey" | "openaiImageApiKey",
        value: string,
